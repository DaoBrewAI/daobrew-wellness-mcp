...
tl=json.load(open(f"{PV}/pattern_timeline_unified.json"))["timeline"]
...
hrv=[{"ts":datetime.datetime.fromisoformat(h["timestamp"].replace("Z","+00:00")).timestamp(),"v":h["value"]}
     for h in json.load(open(f"{SP}/reunify/data_unified/hk_heart_rate_variability.json"))["data"]["samples"]]
...
json.dump(out,open(f"{PV}/enrichment_unified.json","w"),ensure_ascii=False,indent=1)
