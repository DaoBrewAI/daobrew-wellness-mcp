...
const upsert_js_1 = require("../src/engine/upsert.js");
...
const UPSERT_ROOT_ID = (0, Reasoner_js_1.rootCauseId)(UPSERT_TEST_USER);
...
async function withTempGraphDb(run) {
    const tmpDir = (0, node_fs_1.mkdtempSync)((0, node_path_1.join)((0, node_os_1.tmpdir)(), "daobrew-upsert-"));
...
function fixtureInput(userId = UPSERT_TEST_USER) {
    return {
        biometrics: {
            metrics: [{
                    metric: "heart_rate",
...
async function fixtureDelta(userId = UPSERT_TEST_USER) {
    const triplets = (0, triplet_js_1.joinTriplets)(fixtureInput(userId), {
        userId,
        contextWindowSec: 120,
        sampleWindowSec: 60,
    });
    return new Reasoner_js_1.Reasoner().buildDelta({ user_id: userId, triplets });
}
(0, node_test_1.describe)("upsertGraphDelta", () => {
    (0, node_test_1.it)("transactionally writes and arms a GraphDelta idempotently", async (t) => {
        if (!sqliteCliPresent())
            return t.skip("sqlite3 CLI not available");
        await withTempGraphDb(async (dbPath) => {
            const delta = await fixtureDelta();
            const first = await (0, upsert_js_1.upsertGraphDelta)(delta, { nowTs: 1234 });
            ...
            const second = await (0, upsert_js_1.upsertGraphDelta)(delta, { nowTs: 1234 });
            assert.strictEqual(second.status, "written");
            const countsAfterRerun = sqlite(dbPath, "SELECT (SELECT count(*) FROM graph_nodes) || '|' || (SELECT count(*) FROM graph_edges);").trim();
            assert.strictEqual(countsAfterRerun, counts);
            const root = sqlite(dbPath, `SELECT json_extract(props_json,'$.status') || '|' || json_extract(props_json,'$.brief.cause') FROM graph_nodes WHERE id='${UPSERT_ROOT_ID}';`).trim();
            ...
            assert.match(detonator.content[0].text, new RegExp(`cause_id: ${UPSERT_ROOT_ID}`));
            assert.match(detonator.content[0].text, /evidence-backed delivery loop/);
        });
    });
    (0, node_test_1.it)("rejects deltas with unbacked episodes and missing edge refs", async (t) => {
        if (!sqliteCliPresent())
            return t.skip("sqlite3 CLI not available");
        await withTempGraphDb(async () => {
            ...
            await assert.rejects(() => (0, upsert_js_1.upsertGraphDelta)(delta), (err) => {
                assert.ok(err instanceof upsert_js_1.GraphDeltaValidationError);
    ...
    (0, node_test_1.it)("does not re-arm a completed root cause by default", async (t) => {
        if (!sqliteCliPresent())
            return t.skip("sqlite3 CLI not available");
        await withTempGraphDb(async (dbPath) => {
            const delta = await fixtureDelta();
            await (0, upsert_js_1.upsertGraphDelta)(delta, { nowTs: 1234 });
            sqlite(dbPath, `UPDATE graph_nodes SET props_json = json_set(props_json, '$.status', 'done') WHERE id='${UPSERT_ROOT_ID}';`);
            const result = await (0, upsert_js_1.upsertGraphDelta)(delta, { nowTs: 1235 });
            assert.strictEqual(result.status, "skipped_done");
            assert.strictEqual(result.root_armed, false);
            const status = sqlite(dbPath, `SELECT json_extract(props_json,'$.status') FROM graph_nodes WHERE id='${UPSERT_ROOT_ID}';`).trim();
            assert.strictEqual(status, "done");
        });
    });
    (0, node_test_1.it)("skips the watching write when the root is done, preserving the done lifecycle", async (t) => {
        if (!sqliteCliPresent())
            return t.skip("sqlite3 CLI not available");
        await withTempGraphDb(async (dbPath) => {
            const delta = await fixtureDelta();
            await (0, upsert_js_1.upsertGraphDelta)(delta, { nowTs: 1234 });
            await (0, tools_js_1.handleToolCall)("daobrew_detonate_done", {
                cause_id: UPSERT_ROOT_ID,
                artifact_ref: "ai2-demo-story.md",
            }, true);
            const countsBefore = sqlite(dbPath, "SELECT (SELECT count(*) FROM graph_nodes) || '|' || (SELECT count(*) FROM graph_edges);").trim();
            const triplets = (0, triplet_js_1.joinTriplets)(fixtureInput(), {
                userId: UPSERT_TEST_USER,
                contextWindowSec: 120,
                sampleWindowSec: 60,
            });
            const watching = await new Reasoner_js_1.Reasoner().buildWatchingDelta({
                user_id: UPSERT_TEST_USER,
                triplets,
                generated_at_ts: 2234,
            });
            const result = await (0, upsert_js_1.upsertGraphDelta)(watching, { nowTs: 2234 });
            assert.strictEqual(result.status, "skipped_done");
            assert.strictEqual(result.root_armed, false);
            const props = JSON.parse(sqlite(dbPath, `SELECT props_json FROM graph_nodes WHERE id='${UPSERT_ROOT_ID}';`).trim());
            ...
            const doneEdges = sqlite(dbPath, `SELECT count(*) FROM graph_edges WHERE id='${UPSERT_DONE_EDGE_ID}';`).trim();
    ...
    (0, node_test_1.it)("clears completed artifact props when an explicit re-arm is allowed", async (t) => {
        if (!sqliteCliPresent())
            return t.skip("sqlite3 CLI not available");
        await withTempGraphDb(async (dbPath) => {
            const delta = await fixtureDelta();
            await (0, upsert_js_1.upsertGraphDelta)(delta, { nowTs: 1234 });
            sqlite(dbPath, `UPDATE graph_nodes
          ...
          WHERE id='${UPSERT_ROOT_ID}';`);
            const result = await (0, upsert_js_1.upsertGraphDelta)(delta, { nowTs: 1235, allowRearmDone: true });
            const props = JSON.parse(sqlite(dbPath, `SELECT props_json FROM graph_nodes WHERE id='${UPSERT_ROOT_ID}';`).trim());
    ...
    (0, node_test_1.it)("stamps armed_at_ts on the →armed transition and keeps it stable while armed", async (t) => {
        if (!sqliteCliPresent())
            return t.skip("sqlite3 CLI not available");
        await withTempGraphDb(async (dbPath) => {
            const delta = await fixtureDelta();
            ...
            await (0, upsert_js_1.upsertGraphDelta)(delta, { nowTs: 1234 });
            const armed = JSON.parse(sqlite(dbPath, `SELECT props_json FROM graph_nodes WHERE id='${UPSERT_ROOT_ID}';`).trim());
            ...
            await (0, upsert_js_1.upsertGraphDelta)(delta, { nowTs: 9999 });
            const stillArmed = JSON.parse(sqlite(dbPath, `SELECT props_json FROM graph_nodes WHERE id='${UPSERT_ROOT_ID}';`).trim());
            ...
            sqlite(dbPath, `UPDATE graph_nodes SET props_json = json_remove(props_json, '$.armed_at_ts') WHERE id='${UPSERT_ROOT_ID}';`);
            await (0, upsert_js_1.upsertGraphDelta)(delta, { nowTs: 15000 });
            const legacy = JSON.parse(sqlite(dbPath, `SELECT props_json FROM graph_nodes WHERE id='${UPSERT_ROOT_ID}';`).trim());
          ...
          WHERE id='${UPSERT_ROOT_ID}';`);
            await (0, upsert_js_1.upsertGraphDelta)(delta, { nowTs: 20000, allowRearmDone: true });
            const rearmed = JSON.parse(sqlite(dbPath, `SELECT props_json FROM graph_nodes WHERE id='${UPSERT_ROOT_ID}';`).trim());
            assert.strictEqual(rearmed.status, "armed");
            assert.strictEqual(rearmed.armed_at_ts, 20000);
        });
    });
    (0, node_test_1.it)("preserves offer_state while armed and drops it on a fresh arming episode (MRT v1)", async (t) => {
        if (!sqliteCliPresent())
            return t.skip("sqlite3 CLI not available");
        await withTempGraphDb(async (dbPath) => {
            const delta = await fixtureDelta();
            ...
            // like armed_at_ts.
            sqlite(dbPath, `UPDATE graph_nodes SET props_json = json_set(props_json, '$.offer_state', 'held') WHERE id='${UPSERT_ROOT_ID}';`);
            await (0, upsert_js_1.upsertGraphDelta)(delta, { nowTs: 9999 });
            const stillHeld = JSON.parse(sqlite(dbPath, `SELECT props_json FROM graph_nodes WHERE id='${UPSERT_ROOT_ID}';`).trim());
    ...
    (0, node_test_1.it)("preserves ghost carry-forward props across reruns", async (t) => {
        if (!sqliteCliPresent())
            return t.skip("sqlite3 CLI not available");
        await withTempGraphDb(async (dbPath) => {
            const delta = await fixtureDelta();
          ...
          WHERE id='${UPSERT_ROOT_ID}';`);
            await (0, upsert_js_1.upsertGraphDelta)(delta, { nowTs: 1235 });
            const props = JSON.parse(sqlite(dbPath, `SELECT props_json FROM graph_nodes WHERE id='${UPSERT_ROOT_ID}';`).trim());
    ...
    (0, node_test_1.it)("writes WATCHING state and clears stale armed roots without carrying old weeks", async (t) => {
        if (!sqliteCliPresent())
            return t.skip("sqlite3 CLI not available");
        await withTempGraphDb(async (dbPath) => {
            const delta = await fixtureDelta();
          ...
          WHERE id='${UPSERT_ROOT_ID}';`);
            ...
            const result = await (0, upsert_js_1.upsertGraphDelta)(watching, { nowTs: 2234 });
            const root = JSON.parse(sqlite(dbPath, `SELECT props_json FROM graph_nodes WHERE id='${UPSERT_ROOT_ID}';`).trim());
            const edgeCount = sqlite(dbPath, "SELECT count(*) FROM graph_edges;").trim();
            const manifestedCount = sqlite(dbPath, "SELECT count(*) FROM graph_edges WHERE kind='manifested';").trim();
            const nodeCount = sqlite(dbPath, "SELECT count(*) FROM graph_nodes;").trim();
            const titles = sqlite(dbPath, "SELECT group_concat(title, '|') FROM graph_nodes;").trim();
    ...
    (0, node_test_1.it)("writes separate graphs for different users without id collisions", async (t) => {
        if (!sqliteCliPresent())
            return t.skip("sqlite3 CLI not available");
        await withTempGraphDb(async (dbPath) => {
            const localDelta = await fixtureDelta(UPSERT_TEST_USER);
            const otherDelta = await fixtureDelta(UPSERT_OTHER_USER);
            await (0, upsert_js_1.upsertGraphDelta)(localDelta, { nowTs: 1234 });
            const counts = sqlite(dbPath, "SELECT (SELECT count(*) FROM graph_nodes) || '|' || (SELECT count(*) FROM graph_edges);").trim();
            await (0, upsert_js_1.upsertGraphDelta)(otherDelta, { userId: UPSERT_OTHER_USER, nowTs: 1235 });
            const countsAfterOther = sqlite(dbPath, "SELECT (SELECT count(*) FROM graph_nodes) || '|' || (SELECT count(*) FROM graph_edges);").trim();
            assert.notStrictEqual(countsAfterOther, counts);
            const perUser = sqlite(dbPath, "SELECT user_id || ':' || count(*) FROM graph_nodes GROUP BY user_id ORDER BY user_id;").trim().split("\n");
            ...
            const rootIds = sqlite(dbPath, "SELECT id FROM graph_nodes WHERE kind='ghost' ORDER BY user_id;").trim().split("\n");
            assert.deepStrictEqual(rootIds, [UPSERT_ROOT_ID, (0, Reasoner_js_1.rootCauseId)(UPSERT_OTHER_USER)]);
            const beforeRerun = countsAfterOther;
            await (0, upsert_js_1.upsertGraphDelta)(otherDelta, { userId: UPSERT_OTHER_USER, nowTs: 1236 });
            const afterRerun = sqlite(dbPath, "SELECT (SELECT count(*) FROM graph_nodes) || '|' || (SELECT count(*) FROM graph_edges);").trim();
...
function assertScopedQueriesForWiringUser(statements, label) {
    ...
    for (const sql of statements) {
        ...
        assert.ok(sql.includes(`set_config('${GUC}', '${WIRING_USER}', true)`), `${label}: every query must carry the GUC scope for the run's userId, got: ${sql}`);
        assert.strictEqual(occurrences(sql, "set_config"), 1, `${label}: the GUC scope must appear exactly once per statement, got: ${sql}`);
    }
}
function assertScopedExecsForWiringUser(statements, label) {
    assert.ok(statements.length > 0, `${label}: at least one SQL statement must reach the transport`);
    for (const sql of statements) {
        ...
        assert.strictEqual(sql.split("\n").filter((line) => line.trim() === "BEGIN;").length, 1, `${label}: exactly one transaction BEGIN, got: ${sql}`);
        assert.strictEqual(sql.split("\n").filter((line) => line.trim() === "COMMIT;").length, 1, `${label}: exactly one transaction COMMIT, got: ${sql}`);
        assert.ok(sql.includes(`set_config('${GUC}', '${WIRING_USER}', true)`), `${label}: every exec must carry the transaction-local GUC for the run's userId, got: ${sql}`);
        assert.strictEqual(occurrences(sql, "set_config"), 1, `${label}: the GUC scope must appear exactly once per statement, got: ${sql}`);
    }
}
(0, node_test_1.describe)("upsertGraphDelta injected executors", () => {
    (0, node_test_1.it)("co-locates exact-lease guard, graph DML, receipt, and generation in one postgres batch", async () => {
        await withPostgresFakeTransport(async () => {
            ...
            assert.ok(graphWrite < assignment
                && assignment < receipt
                && receipt < invalidate
                && invalidate < marker, "offer, receipt, invalidation, and new current marker must share that exact order");
            ...
            assert.match(sql, new RegExp(`pg_advisory_xact_lock\\(\\s*1144153670, hashtext\\('${generationUser}'\\)\\s*\\)`));
            assert.match(sql, /lease_expires_at_ts > commit_ts/);
            assert.match(sql, new RegExp(`\\(user_id, generation_id, generated_at_ts, is_current\\)\\s+VALUES \\('${generationUser}', '11111111-1111-4111-8111-111111111111', commit_ts, TRUE\\)`));
    ...
    (0, node_test_1.it)("advances skipped_done only through the same exact-lease generation transaction", async () => {
        await withPostgresFakeTransport(async () => {
            ...
            const result = await (0, upsert_js_1.upsertGraphDelta)(delta, {
                ...
                query: async (sql) => {
                    if (sql.includes("props_json ->> 'status'"))
                        return [{ status: "done" }];
                    return [];
                },
                exec: async (sql) => { writes.push(sql); },
    ...
    (0, node_test_1.it)("rejects partial or sqlite generation capabilities before any write", async () => {
        await withPostgresFakeTransport(async () => {
            const partialUser = "CB3D5925-2FB8-4706-8C5B-0D6520A5A0F1";
            const delta = await fixtureDelta(partialUser);
            let writes = 0;
            await assert.rejects(() => (0, upsert_js_1.upsertGraphDelta)(delta, {
                userId: partialUser,
                generationId: "11111111-1111-4111-8111-111111111111",
                query: async () => [],
                exec: async () => { writes += 1; },
    ...
    (0, node_test_1.it)("routes all three internal reads through the injected query and the batch through the injected exec", async () => {
        await withPostgresFakeTransport(async (captured) => {
            ...
            assert.ok(reads.some((sql) => sql.includes("SELECT id FROM graph_nodes")), "existingNodeIds must flow through the injected query");
            assert.ok(reads.some((sql) => sql.includes("props_json ->> 'status'") && sql.includes("LIMIT 1")), "existingRootStatus must flow through the injected query");
            assert.ok(reads.some((sql) => sql.includes("SELECT id, props_json")), "existingGhostStates must flow through the injected query");
    ...
    (0, node_test_1.it)("sqlite batch keeps its own BEGIN IMMEDIATE/COMMIT transaction", async () => {
        ...
        const tmpDir = (0, node_fs_1.mkdtempSync)((0, node_path_1.join)((0, node_os_1.tmpdir)(), "daobrew-upsert-sqlite-batch-"));
        process.env.DAOBREW_GRAPH_STORE = "sqlite";
        process.env.DAOBREW_GRAPH_DB = (0, node_path_1.join)(tmpDir, "sentinel-graph.db");
        try {
            const delta = await fixtureDelta();
            const writes = [];
            const result = await (0, upsert_js_1.upsertGraphDelta)(delta, {
            ...
            assert.strictEqual(result.status, "written");
            assert.strictEqual(writes.length, 1);
            assert.ok(writes[0].trimStart().startsWith("BEGIN IMMEDIATE;"), `sqlite batch must open its own transaction, got: ${writes[0].slice(0, 80)}`);
            ...
            if (savedStore === undefined)
                ...
                process.env.DAOBREW_GRAPH_STORE = savedStore;
            if (savedGraphDb === undefined)
                delete process.env.DAOBREW_GRAPH_DB;
            else
                process.env.DAOBREW_GRAPH_DB = savedGraphDb;
            (0, node_fs_1.rmSync)(tmpDir, { recursive: true, force: true });
        }
    });
    (0, node_test_1.it)("internal reads filter user_id by the scope row under postgres, never by a literal (F1 One-Time-Filter regression)", async () => {
        ...
        await withPostgresFakeTransport(async () => {
            ...
            for (const sql of reads) {
                assert.ok(sql.includes("user_id = __daobrew_scope.uid"), `postgres internal read must filter user_id via the scope row, got: ${sql}`);
                assert.ok(!sql.includes(`user_id = '${UPSERT_TEST_USER}'`), `postgres internal read must NOT compare user_id to a literal (One-Time-Filter fail-closed trap), got: ${sql}`);
            }
        });
    });
    (0, node_test_1.it)("internal reads keep the escaped literal under sqlite (plain queryJson has no scope row)", async (t) => {
        if (!sqliteCliPresent())
            return t.skip("sqlite3 CLI not available");
        ...
        try {
            await withTempGraphDb(async () => {
                const delta = await fixtureDelta();
                const reads = [];
                await (0, upsert_js_1.validateGraphDelta)(delta, {
                    query: async (sql) => {
                        reads.push(sql);
                        return [];
                    },
                ...
                assert.ok(reads[0].includes(`user_id = '${UPSERT_TEST_USER}'`), `sqlite internal read must keep the escaped literal, got: ${reads[0]}`);
    ...
    (0, node_test_1.it)("validateGraphDelta routes its existingNodeIds read through the injected query", async () => {
        await withPostgresFakeTransport(async (captured) => {
            const delta = await fixtureDelta();
            const reads = [];
            await (0, upsert_js_1.validateGraphDelta)(delta, {
    ...
    (0, node_test_1.it)("defaults to module execSql/queryJson with GUC-free SQL when no executors are injected", async () => {
        await withPostgresFakeTransport(async (captured) => {
            const delta = await fixtureDelta();
            const result = await (0, upsert_js_1.upsertGraphDelta)(delta, { nowTs: 1234 });
            assert.strictEqual(result.status, "written");
...
(0, node_test_1.describe)("generation lease commit live postgres", () => {
    (0, node_test_1.it)("co-commits rows+marker and rolls both back on crash or stale-token interleaving", async (t) => {
        ...
        try {
            ...
            const repoRoot = (0, node_fs_1.existsSync)((0, node_path_1.resolve)(process.cwd(), "alembic.ini"))
                ? process.cwd()
      ...
      `);
            ...
            await (0, upsert_js_1.upsertGraphDelta)(successDelta, {
                userId: successUser,
                generationId: successGeneration,
                leaseToken: successLease,
                armedOffer: { treatmentP: 1 },
                nowTs: now,
                exec: (0, user_scope_js_1.scopedExec)(successUser),
                query: (0, user_scope_js_1.scopedQuery)(successUser),
            });
      ...
      `);
            ...
            await assert.rejects(() => (0, user_scope_js_1.scopedExec)(successUser)(`
          UPDATE graph_nodes
      ...
      `);
            ...
            await (0, user_scope_js_1.scopedExec)(successUser)(`
        UPDATE graph_nodes
      ...
      `);
            const slowGenerationExec = (0, user_scope_js_1.scopedExec)(raceUser, async (scopedSql) => {
            ...
            const generationPromise = (0, upsert_js_1.upsertGraphDelta)(raceDelta, {
                userId: raceUser,
                ...
                query: (0, user_scope_js_1.scopedQuery)(raceUser),
            });
        ...
        `);
            ...
            const mutationPromise = (0, user_scope_js_1.scopedExec)(raceUser)(`
        UPDATE graph_nodes
      ...
      `);
            ...
            const crashExec = (0, user_scope_js_1.scopedExec)(crashUser, async (scopedSql) => {
                ...
                await graphDb.execSql(`${scopedSql.slice(0, boundary)}SELECT 1 / 0;\n${scopedSql.slice(boundary)}`);
            });
            await assert.rejects(() => (0, upsert_js_1.upsertGraphDelta)(crashDelta, {
                userId: crashUser,
                ...
                nowTs: now,
                exec: crashExec,
                query: (0, user_scope_js_1.scopedQuery)(crashUser),
            }), /division by zero/);
      ...
      `);
            ...
            await assert.rejects(() => (0, upsert_js_1.upsertGraphDelta)(staleDelta, {
                userId: staleUser,
                generationId: staleGeneration,
                leaseToken: oldLease,
                exec: (0, user_scope_js_1.scopedExec)(staleUser),
                query: (0, user_scope_js_1.scopedQuery)(staleUser),
...
(0, node_test_1.describe)("engine run wiring (postgres graph upsert)", () => {
    (0, node_test_1.it)("runEngineOnce writes the noSignal watching delta through scoped executors", async () => {
        await withPostgresFakeTransport(async (captured) => {
    ...
    (0, node_test_1.it)("runEngineOnce writes the armed delta through scoped executors", async () => {
        await withPostgresFakeTransport(async (captured) => {
            const result = await (0, run_js_1.runEngineOnce)({ once: true, demo: true, userId: WIRING_USER });
            assert.strictEqual(result.status, "written");
            assert.strictEqual(result.graph_store, "postgres");
            const batches = captured.execs.filter((sql) => sql.includes("INSERT INTO graph_nodes"));
