...
const LatentHypothesis_js_1 = require("../src/engine/reasoner/LatentHypothesis.js");
const prompt_js_1 = require("../src/engine/reasoner/prompt.js");
...
const TEST_ROOT_CAUSE_ID = (0, Reasoner_js_1.rootCauseId)(REASONER_TEST_USER);
function joinTriplets(input, options = {}) {
    return (0, triplet_js_1.joinTriplets)(input, { userId: REASONER_TEST_USER, ...options });
}
function patternNode(delta, key) {
    const node = delta.nodes.find((entry) => entry.kind === "pattern" && entry.source_ref === `pattern:${key}`);
    assert.ok(node, `missing pattern:${key}`);
    return node;
}
function patternId(delta, key) {
    return patternNode(delta, key).id;
}
function fixtureInput(userId = REASONER_TEST_USER) {
    return {
        biometrics: {
            metrics: [{
                    metric: "heart_rate",
...
(0, node_test_1.describe)("Reasoner", () => {
    (0, node_test_1.it)("builds a GraphDelta with source-backed episode and armed brief", async () => {
        const triplets = joinTriplets(fixtureInput(), {
            sampleWindowSec: 60,
            contextWindowSec: 120,
            memoryWindowSec: 10,
        });
        const delta = await new Reasoner_js_1.Reasoner().buildDelta({ user_id: REASONER_TEST_USER, triplets });
        ...
        assert.strictEqual(root.props_json?.citation_gate?.has_cited_transcript_span, true);
        ...
        assert.strictEqual(episode.props_json?.citation_gate?.thread_key, "delivery-readiness");
        ...
        assert.strictEqual((episode.props_json?.supporting_calendar_events).length, 1);
        assert.strictEqual((episode.props_json?.supporting_meeting_notes).length, 1);
        ...
        assert.ok((episode.props_json?.sample_refs).includes("healthkit:heart_rate:1970-01-01T00:16:50.000Z"));
        ...
        assert.ok(!delta.edges.some((edge) => edge.kind === "manifested" && (delta.nodes.find((node) => node.id === edge.src_id)?.kind === "meeting")));
        const episodePatternEdge = delta.edges.find((edge) => (edge.kind === "manifested" &&
            edge.src_id === episode.id &&
            edge.dst_id === patternId(delta, "overdrive")));
        assert.ok(episodePatternEdge);
        assert.strictEqual(episodePatternEdge.props_json?.causality_level, "pattern_candidate");
        const triggerRootEdge = delta.edges.find((edge) => (edge.kind === "suggests" &&
            edge.src_id === episode.id &&
            edge.dst_id === TEST_ROOT_CAUSE_ID));
        assert.ok(triggerRootEdge);
        assert.strictEqual(triggerRootEdge.props_json?.causality_level, "source_backed_hypothesis_not_settled_causality");
        assert.ok(delta.edges.every((edge) => edge.src_id.length > 0 && edge.dst_id.length > 0));
        assert.doesNotMatch(JSON.stringify(delta), /We still need to pick one wedge for the demo story and close the read/);
        assert.match(JSON.stringify(delta), /text_redacted/);
        const carryForwardEdge = delta.edges.find((edge) => (edge.kind === "evidence_for" &&
            edge.src_id === episode.id &&
            edge.dst_id === TEST_ROOT_CAUSE_ID));
    ...
    (0, node_test_1.it)("episode props carry biometric_summary and pattern_endorsements", async () => {
        const triplets = joinTriplets(fixtureInput(), {
            sampleWindowSec: 60,
            contextWindowSec: 120,
            memoryWindowSec: 10,
        ...
        const delta = await new Reasoner_js_1.Reasoner().buildDelta({
        ...
        const hr = props.biometric_summary.metrics.find((m) => m.type === "heart_rate");
    ...
    (0, node_test_1.it)("episode without samples keeps props unchanged (no biometric_summary key)", async () => {
    ...
    (0, node_test_1.it)("surfaces project-memory support as first-class memory_hit nodes with suggests edges to the root", async () => {
        const joinOptions = {
            sampleWindowSec: 60,
            contextWindowSec: 120,
            memoryWindowSec: 10,
        ...
        const memEdges = delta.edges.filter((edge) => (edge.props_json?.evidence_role === "memory_support"));
        ...
        const root = delta.nodes.find((node) => node.id === TEST_ROOT_CAUSE_ID);
        assert.ok(root);
        assert.strictEqual((root.props_json?.supporting_project_memories).length, 1);
        // Determinism: an identical input must produce identical memory_hit
        // nodes and edges (full objects, not just ids).
        const again = await new Reasoner_js_1.Reasoner().buildDelta({
            user_id: REASONER_TEST_USER,
            triplets: joinTriplets(fixtureInput(), joinOptions),
        });
        const sortById = (items) => [...items].sort((a, b) => a.id.localeCompare(b.id));
        assert.deepStrictEqual(sortById(again.nodes.filter((node) => node.kind === "memory_hit")), sortById(memoryNodes));
        const memoryEdgesOf = (edges) => edges.filter((edge) => (edge.props_json?.evidence_role === "memory_support"));
        assert.deepStrictEqual(sortById(memoryEdgesOf(again.edges)), sortById(memEdges));
    });
    (0, node_test_1.it)("keeps memory_hit node ids stable across different weeks (week-agnostic ids match ux_nodes_dedup)", async () => {
        const joinOptions = {
            sampleWindowSec: 60,
            contextWindowSec: 120,
            memoryWindowSec: 10,
        };
        const weekA = await new Reasoner_js_1.Reasoner().buildDelta({
            user_id: REASONER_TEST_USER,
            triplets: joinTriplets(fixtureInput(), joinOptions),
        });
        ...
        const weekB = await new Reasoner_js_1.Reasoner().buildDelta({
            user_id: REASONER_TEST_USER,
            triplets: joinTriplets(shifted, joinOptions),
        });
        const idsOf = (delta) => delta.nodes
        ...
        const weekOf = (delta) => delta.edges
            .find((edge) => edge.props_json?.evidence_role === "memory_support")
    ...
    (0, node_test_1.it)("collapses memories sharing one source_ref into a single memory_hit node per delta", async () => {
        const joinOptions = {
            sampleWindowSec: 60,
            contextWindowSec: 120,
            memoryWindowSec: 10,
    ...
    (0, node_test_1.it)("ranks memory support by context-term overlap before source importance", async () => {
        const joinOptions = {
            sampleWindowSec: 60,
            contextWindowSec: 120,
            memoryWindowSec: 10,
        ...
        const citing = delta.nodes.find((node) => (Array.isArray(node.props_json?.supporting_project_memories) &&
            (node.props_json?.supporting_project_memories).length === 2));
        assert.ok(citing, "both qualifying memories must appear in supporting_project_memories");
        const cited = (citing.props_json?.supporting_project_memories).map((m) => m.id);
        assert.deepStrictEqual(cited, ["memory-claude-relevant", "memory-codex-junk"], "context-relevant memory must outrank the higher-importance junk digest");
    });
    (0, node_test_1.it)("keeps equal-overlap memory support ties in a stable order regardless of input order", async () => {
        const joinOptions = {
            sampleWindowSec: 60,
            contextWindowSec: 120,
            memoryWindowSec: 10,
        };
        const tieMemories = (base) => [
            {
                ...base,
                id: "memory-tie-a",
                source: "codex_project_session",
        ...
        const buildWith = async (reversed) => {
            const input = fixtureInput();
            const ties = tieMemories(input.memory[0]);
            input.memory = [...input.memory, ...(reversed ? [...ties].reverse() : ties)];
            return new Reasoner_js_1.Reasoner().buildDelta({ user_id: REASONER_TEST_USER, triplets: joinTriplets(input, joinOptions) });
        };
        const citedIds = (delta) => {
            const citing = delta.nodes.find((node) => (Array.isArray(node.props_json?.supporting_project_memories) &&
                (node.props_json?.supporting_project_memories).length === 3));
            assert.ok(citing, "all three qualifying memories must be cited");
            return (citing.props_json?.supporting_project_memories).map((m) => m.id);
        };
    ...
    (0, node_test_1.it)("hides calendar-only source rows instead of promoting them as trigger bundles", async () => {
        const input = fixtureInput();
        input.granola = [];
        ...
        await assert.rejects(() => new Reasoner_js_1.Reasoner().buildDelta({ user_id: REASONER_TEST_USER, triplets }), Reasoner_js_1.NotEnoughSignalError);
    });
    (0, node_test_1.it)("surfaces an early causal result for worn biometrics plus one non-logistics calendar signal", async () => {
        const input = fixtureInput();
    ...
    (0, node_test_1.it)("keeps calendar plus project-memory overlap at insufficient evidence without a cited transcript span", async () => {
        const input = fixtureInput();
        input.granola = [];
        input.calendar[0].source = "google_calendar";
    ...
    (0, node_test_1.it)("does not let source semantics assign biometric Patterns", async () => {
        const input = fixtureInput();
        ...
        assert.strictEqual(delta.nodes.find((node) => node.kind === "ghost" && node.source_ref === "reasoner:ghost-b2b-track")?.title, "B2B/B2C positioning evidence gap");
        assert.ok(delta.assumptions?.some((assumption) => (assumption.key === "pattern_source" &&
            String(assumption.value).includes("never_assign_patterns"))));
    });
    (0, node_test_1.it)("selects visible episodes with a balanced per-pattern cap", async () => {
        const input = fixtureInput();
        ...
        const manifestedCounts = delta.edges
            ...
            if (patternRef)
                acc.set(patternRef, (acc.get(patternRef) ?? 0) + 1);
            return acc;
        }, new Map());
        ...
        assert.ok(delta.assumptions?.some((assumption) => (assumption.key === "selection" &&
            String(assumption.value).includes("max_7_stress_moments_per_week"))));
    });
    (0, node_test_1.it)("is deterministic for repeated input", async () => {
        const triplets = joinTriplets(fixtureInput());
        ...
        assert.deepStrictEqual(first.nodes.map((node) => [node.id, node.kind, node.source_ref]), second.nodes.map((node) => [node.id, node.kind, node.source_ref]));
        assert.deepStrictEqual(first.edges.map((edge) => [edge.id, edge.src_id, edge.dst_id, edge.kind]), second.edges.map((edge) => [edge.id, edge.src_id, edge.dst_id, edge.kind]));
    });
    (0, node_test_1.it)("rejects local identity and scopes UUID user graph ids", async () => {
        const input = fixtureInput();
        assert.throws(() => joinTriplets(input, { userId: "local" }), /user_id is required/);
        assert.throws(() => (0, Reasoner_js_1.rootCauseId)("local"), /user_id is required/);
        ...
        assert.strictEqual(other.armed_root_cause.node_id, (0, Reasoner_js_1.rootCauseId)(OTHER_REASONER_TEST_USER));
    ...
    (0, node_test_1.it)("uses one neutral persistent root identity across WATCHING and armed states for non-local users", async () => {
        ...
        assert.strictEqual(armedRoot.id, (0, Reasoner_js_1.rootCauseId)(userId));
        ...
        assert.notStrictEqual((0, Reasoner_js_1.rootCauseId)("6426A15D-61C3-4C5B-BF19-5CBB7152E9E8"), armedRoot.id);
    });
    (0, node_test_1.it)("rejects empty triplets instead of fabricating a cause", async () => {
        await assert.rejects(() => new Reasoner_js_1.Reasoner().buildDelta({ user_id: REASONER_TEST_USER, triplets: [] }), Reasoner_js_1.NotEnoughSignalError);
    });
    (0, node_test_1.it)("does not fabricate a root when only balanced biometric states exist", async () => {
        const input = fixtureInput();
    ...
    (0, node_test_1.it)("surfaces an early causal result for worn biometrics plus one memory insight", async () => {
        const input = fixtureInput();
        input.biometrics.metrics[0].samples = [
            input.biometrics.metrics[0].samples[0],
            { ...input.biometrics.metrics[0].samples[0], timestamp: "1970-01-01T00:16:55.000Z", graph_source_ref: "healthkit:heart_rate:1970-01-01T00:16:55.000Z" },
    ...
    (0, node_test_1.it)("keeps biometrics-only episodes hidden even with three non-carried samples", async () => {
        const input = fixtureInput();
        input.biometrics.metrics[0].samples = [
            input.biometrics.metrics[0].samples[0],
            { ...input.biometrics.metrics[0].samples[0], timestamp: "1970-01-01T00:16:55.000Z", graph_source_ref: "healthkit:heart_rate:1970-01-01T00:16:55.000Z" },
    ...
    (0, node_test_1.it)("does not count carried samples toward early causal availability", async () => {
        const input = fixtureInput();
        input.biometrics.metrics[0].samples = [
            input.biometrics.metrics[0].samples[0],
            { ...input.biometrics.metrics[0].samples[0], timestamp: "1970-01-01T00:16:55.000Z", graph_source_ref: "healthkit:heart_rate:1970-01-01T00:16:55.000Z" },
    ...
    (0, node_test_1.it)("builds a prompt from deterministic triplet refs", () => {
        const prompt = (0, prompt_js_1.buildReasonerPrompt)({ triplets: joinTriplets(fixtureInput()) });
...
(0, node_test_1.it)("rejects Welcome David when the meeting evidence has no stress-load signal", async () => {
    const input = fixtureInput();
...
(0, node_test_1.it)("drops cross-day post-hoc source rows that start after the biometric episode", async () => {
    const input = fixtureInput();
...
(0, node_test_1.it)("keeps planned Calendar plus Granola context after a bucketed biometric timestamp", async () => {
    const input = fixtureInput();
...
(0, node_test_1.it)("demotes no-summary Granola rows to insufficient evidence without other support", async () => {
    const input = fixtureInput();
...
(0, node_test_1.it)("does not arm or name a root from generic event titles alone", async () => {
    for (const title of ["Event", "Weekly sync - DaoBrew", "AI2 meeting"]) {
        ...
        await assert.rejects(() => new Reasoner_js_1.Reasoner().buildDelta({ user_id: REASONER_TEST_USER, triplets }), Reasoner_js_1.NotEnoughSignalError, `${title} must stay an evidence chip, not a root`);
    }
});
(0, node_test_1.it)("builds a first-class WATCHING delta for insufficient evidence", async () => {
    const input = fixtureInput();
    ...
    const delta = await new Reasoner_js_1.Reasoner().buildWatchingDelta({
        user_id: REASONER_TEST_USER,
        triplets,
        generated_at_ts: 1234,
    ...
    assert.strictEqual(root.props_json?.citation_gate?.has_cited_transcript_span, false);
...
(0, node_test_1.it)("surfaces memory_hit nodes on a WATCHING root when project memory supports the context", async () => {
    const input = fixtureInput();
    input.calendar[0].title = "Weekly sync - DaoBrew";
    input.granola[0].title = "Weekly sync - DaoBrew";
...
(0, node_test_1.it)("uses cited transcript gap language instead of event titles for root text", async () => {
    const input = fixtureInput();
    input.calendar[0].title = "Weekly sync - DaoBrew";
...
(0, node_test_1.it)("infers the deeper B2B/B2C wedge decision instead of the surface value-proposition question", async () => {
    const input = fixtureInput();
    ...
    assert.strictEqual(root.props_json?.citation_gate?.thread_key, "converge-the-story");
    assert.strictEqual(root.props_json?.claim_level, "source_backed_hypothesis_not_settled_causality");
    assert.strictEqual(episode.props_json?.claim_level, "source_backed_hypothesis_not_settled_causality");
    assert.deepStrictEqual(episode.props_json?.evidence_ticks, {
        calendar: true,
...
(0, node_test_1.it)("accepts a same-local-day source-backed hypothesis even when the meeting is hours from the biometric episode", async () => {
    const input = fixtureInput();
    ...
    assert.strictEqual(episode.props_json?.claim_level, "source_backed_hypothesis_not_settled_causality");
    assert.deepStrictEqual((episode.props_json?.supporting_calendar_events).map((event) => event.title), ["AI2 interview"]);
    assert.ok(delta.edges.some((edge) => edge.kind === "manifested" && edge.dst_id === patternId(delta, "overdrive")));
});
(0, node_test_1.it)("does not fabricate a semantic latent root when transcript spans lack Project Memory support", async () => {
    const input = fixtureInput();
    input.calendar[0].title = "AI2 meeting";
    input.granola[0].title = "AI2 meeting";
    input.granola[0].summary = "The summary says the team discussed product positioning.";
...
(0, node_test_1.it)("exposes deterministic latent-hypothesis verifier gates separately from proposal", () => {
    const input = fixtureInput();
    input.calendar[0].title = "AI2 meeting";
    input.granola[0].title = "AI2 meeting";
    input.granola[0].summary = "The summary says the team discussed product positioning.";
    ...
    const proposal = (0, LatentHypothesis_js_1.proposeLatentHypotheses)({
    ...
    assert.ok(proposal);
    const verification = (0, LatentHypothesis_js_1.verifyLatentHypothesis)(proposal, {
        meetings: input.granola,
        memories: input.memory,
        biometric_patterns: ["overdrive"],
        event_titles: input.calendar.map((event) => event.title),
    ...
    const titleOnly = (0, LatentHypothesis_js_1.verifyLatentHypothesis)({
        ...proposal,
        cause: "AI2 meeting",
    }, {
        meetings: input.granola,
    ...
    const clinical = (0, LatentHypothesis_js_1.verifyLatentHypothesis)({
        ...proposal,
...
(0, node_test_1.it)("proposes the causal-model definition thread from action-item style meeting notes", () => {
    const input = fixtureInput();
    ...
    const proposal = (0, LatentHypothesis_js_1.proposeLatentHypotheses)(context)
    ...
    const verification = (0, LatentHypothesis_js_1.verifyLatentHypothesis)(proposal, context);
    assert.strictEqual(verification.accepted, true);
    assert.strictEqual(verification.gates.citation, "pass");
    assert.strictEqual(verification.gates.time_plausibility, "pass");
    assert.strictEqual(verification.gates.recurrence_cross_source, "pass");
...
(0, node_test_1.describe)("time_plausibility filter-not-veto", () => {
    ...
    function meetingFixture(id, occurredAtTs, spanText) {
        return {
            id,
            user_id: REASONER_TEST_USER,
            source: "granola",
    ...
    (0, node_test_1.it)("passes with multi-day evidence and cites only anchor-day spans while recurrence keeps full history", () => {
        const meetings = [
        ...
        const proposal = (0, LatentHypothesis_js_1.proposeLatentHypotheses)(context).find((entry) => entry.thread_key === "converge-the-story");
        assert.ok(proposal);
        assert.strictEqual(proposal.evidence_spans.length, 2);
        assert.strictEqual(proposal.recurrence_weeks, 2);
        const candidate = (0, LatentHypothesis_js_1.detectLatentHypothesis)(context);
    ...
    (0, node_test_1.it)("still vetoes when the anchor day has zero same-day spans", () => {
        ...
        assert.strictEqual((0, LatentHypothesis_js_1.detectLatentHypothesis)(context), null);
    });
    (0, node_test_1.it)("keeps pass-through behavior when anchor_ts is not finite", () => {
        const meetings = [
            meetingFixture("m1", TS_WEEK1, weekOneSpanText),
            meetingFixture("m2", TS_WEEK2, weekTwoSpanText),
        ];
...
(0, node_test_1.it)("defaults to insufficient evidence when biometric state has no eligible source bundle", async () => {
    const input = fixtureInput();
    input.calendar = [];
    input.granola = [];
    input.memory = [];
...
(0, node_test_1.it)("keeps same-event trigger bundles separate when backend biometrics map to different Patterns", async () => {
    const input = fixtureInput();
    ...
    const delta = await new Reasoner_js_1.Reasoner().buildDelta({ user_id: REASONER_TEST_USER, triplets });
    const episodeNodes = delta.nodes.filter((node) => node.kind === "episode");
    const manifestedByEpisode = new Map();
    ...
    assert.ok(episodeNodes.every((node) => {
        const lenses = node.props_json?.pattern_lenses;
