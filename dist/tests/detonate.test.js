...
const assignments_js_1 = require("../src/engine/assignments.js");
/**
...
const SEED_SCRIPT = (0, node_path_1.join)(__dirname, "..", "..", "scripts", "seed-demo-graph.mjs");
...
function writeLicenseConfig(dir, extra) {
...
async function acquireEnvironmentSuite() {
    let release;
...
(0, node_test_1.describe)("detonator integration", { concurrency: 1 }, () => {
    (0, node_test_1.describe)("detonator — mock mode (no graph DB)", () => {
        let tmpDir;
        let releaseEnvironment;
        (0, node_test_1.before)(async () => {
            releaseEnvironment = await acquireEnvironmentSuite();
            tmpDir = (0, node_fs_1.mkdtempSync)((0, node_path_1.join)((0, node_os_1.tmpdir)(), "daobrew-detonate-"));
            process.env.DAOBREW_GRAPH_DB = (0, node_path_1.join)(tmpDir, "missing.db"); // guaranteed absent
            (0, mock_js_1._resetMockDetonator)();
        });
        (0, node_test_1.after)(() => {
        ...
        (0, node_test_1.it)("daobrew_detonate returns the full task package", async () => {
            const result = await (0, tools_js_1.handleToolCall)("daobrew_detonate", {}, true);
        ...
        (0, node_test_1.it)("daobrew_detonate_done closes the mock cause; re-detonate finds nothing", async () => {
            const done = await (0, tools_js_1.handleToolCall)("daobrew_detonate_done", { cause_id: "ghost_ai2_demo_story", artifact_ref: "ai2-demo-story.md" }, true);
            assert.match(done.content[0].text, /completed \(mock\)/);
            const again = await (0, tools_js_1.handleToolCall)("daobrew_detonate", {}, true);
            assert.match(again.content[0].text, /No ready Sentinel action package/);
        });
        (0, node_test_1.it)("daobrew_detonate without mock and without a license returns payment guide", async () => {
            process.env.DAOBREW_CONFIG_FILE = (0, node_path_1.join)(tmpDir, "missing-config.json");
            const result = await (0, tools_js_1.handleToolCall)("daobrew_detonate", {}, false);
        ...
        (0, node_test_1.it)("daobrew_detonate_done validates required args", async () => {
            await assert.rejects(() => (0, tools_js_1.handleToolCall)("daobrew_detonate_done", { cause_id: "x" }, true), /requires cause_id and artifact_ref/);
        });
        (0, node_test_1.it)("daobrew_schedule_block guards non-macOS platforms", async (t) => {
            if (process.platform === "darwin")
                return t.skip("macOS — would write a real calendar event");
            const result = await (0, tools_js_1.handleToolCall)("daobrew_schedule_block", { title: "Test block" }, true);
            assert.strictEqual(result.status, "unsupported_platform");
        });
    });
    (0, node_test_1.describe)("detonate_done — user_acceptance outcome close", () => {
        let tmpDir;
        let releaseEnvironment;
        (0, node_test_1.before)(async () => {
            releaseEnvironment = await acquireEnvironmentSuite();
            tmpDir = (0, node_fs_1.mkdtempSync)((0, node_path_1.join)((0, node_os_1.tmpdir)(), "daobrew-acceptance-"));
            process.env.DAOBREW_GRAPH_DB = (0, node_path_1.join)(tmpDir, "missing.db"); // guaranteed absent
            (0, mock_js_1._resetMockDetonator)();
        });
        (0, node_test_1.after)(() => {
        ...
        (0, node_test_1.it)("tool schema exposes optional user_acceptance without changing required args", () => {
            const done = tools_js_1.toolDefinitions.find((tool) => tool.name === "daobrew_detonate_done");
            assert.ok(done);
            const schema = done.inputSchema;
        ...
        (0, node_test_1.it)("the brief's close-loop line asks for the optional user_acceptance", async () => {
            const result = await (0, tools_js_1.handleToolCall)("daobrew_detonate", {}, true);
            assert.match(result.content[0].text, /user_acceptance: "accepted" \| "edited" \| "rejected"/);
        });
        (0, node_test_1.it)("rejects an invalid user_acceptance value at the tool boundary", async () => {
            await assert.rejects(() => (0, tools_js_1.handleToolCall)("daobrew_detonate_done", { cause_id: "x", artifact_ref: "y", user_acceptance: "sideways" }, true), /accepted.*edited.*rejected/s);
        });
        (0, node_test_1.it)("mock close with user_acceptance stays backward compatible", async () => {
            const done = await (0, tools_js_1.handleToolCall)("daobrew_detonate_done", { cause_id: "ghost_ai2_demo_story", artifact_ref: "ai2-demo-story.md", user_acceptance: "accepted" }, true);
            assert.match(done.content[0].text, /completed \(mock\)/);
        });
    });
    (0, node_test_1.describe)("intervention assignment log (thin MRT contracts)", () => {
        ...
        function fakeExec(log) {
            return async (sql) => { log.push(sql); };
        }
        (0, node_test_1.it)("assignmentId is deterministic: ivn_ prefix + 24 hex, stable across calls", () => {
            const a = (0, assignments_js_1.assignmentId)(base);
            const b = (0, assignments_js_1.assignmentId)(base);
            assert.match(a, /^ivn_[0-9a-f]{24}$/);
            assert.strictEqual(a, b);
            assert.notStrictEqual(a, (0, assignments_js_1.assignmentId)({ ...base, ghostId: "ghost_y" }));
            assert.notStrictEqual(a, (0, assignments_js_1.assignmentId)({ ...base, userId: "other" }));
            assert.notStrictEqual(a, (0, assignments_js_1.assignmentId)({ ...base, armedAtTs: 1_750_000_001 }));
        });
        (0, node_test_1.it)("episode key prefers armed_at_ts over created_at_ts (legacy fallback only)", () => {
            ...
            assert.strictEqual((0, assignments_js_1.armedEpisodeTs)({ armed_at_ts: 111 }, 100), 111);
            assert.strictEqual((0, assignments_js_1.armedEpisodeTs)({}, 100), 100);
            assert.strictEqual((0, assignments_js_1.armedEpisodeTs)({ armed_at_ts: "junk" }, 100), 100);
            assert.strictEqual((0, assignments_js_1.armedEpisodeTs)({}, null), null);
            // Retry within one arming → SAME id; re-arm (fresh stamp) → NEW id.
            const key = (props) => (0, assignments_js_1.assignmentId)({ userId: DETONATE_TEST_USER, ghostId: "ghost_x", armedAtTs: (0, assignments_js_1.armedEpisodeTs)(props, 100) });
            assert.strictEqual(key({ armed_at_ts: 111 }), key({ armed_at_ts: 111 }));
            assert.notStrictEqual(key({ armed_at_ts: 111 }), key({ armed_at_ts: 222 }));
        });
        (0, node_test_1.it)("logs one idempotent INSERT carrying the full assignment contract (postgres)", async () => {
            const log = [];
            const result = await (0, assignments_js_1.logInterventionAssignment)({
            ...
            assert.ok(sql.includes((0, assignments_js_1.assignmentId)(base)));
        ...
        (0, node_test_1.it)("missing vocabulary props become NULL columns; the insert still happens", async () => {
            const log = [];
            const result = await (0, assignments_js_1.logInterventionAssignment)({
        ...
        (0, node_test_1.it)("sqlite store skips assignment logging entirely (warm-tier is postgres-only)", async () => {
            const log = [];
            const result = await (0, assignments_js_1.logInterventionAssignment)({
        ...
        (0, node_test_1.it)("a failing exec resolves with a warning — never rejects (fail-soft)", async () => {
            ...
            const closed = await (0, assignments_js_1.closeInterventionAssignment)({
        ...
        (0, node_test_1.it)("close stamps artifact_done + closed_at_ts, and user_acceptance only when given", async () => {
            const log = [];
            ...
            assert.ok(log[0].includes(`WHERE id = '${(0, assignments_js_1.assignmentId)(base)}'`));
            const bare = [];
            await (0, assignments_js_1.closeInterventionAssignment)({
            ...
            const skipped = [];
            const result = await (0, assignments_js_1.closeInterventionAssignment)({
    ...
    (0, node_test_1.describe)("detonator — seeded SQLite round-trip", () => {
        ...
        (0, node_test_1.before)(async () => {
            releaseEnvironment = await acquireEnvironmentSuite();
            tmpDir = (0, node_fs_1.mkdtempSync)((0, node_path_1.join)((0, node_os_1.tmpdir)(), "daobrew-graph-"));
            dbPath = (0, node_path_1.join)(tmpDir, "sentinel-graph.db");
        ...
        (0, node_test_1.it)("seed → detonate → done → re-detonate is a closed, idempotent loop", async (t) => {
            if (!hasCli)
            ...
            (0, node_child_process_1.execFileSync)(process.execPath, [SEED_SCRIPT], { env: { ...process.env, DAOBREW_GRAPH_DB: dbPath } });
            assert.ok((0, node_fs_1.existsSync)(dbPath), "seed script should create the DB");
            // Re-seed must not duplicate or fail (INSERT OR IGNORE / REPLACE)
            (0, node_child_process_1.execFileSync)(process.execPath, [SEED_SCRIPT], { env: { ...process.env, DAOBREW_GRAPH_DB: dbPath } });
            const brief = await (0, tools_js_1.handleToolCall)("daobrew_detonate", {}, false);
            ...
            const done = await (0, tools_js_1.handleToolCall)("daobrew_detonate_done", { cause_id: "ghost_ai2_demo_story", artifact_ref: "/tmp/ai2-demo-story.md", user_acceptance: "edited" }, false);
            assert.match(done.content[0].text, /Sentinel action package completed/);
            // Optional Y_primary signal persists into ghost props on BOTH stores.
            const acceptance = (0, node_child_process_1.execFileSync)("sqlite3", [dbPath, "SELECT json_extract(props_json,'$.user_acceptance') FROM graph_nodes WHERE id='ghost_ai2_demo_story'"], { encoding: "utf-8" }).trim();
            assert.strictEqual(acceptance, "edited");
            const status = (0, node_child_process_1.execFileSync)("sqlite3", [dbPath, "SELECT json_extract(props_json,'$.status') || '|' || json_extract(props_json,'$.artifact_ref') FROM graph_nodes WHERE id='ghost_ai2_demo_story'"], { encoding: "utf-8" }).trim();
            ...
            const doneAt = (0, node_child_process_1.execFileSync)("sqlite3", [dbPath, "SELECT json_extract(props_json,'$.done_at_ts') FROM graph_nodes WHERE id='ghost_ai2_demo_story'"], { encoding: "utf-8" }).trim();
            ...
            const causedEdges = (0, node_child_process_1.execFileSync)("sqlite3", [dbPath, "SELECT count(*) FROM graph_edges WHERE dst_id='ghost_ai2_demo_story' AND kind='caused'"], { encoding: "utf-8" }).trim();
            assert.strictEqual(causedEdges, "1");
            const again = await (0, tools_js_1.handleToolCall)("daobrew_detonate", {}, false);
            assert.match(again.content[0].text, /No ready Sentinel action package/);
            // Re-seed re-arms the ghost and clears the completion edge (fresh video take)
            (0, node_child_process_1.execFileSync)(process.execPath, [SEED_SCRIPT], { env: { ...process.env, DAOBREW_GRAPH_DB: dbPath } });
            const rearmed = await (0, tools_js_1.handleToolCall)("daobrew_detonate", {}, false);
            assert.match(rearmed.content[0].text, /cause_id: ghost_ai2_demo_story/);
            // Backward compat on disk: closing WITHOUT user_acceptance stamps no key.
            await (0, tools_js_1.handleToolCall)("daobrew_detonate_done", { cause_id: "ghost_ai2_demo_story", artifact_ref: "/tmp/ai2-demo-story-take2.md" }, false);
            const bare = (0, node_child_process_1.execFileSync)("sqlite3", [dbPath, "SELECT COALESCE(json_extract(props_json,'$.user_acceptance'),'ABSENT') FROM graph_nodes WHERE id='ghost_ai2_demo_story'"], { encoding: "utf-8" }).trim();
    ...
    async function withFakedGraphTransport(opts, fn) {
        const mod = graphDb;
        ...
        const tmp = (0, node_fs_1.mkdtempSync)((0, node_path_1.join)((0, node_os_1.tmpdir)(), "daobrew-pg-scope-"));
        const captured = { queries: [], execs: [] };
        ...
        try {
            ...
            process.env.DAOBREW_CONFIG_FILE = writeLicenseConfig(tmp, opts.configUserId === undefined ? undefined : { user_id: opts.configUserId });
            if (opts.envUserId === undefined)
                delete process.env.DAOBREW_USER_ID;
            else
                process.env.DAOBREW_USER_ID = opts.envUserId;
    ...
    function assertScopedQueryOnce(sql, userId, label) {
        ...
        assert.strictEqual(occurrences(sql, `set_config('${GUC}', '${userId}', true)`), 1, `${label}: must carry the GUC for '${userId}' exactly once, got: ${sql}`);
        assert.strictEqual(occurrences(sql, "set_config"), 1, `${label}: set_config must appear exactly once per statement batch, got: ${sql}`);
    }
    function assertScopedExecOnce(sql, userId, label) {
        ...
        assert.strictEqual(occurrences(sql, `set_config('${GUC}', '${userId}', true)`), 1, `${label}: must carry the transaction-local GUC for '${userId}' exactly once, got: ${sql}`);
        assert.strictEqual(occurrences(sql, "set_config"), 1, `${label}: set_config must appear exactly once per statement batch, got: ${sql}`);
    }
    (0, node_test_1.describe)("tool-layer identity (toolUserId)", () => {
        let tmpDir;
        const savedEnv = { user: undefined, config: undefined };
        let releaseEnvironment;
        (0, node_test_1.before)(async () => {
            releaseEnvironment = await acquireEnvironmentSuite();
            tmpDir = (0, node_fs_1.mkdtempSync)((0, node_path_1.join)((0, node_os_1.tmpdir)(), "daobrew-tool-id-"));
        ...
        (0, node_test_1.after)(() => {
            if (savedEnv.user === undefined)
                ...
                process.env.DAOBREW_USER_ID = savedEnv.user;
            if (savedEnv.config === undefined)
                delete process.env.DAOBREW_CONFIG_FILE;
            else
                process.env.DAOBREW_CONFIG_FILE = savedEnv.config;
            (0, node_fs_1.rmSync)(tmpDir, { recursive: true, force: true });
            releaseEnvironment();
        });
        (0, node_test_1.it)("resolves env > config user_id and fails closed without a canonical UUID", () => {
            // Nothing set anywhere → fail closed.
            delete process.env.DAOBREW_USER_ID;
            process.env.DAOBREW_CONFIG_FILE = (0, node_path_1.join)(tmpDir, "missing-config.json");
            assert.strictEqual(process.env.DAOBREW_USER_ID, undefined);
            assert.throws(() => (0, tools_js_1.toolUserId)(), /no canonical UUID identity/);
            // Config user_id is the fallback when the env var is absent.
            const configPath = (0, node_path_1.join)(tmpDir, "config.json");
            (0, node_fs_1.writeFileSync)(configPath, JSON.stringify({ user_id: DETONATE_TEST_USER.toLowerCase() }));
            process.env.DAOBREW_CONFIG_FILE = configPath;
            assert.strictEqual((0, tools_js_1.toolUserId)(), DETONATE_TEST_USER);
            ...
            assert.strictEqual((0, tools_js_1.toolUserId)(), DETONATE_ENV_USER);
            process.env.DAOBREW_USER_ID = "   ";
            assert.throws(() => (0, tools_js_1.toolUserId)(), /no canonical UUID identity/);
            // Blank config user_id also means unset → fail closed.
            delete process.env.DAOBREW_USER_ID;
            (0, node_fs_1.writeFileSync)(configPath, JSON.stringify({ user_id: "   " }));
            assert.throws(() => (0, tools_js_1.toolUserId)(), /no canonical UUID identity/);
        });
    });
    (0, node_test_1.describe)("detonator — postgres tool-layer scoping (faked transport)", () => {
        let releaseEnvironment;
        (0, node_test_1.before)(async () => {
            releaseEnvironment = await acquireEnvironmentSuite();
        });
        ...
        (0, node_test_1.it)("detonate scopes the armed-ghost SELECT and the assignment INSERT with the config user", async () => {
            await withFakedGraphTransport({ rows: [ARMED_GHOST_ROW], store: "postgres", configUserId: DETONATE_TEST_USER }, async (captured) => {
                const result = await (0, tools_js_1.handleToolCall)("daobrew_detonate", {}, false);
                assert.match(result.content[0].text, /cause_id: ghost_pg_scope/);
                ...
                assert.ok(captured.execs[0].includes(`'${DETONATE_ROW_USER}'`), `assignment payload user_id must still come from rows[0].user_id, got: ${captured.execs[0]}`);
            });
        });
        (0, node_test_1.it)("DAOBREW_USER_ID env overrides the config user for scoping", async () => {
            await withFakedGraphTransport({ rows: [ARMED_GHOST_ROW], store: "postgres", configUserId: DETONATE_TEST_USER, envUserId: DETONATE_ENV_USER }, async (captured) => {
                await (0, tools_js_1.handleToolCall)("daobrew_detonate", {}, false);
        ...
        (0, node_test_1.it)("detonate_done scopes the SELECT, the UPDATE/edge-INSERT batch, and the outcome close — one GUC each", async () => {
            await withFakedGraphTransport({ rows: [ARMED_GHOST_ROW], store: "postgres", configUserId: DETONATE_TEST_USER }, async (captured) => {
                const done = await (0, tools_js_1.handleToolCall)("daobrew_detonate_done", { cause_id: "ghost_pg_scope", artifact_ref: "/tmp/pg-scope.md", user_acceptance: "accepted" }, false);
                assert.match(done.content[0].text, /Sentinel action package completed/);
        ...
        (0, node_test_1.it)("sqlite store composes byte-identical unscoped SQL — no set_config anywhere", async () => {
            await withFakedGraphTransport({ rows: [ARMED_GHOST_ROW], store: "sqlite", configUserId: DETONATE_TEST_USER, envUserId: DETONATE_ENV_USER }, async (captured) => {
                await (0, tools_js_1.handleToolCall)("daobrew_detonate", {}, false);
                await (0, tools_js_1.handleToolCall)("daobrew_detonate_done", { cause_id: "ghost_pg_scope", artifact_ref: "/tmp/pg-scope.md" }, false);
                ...
                assert.ok(captured.queries[0].trimStart().startsWith("SELECT id, user_id, created_at_ts, title, props_json FROM graph_nodes"), `sqlite detonate SELECT must be the raw statement, got: ${captured.queries[0]}`);
