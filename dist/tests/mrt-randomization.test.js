...
const development_tools_js_1 = require("../src/development-tools.js");
...
(0, node_test_1.describe)("drawOfferArm (hash-deterministic episode draw)", () => {
    (0, node_test_1.it)("is deterministic: same episode key always draws the same arm and value", () => {
        const a = (0, assignments_js_1.drawOfferArm)(EPISODE.userId, EPISODE.ghostId, EPISODE.armedAtTs, 0.9);
        const b = (0, assignments_js_1.drawOfferArm)(EPISODE.userId, EPISODE.ghostId, EPISODE.armedAtTs, 0.9);
        assert.deepStrictEqual(a, b);
        assert.ok(a.draw >= 0 && a.draw < 1, "draw must be uniform in [0,1)");
    });
    (0, node_test_1.it)("changes the draw when any episode key component changes", () => {
        const base = (0, assignments_js_1.drawOfferArm)(EPISODE.userId, EPISODE.ghostId, EPISODE.armedAtTs, 0.9);
        const byUser = (0, assignments_js_1.drawOfferArm)("other", EPISODE.ghostId, EPISODE.armedAtTs, 0.9);
        const byGhost = (0, assignments_js_1.drawOfferArm)(EPISODE.userId, "ghost_y", EPISODE.armedAtTs, 0.9);
        const byEpisode = (0, assignments_js_1.drawOfferArm)(EPISODE.userId, EPISODE.ghostId, EPISODE.armedAtTs + 1, 0.9);
    ...
    (0, node_test_1.it)("p=1 always assigns treatment; p=0 always assigns control (draw < p on [0,1))", () => {
        for (let i = 0; i < 50; i += 1) {
            const treated = (0, assignments_js_1.drawOfferArm)("u", `ghost_${i}`, 1_000 + i, 1);
            assert.strictEqual(treated.arm, "task_package");
            assert.strictEqual(treated.assignmentProbability, 1);
            const held = (0, assignments_js_1.drawOfferArm)("u", `ghost_${i}`, 1_000 + i, 0);
            assert.strictEqual(held.arm, "hold_noop");
            assert.strictEqual(held.assignmentProbability, 1);
        }
    });
    (0, node_test_1.it)("clamps p into [0,1] and records the ASSIGNED arm's probability", () => {
        const over = (0, assignments_js_1.drawOfferArm)(EPISODE.userId, EPISODE.ghostId, EPISODE.armedAtTs, 1.5);
        assert.strictEqual(over.treatmentProbability, 1);
        assert.strictEqual(over.arm, "task_package");
        const under = (0, assignments_js_1.drawOfferArm)(EPISODE.userId, EPISODE.ghostId, EPISODE.armedAtTs, -0.2);
        ...
        for (let i = 0; i < 200; i += 1) {
            const draw = (0, assignments_js_1.drawOfferArm)("dist-user", `ghost_${i}`, 42, p);
            assert.strictEqual(draw.assignmentProbability, draw.arm === "task_package" ? 0.9 : 0.1);
        }
    });
    (0, node_test_1.it)("distribution sanity on fixed fixtures: p=0.9 holds roughly 10% of 400 episodes", () => {
        let held = 0;
        for (let i = 0; i < 400; i += 1) {
            if ((0, assignments_js_1.drawOfferArm)("dist-user", `ghost_${i}`, 42, 0.9).arm === "hold_noop")
                held += 1;
        }
    ...
    (0, node_test_1.it)("null armedAtTs keys the draw exactly like assignmentId does (legacy episode)", () => {
        const a = (0, assignments_js_1.drawOfferArm)("u", "g", null, 0.5);
        const b = (0, assignments_js_1.drawOfferArm)("u", "g", null, 0.5);
        assert.strictEqual(a.draw, b.draw);
        assert.notStrictEqual(a.draw, (0, assignments_js_1.drawOfferArm)("u", "g", 0, 0.5).draw);
    });
});
(0, node_test_1.describe)("mrtTreatmentP (env DAOBREW_MRT_TREATMENT_P)", () => {
    (0, node_test_1.it)("defaults to 0.9 when unset, blank, or junk", () => {
        assert.strictEqual(assignments_js_1.DEFAULT_MRT_TREATMENT_P, 0.9);
        assert.strictEqual((0, assignments_js_1.mrtTreatmentP)(undefined), 0.9);
        assert.strictEqual((0, assignments_js_1.mrtTreatmentP)("   "), 0.9);
        assert.strictEqual((0, assignments_js_1.mrtTreatmentP)("not-a-number"), 0.9);
    });
    (0, node_test_1.it)("parses and clamps to [0,1]", () => {
        assert.strictEqual((0, assignments_js_1.mrtTreatmentP)("0.5"), 0.5);
        assert.strictEqual((0, assignments_js_1.mrtTreatmentP)("0"), 0);
        assert.strictEqual((0, assignments_js_1.mrtTreatmentP)("1"), 1);
        assert.strictEqual((0, assignments_js_1.mrtTreatmentP)("1.7"), 1);
        assert.strictEqual((0, assignments_js_1.mrtTreatmentP)("-3"), 0);
    });
    (0, node_test_1.it)("reads process.env.DAOBREW_MRT_TREATMENT_P by default", () => {
        const saved = process.env.DAOBREW_MRT_TREATMENT_P;
        try {
            process.env.DAOBREW_MRT_TREATMENT_P = "0.25";
            assert.strictEqual((0, assignments_js_1.mrtTreatmentP)(), 0.25);
            ...
            if (saved === undefined)
                ...
                process.env.DAOBREW_MRT_TREATMENT_P = saved;
        }
    });
});
(0, node_test_1.describe)("prepareArmedOffer (atomic graph-generation decision)", () => {
    (0, node_test_1.it)("derives offer_state and assignment SQL from one exact episode draw", () => {
        const prepared = (0, assignments_js_1.prepareArmedOffer)({
        ...
        const sql = (0, assignments_js_1.interventionAssignmentSql)({
            ...EPISODE,
            ghostProps: prepared.ghostProps,
            nowTs: EPISODE.armedAtTs + 1,
            assignment: prepared.assignment,
        });
        assert.ok(sql.includes((0, assignments_js_1.assignmentId)(EPISODE)));
        assert.match(sql, /'hold_noop', 1, 'randomized_offer_v1'/);
        assert.match(sql, /ON CONFLICT \(id\) DO NOTHING/);
    });
});
(0, node_test_1.describe)("randomized assignment logging (route_policy=randomized_offer_v1)", () => {
    ...
    (0, node_test_1.it)("logs a held (control) assignment carrying both eligible arms and the assigned arm's probability", async () => {
        const log = [];
        const result = await (0, assignments_js_1.logInterventionAssignment)({
            ...
            assignment: {
                ...
                routePolicy: assignments_js_1.RANDOMIZED_ROUTE_POLICY,
                eligibleActions: assignments_js_1.OFFER_ELIGIBLE_ACTIONS,
            },
        });
        assert.strictEqual(result.logged, true);
        assert.strictEqual(log.length, 1);
        const sql = log[0];
        assert.ok(sql.includes((0, assignments_js_1.assignmentId)(EPISODE)), "same deterministic episode id as detonate");
        assert.match(sql, /'\["task_package","hold_noop"\]'::jsonb/);
        assert.match(sql, /'hold_noop', 0\.1, 'randomized_offer_v1'/);
        assert.match(sql, /ON CONFLICT \(id\) DO NOTHING/);
    });
    (0, node_test_1.it)("logs a treatment assignment with probability p under the randomized policy", async () => {
        const log = [];
    ...
    (0, node_test_1.it)("without an assignment override the deterministic detonate contract is byte-identical", async () => {
        const log = [];
        await (0, assignments_js_1.logInterventionAssignment)({
...
(0, node_test_1.describe)("assignArmedOffer (arming-time randomization)", () => {
    const USER = MRT_TEST_USER;
    const GHOST = "ghost_armed_root";
    const ARMED_AT = 1_750_000_100;
    const armedRow = (props = {}) => ({
        id: GHOST,
        user_id: USER,
    ...
    (0, node_test_1.it)("p=1 assigns treatment: logs task_package and stamps offer_state='offered'", async () => {
        const h = harness([armedRow()]);
        const result = await (0, assignments_js_1.assignArmedOffer)({
        ...
        assert.ok(insert.includes((0, assignments_js_1.assignmentId)({ userId: USER, ghostId: GHOST, armedAtTs: ARMED_AT })), "episode id must match what detonate/detonate_done derive");
    ...
    (0, node_test_1.it)("the armed-root read filters user_id by the scope row, never by a literal (F1 One-Time-Filter regression)", async () => {
        ...
        const h = harness([armedRow()]);
        await (0, assignments_js_1.assignArmedOffer)({
            userId: USER, ghostId: GHOST, storeKind: "postgres",
            exec: h.exec, query: h.query, nowTs: 1_750_000_200, treatmentP: 1,
        });
        assert.strictEqual(h.queries.length, 1);
        assert.ok(h.queries[0].includes("user_id = __daobrew_scope.uid"), `armed-root read must filter user_id via the scope row, got: ${h.queries[0]}`);
        assert.ok(!h.queries[0].includes(`user_id = '${USER}'`), `armed-root read must NOT compare user_id to a literal, got: ${h.queries[0]}`);
    });
    (0, node_test_1.it)("p=0 assigns control: logs hold_noop and stamps offer_state='held'", async () => {
        const h = harness([armedRow()]);
        const result = await (0, assignments_js_1.assignArmedOffer)({
            userId: USER, ghostId: GHOST, storeKind: "postgres",
    ...
    (0, node_test_1.it)("re-runs are idempotent: same episode re-draws the same arm with identical SQL", async () => {
        const deps = (h) => ({
            userId: USER, ghostId: GHOST, storeKind: "postgres",
        ...
        const a = await (0, assignments_js_1.assignArmedOffer)(deps(first));
        const b = await (0, assignments_js_1.assignArmedOffer)(deps(second));
        assert.strictEqual(a.offerState, b.offerState);
        assert.deepStrictEqual(first.execs, second.execs);
    });
    (0, node_test_1.it)("skips on non-postgres stores exactly like assignment logging does", async () => {
        const h = harness([armedRow()]);
        const result = await (0, assignments_js_1.assignArmedOffer)({
    ...
    (0, node_test_1.it)("skips when the root is missing or no longer armed", async () => {
        const missing = harness([]);
        const gone = await (0, assignments_js_1.assignArmedOffer)({
            userId: USER, ghostId: GHOST, storeKind: "postgres",
        ...
        const closed = await (0, assignments_js_1.assignArmedOffer)({
            userId: USER, ghostId: GHOST, storeKind: "postgres",
    ...
    (0, node_test_1.it)("is fail-soft: a throwing transport resolves with a warning, never rejects", async () => {
        ...
        const h = harness([armedRow()]);
        const partial = await (0, assignments_js_1.assignArmedOffer)({
            userId: USER, ghostId: GHOST, storeKind: "postgres",
    ...
    (0, node_test_1.it)("legacy stampless episode keys on created_at_ts, matching the detonate-side key", async () => {
        const row = armedRow();
        delete row.props_json.armed_at_ts;
        const h = harness([row]);
        const result = await (0, assignments_js_1.assignArmedOffer)({
            userId: USER, ghostId: GHOST, storeKind: "postgres",
        ...
        assert.ok(h.execs[0].includes((0, assignments_js_1.assignmentId)({ userId: USER, ghostId: GHOST, armedAtTs: 1_750_000_000 })));
    });
});
async function withFakedGraphTransport(rows, fn) {
    const mod = graphDb;
...
(0, node_test_1.describe)("daobrew_status CTA suppression (MRT control arm)", () => {
    (0, node_test_1.it)("surfaces a ready action package with a detonate CTA for an offered root", async () => {
        await withFakedGraphTransport([OFFERED_GHOST_ROW], async () => {
            const result = await (0, development_tools_js_1.handleToolCall)("daobrew_status", {}, true);
    ...
    (0, node_test_1.it)("legacy armed roots without offer_state keep the CTA (pre-MRT episodes)", async () => {
        const legacy = { ...OFFERED_GHOST_ROW, id: "ghost_legacy_cta", props_json: { ...OFFERED_GHOST_ROW.props_json } };
        delete legacy.props_json.offer_state;
        await withFakedGraphTransport([legacy], async () => {
            const result = await (0, development_tools_js_1.handleToolCall)("daobrew_status", {}, true);
            assert.strictEqual(result.sentinel_action_package?.state, "ready");
        });
    });
    (0, node_test_1.it)("suppresses the proactive CTA for a held root: watching info only, no detonate mention", async () => {
    ...
    (0, node_test_1.it)("reports null when no root is armed, and stays fail-soft on a broken graph read", async () => {
...
(0, node_test_1.describe)("daobrew_detonate on a held episode (intent-to-treat override)", () => {
    (0, node_test_1.it)("is ALWAYS honored: serves the brief and re-issues the idempotent assignment INSERT", async () => {
        await withFakedGraphTransport([HELD_GHOST_ROW], async (captured) => {
            ...
            const result = await (0, development_tools_js_1.handleToolCall)("daobrew_detonate", {}, true);
            assert.ok(result.__mcp_content__, "the brief must be served despite offer_state='held'");
            assert.match(result.content[0].text, /cause_id: ghost_held_cta/);
            ...
            assert.match(insert, /INSERT INTO intervention_assignments/);
            assert.match(insert, /ON CONFLICT \(id\) DO NOTHING/);
            const episodeId = (0, assignments_js_1.assignmentId)({ userId: MRT_TEST_USER, ghostId: "ghost_held_cta", armedAtTs: 1_750_000_100 });
            assert.ok(insert.includes(episodeId), "same deterministic episode id as the arming-time hold_noop row");
        });
    });
    (0, node_test_1.it)("detonate_done on the held episode closes the SAME assignment row (artifact_done marks the override)", async () => {
        await withFakedGraphTransport([HELD_GHOST_ROW], async (captured) => {
            const done = await (0, development_tools_js_1.handleToolCall)("daobrew_detonate_done", { cause_id: "ghost_held_cta", artifact_ref: "/tmp/held-override.md", user_acceptance: "accepted" }, true);
