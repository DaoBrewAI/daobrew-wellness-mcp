...
const insight_lifecycle_js_1 = require("../src/engine/ingest/insight-lifecycle.js");
function makeRow(id, sourceRef, overrides = {}) {
    return {
        id,
...
(0, node_test_1.describe)("planInsightLifecycle", () => {
    (0, node_test_1.it)("replays an exact-content add decision without calling the llm", async () => {
        const row = makeRow("insight-1", "/sessions/a.jsonl");
        const plan = await (0, insight_lifecycle_js_1.planInsightLifecycle)({
            userId: "u1",
            rows: [row],
            query: async (sql) => {
                if (/FROM insight_lifecycle_decisions/.test(sql)) {
                    return [{
                            ...
                            incoming_content_hash: (0, insight_lifecycle_js_1.insightContentHash)(row),
                            decision_kind: "add",
    ...
    (0, node_test_1.it)("replays a noop-duplicate decision by suppressing the incoming row and reinforcing the winner", async () => {
        ...
        assert.equal(plan.rows[0].strength, insight_lifecycle_js_1.SUPPRESSED_INSIGHT_STRENGTH);
    ...
    (0, node_test_1.it)("classifies add, supersedes, and noop_duplicate in one llm batch and persists replay rows", async () => {
            ...
            decisions: [
                {
                    ...
                    incoming_content_hash: (0, insight_lifecycle_js_1.insightContentHash)(addRow),
                    decision_kind: "add",
                    target_id: null,
                    importance: 0.91,
                    rationale: "new fact",
                },
                {
                    ...
                    incoming_content_hash: (0, insight_lifecycle_js_1.insightContentHash)(supersedeRow),
                ...
                },
                {
                    ...
                    incoming_content_hash: (0, insight_lifecycle_js_1.insightContentHash)(noopRow),
                    decision_kind: "noop_duplicate",
                    target_id: "winner-2",
                    importance: 0.63,
                    rationale: "same fact",
        ...
        const plan = await (0, insight_lifecycle_js_1.planInsightLifecycle)({
            ...
            query: async (sql) => {
                ...
                if (/ORDER BY created_at_ts DESC/.test(sql))
                    return [];
        ...
        assert.equal(plan.rows[2].strength, insight_lifecycle_js_1.SUPPRESSED_INSIGHT_STRENGTH);
        assert.deepEqual(plan.decisionCounts, {
            add: 1,
            supersedes: 1,
            noopDuplicate: 1,
    ...
    (0, node_test_1.it)("fails open with one warning when the llm payload is malformed", async () => {
    ...
    (0, node_test_1.it)("caps new classifications by content hash budget and lets overflow rows fall through unchanged", async () => {
            ...
            decisions: [
                {
                    ...
                    incoming_content_hash: (0, insight_lifecycle_js_1.insightContentHash)(first),
                    decision_kind: "add",
                    target_id: null,
                    importance: 0.95,
        ...
        const plan = await (0, insight_lifecycle_js_1.planInsightLifecycle)({
            ...
            query: async (sql) => {
                if (/FROM insight_lifecycle_decisions/.test(sql))
                    return [];
                if (/ORDER BY embedding <=>/.test(sql) || /ORDER BY created_at_ts DESC/.test(sql))
                    return [];
                return [];
            },
            storeKind: () => "postgres",
    ...
    (0, node_test_1.it)("replays prior lifecycle decisions on the next ingest cycle without a second llm call", async () => {
        const addRow = makeRow("incoming-add", "/sessions/add.jsonl");
        ...
        const firstCycle = await (0, insight_lifecycle_js_1.planInsightLifecycle)({
            userId: "u1",
            rows: [addRow, noopRow],
            llm,
            embedRows: async (texts) => texts.map((_text, index) => new Array(768).fill(index / 10)),
        ...
        const secondCycle = await (0, insight_lifecycle_js_1.planInsightLifecycle)({
            userId: "u1",
            rows: [addRow, noopRow],
            query: async (sql) => {
                if (/FROM insight_lifecycle_decisions/.test(sql)) {
        ...
        assert.equal(secondCycle.rows[1].strength, insight_lifecycle_js_1.SUPPRESSED_INSIGHT_STRENGTH);
    ...
    (0, node_test_1.it)("excludes the incoming natural-key row from semantic lifecycle candidates", async () => {
    ...
    (0, node_test_1.it)("discards partial current-batch ownership when a later lifecycle decision fails open", async () => {
        ...
        const { llm } = stubLlm({
            decisions: [
                ...
                },
                {
                    ...
                    incoming_content_hash: (0, insight_lifecycle_js_1.insightContentHash)(second),
                    decision_kind: "noop_duplicate",
