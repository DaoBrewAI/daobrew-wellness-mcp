...
(0, node_test_1.describe)("e2e mock-mode flow (6-tool surface)", () => {
    ...
    (0, node_test_1.it)("lists all 10 tool definitions (wellness 7 + detonator 3)", () => {
        assert.strictEqual(development_tools_js_1.toolDefinitions.length, 10);
        const names = development_tools_js_1.toolDefinitions.map(t => t.name);
        ...
        assert.ok(names.includes("daobrew_detonate"));
        assert.ok(names.includes("daobrew_schedule_block"));
        assert.ok(names.includes("daobrew_detonate_done"));
    });
    (0, node_test_1.it)("daobrew_check returns content blocks (text-only when no session)", async () => {
        (0, session_js_1.clearSession)();
        const result = await (0, development_tools_js_1.handleToolCall)("daobrew_check", { force_refresh: true }, true);
    ...
    (0, node_test_1.it)("daobrew_breathe rejects ambient without optin", async () => {
        ...
        const result = await (0, development_tools_js_1.handleToolCall)("daobrew_breathe", {
    ...
    (0, node_test_1.it)("daobrew_stop returns no_active_session when idle", async () => {
        (0, session_js_1.clearSession)();
        const result = await (0, development_tools_js_1.handleToolCall)("daobrew_stop", {}, true);
    ...
    (0, node_test_1.it)("daobrew_settings updates preferences", async () => {
        const result = await (0, development_tools_js_1.handleToolCall)("daobrew_settings", {
    ...
    (0, node_test_1.it)("daobrew_connect_watch returns mock_mode in mock", async () => {
        const result = await (0, development_tools_js_1.handleToolCall)("daobrew_connect_watch", {}, true);
    ...
    (0, node_test_1.it)("daobrew_history returns entries", async () => {
        const result = await (0, development_tools_js_1.handleToolCall)("daobrew_history", { days: 7 }, true);
        assert.ok(Array.isArray(result));
        assert.ok(result.length >= 3);
    });
    (0, node_test_1.it)("rejects invalid stress_pattern", async () => {
        await assert.rejects(() => (0, development_tools_js_1.handleToolCall)("daobrew_breathe", { stress_pattern: "invalid" }, true), /Invalid stress_pattern/);
    });
    (0, node_test_1.it)("rejects unknown tool", async () => {
        await assert.rejects(() => (0, development_tools_js_1.handleToolCall)("nonexistent_tool", {}, true), /Unknown tool/);
    });
});
