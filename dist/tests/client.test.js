...
(0, node_test_1.describe)("DaoBrewClient", () => {
    (0, node_test_1.it)("constructor sets defaults", () => {
        const client = new client_js_1.DaoBrewClient({ deviceCredential: DEVICE_CREDENTIAL });
        assert.ok(client);
    });
    ...
    (0, node_test_1.it)("checks backend contract compatibility through the version route", async () => {
        await withJsonServer((req) => {
            ...
            const client = new client_js_1.DaoBrewClient({ deviceCredential: DEVICE_CREDENTIAL, baseUrl, timeoutMs: 1000 });
            assert.equal(await client.assertBackendCompatible(), 1);
        });
    });
    (0, node_test_1.it)("fails closed for older or malformed backend contract readiness", async () => {
    ...
    (0, node_test_1.it)("keeps the bearer credential out of curl argv", () => {
        const spec = (0, client_js_1.buildCurlRequestSpec)({
            deviceCredential: DEVICE_CREDENTIAL,
            baseUrl: "https://api.example.test/api/v1",
        }, "/device/health/samples", { method: "POST", body: JSON.stringify({ samples: [] }) });
    ...
    (0, node_test_1.it)("getHealthkitHistory calls backend history endpoint", async () => {
        await withJsonServer((req) => {
    ...
    (0, node_test_1.it)("uses bearer-only principal selection and never sends X-Device-ID", async () => {
        await withJsonServer((req) => {
    ...
    (0, node_test_1.it)("streams a wide health backfill body instead of placing it in argv", async () => {
        const server = (0, node_http_1.createServer)((req, res) => {
            const chunks = [];
    ...
    (0, node_test_1.it)("requests a server-owned causal run without a client identity payload", async () => {
        let body = "";
        const server = (0, node_http_1.createServer)((req, res) => {
    ...
    (0, node_test_1.it)("getStateHistory passes optional limit and time bounds", async () => {
        await withJsonServer((req) => {
    ...
    (0, node_test_1.it)("getStateHistory omits query parameters when unspecified", async () => {
        await withJsonServer((req) => {
