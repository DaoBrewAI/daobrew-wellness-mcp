...
(0, node_test_1.describe)("joinTriplets", () => {
    (0, node_test_1.it)("joins biometric episodes to nearby calendar, meeting, and memory context", () => {
    ...
    (0, node_test_1.it)("joins same-local-day source context by default for long-horizon attribution", () => {
        ...
        const triplets = (0, triplet_js_1.joinTriplets)(input, { userId: TRIPLET_TEST_USER });
        assert.strictEqual(triplets.length, 1);
        assert.strictEqual(triplets[0].events[0].graph_source_ref, "calendar:evt-1");
        assert.strictEqual(triplets[0].meetings[0].graph_source_ref, "granola:note-1");
    });
    (0, node_test_1.it)("does not join source rows outside the default one-day attribution horizon", () => {
        const input = fixtureInput();
        const stateTs = Math.trunc(Date.parse("2026-05-29T21:00:00-07:00") / 1000);
    ...
    (0, node_test_1.it)("is deterministic when input order changes", () => {
        const input = fixtureInput();
        const reversed = {
            biometrics: {
                ...input.biometrics,
        ...
        assert.deepStrictEqual((0, triplet_js_1.joinTriplets)(input, { userId: TRIPLET_TEST_USER }).map((triplet) => ({ id: triplet.id, refs: triplet.source_refs })), (0, triplet_js_1.joinTriplets)(reversed, { userId: TRIPLET_TEST_USER }).map((triplet) => ({ id: triplet.id, refs: triplet.source_refs })));
    });
    (0, node_test_1.it)("keeps context-relevant memories inside the per-triplet cap over higher-importance junk", () => {
        const input = fixtureInput();
        ...
        const triplets = (0, triplet_js_1.joinTriplets)(input, {
            userId: TRIPLET_TEST_USER,
            sampleWindowSec: 60,
            contextWindowSec: 120,
            memoryWindowSec: 10,
        ...
        assert.strictEqual(ids[0], "memory-relevant", "context-relevant memory must survive the cap and rank first");
    });
    (0, node_test_1.it)("uses semantic memory scores for the overlap arm while keeping the time-window arm intact", () => {
        const input = fixtureInput();
        const base = input.memory[0];
        input.memory = [
            {
        ...
        const lexicalTriplets = (0, triplet_js_1.joinTriplets)(input, {
            userId: TRIPLET_TEST_USER,
            sampleWindowSec: 60,
            contextWindowSec: 120,
            memoryWindowSec: 10,
        });
        assert.deepStrictEqual(lexicalTriplets[0].memories.map((memory) => memory.id), ["memory-window"], "without semantic evidence the old lexical path stays unchanged");
        const semanticTriplets = (0, triplet_js_1.joinTriplets)(input, {
            userId: TRIPLET_TEST_USER,
            sampleWindowSec: 60,
            contextWindowSec: 120,
            memoryWindowSec: 10,
        ...
        assert.deepStrictEqual(semanticTriplets[0].memories.map((memory) => memory.id), ["memory-semantic", "memory-window"]);
    });
    (0, node_test_1.it)("does not invent episodes when backend state history is empty", () => {
        const input = fixtureInput();
        input.biometrics.states = [];
        assert.deepStrictEqual((0, triplet_js_1.joinTriplets)(input, { userId: TRIPLET_TEST_USER }), []);
    });
    (0, node_test_1.it)("carries the nearest prior per-metric sample within the LOCF staleness cap", () => {
        const input = fixtureInput();
    ...
    (0, node_test_1.it)("does not carry samples older than the LOCF staleness cap boundary", () => {
        const input = fixtureInput();
