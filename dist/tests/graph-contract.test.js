...
const REPO_ROOT = (() => {
    for (const candidate of [(0, node_path_1.resolve)(__dirname, "..", ".."), (0, node_path_1.resolve)(__dirname, "..")]) {
        if ((0, node_fs_1.existsSync)((0, node_path_1.join)(candidate, "package.json")) &&
            (0, node_fs_1.existsSync)((0, node_path_1.join)(candidate, "src", "engine", "schema.ts"))) {
            return candidate;
        }
    }
...
const SENTINEL_MAC_DIR = process.env.DAOBREW_SENTINEL_MAC_DIR?.trim() ||
    (0, node_path_1.resolve)(REPO_ROOT, "..", "DaobrewSentinelMac");
const KNOWLEDGE_GRAPH_SWIFT = (0, node_path_1.join)(SENTINEL_MAC_DIR, "Sources", "SentinelMac", "Models", "KnowledgeGraph.swift");
const GRAPH_STORE_SWIFT = (0, node_path_1.join)(SENTINEL_MAC_DIR, "Sources", "SentinelMac", "Network", "SentinelGraphStore.swift");
const DETONATOR_STATE_SWIFT = (0, node_path_1.join)(SENTINEL_MAC_DIR, "Sources", "SentinelMac", "Models", "DetonatorState.swift");
...
function readSource(path) {
    assert.ok((0, node_fs_1.existsSync)(path), `graph-contract: missing source file ${path}`);
    return (0, node_fs_1.readFileSync)(path, "utf-8");
}
/** Slice a Swift function body: from its `func <name>` marker to the next `func `. */
function sliceSwiftFunc(source, funcName, file) {
    const marker = `func ${funcName}`;
    const start = source.indexOf(marker);
    assert.ok(start >= 0, `${file}: "${marker}" not found — reader source moved; update the extraction regex in graph-contract.test.ts`);
    const rest = source.slice(start + marker.length);
    const next = rest.search(/\n\s*(?:private\s+|static\s+)*func\s/);
    return next === -1 ? rest : rest.slice(0, next);
}
/** Extract `case "<literal>":` raw strings from a Swift switch body. */
function extractSwitchCaseStrings(block, context) {
    const kinds = [...block.matchAll(/case\s+"([a-z0-9_]+)"\s*:/g)].map((match) => match[1]);
    assert.ok(kinds.length > 0, `${context}: no case "…" strings extracted — reader source moved; update the extraction regex`);
    return kinds;
}
/** Extract all '<literal>' strings from a SQL CHECK(kind IN (...)) list. */
function extractCheckKindLists(schemaSource) {
    const lists = [...schemaSource.matchAll(/CHECK\(kind IN \(([^)]+)\)\)/g)]
        .map((match) => [...match[1].matchAll(/'([a-z0-9_]+)'/g)].map((entry) => entry[1]));
    assert.ok(lists.length >= 2, "src/engine/schema.ts: CHECK(kind IN (...)) constraints not found — schema DDL moved; update graph-contract.test.ts");
    return lists;
}
// ---------------------------------------------------------------------------
...
const ENGINE_GHOST_STATUSES = ["watching", "suggested", "armed", "done"];
// ---------------------------------------------------------------------------
// Tests
// ---------------------------------------------------------------------------
(0, node_test_1.describe)("graph contract: engine writer vs SentinelMac reader", () => {
    (0, node_test_1.it)("schema DDL CHECK constraints match the exported kind constants", () => {
        const schemaSource = readSource((0, node_path_1.join)(REPO_ROOT, "src", "engine", "schema.ts"));
        ...
        assert.deepStrictEqual([...nodeCheck].sort(), [...schema_js_1.GRAPH_NODE_KINDS].sort(), "GRAPH_NODE_KINDS and the graph_nodes CHECK constraint drifted apart in schema.ts");
        assert.deepStrictEqual([...edgeCheck].sort(), [...schema_js_1.GRAPH_EDGE_KINDS].sort(), "GRAPH_EDGE_KINDS and the graph_edges CHECK constraint drifted apart in schema.ts");
    });
    (0, node_test_1.it)("every node kind the engine can write is handled by the Swift reader", (t) => {
        if (!(0, node_fs_1.existsSync)(KNOWLEDGE_GRAPH_SWIFT))
            return t.skip(READER_MISSING);
        const swift = readSource(KNOWLEDGE_GRAPH_SWIFT);
        const readerNodeKinds = extractSwitchCaseStrings(sliceSwiftFunc(swift, "mapNodeKind", "KnowledgeGraph.swift"), "KnowledgeGraph.swift mapNodeKind");
        for (const kind of schema_js_1.GRAPH_NODE_KINDS) {
            assert.ok(readerNodeKinds.includes(kind), `node kind "${kind}" is writable by the engine (schema.ts GRAPH_NODE_KINDS / upsert gate) ` +
                `but KnowledgeGraph.swift mapNodeKind drops it — the node would silently vanish from the canvas`);
        }
        const readerOnly = readerNodeKinds.filter((kind) => !schema_js_1.GRAPH_NODE_KINDS.includes(kind));
    ...
    (0, node_test_1.it)("every edge kind the engine can write is handled by the Swift reader", (t) => {
        if (!(0, node_fs_1.existsSync)(KNOWLEDGE_GRAPH_SWIFT))
            return t.skip(READER_MISSING);
        const swift = readSource(KNOWLEDGE_GRAPH_SWIFT);
        const readerEdgeKinds = extractSwitchCaseStrings(sliceSwiftFunc(swift, "mapEdgeKind", "KnowledgeGraph.swift"), "KnowledgeGraph.swift mapEdgeKind");
        for (const kind of schema_js_1.GRAPH_EDGE_KINDS) {
            assert.ok(readerEdgeKinds.includes(kind), `edge kind "${kind}" is writable by the engine (schema.ts GRAPH_EDGE_KINDS / upsert gate) ` +
                `but KnowledgeGraph.swift mapEdgeKind drops it — the edge would silently vanish from the canvas`);
        }
        const readerOnly = readerEdgeKinds.filter((kind) => !schema_js_1.GRAPH_EDGE_KINDS.includes(kind));
        if (readerOnly.length > 0) {
            console.log(`[graph-contract] reader-only edge kinds (engine never writes): ${readerOnly.join(", ")}`);
        }
    });
    (0, node_test_1.it)("reasoner runtime output stays inside the schema kind sets", async () => {
        const { armed, watching } = await writerDeltas();
        const emittedNodeKinds = new Set();
        const emittedEdgeKinds = new Set();
        for (const delta of [armed, watching]) {
            for (const node of delta.nodes)
                emittedNodeKinds.add(node.kind);
            for (const edge of delta.edges)
                emittedEdgeKinds.add(edge.kind);
        }
        for (const kind of emittedNodeKinds) {
            assert.ok(schema_js_1.GRAPH_NODE_KINDS.includes(kind), `Reasoner emitted node kind "${kind}" that the upsert gate / schema CHECK would reject`);
        }
        for (const kind of emittedEdgeKinds) {
            assert.ok(schema_js_1.GRAPH_EDGE_KINDS.includes(kind), `Reasoner emitted edge kind "${kind}" that the upsert gate / schema CHECK would reject`);
        }
        ...
        const neverEmitted = schema_js_1.GRAPH_NODE_KINDS
    ...
    (0, node_test_1.it)("ghost nodes carry every props_json key the Swift reader renders", async () => {
        const { armed, watching } = await writerDeltas();
        for (const [variant, props, extraKeys] of [
            ["armed root", rootProps(armed), GHOST_ARMED_PROPS],
            ["watching root", rootProps(watching), GHOST_WATCHING_PROPS],
        ]) {
            for (const key of [...GHOST_CORE_PROPS, ...extraKeys]) {
                assert.ok(key in props, `${variant} ghost props_json is missing "${key}" — KnowledgeGraph.swift reads it (see citations in graph-contract.test.ts)`);
            }
            // Nested shape the reader digs into (KnowledgeGraph.swift ~L470, ~L715):
            const brief = props.brief;
            assert.ok(typeof brief?.cause === "string" && brief.cause.trim().length > 0, `${variant} ghost brief.cause missing/empty — reader uses it for the ghost headline and title defense`);
            const suggestedBlock = brief.suggested_block;
            assert.ok(typeof suggestedBlock?.title === "string" && suggestedBlock.title.trim().length > 0, `${variant} ghost brief.suggested_block.title missing/empty — reader uses it for WeeklyInsight.suggestedAction`);
            // Status values must stay inside the reader's known lifecycle words.
            assert.ok(typeof props.status === "string", `${variant} ghost props_json.status must be a string`);
        }
        assert.strictEqual(rootProps(watching).status, "watching");
        assert.strictEqual(rootProps(armed).status, "suggested");
    });
    (0, node_test_1.it)("Swift reader still reads the ghost props this contract asserts", (t) => {
        if (!(0, node_fs_1.existsSync)(KNOWLEDGE_GRAPH_SWIFT))
            return t.skip(READER_MISSING);
        const swift = readSource(KNOWLEDGE_GRAPH_SWIFT);
        const assertedKeys = [
        ...
        for (const key of assertedKeys) {
            assert.ok(swift.includes(`["${key}"]`), `KnowledgeGraph.swift no longer subscripts props key "${key}" — the reader contract drifted; ` +
                "update the GHOST_*_PROPS lists in graph-contract.test.ts to match what the reader actually renders");
        }
    });
    (0, node_test_1.it)("detonate lifecycle: status + artifact_ref stay aligned across writer and reader", (t) => {
        if (!(0, node_fs_1.existsSync)(GRAPH_STORE_SWIFT) || !(0, node_fs_1.existsSync)(DETONATOR_STATE_SWIFT) || !(0, node_fs_1.existsSync)(KNOWLEDGE_GRAPH_SWIFT)) {
            return t.skip(READER_MISSING);
        }
        // Writer: upsert.ts arms the root and disarms competing ghosts.
        const upsertSource = readSource((0, node_path_1.join)(REPO_ROOT, "src", "engine", "upsert.ts"));
        assert.match(upsertSource, /status:\s*"armed"/, "upsert.ts mergeRootProps no longer sets status \"armed\" — detonator arming contract moved");
        assert.match(upsertSource, /'\$\.status',\s*'suggested'/, "upsert.ts disarmOtherGhostsSql no longer resets losers to 'suggested' — update the contract");
        // Writer: tools.ts daobrew_detonate_done closes the loop with done + artifact_ref.
        const toolsSource = readSource((0, node_path_1.join)(REPO_ROOT, "src", "tools.ts"));
        assert.match(toolsSource, /'\$\.status',\s*'done',\s*'\$\.artifact_ref'/, "src/tools.ts daobrew_detonate_done no longer writes $.status='done' + $.artifact_ref (sqlite path)");
        assert.match(toolsSource, /'status',\s*'done',\s*'artifact_ref'/, "src/tools.ts daobrew_detonate_done no longer writes status/artifact_ref (postgres jsonb path)");
        ...
        assert.match(storeSource, /json_extract\(props_json,\s*'\$\.status'\)/, "SentinelGraphStore.swift no longer reads $.status — reader source moved; update the extraction regex");
        assert.match(storeSource, /'\$\.artifact_ref'/, "SentinelGraphStore.swift no longer reads $.artifact_ref — reader source moved; update the extraction regex");
        // Reader: DetonatorState renders from the same two keys (~L448).
        const detonatorSource = readSource(DETONATOR_STATE_SWIFT);
        assert.ok(detonatorSource.includes('["artifact_ref"]') && detonatorSource.includes('["status"]'), "DetonatorState.swift no longer reads props status/artifact_ref — reader contract drifted");
        ...
        assert.ok(statusWordsMatch, "KnowledgeGraph.swift statusWords set not found — reader source moved; update the extraction regex");
        const readerStatusWords = [...statusWordsMatch[1].matchAll(/"([a-z_]+)"/g)].map((match) => match[1]);
        for (const status of ENGINE_GHOST_STATUSES) {
            assert.ok(readerStatusWords.includes(status), `engine can persist ghost status "${status}" but KnowledgeGraph.swift statusWords does not know it`);
        }
    });
});
