...
const mockWellnessState = (...args) => developmentMock().mockWellnessState(...args);
const mockStartSession = (...args) => developmentMock().mockStartSession(...args);
const mockSessionHistory = (...args) => developmentMock().mockSessionHistory(...args);
const mockDetonateBrief = () => developmentMock().mockDetonateBrief();
const mockDetonateDone = (causeId) => developmentMock().mockDetonateDone(causeId);
// ──────────────────────────────────────────────────────────────────────────────
...
function mockBreathState(pattern, sessionStartMs, plannedDurationSec) {
...
async function handleToolCall(name, args, isMock, _apiKey, isDemo = false, client) {
    ...
    function requireClient() {
        if (!client)
            throw new Error("DaoBrewClient not initialized. Sign in to DaoBrew on this Mac to enroll this device, or use --mock.");
        return client;
    }
    switch (name) {
        case "daobrew_check":
...
async function handleConnectWatch(isMock, client) {
    ...
    if (isMock) {
        return {
            ...
            instructions: "Running in mock mode — no real backend connection. Sign in to DaoBrew on this Mac to enroll this device.",
        };
    }
    if (!client) {
        return {
    ...
    try {
        ...
        return {
            ...
            install_url: "https://testflight.apple.com/join/6XTNFvv5",
            instructions: "Backend is unavailable. Check that this Mac is enrolled and the backend is online, then try again.",
        };
    }
}
// ──────────────────────────────────────────────────────────────────────────────
 ...
 */
function toolUserId() {
    const plan = (0, identity_js_1.resolveUserId)((0, local_config_js_1.readLocalConfig)());
    if (!plan.ok)
    ...
    if ((0, graph_db_js_1.graphDbExists)()) {
        ...
        const sql = (0, graph_db_js_1.graphStoreKind)() === "postgres"
         ...
         INSERT INTO graph_edges(id, user_id, src_id, dst_id, kind, label, created_at_ts)
           SELECT 'e_done_' || ${(0, graph_db_js_1.q)(causeId)}, p.user_id, p.id, ${(0, graph_db_js_1.q)(causeId)}, 'caused', 'detonated', ${now}
           FROM graph_nodes p
           WHERE p.kind = 'pattern'
             AND p.user_id = (SELECT user_id FROM graph_nodes WHERE id = ${(0, graph_db_js_1.q)(causeId)})
