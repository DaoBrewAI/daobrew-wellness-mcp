...
async function downloadAudio(filename) {
    const dest = (0, path_1.join)(AUDIO_CACHE_DIR, filename);
    if ((0, fs_1.existsSync)(dest))
        return dest;
    if (!(0, fs_1.existsSync)(AUDIO_CACHE_DIR))
        (0, fs_1.mkdirSync)(AUDIO_CACHE_DIR, { recursive: true });
    try {
        const resp = await fetch(`${AUDIO_CDN}/${filename}`);
        if (!resp.ok)
            return null;
        const buf = Buffer.from(await resp.arrayBuffer());
        (0, fs_1.writeFileSync)(dest, buf);
        return dest;
    }
    catch {
        return null;
...
async function resolveAudioFile(element, task = "breathing") {
...
async function playAudio(element, task = "breathing", volume = 0.4) {
    ...
    try {
        const child = process.platform === "darwin"
            ? (0, child_process_1.spawn)("afplay", ["-v", String(volume), audioPath], {
                stdio: "ignore", detached: true,
            })
            : (0, child_process_1.spawn)("mpv", [
