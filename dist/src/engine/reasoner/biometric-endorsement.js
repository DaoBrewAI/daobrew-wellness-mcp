...
exports.buildBiometricEndorsement = buildBiometricEndorsement;
...
function metricHead(m) {
    const unit = m.unit || DEFAULT_UNITS[m.type] || "";
    const value = unit ? `${m.avg} ${unit}` : `${m.avg}`;
    if (CUMULATIVE_TYPES.has(m.type))
        return `${label(m.type)} ${value}`;
    return `${label(m.type)} ${value} avg`;
}
function fmtTime(ts, timeZone) {
...
function buildBiometricEndorsement(input) {
    ...
    const metrics = [...byType.entries()].map(([type, arr]) => {
    ...
    const summary = `${parts.join(" · ")} · ${samples.length} samples${sparse ? " (sparse)" : ""}, ${windowStr}`;
    ...
    const pick = (want) => byDelta.find(want) ?? byDelta[0];
