...
exports.TASKMAP_DECOMPOSITION_VALIDATION_LIMITS_V1 = exports.TASKMAP_DECOMPOSITION_VALIDATION_STEP_IDS = exports.TASKMAP_DECOMPOSITION_VALIDATION_VERSION = void 0;
exports.validateTaskMapDecomposition = validateTaskMapDecomposition;
exports.validateTaskMapDecompositionValidationResult = validateTaskMapDecompositionValidationResult;
...
exports.TASKMAP_DECOMPOSITION_VALIDATION_VERSION = "taskmap-decomposition-validation.v1";
exports.TASKMAP_DECOMPOSITION_VALIDATION_STEP_IDS = Object.freeze([
    "field_completeness",
    "citation_validity",
    "acyclicity",
    "edge_type_legality",
...
exports.TASKMAP_DECOMPOSITION_VALIDATION_LIMITS_V1 = Object.freeze({
    maxSubtasks: 16,
    maxEdges: 64,
...
function exactKeyReasons(value, expected, label) {
...
function boundedId(value) {
    return boundedText(value, exports.TASKMAP_DECOMPOSITION_VALIDATION_LIMITS_V1.maxIdCharacters)
        && value === value.trim();
}
function boundedModelId(value) {
    return boundedText(value, method_library_js_1.TASKMAP_METHOD_LIBRARY_LIMITS_V1.maxModelCharacters)
        && value === value.trim();
}
...
function digestOrInvalidMarker(value, marker) {
    try {
        return (0, source_contracts_js_1.taskMapContractDigest)(value);
    }
    catch (_error) {
        return (0, source_contracts_js_1.taskMapContractDigest)({ invalidCanonicalInput: marker });
    }
}
function candidateSubtasks(candidate) {
    if (!plain(candidate) || !Array.isArray(candidate.subtasks))
        return [];
    return candidate.subtasks.slice(0, exports.TASKMAP_DECOMPOSITION_VALIDATION_LIMITS_V1.maxSubtasks + 1);
}
function candidateEdges(candidate) {
    if (!plain(candidate) || !Array.isArray(candidate.edges))
        return [];
    return candidate.edges.slice(0, exports.TASKMAP_DECOMPOSITION_VALIDATION_LIMITS_V1.maxEdges + 1);
}
function sourceResultFrom(context) {
...
function validLlmProviderTransportCorrelation(provider, transport) {
    return (provider === "offline-replay" && transport === "injected-offline")
        || (typeof provider === "string" && LIVE_LLM_PROVIDERS.has(provider)
            && transport === provider);
}
function sourceProposalShapeReasons(proposal, index) {
    const reasons = [];
    if (!plain(proposal))
        return [`Task-7 proposal ${index} must be an object`];
    reasons.push(...exactKeyReasons(proposal, SOURCE_PROPOSAL_KEYS, `Task-7 proposal ${index}`));
    if (!boundedId(proposal.proposalId))
        reasons.push(`Task-7 proposal ${index} id is invalid`);
    if (!boundedId(proposal.methodId))
        reasons.push(`Task-7 proposal ${index} method id is invalid`);
    if (!Array.isArray(proposal.subtasks) || proposal.subtasks.length === 0
        || proposal.subtasks.length > exports.TASKMAP_DECOMPOSITION_VALIDATION_LIMITS_V1.maxSubtasks) {
    ...
    for (const [subtaskIndex, subtask] of proposal.subtasks.entries()) {
        ...
        reasons.push(...exactKeyReasons(subtask, SUBTASK_KEYS, `Task-7 proposal ${index} subtask ${subtaskIndex}`));
        ...
        if (!boundedText(subtask.title, exports.TASKMAP_DECOMPOSITION_VALIDATION_LIMITS_V1.maxTitleCharacters)) {
            reasons.push(`Task-7 proposal ${index} subtask ${subtaskIndex} title is invalid`);
        }
        if (!boundedText(subtask.summary, exports.TASKMAP_DECOMPOSITION_VALIDATION_LIMITS_V1.maxSummaryCharacters)) {
        ...
        if (!Array.isArray(subtask.citationPointerIds)
            ...
            || subtask.citationPointerIds.length
                > exports.TASKMAP_DECOMPOSITION_VALIDATION_LIMITS_V1.maxCitationsPerItem
...
function task7BindingReasons(candidate, context) {
    const reasons = [];
    const sourceResult = sourceResultFrom(context);
    const sourceBytes = jsonBytes(sourceResult);
    if (sourceBytes === null
        || sourceBytes > exports.TASKMAP_DECOMPOSITION_VALIDATION_LIMITS_V1.maxSourceResultBytes) {
        return ["referenced Task-7 result is not bounded canonical JSON"];
    }
    if (!plain(sourceResult))
        return ["referenced Task-7 result must be an object"];
    reasons.push(...exactKeyReasons(sourceResult, SOURCE_RESULT_KEYS, "referenced Task-7 result"));
    if (sourceResult.contractVersion !== method_library_js_1.TASKMAP_DECOMPOSITION_PROPOSAL_VERSION
    ...
    if (sourceResult.proposals !== undefined && Array.isArray(sourceResult.proposals)) {
        ...
        const proposalIds = sourceResult.proposals.flatMap((proposal) => plain(proposal) && typeof proposal.proposalId === "string" ? [proposal.proposalId] : []);
    ...
    if (!plain(sourceResult.llm)) {
        ...
        if (sourceResult.llm.stationId !== method_library_js_1.TASKMAP_DECOMPOSITION_STATION_ID
    ...
    if (sourceResult.source === "method_library") {
        ...
        if (sourceResult.unavailableReason !== null || !plain(sourceResult.llm)
            ...
            || !validLlmProviderTransportCorrelation(sourceResult.llm.providerId, sourceResult.llm.transport)
    ...
    else if (exactKeyReasons(sourceResult.authority, SOURCE_AUTHORITY_KEYS, "referenced Task-7 authority").length > 0 || sourceResult.authority.edgesWritten !== false
    ...
    else if (exactKeyReasons(sourceResult.privacy, SOURCE_PRIVACY_KEYS, "referenced Task-7 privacy").length > 0 || sourceResult.privacy.sourceBodiesStored !== false
    ...
    try {
        ...
        if ((0, source_contracts_js_1.taskMapContractDigest)(core) !== sourceResult.artifactDigest) {
    ...
    const selected = Array.isArray(sourceResult.proposals)
        ? sourceResult.proposals.filter((proposal) => plain(proposal) && proposal.proposalId === candidate.proposalId)
        : [];
    if (selected.length !== 1 || !plain(selected[0])) {
        reasons.push("candidate proposal is absent from the referenced Task-7 result");
        return uniqueSorted(reasons);
    }
    const selectedDigest = (0, source_contracts_js_1.taskMapContractDigest)(selected[0]);
    ...
    const selectedOrdinal = sourceResult.proposals.indexOf(selected[0]);
    const selectedSubtaskValues = Array.isArray(selected[0].subtasks)
        ? selected[0].subtasks
        : [];
    ...
    if (typeof candidate.parentTaskId !== "string" || selectedOrdinal < 0
        ...
        const expectedProposalId = `tmdecomp_${(0, source_contracts_js_1.taskMapContractDigest)({
        ...
        for (const [index, selectedSubtask] of selectedSubtaskValues.entries()) {
            ...
            const expectedSubtaskId = `tmds_${(0, source_contracts_js_1.taskMapContractDigest)({
    ...
    try {
        if ((0, source_contracts_js_1.taskMapContractCanonicalJson)(candidateProposal)
            !== (0, source_contracts_js_1.taskMapContractCanonicalJson)(selected[0])) {
...
function projectionFrom(context) {
    if (!plain(context) || !plain(context.projection))
        return null;
    return context.projection;
}
function fieldCompletenessReasons(candidate, context) {
    const reasons = [];
    const bytes = jsonBytes(candidate);
    if (bytes === null
        || bytes > exports.TASKMAP_DECOMPOSITION_VALIDATION_LIMITS_V1.maxCandidateBytes) {
    ...
    if (!plain(candidate)) {
        ...
        if (!boundedId(candidate.proposalId))
            reasons.push("proposalId is missing or unbounded");
        if (!boundedId(candidate.methodId))
            reasons.push("methodId is missing or unbounded");
        if (typeof candidate.sourceResultArtifactDigest !== "string"
            || !SHA256.test(candidate.sourceResultArtifactDigest)) {
        ...
        if (!boundedId(candidate.parentTaskId))
            reasons.push("parentTaskId is missing or unbounded");
        if (!Array.isArray(candidate.subtasks) || candidate.subtasks.length === 0
            || candidate.subtasks.length > exports.TASKMAP_DECOMPOSITION_VALIDATION_LIMITS_V1.maxSubtasks) {
            reasons.push("subtask collection is missing, empty, or unbounded");
        }
        if (!Array.isArray(candidate.edges)
            || candidate.edges.length > exports.TASKMAP_DECOMPOSITION_VALIDATION_LIMITS_V1.maxEdges) {
    ...
    if (!plain(context)) {
        ...
        if (!plain(context.projection))
            reasons.push("validation projection is missing");
        if (!plain(context.sourceResult))
            reasons.push("referenced Task-7 result is missing");
    }
    const projection = projectionFrom(context);
    if (projection !== null) {
        try {
            reasons.push(...(0, harness_js_1.taskMapProjectionArtifactValidationReasons)(projection)
    ...
    for (const [index, subtask] of candidateSubtasks(candidate).entries()) {
        ...
        if (!boundedId(subtask.subtaskId))
            reasons.push(`subtask ${index} id is missing or unbounded`);
        else
            subtaskIds.push(subtask.subtaskId);
        if (!boundedText(subtask.title, exports.TASKMAP_DECOMPOSITION_VALIDATION_LIMITS_V1.maxTitleCharacters)) {
    ...
    if (new Set(subtaskIds).size !== subtaskIds.length)
        reasons.push("subtask ids must be unique");
    const edgeIds = [];
    for (const [index, edge] of candidateEdges(candidate).entries()) {
        ...
        if (!boundedId(edge.id))
            reasons.push(`edge ${index} id is missing or unbounded`);
        else
            edgeIds.push(edge.id);
        if (!boundedId(edge.from) || !boundedId(edge.to)) {
            reasons.push(`edge ${index} endpoints are missing or unbounded`);
        }
        if (!boundedId(edge.relation))
            reasons.push(`edge ${index} relation is missing or unbounded`);
    ...
    if (new Set(edgeIds).size !== edgeIds.length)
        reasons.push("edge ids must be unique");
    return uniqueSorted(reasons);
}
function citationValidityReasons(candidate, context) {
    const projection = projectionFrom(context);
    if (projection === null || !Array.isArray(projection.sources)) {
        return ["projection sources are unavailable for citation validation"];
    }
    const sources = new Map(projection.sources.flatMap((source) => plain(source) && typeof source.id === "string"
        ? [[source.id, source]]
        : []));
    const reasons = [];
    const endpointSupports = new Map();
    for (const root of Array.isArray(projection.roots) ? projection.roots : []) {
        if (!plain(root) || typeof root.id !== "string")
            continue;
        endpointSupports.set(root.id, new Set(Array.isArray(root.citations)
            ? root.citations.flatMap((citation) => plain(citation) && typeof citation.pointerId === "string" ? [citation.pointerId] : [])
            : []));
    }
    ...
    for (const [kind, values] of [
        ...
        for (const [index, value] of values.entries()) {
            if (!plain(value) || !Array.isArray(value.citationPointerIds))
                continue;
...
function executionEdges(candidate, context) {
    const projection = projectionFrom(context);
...
function acyclicityReasons(candidate, context) {
...
function edgeTypeLegalityReasons(candidate, context) {
    const projection = projectionFrom(context);
    ...
    const proposedSubtaskIds = new Set();
    for (const [index, subtask] of candidateSubtasks(candidate).entries()) {
        if (!plain(subtask) || typeof subtask.subtaskId !== "string")
            continue;
    ...
    for (const existingEdge of projection.edges) {
        if (!plain(existingEdge) || typeof existingEdge.from !== "string"
            || typeof existingEdge.to !== "string" || typeof existingEdge.relation !== "string")
            continue;
        const identity = `${existingEdge.from}\u0000${existingEdge.relation}\u0000${existingEdge.to}`;
        if (edgeIdentities.has(identity))
            reasons.push("projection contains a duplicate edge identity");
    ...
    for (const [index, edge] of candidateEdges(candidate).entries()) {
        ...
        if (typeof edge.relation === "string") {
            ...
            if (edgeIdentities.has(identity))
                reasons.push(`edge ${index} duplicates an existing edge identity`);
            edgeIdentities.add(identity);
        }
        ...
        if (edge.from === edge.to)
            reasons.push(`edge ${index} cannot be a self edge`);
        ...
        else if (["depends_on", "blocks", "supersedes", "informed_by"].includes(edge.relation) && (from !== "task" || to !== "task")) {
    ...
    if (plain(candidate) && typeof candidate.parentTaskId === "string") {
        ...
        for (const edge of candidateEdges(candidate)) {
            if (!plain(edge) || typeof edge.from !== "string" || typeof edge.to !== "string")
                continue;
...
function privacyBoundaryReasons(candidate) {
    const bytes = jsonBytes(candidate);
    if (bytes === null
        || bytes > exports.TASKMAP_DECOMPOSITION_VALIDATION_LIMITS_V1.maxCandidateBytes) {
        return ["privacy validation rejected an unbounded candidate"];
    }
    return uniqueSorted((0, harness_js_1.taskMapProjectionPrivacyLeakReasons)(candidate));
}
function step(stepId, reasons) {
    const normalized = uniqueSorted(reasons);
    return { stepId, passed: normalized.length === 0, reasons: normalized };
}
function validateTaskMapDecomposition(candidate, context) {
    const steps = [
    ...
    const projection = projectionFrom(context);
    const core = {
        contractVersion: exports.TASKMAP_DECOMPOSITION_VALIDATION_VERSION,
    ...
    if (Buffer.byteLength((0, source_contracts_js_1.taskMapContractCanonicalJson)(result), "utf8")
        > exports.TASKMAP_DECOMPOSITION_VALIDATION_LIMITS_V1.maxArtifactBytes) {
        fail("validation artifact exceeds its bounded ceiling");
    }
    return result;
}
function assertResultShape(value) {
    if (!plain(value))
        fail("result must be an object");
    if (exactKeyReasons(value, RESULT_KEYS, "result").length > 0
        || value.contractVersion !== exports.TASKMAP_DECOMPOSITION_VALIDATION_VERSION
        ...
        || value.steps.length !== exports.TASKMAP_DECOMPOSITION_VALIDATION_STEP_IDS.length
        || !plain(value.authority) || !plain(value.privacy))
        fail("result shape is invalid");
    if (exactKeyReasons(value.authority, AUTHORITY_KEYS, "result authority").length > 0
        ...
        || value.authority.requiresOwnerAcceptance !== true)
        fail("result authority is invalid");
    if (exactKeyReasons(value.privacy, RESULT_PRIVACY_KEYS, "result privacy").length > 0
        ...
        fail("result privacy is invalid");
    for (const [index, candidateStep] of value.steps.entries()) {
        if (!plain(candidateStep)
            ...
            || candidateStep.stepId !== exports.TASKMAP_DECOMPOSITION_VALIDATION_STEP_IDS[index]
...
function validateTaskMapDecompositionValidationResult(value, candidate, context) {
    const suppliedBytes = jsonBytes(value);
    if (suppliedBytes === null
        || suppliedBytes > exports.TASKMAP_DECOMPOSITION_VALIDATION_LIMITS_V1.maxArtifactBytes) {
    ...
    if ((0, source_contracts_js_1.taskMapContractCanonicalJson)(value) !== (0, source_contracts_js_1.taskMapContractCanonicalJson)(rebuilt)) {
