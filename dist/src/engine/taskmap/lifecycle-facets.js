...
exports.TASKMAP_LIFECYCLE_FACETS_POLICY_V1 = exports.TASKMAP_LIFECYCLE_FACETS_LIMITS_V1 = exports.TASKMAP_LIFECYCLE_FACETS_POLICY_VERSION = exports.TASKMAP_LIFECYCLE_FACETS_VERSION = void 0;
exports.buildTaskMapLifecycleFacets = buildTaskMapLifecycleFacets;
exports.unsafeBuildTaskMapLifecycleFacetsFromAuthenticatedFixturesForTest = unsafeBuildTaskMapLifecycleFacetsFromAuthenticatedFixturesForTest;
exports.assertTaskMapLifecycleFacets = assertTaskMapLifecycleFacets;
exports.taskMapLifecycleFacetsCanonicalBytes = taskMapLifecycleFacetsCanonicalBytes;
...
exports.TASKMAP_LIFECYCLE_FACETS_VERSION = "taskmap-lifecycle-facets.v1";
exports.TASKMAP_LIFECYCLE_FACETS_POLICY_VERSION = "taskmap-lifecycle-facets-policy.1";
exports.TASKMAP_LIFECYCLE_FACETS_LIMITS_V1 = Object.freeze({
...
exports.TASKMAP_LIFECYCLE_FACETS_POLICY_V1 = Object.freeze({
    contractVersion: exports.TASKMAP_LIFECYCLE_FACETS_POLICY_VERSION,
...
const READ_P10_2_STORE = identity_dedupe_projection_js_1.readTaskMapIdentityDedupeProjectionStore;
const BUILD_P10_3A_WORK_CONTROL = work_control_decision_js_1.buildTaskMapWorkControlDecision;
const ASSERT_P10_3A_WORK_CONTROL = work_control_decision_js_1.assertTaskMapWorkControlDecision;
const DIFF_PROJECTIONS = source_contracts_js_1.diffTaskMapProjections;
const TASKMAP_CONTRACT_DIGEST = source_contracts_js_1.taskMapContractDigest;
...
const HASH_UPDATE = GET_OWN_PROPERTY_DESCRIPTOR(HASH_PROTOTYPE, "update")?.value;
const HASH_DIGEST = GET_OWN_PROPERTY_DESCRIPTOR(HASH_PROTOTYPE, "digest")?.value;
...
const POLICY_DIGEST = digestCanonical(exports.TASKMAP_LIFECYCLE_FACETS_POLICY_V1);
...
function assertPrototypeIntegrity(prototype, expected, label) {
    ...
    for (let index = 0; index < expected.length; index += 1) {
        ...
        if (keys[index] !== expectedRow.key
            || !sameDescriptor(GET_OWN_PROPERTY_DESCRIPTOR(prototype, expectedRow.key), expectedRow.descriptor)) {
            fail(`${label} prototype changed during validation`);
        }
    }
}
function assertRuntimeIntegrity() {
    ...
    if (identity_dedupe_projection_js_1.readTaskMapIdentityDedupeProjectionStore !== READ_P10_2_STORE
        || work_control_decision_js_1.buildTaskMapWorkControlDecision !== BUILD_P10_3A_WORK_CONTROL
        || work_control_decision_js_1.assertTaskMapWorkControlDecision !== ASSERT_P10_3A_WORK_CONTROL
        || source_contracts_js_1.diffTaskMapProjections !== DIFF_PROJECTIONS
        || source_contracts_js_1.taskMapContractDigest !== TASKMAP_CONTRACT_DIGEST
        || node_crypto_1.createHash !== CREATE_HASH
...
function countNode(state, label) {
    state.nodes += 1;
    if (state.nodes > exports.TASKMAP_LIFECYCLE_FACETS_LIMITS_V1.maxNodes) {
...
function countDescriptor(state, label) {
    ...
    if (state.descriptors
        > exports.TASKMAP_LIFECYCLE_FACETS_LIMITS_V1.maxDescriptors) {
...
function chargeOwnDescriptor(state, key, label) {
    ...
    if (key.length > exports.TASKMAP_LIFECYCLE_FACETS_LIMITS_V1.maxStringLength) {
        fail(`${label} contains an oversized object key`);
    }
    addPrecloneBytes(state, BUFFER_BYTE_LENGTH(JSON_STRINGIFY(key), "utf8") + 1, label);
}
function preflightJson(value, label, state, depth = 0) {
    if (depth > exports.TASKMAP_LIFECYCLE_FACETS_LIMITS_V1.maxDepth) {
    ...
    if (typeof value === "string") {
        if (value.length > exports.TASKMAP_LIFECYCLE_FACETS_LIMITS_V1.maxStringLength) {
            fail(`${label} exceeds the string ceiling`);
        }
        addPrecloneBytes(state, BUFFER_BYTE_LENGTH(JSON_STRINGIFY(value), "utf8"), label);
        return;
    }
    ...
    if (ARRAY_IS_ARRAY(value)) {
        ...
        if (value.length > exports.TASKMAP_LIFECYCLE_FACETS_LIMITS_V1.maxArrayLength) {
    ...
    if (keys.length > exports.TASKMAP_LIFECYCLE_FACETS_LIMITS_V1.maxObjectKeys) {
...
function snapshotJson(value, label, byteLimit = exports.TASKMAP_LIFECYCLE_FACETS_LIMITS_V1.maxCanonicalInputBytes) {
...
function canonicalJson(value) {
    ...
    if (value !== null && typeof value === "object") {
        ...
        const items = [];
        for (const key of keys) {
            items.push(`${JSON_STRINGIFY(key)}:${canonicalJson(value[key])}`);
        }
        return `{${items.join(",")}}`;
    }
...
function assertSameAuthenticatedHead(firstStore, secondStore) {
    ...
    if (firstStore.entries.length !== secondStore.entries.length
...
function lifecycleFromP10_2(value) {
    if (value === "open")
...
function expectedChange(delta) {
    if (delta.adjudicatedDelta === "open"
        && delta.previousState === "absent"
        && delta.currentState === "open") {
        return "new";
    }
    if (delta.adjudicatedDelta === "updated")
        return "updated";
    if (delta.adjudicatedDelta === "resolved")
        return "resolved";
    if (delta.adjudicatedDelta === "superseded")
        return "superseded";
    if (delta.adjudicatedDelta === "rejected")
        return "rejected";
    return "unchanged";
}
function assertLifecycleTruth(previousState, currentState, adjudicatedDelta, label) {
    const valid = (adjudicatedDelta === "open"
...
function assertWorkControlPredecessor(entry, projection, workControl) {
    const predecessor = workControl.predecessor;
    const projectionDigest = DIFF_PROJECTIONS(null, projection).currentProjectionDigest;
    if (predecessor.ownerScopeDigest !== entry.ownerScopeDigest
...
function projectionTaskIndex(projection) {
    if (projection.tasks.length
        > exports.TASKMAP_LIFECYCLE_FACETS_LIMITS_V1.maxProjectionTasks) {
        fail("projection task count exceeds the lifecycle-facet ceiling");
    }
    const roots = new MAP_CONSTRUCTOR();
    for (const root of projection.roots) {
        assertHashedId(root.id, HASHED_IDS.root, "projection root ID");
        if (roots.has(root.id))
            fail("projection root IDs are not unique");
        const taskIds = new SET_CONSTRUCTOR();
    ...
    const tasks = new MAP_CONSTRUCTOR();
    for (const task of projection.tasks) {
        ...
        if (tasks.has(task.id))
            fail("projection task IDs are not unique");
    ...
    const rootTaskIds = new SET_CONSTRUCTOR();
...
function predecessorFrom(entry, workControl) {
    return {
...
function buildRows(entry, projection, workControl) {
    const projectionTasks = projectionTaskIndex(projection);
    const taskIdByRowDigest = new MAP_CONSTRUCTOR();
    ...
    if (workControl.workDecisions.length
        > exports.TASKMAP_LIFECYCLE_FACETS_LIMITS_V1.maxWorks) {
        fail("work count exceeds the lifecycle-facet ceiling");
    }
    const usedTaskIds = new SET_CONSTRUCTOR();
    const rows = [];
    for (const decision of workControl.workDecisions) {
        ...
        expectedTaskRows.sort((left, right) => (compareText(left.taskId, right.taskId)
            || compareText(left.projectionRowDigest, right.projectionRowDigest)));
        expectedRejectionRows.sort((left, right) => (compareText(left.projectionReferenceId, right.projectionReferenceId)
            || compareText(left.projectionRowDigest, right.projectionRowDigest)));
        const expectedTaskIds = expectedTaskRows.map((row) => row.taskId).sort(compareText);
        ...
        const expectedRowDigests = work.projectionReferences
            ...
            .sort(compareText);
        if (canonicalJson(decision.projectionTaskRows)
            ...
            fail("P10.3a projection rows/digests diverge from P10.2 references");
        }
        const lifecycle = lifecycleFromP10_2(work.lifecycleState);
        if (lifecycle !== decision.lifecycleDecision
        ...
        assertLifecycleTruth(delta.previousState, delta.currentState, delta.adjudicatedDelta, `work ${decision.workId}`);
        const taskIds = [...decision.projectionTaskIds].sort(compareText);
        if (taskIds.length !== decision.projectionTaskIds.length
            || taskIds.some((taskId, index) => (taskId !== decision.projectionTaskIds[index]))) {
...
function buildFromAuthenticatedInputs(firstStore, secondStore, projectionInput, workControlInput) {
    assertRuntimeIntegrity();
    const projection = snapshotJson(projectionInput, "lifecycle-facet projection");
    ...
    const core = artifactCore({
        contractVersion: exports.TASKMAP_LIFECYCLE_FACETS_VERSION,
        originDigest,
        policyVersion: exports.TASKMAP_LIFECYCLE_FACETS_POLICY_VERSION,
 ...
 */
async function buildTaskMapLifecycleFacets(input) {
    const snapshot = snapshotJson(input, "lifecycle-facet build input");
    assertExactKeys(snapshot, ["currentRoot", "runRoot", "sidecarRoot", "projection"], "lifecycle-facet build input");
    const roots = {
    ...
    assertRuntimeIntegrity();
    return buildFromAuthenticatedInputs(firstStore, secondStore, snapshot.projection, workControl.artifact);
}
/**
 ...
 */
function unsafeBuildTaskMapLifecycleFacetsFromAuthenticatedFixturesForTest(input) {
    const snapshot = snapshotJson(input, "unsafe lifecycle-facet authenticated fixture input");
    assertExactKeys(snapshot, ["firstStore", "secondStore", "projection", "workControl"], "unsafe lifecycle-facet authenticated fixture input");
    return buildFromAuthenticatedInputs(snapshot.firstStore, snapshot.secondStore, snapshot.projection, snapshot.workControl);
}
function assertPredecessor(predecessor) {
    ...
    if (!FIXED_GENERATION.test(predecessor.generation)
        || BIGINT_CONSTRUCTOR(predecessor.generation) === 0n) {
        fail("predecessor generation is invalid");
    }
    assertHashedId(predecessor.currentRefId, HASHED_IDS.currentRef, "current-ref ID");
    assertDigest(predecessor.currentRefDigest, "current-ref digest");
    assertHashedId(predecessor.entryId, HASHED_IDS.entry, "entry ID");
    assertHashedId(predecessor.acceptedOriginBundleId, HASHED_IDS.bundle, "accepted-origin bundle ID");
    assertDigest(predecessor.acceptedOriginReplayDigest, "accepted-origin replay digest");
    ...
    assertDigest(predecessor.sourceSemanticInputDigest, "source-semantic-input digest");
    assertHashedId(predecessor.workControlArtifactId, HASHED_IDS.workControl, "work-control artifact ID");
    assertDigest(predecessor.workControlArtifactDigest, "work-control artifact digest");
    if (predecessor.workControlArtifactId
        !== `tmworkcontroldecision_${predecessor.workControlArtifactDigest}`) {
        fail("work-control artifact ID does not bind its digest");
    }
    assertDigest(predecessor.workControlOriginDigest, "work-control origin digest");
    assertHashedId(predecessor.projectionRunId, HASHED_IDS.projectionRun, "projection run ID");
...
function assertArtifactPrivacy(value, path = "artifact") {
    ...
    for (const key of OBJECT_KEYS(value)) {
        if (/^(?:title|summary|body|transcript|candidateToken|candidateText|route|returnRoute|sourceObjectId|sourceRevision|ownerId|participant|email|localPath|credential|secret)$/i
            .test(key)) {
            fail(`${path}.${key} is forbidden by the privacy contract`);
        }
        assertArtifactPrivacy(value[key], `${path}.${key}`);
    }
}
/**
 ...
 */
function assertTaskMapLifecycleFacets(value) {
    const artifact = snapshotJson(value, "lifecycle-facet artifact", exports.TASKMAP_LIFECYCLE_FACETS_LIMITS_V1.maxCanonicalArtifactBytes);
    ...
    if (artifact.contractVersion !== exports.TASKMAP_LIFECYCLE_FACETS_VERSION) {
        fail("artifact contract version is unsupported");
    }
    if (artifact.policyVersion !== exports.TASKMAP_LIFECYCLE_FACETS_POLICY_VERSION
    ...
    if (!ARRAY_IS_ARRAY(artifact.rows)
        || artifact.rows.length
            > exports.TASKMAP_LIFECYCLE_FACETS_LIMITS_V1.maxWorks) {
        fail("artifact rows exceed the work ceiling");
    }
    const workIds = new SET_CONSTRUCTOR();
    const taskIds = new SET_CONSTRUCTOR();
    const deltaIds = new SET_CONSTRUCTOR();
    const lifecycleEventIds = new SET_CONSTRUCTOR();
    let previousWorkId;
    for (const [index, row] of artifact.rows.entries()) {
        ...
        if (!["accepted_open", "source_complete", "superseded", "rejected"]
            .includes(row.lifecycle)) {
            fail(`${label} lifecycle is unsupported`);
        }
        if (!["new", "updated", "resolved", "superseded", "rejected", "unchanged"]
            .includes(row.change)) {
        ...
        assertExactKeys(row.lifecycleDelta, [
            "deltaId",
            "previousState",
            "currentState",
            "adjudicatedDelta",
        ...
        assertHashedId(row.lifecycleDelta.deltaId, HASHED_IDS.delta, `${label} delta ID`);
        ...
        assertDigest(row.lifecycleDelta.adjudicationDigest, `${label} adjudication digest`);
        assertHashedId(row.lifecycleDelta.lifecycleEventId, HASHED_IDS.lifecycleEvent, `${label} lifecycle event ID`);
        ...
        assertLifecycleTruth(row.lifecycleDelta.previousState, row.lifecycleDelta.currentState, row.lifecycleDelta.adjudicatedDelta, label);
        if (lifecycleFromP10_2(row.lifecycleDelta.currentState) !== row.lifecycle) {
    ...
    if (workIds.size !== artifact.rows.length
    ...
    assertExactKeys(artifact.privacy, OBJECT_KEYS(PRIVACY), "lifecycle-facet privacy");
    for (const key of OBJECT_KEYS(PRIVACY)) {
    ...
    if (BUFFER_BYTE_LENGTH(bytes, "utf8")
        > exports.TASKMAP_LIFECYCLE_FACETS_LIMITS_V1.maxCanonicalArtifactBytes) {
        fail("artifact exceeds the canonical byte ceiling");
    }
    return deepFreeze(artifact);
}
function taskMapLifecycleFacetsCanonicalBytes(value) {
