...
exports.buildTaskMapBodyContextDisclosure = buildTaskMapBodyContextDisclosure;
...
function isStrictDay(value) {
    if (!STRICT_DAY.test(value))
        return false;
...
function validateSafeContext(context) {
    if (context.contractVersion !== OURA_CONTEXT_CONTRACT_VERSION
        ||
    ...
    if (!SAFE_CLASSIFIER_VERSION.test(context.classifier.version)
    ...
    if (context.privacy.rawBiometricsStored
    ...
    if (classifiedDays !== context.coverage.classifiedDays
...
function actionableBodyEvents(input, projection) {
    ...
    return input.events
        ...
        && now - Date.parse(event.occurredAt) <= harness_js_1.TASKMAP_ALGORITHM_POLICY.bodyFreshnessWindowMs))
        .sort((left, right) => left.dayKey.localeCompare(right.dayKey));
}
function nodeMatches(citationEventIds, bodyEvents, events, pointers) {
    const evidence = citationEventIds
        .map((id) => events.get(id))
        .filter((event) => (event !== undefined && event.bodyJoinEligible === true));
    const matches = bodyEvents.flatMap((body) => {
        const bodyTime = Date.parse(body.occurredAt);
        const backingPointers = new Set(evidence
            .filter((event) => (Math.abs(Date.parse(event.occurredAt) - bodyTime)
            <= harness_js_1.TASKMAP_ALGORITHM_POLICY.bodyJoinWindowMs))
            .filter((event) => pointers.has(event.pointerId))
            .map((event) => event.pointerId));
        if (backingPointers.size === 0)
            return [];
        return [{
                dayKey: body.dayKey,
                axis: body.bodyAxis,
                category: body.bodyCategory,
 ...
 */
function buildTaskMapBodyContextDisclosure(input, projection, context) {
    validateSafeContext(context);
    if (projection.runStatus !== "accepted"
        || projection.rejections.length !== 0
        || projection.inputDigest !== (0, harness_js_1.taskMapInputDigest)(input)
    ...
    const nodes = [...projection.roots, ...projection.tasks].flatMap((node) => {
        const matches = nodeMatches(node.citations.map((citation) => citation.eventId), bodyEvents, events, pointers);
