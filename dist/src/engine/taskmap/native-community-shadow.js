...
exports.TaskMapNativeCommunityShadowUnavailableError = exports.TASKMAP_NATIVE_COMMUNITY_ROOT_EVIDENCE_LIMITS_V1 = exports.TASKMAP_NATIVE_COMMUNITY_SHADOW_LIMITS_V1 = exports.TASKMAP_NATIVE_COMMUNITY_GRAPH_COVERAGE_POLICY_V1 = exports.TASKMAP_NATIVE_COMMUNITY_GRAPH_COVERAGE_VERSION = exports.TASKMAP_NATIVE_COMMUNITY_SHADOW_UNAVAILABLE_VERSION = exports.TASKMAP_NATIVE_COMMUNITY_SHADOW_VERSION = void 0;
exports.taskMapNativeCommunityCandidateNodeBindings = taskMapNativeCommunityCandidateNodeBindings;
exports.buildTaskMapNativeCommunityShadowUnavailableReceipt = buildTaskMapNativeCommunityShadowUnavailableReceipt;
exports.buildTaskMapNativeCommunityPlan = buildTaskMapNativeCommunityPlan;
exports.mapTaskMapNativeCommunityPlanToAgentRoots = mapTaskMapNativeCommunityPlanToAgentRoots;
exports.buildTaskMapNativeCommunityRootEvidence = buildTaskMapNativeCommunityRootEvidence;
exports.buildTaskMapNativeCommunityShadow = buildTaskMapNativeCommunityShadow;
...
const community_title_replay_js_1 = require("./community-title-replay.js");
const community_semantic_evidence_js_1 = require("./community-semantic-evidence.js");
...
exports.TASKMAP_NATIVE_COMMUNITY_SHADOW_VERSION = "taskmap-native-community-shadow.v1";
exports.TASKMAP_NATIVE_COMMUNITY_SHADOW_UNAVAILABLE_VERSION = "taskmap-native-community-shadow-unavailable.v1";
exports.TASKMAP_NATIVE_COMMUNITY_GRAPH_COVERAGE_VERSION = "taskmap-native-community-graph-coverage.v1";
exports.TASKMAP_NATIVE_COMMUNITY_GRAPH_COVERAGE_POLICY_V1 = Object.freeze({
    directoryLimit: 1_024,
    candidateLimit: 4_096,
    attemptLimit: 1_024,
    globalScanByteLimit: 64 * 1_024 * 1_024,
    perFileScanByteLimit: 512 * 1_024,
    observationLimit: agent_session_producer_freshness_js_1.TASKMAP_GRAPH_BRAIN_LIMITS_V1.maxObservations,
    rawByteLimit: agent_session_producer_freshness_js_1.TASKMAP_GRAPH_BRAIN_LIMITS_V1.maxRawBytesGlobal,
    maxGraphEpisodes: agent_session_producer_freshness_js_1.TASKMAP_GRAPH_BRAIN_LIMITS_V1.maxEpisodesGlobal,
});
exports.TASKMAP_NATIVE_COMMUNITY_SHADOW_LIMITS_V1 = Object.freeze({
    maximumInputBytes: 2 * 1_024 * 1_024,
    maximumOutputBytes: 2 * 1_024 * 1_024,
    maximumAgentNodes: community_graph_brain_js_1.TASKMAP_GRAPH_BRAIN_LIMITS_V1.agentEpisodeCapacity,
    maximumMeetingCalendarNodes: community_graph_brain_js_1.TASKMAP_GRAPH_BRAIN_LIMITS_V1.meetingCalendarHeadroom,
});
exports.TASKMAP_NATIVE_COMMUNITY_ROOT_EVIDENCE_LIMITS_V1 = Object.freeze({
    maximumEventsPerRoot: 5,
    maximumEventsTotal: 128,
});
class TaskMapNativeCommunityShadowUnavailableError extends TypeError {
    code;
    constructor(code) {
...
exports.TaskMapNativeCommunityShadowUnavailableError = TaskMapNativeCommunityShadowUnavailableError;
...
const PERSISTENT_ID_CREDENTIAL = /(?:^|[._:=\-\s])(?:api[._-]?key|access[._-]?token|refresh[._-]?token|token|bearer|credential|password|secret)(?:$|[._:=\-\s])/iu;
const PERSISTENT_ID_PROVIDER_TOKEN = /(?:sk-(?:proj-)?[A-Za-z0-9_-]{16,}|gh[pousr]_[A-Za-z0-9]{16,}|xox[baprs]-[A-Za-z0-9-]{12,}|aiza[A-Za-z0-9_-]{12,}|grn_[A-Za-z0-9_-]{12,}|dbk_[A-Za-z0-9_-]{12,})/iu;
const PERSISTENT_ID_AWS_ACCESS_KEY = /(?:AKIA|ASIA)[0-9A-Z]{16}/u;
const PERSISTENT_ID_JWT = /[A-Za-z0-9_-]{8,}\.[A-Za-z0-9_-]{8,}\.[A-Za-z0-9_-]{8,}/u;
...
const INERT_GROUPING_REPLAY_PATH = "/dev/null/taskmap-native-community-shadow-grouping";
const INERT_EMBEDDING_CACHE_PATH = "/dev/null/taskmap-native-community-shadow-embeddings";
...
function assertSafeId(value) {
    if (typeof value !== "string"
        || value.length === 0
        || value.length > community_graph_brain_js_1.TASKMAP_GRAPH_BRAIN_LIMITS_V1.maxOpaqueIdCharacters
        || CONTROL.test(value))
        fail("invalid_input");
}
function assertPrivacySafePersistentId(value) {
    assertSafeId(value);
...
function hasSensitivePersistedText(value) {
    return SENSITIVE_PERSISTED_TEXT_PATTERNS.some((pattern) => pattern.test(value));
}
function redactSensitiveNodeText(value) {
    let redacted = (0, text_contract_js_1.toWellFormedText)(value);
    ...
    return (0, text_contract_js_1.boundedUtf16)(redacted.replace(/\p{Cc}/gu, " ").replace(/\s+/gu, " ").trim(), community_graph_brain_js_1.TASKMAP_GRAPH_BRAIN_LIMITS_V1.maxNodeTextCharacters);
}
function assertPrivacySafeProposalSet(proposalSet) {
    ...
    for (const proposal of proposalSet.proposals) {
        ...
        if (proposal.title !== (0, text_contract_js_1.toWellFormedText)(proposal.title)
...
function hasUnanchoredSensitiveArtifactValue(value) {
    if (typeof value === "string")
        return hasSensitivePersistedText(value);
...
function assertPrivacySafeArtifactBase(value) {
    if ((0, harness_js_1.taskMapProjectionPrivacyLeakReasons)(value).length > 0
...
function assertCanonicalByteBudget(value, maximum) {
    let bytes;
    try {
        bytes = Buffer.byteLength((0, source_contracts_js_1.taskMapContractCanonicalJson)(value), "utf8");
    }
    catch {
        fail("invalid_input");
    }
...
function validateLegacyBindings(value) {
    if (!plain(value)
...
function validateLegacyFlags(value) {
    if (!plain(value)
        || !exactKeys(value, new Set([
...
function validateGraphCollectionCoverage(value) {
    if (!plain(value))
        fail("invalid_input");
    const coverage = value;
    if (!exactKeys(coverage, new Set(["authority", "completeness", "contractVersion", "discovery", "distribution", "observations", "privacy", "reads", "scan", "truncationReasons"]))
        || coverage.contractVersion !== exports.TASKMAP_NATIVE_COMMUNITY_GRAPH_COVERAGE_VERSION
        || coverage.authority !== "none"
        || !["complete", "bounded_partial", "unknown"].includes(String(coverage.completeness)))
        fail("invalid_input");
    const expectedKeys = {
    ...
    for (const key of ["discovery", "reads", "scan", "observations", "distribution"]) {
        if (!plain(coverage[key]))
            fail("invalid_input");
        if (!exactKeys(coverage[key], expectedKeys[key]))
            fail("invalid_input");
    ...
    if (!Array.isArray(coverage.truncationReasons))
        fail("invalid_input");
    const truncationReasons = coverage.truncationReasons;
    ...
    if (truncationReasons.some((reason) => typeof reason !== "string" || !allowedReasons.has(reason)))
        fail("invalid_input");
    const canonicalReasons = [...truncationReasons].sort(compareCodePoint);
    if (new Set(canonicalReasons).size !== canonicalReasons.length || canonicalReasons.some((reason, index) => reason !== truncationReasons[index]))
        fail("invalid_input");
    const discovery = coverage.discovery;
    const reads = coverage.reads;
    const scan = coverage.scan;
    const observations = coverage.observations;
    const reasons = new Set(coverage.truncationReasons);
    if (typeof discovery.directoryLimitReached !== "boolean"
        || typeof discovery.candidateLimitReached !== "boolean"
        || reads.attemptedFiles > reads.attemptLimit
        || discovery.directoriesVisited > discovery.directoryLimit
        || discovery.candidatesDiscovered > discovery.candidateLimit
        || scan.chargedBytes > scan.globalByteLimit
        || observations.selectedObservations > observations.observationLimit
        || observations.rawBytesSelected > observations.rawByteLimit
        || observations.graphEpisodesSelected > observations.maxGraphEpisodes
        || discovery.directoryLimit !== exports.TASKMAP_NATIVE_COMMUNITY_GRAPH_COVERAGE_POLICY_V1.directoryLimit
        || discovery.candidateLimit !== exports.TASKMAP_NATIVE_COMMUNITY_GRAPH_COVERAGE_POLICY_V1.candidateLimit
        || reads.attemptLimit !== exports.TASKMAP_NATIVE_COMMUNITY_GRAPH_COVERAGE_POLICY_V1.attemptLimit
        || scan.globalByteLimit !== exports.TASKMAP_NATIVE_COMMUNITY_GRAPH_COVERAGE_POLICY_V1.globalScanByteLimit
        || scan.perFileByteLimit !== exports.TASKMAP_NATIVE_COMMUNITY_GRAPH_COVERAGE_POLICY_V1.perFileScanByteLimit
        || observations.observationLimit !== exports.TASKMAP_NATIVE_COMMUNITY_GRAPH_COVERAGE_POLICY_V1.observationLimit
        || observations.rawByteLimit !== exports.TASKMAP_NATIVE_COMMUNITY_GRAPH_COVERAGE_POLICY_V1.rawByteLimit
        || observations.maxGraphEpisodes !== exports.TASKMAP_NATIVE_COMMUNITY_GRAPH_COVERAGE_POLICY_V1.maxGraphEpisodes
        ...
        || (coverage.completeness !== "unknown" && (reads.droppedAttemptLimit > 0) !== reasons.has("read_attempt_limit"))
        || (coverage.completeness !== "unknown" && (reads.droppedInvalid > 0) !== reasons.has("invalid_or_raced"))
        || (coverage.completeness !== "unknown" && (scan.droppedScanBudget > 0) !== reasons.has("scan_byte_limit"))
        || (coverage.completeness !== "unknown" && (observations.droppedRawByteBudget > 0) !== reasons.has("raw_byte_limit"))
        || (coverage.completeness !== "unknown" && (observations.droppedObservationLimit > 0) !== reasons.has("observation_limit"))
        || (coverage.completeness !== "unknown" && (observations.droppedGraphEpisodes > 0) !== reasons.has("graph_episode_limit")))
        fail("invalid_input");
}
function validateCoverageFeedBinding(coverage, feed) {
    ...
    if (coverage.observations.selectedObservations !== inputObservations
...
function validateFeedEpisode(value) {
    ...
    if (value.graphEpisodeId !== stableId("tmagraph", value.graphEpisodeDigest)
        ...
        || value.userDirectiveSummary.length
            > community_graph_brain_js_1.TASKMAP_GRAPH_BRAIN_LIMITS_V1.maxNodeTextCharacters
    ...
    for (const key of [
        ...
        if (!Array.isArray(digests) || digests.length > 2)
            fail("invalid_input");
    ...
    const episode = value;
    const { episodeId, episodeRevisionDigest, graphEpisodeId: _graphEpisodeId, graphEpisodeDigest: _graphEpisodeDigest, graphEpisodeRevisionDigest: _graphEpisodeRevisionDigest, workstreamIdentityDigest: _workstreamIdentityDigest, routingKind: _routingKind, providerNeutralRoutingDigest: _providerNeutralRoutingDigest, isoWeek: _isoWeek, recurrenceCount: _recurrenceCount, firstSeenAt: _firstSeenAt, ...producerEpisodeBase } = episode;
    if (episodeId !== `tmaepisode_${(0, source_contracts_js_1.taskMapContractDigest)(episode.episodeIdentityDigest).slice(0, 16)}`
        || episodeRevisionDigest !== (0, source_contracts_js_1.taskMapContractDigest)(producerEpisodeBase))
        fail("invalid_input");
}
function validateAgentFeed(value, ownerScopeDigest, requestedAt) {
    if (value === null)
        return;
    if (!plain(value)
        || !exactKeys(value, new Set([
        ...
        || value.contractVersion !== agent_session_semantic_admission_js_1.TASKMAP_AGENT_SESSION_GRAPH_FEED_VERSION
        || value.ownerScopeDigest !== ownerScopeDigest
        || value.authority !== "none"
        || value.acceptedMembershipAuthority !== false
        || (0, source_contracts_js_1.taskMapContractCanonicalJson)(value.limits)
            !== (0, source_contracts_js_1.taskMapContractCanonicalJson)(agent_session_producer_freshness_js_1.TASKMAP_GRAPH_BRAIN_LIMITS_V1)
        || !Array.isArray(value.episodes)
        || value.episodes.length
            > exports.TASKMAP_NATIVE_COMMUNITY_SHADOW_LIMITS_V1.maximumAgentNodes
        || !plain(value.counts)
    ...
    for (const key of [
        ...
        assertCount(value.counts[key]);
    const eligibleEpisodes = value.counts.eligibleEpisodes;
    const deduplicatedEpisodes = value.counts.deduplicatedEpisodes;
    const selectedEpisodes = value.counts.selectedEpisodes;
    if (selectedEpisodes !== value.episodes.length
    ...
    if (feedDigest !== (0, source_contracts_js_1.taskMapContractDigest)(base)
        || feedId !== stableId("tmagraphfeed", feedDigest))
        fail("invalid_input");
}
function validateSemanticOptions(value) {
    ...
    for (const model of [stationModel, value.embeddingModelId]) {
        if (typeof model === "string"
            && ((0, harness_js_1.taskMapProjectionPrivacyLeakReasons)(model).length > 0
...
function validatePointer(value) {
    ...
    if (value.sourceVersion !== undefined)
        assertDigest(value.sourceVersion);
...
function validateEvent(value) {
    ...
    if (!Array.isArray(value.objectRefs)
        || value.objectRefs.length
            > community_graph_brain_js_1.TASKMAP_GRAPH_BRAIN_LIMITS_V1.maxExternalRefsPerNode
        || value.objectRefs.some((ref) => typeof ref !== "string" || ref.length === 0 || ref.length > 512)
...
function validateSourceBinding(value) {
    ...
    if (typeof value.observedRevision !== "string"
...
function validateEvidenceBinding(value) {
    if (!plain(value)
        || !exactKeys(value, new Set(["disposition", "eventId", "rootLinkRefs"]), EVIDENCE_BINDING_KEYS))
        fail("invalid_input");
    assertSafeId(value.eventId);
    if (!Array.isArray(value.rootLinkRefs)
        || value.rootLinkRefs.some((ref) => typeof ref !== "string" || !/^external:[a-f0-9]{64}$/.test(ref))
        || new Set(value.rootLinkRefs).size !== value.rootLinkRefs.length)
        fail("invalid_input");
}
function sourceFamily(pointer, sourceBinding) {
    if ((pointer.sourceKind === "gemini_meet" || pointer.sourceKind === "granola")
        && sourceBinding.semanticClass === "meeting_context")
        return "meeting";
    if ((pointer.sourceKind === "google_calendar"
        || pointer.sourceKind === "local_calendar")
        && sourceBinding.semanticClass === "context_only")
        return "calendar";
    return null;
}
function validateCandidateBinding(event, pointer, sourceBinding, evidenceBinding) {
    ...
    if (family === null
        ...
        || evidenceBinding.rootLinkRefs.some((ref) => !event.objectRefs.includes(ref) || ref === candidateIdentityRef)
        ...
        fail("invalid_input");
    const externalRefs = event.objectRefs.filter((ref) => /^external:[a-f0-9]{64}$/.test(ref));
    const expectedExternalRefs = new Set([
        candidateIdentityRef,
        ...evidenceBinding.rootLinkRefs,
    ]);
    if (externalRefs.length !== expectedExternalRefs.size
        || externalRefs.some((ref) => !expectedExternalRefs.has(ref)))
        fail("invalid_input");
    const expectedCandidateIdentityRef = `external:${(0, meeting_producer_freshness_js_1.taskMapMeetingStatementReferenceDigest)({
        kind: evidenceBinding.candidateKind,
        title: event.title,
        summary: event.summary,
        explicitExternalReferenceDigests: evidenceBinding.rootLinkRefs.map((ref) => ref.slice("external:".length)),
        ...(evidenceBinding.mentionIdentityDigest === undefined
            ? {}
...
function validateSemanticFragment(value, ownerScopeDigest, requestedAt) {
    if (value === undefined)
        return undefined;
    if (!plain(value)
        || !exactKeys(value, new Set([
        ...
        || value.contractVersion !== native_semantic_builder_adapter_js_1.TASKMAP_NATIVE_SEMANTIC_BUILDER_INPUT_VERSION
        || value.ownerScopeDigest !== ownerScopeDigest
        ...
        || value.sourceBindings.length
            > native_semantic_builder_adapter_js_1.TASKMAP_NATIVE_SEMANTIC_BUILDER_LIMITS_V1.maximumSourceBindings
        || value.evidenceBindings.length
            > native_semantic_builder_adapter_js_1.TASKMAP_NATIVE_SEMANTIC_BUILDER_LIMITS_V1.maximumEvidenceBindings
        || value.taskMapInput.pointers.length
            > native_semantic_builder_adapter_js_1.TASKMAP_NATIVE_SEMANTIC_BUILDER_LIMITS_V1.maximumPointers
        || value.taskMapInput.events.length
            > native_semantic_builder_adapter_js_1.TASKMAP_NATIVE_SEMANTIC_BUILDER_LIMITS_V1.maximumTotalEvents
        || value.taskMapInput.contractVersion !== types_js_1.TASKMAP_CONTRACT_VERSION
        || value.producer.id !== meeting_producer_freshness_js_1.TASKMAP_MEETING_PRODUCER_RESULT_VERSION
        || value.producer.version !== meeting_producer_freshness_js_1.TASKMAP_MEETING_PRODUCER_VERSION
    ...
    if (validThroughMs - producedAtMs !== meeting_producer_freshness_js_1.TASKMAP_MEETING_PRODUCER_MAX_AGE_MS
    ...
    if (pointers.size !== value.taskMapInput.pointers.length
    ...
    for (const event of events.values()) {
        ...
        if (pointer === undefined || source === undefined)
            fail("invalid_input");
        if (evidence.disposition === "candidate_only") {
            validateCandidateBinding(event, pointer, source, evidence);
        }
    }
    if ((0, harness_js_1.taskMapInputValidationReasons)(value.taskMapInput).length > 0)
        fail("invalid_input");
    return value;
}
function validatePreviousAcceptedRoots(value) {
    if (!Array.isArray(value)
        || value.length
    ...
    for (const root of value) {
        if (!plain(root)
            ...
            || root.memberNodeIds.length
                > community_root_proposals_js_1.TASKMAP_COMMUNITY_ROOT_PROPOSAL_LIMITS_V1
        ...
        if (rootIds.has(root.rootProposalId)
    ...
    })).sort((left, right) => compareCodePoint(left.rootProposalId, right.rootProposalId));
    if (Buffer.byteLength((0, source_contracts_js_1.taskMapContractCanonicalJson)(normalized), "utf8")
        > community_root_proposals_js_1.TASKMAP_COMMUNITY_ROOT_PROPOSAL_LIMITS_V1.maxPreviousRootsBytes)
        fail("invalid_input");
}
function validateInput(value) {
    ...
    assertCanonicalByteBudget(value, exports.TASKMAP_NATIVE_COMMUNITY_SHADOW_LIMITS_V1.maximumInputBytes);
    ...
    validateAgentFeed(value.agentSessionGraphFeed, value.ownerScopeDigest, value.requestedAt);
    validateCoverageFeedBinding(value.graphCollectionCoverage, value.agentSessionGraphFeed);
    validateSemanticOptions(value.semanticEvidence);
    validatePreviousAcceptedRoots(value.previousAcceptedRoots);
    validateSemanticFragment(value.candidateSemanticFragment, value.ownerScopeDigest, value.requestedAt);
    return value;
}
function agentNodes(feed) {
    if (feed === null)
        return [];
...
function candidateDrafts(fragment) {
    if (fragment === undefined)
        return [];
    const inactiveEventIds = new Set();
    const retractorEventIds = new Set();
    ...
    for (const evidenceBinding of fragment.evidenceBindings) {
        if (evidenceBinding.disposition !== "candidate_only")
            continue;
        if (inactiveEventIds.has(evidenceBinding.eventId)
        ...
        const family = validateCandidateBinding(event, pointer, sourceBinding, evidenceBinding);
        const key = `${family}\u0000${evidenceBinding.candidateIdentityRef}\u0000${sourceBinding.semanticOriginId}`;
        const candidate = {
...
function fairFamilyWeekQueue(drafts) {
    ...
    const weeks = [...byWeek.entries()]
        .map(([week, members]) => ({
        week,
        members: members.sort((left, right) => compareCodePoint(left.event.id, right.event.id)),
    }))
...
function fairCandidateHead(drafts) {
    const familyQueues = ["calendar", "meeting"].map((family) => ({
        family,
        queue: fairFamilyWeekQueue(drafts.filter((draft) => draft.sourceFamily === family)),
    }));
    const selected = [];
    selection: for (let round = 0;; round += 1) {
        ...
        for (const family of familyQueues) {
            ...
            added = true;
            if (selected.length
                === exports.TASKMAP_NATIVE_COMMUNITY_SHADOW_LIMITS_V1
...
function candidateNode(ownerScopeDigest, draft) {
    const nodeIdentityDigest = (0, source_contracts_js_1.taskMapContractDigest)({
    ...
    const routingDigest = (0, source_contracts_js_1.taskMapContractDigest)({
    ...
    if (text.length === 0 || CONTROL.test(text))
        fail("invalid_input");
    return {
        ...
        externalRefs,
        embedding: null,
        isCalendarCommitment: draft.sourceFamily === "calendar"
            && draft.evidenceBinding.candidateKind === "commitment",
    };
}
/** Exact candidate-to-node bridge for the review hierarchy. */
function taskMapNativeCommunityCandidateNodeBindings(fragment, ownerScopeDigest, requestedAt) {
    const validated = validateSemanticFragment(fragment, ownerScopeDigest, requestedAt);
    if (validated === undefined)
        return [];
    return candidateDrafts(validated).map((draft) => {
        const match = /^external:([a-f0-9]{64})$/.exec(draft.candidateIdentityRef);
        if (match === null)
            fail("invalid_input");
        return {
...
function sourceFamilyCounts(nodes) {
    const counts = {
        agent: 0,
        meeting: 0,
        calendar: 0,
...
function unavailableReceiptBase(input) {
    return {
        contractVersion: exports.TASKMAP_NATIVE_COMMUNITY_SHADOW_UNAVAILABLE_VERSION,
...
function buildTaskMapNativeCommunityShadowUnavailableReceipt(input) {
    if (!plain(input)
    ...
    const receiptDigest = (0, source_contracts_js_1.taskMapContractDigest)(base);
    const result = {
        ...base,
        receiptId: stableId("tmcommunityunavailable", receiptDigest),
        receiptDigest,
    };
    if (Buffer.byteLength((0, source_contracts_js_1.taskMapContractCanonicalJson)(result), "utf8") >= 4_096) {
 ...
 */
async function buildTaskMapNativeCommunityPlan(input) {
    ...
    return deepFreeze({
        ...base,
        planDigest: (0, source_contracts_js_1.taskMapContractDigest)(base),
        groupingAvailable: artifact.semanticEvidenceReport.grouping.state === "available",
    });
}
/**
 ...
 */
function mapTaskMapNativeCommunityPlanToAgentRoots(input) {
    (0, agent_session_semantic_admission_js_1.assertTaskMapAgentSessionSemanticAdmission)(input.admission);
    validateAgentFeed(input.feed, input.admission.ownerScopeDigest, input.admission.producedAt);
    if (input.plan.feedDigest !== input.feed.feedDigest)
        fail("invalid_input");
    const episodeById = new Map(input.feed.episodes.map((episode) => [
        episode.graphEpisodeId,
        episode,
    ]));
    const exactKey = (value) => `${value.workstreamIdentityDigest}\u0000${value.directiveSemanticDigest}`;
    const rootIdsByExactKey = new Map();
    const proposalById = new Map(input.plan.proposalSet.proposals.map((proposal) => [proposal.rootProposalId, proposal]));
    for (const proposal of input.plan.proposalSet.proposals) {
        for (const memberNodeId of proposal.memberNodeIds) {
            ...
            const rootIds = rootIdsByExactKey.get(key) ?? new Set();
            rootIds.add(proposal.rootProposalId);
            rootIdsByExactKey.set(key, rootIds);
        }
    }
    const clusterIdsByRoot = new Map();
    for (const cluster of input.admission.clusters) {
        ...
        if (!proposalById.has(rootProposalId))
            continue;
    ...
    return deepFreeze({
        roots: [...clusterIdsByRoot.entries()].map(([rootProposalId, clusterIdentityDigests,]) => ({
            rootProposalId,
            title: proposalById.get(rootProposalId).title,
            clusterIdentityDigests: [...new Set(clusterIdentityDigests)].sort(compareCodePoint),
        })).sort((left, right) => compareCodePoint(left.rootProposalId, right.rootProposalId)),
    });
}
function historicalAgentTupleKey(value) {
    return `${value.workstreamIdentityDigest}\u0000${value.directiveSemanticDigest}`;
}
function buildTaskMapNativeCommunityRootEvidence(input) {
    validateAgentFeed(input.feed, input.feed.ownerScopeDigest, input.generatedAt);
    if (input.plan.feedDigest !== input.feed.feedDigest
        || input.plan.planDigest !== (0, source_contracts_js_1.taskMapContractDigest)({
    ...
    if (currentAdmission !== null) {
        (0, agent_session_semantic_admission_js_1.assertTaskMapAgentSessionSemanticAdmission)(currentAdmission);
        if (currentAdmission.ownerScopeDigest !== input.feed.ownerScopeDigest) {
            fail("invalid_input");
        }
    }
    const excludedTuples = new Set(currentAdmission?.clusters.map(historicalAgentTupleKey) ?? []);
    const episodeById = new Map(input.feed.episodes.map((episode) => [
        episode.graphEpisodeId,
        episode,
    ]));
    const proposalIdsByEpisode = new Map();
    for (const proposal of input.plan.proposalSet.proposals) {
        for (const memberNodeId of proposal.memberNodeIds) {
            if (!episodeById.has(memberNodeId))
                continue;
    ...
    const proposalIdsByTuple = new Map();
    ...
    const compareEpisodePriority = (left, right) => Date.parse(right.occurredAt) - Date.parse(left.occurredAt)
    ...
    const proposalRows = input.plan.proposalSet.proposals.flatMap((proposal) => {
        ...
        for (const memberNodeId of proposal.memberNodeIds) {
            ...
            if (episode === undefined
                ...
                continue;
            const tuple = historicalAgentTupleKey(episode);
            if (proposalIdsByTuple.get(tuple)?.size !== 1)
                continue;
        ...
        return [{
                proposal,
                evidenceCandidates: allEpisodes.filter((episode) => !excludedTuples.has(historicalAgentTupleKey(episode))).slice(0, exports.TASKMAP_NATIVE_COMMUNITY_ROOT_EVIDENCE_LIMITS_V1
    ...
    for (let round = 0; round
        < exports.TASKMAP_NATIVE_COMMUNITY_ROOT_EVIDENCE_LIMITS_V1.maximumEventsPerRoot; round += 1) {
        for (const row of proposalRows) {
            if (selectedEventCount
                >= exports.TASKMAP_NATIVE_COMMUNITY_ROOT_EVIDENCE_LIMITS_V1.maximumEventsTotal)
                break;
    ...
    const activeRows = proposalRows.filter((row) => row.selectedEvidence.length > 0);
    ...
    const episodeIdentity = (episode) => ({
        pointerId: `agent-community-evidence-${(0, source_contracts_js_1.taskMapContractDigest)({
    ...
    const evidenceTitle = (episode) => (0, text_contract_js_1.boundedUtf16)(redactSensitiveNodeText(episode.userDirectiveSummary), 96);
    const evidenceSummary = (episode) => (0, text_contract_js_1.boundedUtf16)(redactSensitiveNodeText(episode.assistantOutcomeSummary ?? episode.userDirectiveSummary), 200);
    const pointers = [];
    ...
    const communityRef = (proposalDigest) => `community:${proposalDigest}`;
    const communityRootIdentityRef = (rootProposalId) => `community:root:${rootProposalId}`;
    const communityMemberIdentityRef = (graphEpisodeId) => `community:member:${graphEpisodeId}`;
    const eventId = (rootProposalId, episode) => `agent-community-event-${(0, source_contracts_js_1.taskMapContractDigest)({
        rootProposalId,
        graphEpisodeId: episode.graphEpisodeId,
    }).slice(0, 16)}`;
    const events = activeRows.flatMap((row) => row.selectedEvidence.map((episode) => {
        const identity = episodeIdentity(episode);
        return {
    ...
    const rootProposals = activeRows.map((row) => ({
        proposalId: row.proposal.rootProposalId,
        title: row.proposal.title,
        summary: (0, text_contract_js_1.boundedUtf16)(row.selectedEvidence.map(evidenceTitle).join(" · "), 200),
        evidenceEventIds: row.selectedEvidence.map((episode) => eventId(row.proposal.rootProposalId, episode)).sort(compareCodePoint),
        memberObjectRefs: [
            communityRef(row.proposal.proposalDigest),
            communityRootIdentityRef(row.proposal.rootProposalId),
            ...new Set(row.selectedEvidence.map((episode) => communityMemberIdentityRef(episode.graphEpisodeId))),
            ...new Set(row.selectedEvidence.map((episode) => episodeIdentity(episode).workstreamRef)),
        ].sort(compareCodePoint),
        confidence: 0.8,
    }));
...
function buildArtifactBase(input) {
    return {
        contractVersion: exports.TASKMAP_NATIVE_COMMUNITY_SHADOW_VERSION,
        ...
        graphCollectionCoverage: structuredClone(input.validated.graphCollectionCoverage),
 ...
 */
async function buildTaskMapNativeCommunityShadow(input) {
    let validated;
    try {
    ...
    try {
        const candidateHead = fairCandidateHead(candidateDrafts(validated.candidateSemanticFragment));
        const nodes = [
            ...agentNodes(validated.agentSessionGraphFeed),
            ...candidateHead.map((draft) => candidateNode(validated.ownerScopeDigest, draft)),
        ].sort((left, right) => compareCodePoint(left.nodeId, right.nodeId));
        if (nodes.length > community_graph_brain_js_1.TASKMAP_GRAPH_BRAIN_LIMITS_V1.maxNodes
            || new Set(nodes.map((node) => node.nodeId)).size !== nodes.length)
            fail("invalid_input");
        const { titleReplayPath, ...semanticOptions } = validated.semanticEvidence;
        const semantic = await (0, community_semantic_evidence_js_1.buildTaskMapCommunitySemanticEvidence)({
            nodes,
            ...semanticOptions,
            groupingReplayPath: semanticOptions.groupingReplayPath
                ?? INERT_GROUPING_REPLAY_PATH,
            embeddingCachePath: semanticOptions.embeddingCachePath
                ?? INERT_EMBEDDING_CACHE_PATH,
        });
        if (semantic.report.grouping.producedAt !== null) {
            assertTimestamp(semantic.report.grouping.producedAt);
        }
        const graph = (0, community_graph_brain_js_1.buildTaskMapCommunityGraph)({
            nodes: semantic.nodes,
            semanticGroups: semantic.semanticGroups,
        });
        const nodeLookup = new Map(semantic.nodes.map((node) => [node.nodeId, node]));
        const nodeLookupDigest = (0, community_root_proposals_js_1.taskMapCommunityRootNodeLookupDigest)(nodeLookup);
        ...
        const titleRequest = titleReplayScope === null
            ? null
            : (0, community_root_proposals_js_1.taskMapCommunityTitleBatchRequestDigests)({
                graphOutput: graph,
                nodeLookup,
            });
        let proposalSet;
        if (titleReplayScope !== null && titleRequest !== null) {
            const recorded = await (0, community_title_replay_js_1.loadRecordedCommunityTitleEnvelope)(titleReplayScope, titleRequest.inputDigest);
            if (recorded !== null) {
                try {
                    proposalSet = await (0, community_root_proposals_js_1.buildTaskMapCommunityRootProposals)({
                        graphOutput: graph,
                        nodeLookup,
                        nodeLookupDigest,
        ...
        if (proposalSet === undefined) {
            ...
            proposalSet = await (0, community_root_proposals_js_1.buildTaskMapCommunityRootProposals)({
                ...
                llmStation: liveStation === null || titleReplayScope === null
                    ? liveStation
                    : (0, community_title_replay_js_1.withCommunityTitleEnvelopeRecording)(liveStation, titleReplayScope),
                signal: semanticOptions.signal,
            });
        }
        ...
        const base = buildArtifactBase({
            ...
            edgeDigest: (0, source_contracts_js_1.taskMapContractDigest)(graph.edges),
            proposalSet,
        });
        assertPrivacySafeArtifactBase(base);
        const artifactDigest = (0, source_contracts_js_1.taskMapContractDigest)(base);
        const result = {
            ...base,
        ...
        if (Buffer.byteLength((0, source_contracts_js_1.taskMapContractCanonicalJson)(result), "utf8")
            > exports.TASKMAP_NATIVE_COMMUNITY_SHADOW_LIMITS_V1.maximumOutputBytes)
            fail("output_limit_exceeded");
        return deepFreeze(result);
    }
    catch (error) {
