...
exports.backfillTaskMapIdentityDedupeRollbackObservation = exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1 = exports.TASKMAP_IDENTITY_DEDUPE_STORE_ENTRY_VERSION = exports.TASKMAP_IDENTITY_DECISION_PROOF_VERSION = exports.TASKMAP_IDENTITY_DEDUPE_POLICY_VERSION = exports.TASKMAP_IDENTITY_DEDUPE_REPLAY_CLOSURE_VERSION = exports.TASKMAP_IDENTITY_DEDUPE_DIFF_VERSION = exports.TASKMAP_IDENTITY_DEDUPE_PROJECTION_VERSION = void 0;
exports.taskMapIdentityDedupeUtf8WriteChunks = taskMapIdentityDedupeUtf8WriteChunks;
exports.assertTaskMapIdentityDedupeProjection = assertTaskMapIdentityDedupeProjection;
exports.assertTaskMapIdentityDedupeDiff = assertTaskMapIdentityDedupeDiff;
exports.taskMapIdentityDedupeProjectionCanonicalBytes = taskMapIdentityDedupeProjectionCanonicalBytes;
exports.taskMapIdentityDedupeDiffCanonicalBytes = taskMapIdentityDedupeDiffCanonicalBytes;
exports.taskMapIdentityDedupeReplayClosureDigest = taskMapIdentityDedupeReplayClosureDigest;
exports.unsafeBuildTaskMapIdentityDedupeProjectionFromVerifiedInputsForTest = unsafeBuildTaskMapIdentityDedupeProjectionFromVerifiedInputsForTest;
exports.assertTaskMapIdentityDedupeStoreEntry = assertTaskMapIdentityDedupeStoreEntry;
exports.initializeTaskMapIdentityDedupeProjectionStore = initializeTaskMapIdentityDedupeProjectionStore;
exports.unsafeReadTaskMapIdentityDedupeProjectionStoreStructuralForTest = unsafeReadTaskMapIdentityDedupeProjectionStoreStructuralForTest;
exports.readTaskMapIdentityDedupeProjectionStore = readTaskMapIdentityDedupeProjectionStore;
exports.recoverTaskMapIdentityDedupeProjectionStore = recoverTaskMapIdentityDedupeProjectionStore;
exports.assertTaskMapIdentityDedupeStoreAppendCapacity = assertTaskMapIdentityDedupeStoreAppendCapacity;
exports.buildTaskMapIdentityDedupeProjection = buildTaskMapIdentityDedupeProjection;
exports.buildAndPublishTaskMapIdentityDedupeProjection = buildAndPublishTaskMapIdentityDedupeProjection;
exports.backfillTaskMapIdentityDedupeProjectionGeneration = backfillTaskMapIdentityDedupeProjectionGeneration;
exports.backfillTaskMapIdentityDedupeNoAcceptedGeneration = backfillTaskMapIdentityDedupeNoAcceptedGeneration;
exports.publishTaskMapIdentityDedupeRollbackObservation = publishTaskMapIdentityDedupeRollbackObservation;
...
const refresh_current_ref_js_1 = require("./refresh-current-ref.js");
exports.TASKMAP_IDENTITY_DEDUPE_PROJECTION_VERSION = "taskmap-identity-dedupe-projection.v1";
exports.TASKMAP_IDENTITY_DEDUPE_DIFF_VERSION = "taskmap-identity-dedupe-diff.v1";
exports.TASKMAP_IDENTITY_DEDUPE_REPLAY_CLOSURE_VERSION = "taskmap-identity-dedupe-replay-closure.v1";
exports.TASKMAP_IDENTITY_DEDUPE_POLICY_VERSION = "taskmap-identity-dedupe-policy.1";
exports.TASKMAP_IDENTITY_DECISION_PROOF_VERSION = "taskmap-identity-decision-proof.v1";
exports.TASKMAP_IDENTITY_DEDUPE_STORE_ENTRY_VERSION = "taskmap-identity-dedupe-store-entry.v1";
...
exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1 = Object.freeze({
...
const HASHED_IDS = Object.freeze({
    ...
    event: /^(?:tmmeetingevent|tmsessionevent|tmlifecycleevent)_[a-f0-9]{64}$/,
...
const STORE_WRITER_JOURNAL_VERSION = "taskmap-identity-dedupe-writer-journal.v1";
...
const AGENT_SOURCE_KINDS = new Set([
    "codex_session",
    "claude_session",
    "cursor_session",
]);
const WORK_OBJECT_TYPES = new Set([
    "authoritative_task",
]);
...
function stableId(prefix, value) {
    return `${prefix}_${(0, source_contracts_js_1.taskMapContractDigest)(value)}`;
}
function fail(message) {
    throw new Error(`P10.2 identity/dedupe projection: ${message}`);
}
function decodeExactUtf8(buffer, label) {
    let decoded;
    try {
...
function taskMapIdentityDedupeUtf8WriteChunks(value) {
...
function assertArrayPrototypeIntegrity() {
    ...
    for (let index = 0; index < ARRAY_PROTOTYPE_DESCRIPTORS.length; index += 1) {
        const expected = ARRAY_PROTOTYPE_DESCRIPTORS[index];
        if (currentKeys[index] !== expected.key) {
            fail("Array prototype dependency set is not intact");
        }
        const current = GET_OWN_PROPERTY_DESCRIPTOR(Array.prototype, expected.key);
...
function assertArrayBound(value, limit, label) {
...
function assertExactKeys(value, required, optional, label) {
    ...
    if (ownKeys.length > exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxObjectKeys) {
    ...
    const hasDescriptor = (key) => descriptors.has(key);
...
function preflightJson(value, label, state, depth = 0) {
    state.nodes += 1;
    if (state.nodes > exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxNodes
        || depth > exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxDepth) {
    ...
    const chargeOwnDescriptor = (key) => {
        state.descriptors += 1;
        if (state.descriptors
            > exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxDescriptors) {
            fail(`${label} exceeds the pre-clone descriptor bound`);
        }
        if (key.length > exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxStringLength) {
    ...
    if (typeof value === "string") {
        if (value.length > exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxStringLength) {
    ...
    if (value !== null
        && typeof value === "object"
        && node_util_1.types.isProxy(value)) {
        fail(`${label} must not be proxy-backed`);
    }
    if (Array.isArray(value)) {
        ...
        if (value.length > exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxArrayLength) {
...
function snapshotInput(value) {
    ...
    preflightJson({
        ...
        descriptors: 0,
        bytes: 0,
        byteLimit: exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxSuppliedIdentityProofBytes,
    });
    preflightJson(value, "build input", {
        nodes: 0,
        descriptors: 0,
        bytes: 0,
        byteLimit: exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxCanonicalInputBytes,
    });
    let snapshot;
    try {
    ...
    const bytes = Buffer.byteLength((0, source_contracts_js_1.taskMapContractCanonicalJson)(snapshot), "utf8");
    if (bytes > exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxCanonicalInputBytes) {
        fail("build input exceeds the canonical byte bound");
    }
    return snapshot;
}
function snapshotStoreRoots(value, label = "sidecar store roots") {
    assertExactKeys(value, ["currentRoot", "runRoot", "sidecarRoot"], [], label);
    if (typeof value.currentRoot !== "string"
...
function snapshotStoreInput(value) {
    assertExactKeys(value, [
        "currentRoot",
        "runRoot",
        "sidecarRoot",
...
function snapshotBackfillInput(value) {
    assertExactKeys(value, [
        "currentRoot",
        "runRoot",
        "sidecarRoot",
    ...
    if (typeof value.targetGeneration !== "string"
    ...
    const { targetGeneration, ...storeInput } = value;
    const snapshot = snapshotStoreInput(storeInput);
    return { ...snapshot, targetGeneration };
}
function snapshotObservationInput(value) {
    assertExactKeys(value, [
        "currentRoot",
        "runRoot",
        "sidecarRoot",
    ...
    assertFixedGeneration(value.targetGeneration, "generation observation target");
    try {
        return structuredClone(value);
    }
    catch {
...
function snapshotReplayDigestInput(value) {
    assertExactKeys(value, [
        "sourceSnapshot",
        "projection",
        "suppliedProofs",
    ...
    assertExactKeys(value.suppliedProofs, [
        "aliases",
        "workBindings",
        "sessionLineage",
        "lifecycleAdjudications",
...
function expectedGeneration(index) {
    return BigInt(index + 1).toString().padStart(20, "0");
}
function assertHealthyCurrentHistory(value) {
    if (exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxStoreEntries
        !== refresh_current_ref_js_1.TASKMAP_REFRESH_CURRENT_LIMITS_V1.maxGenerations) {
    ...
    if (value.generations.length
        > exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxStoreEntries) {
        fail("current snapshot exceeds P10.2 generation capacity");
    }
    if (!Array.isArray(value.unknownGenerationNames)
    ...
    const generations = value.generations.map((generation, index) => {
        const normalized = (0, refresh_current_ref_js_1.assertTaskMapRefreshCurrentRef)(generation);
    ...
    if (generations.some((generation) => (generation.ownerScopeDigest !== head.ownerScopeDigest))) {
...
function assertHealthyCurrentSnapshot(value) {
    ...
    const originRef = generations.find((candidate) => (candidate.generation === accepted.originGeneration));
...
function snapshotBindingProjection(snapshot) {
    ...
    return [...rows.values()].sort((left, right) => (compareText(left.bindingDigest, right.bindingDigest)));
}
...
function expectedSnapshotProof(snapshot, origin) {
    ...
    if (!canonicalEqual(bindingProjection, plannedBindings)
    ...
    const core = {
        contractVersion: refresh_run_bundle_js_1.TASKMAP_REFRESH_RUN_SOURCE_SNAPSHOT_PROOF_VERSION,
        ...
        sourceRevisionSetProofDigest: (0, source_contracts_js_1.taskMapContractDigest)(origin.plan.sourceRevisionSets),
...
function assertAcceptedOrigin(origin, accepted, ownerScopeDigest, sourceSnapshot) {
    assertExactKeys(origin, [
        ...
        "batch",
        "checkpoints",
    ], ["sourceSnapshotProof"], "accepted origin");
    const plan = (0, refresh_plan_js_1.assertTaskMapRefreshPlan)(origin.plan);
    const batch = (0, refresh_plan_js_1.assertTaskMapReadyBatch)(plan, origin.batch);
    if (!canonicalEqual(plan, origin.plan)
        || !canonicalEqual(batch, origin.batch)) {
        fail("accepted origin plan or batch is not canonical");
    }
    assertExactKeys(origin.manifest, [
        "contractVersion",
        "bundleId",
        "bundleContentDigest",
        "ownerScopeDigest",
    ...
    assertExactKeys(origin.commitMarker, ["contractVersion", "bundleId", "manifestByteSha256"], [], "accepted origin commit marker");
    if (origin.manifest.contractVersion !== refresh_run_bundle_js_1.TASKMAP_REFRESH_RUN_BUNDLE_VERSION
        || origin.commitMarker.contractVersion
            !== refresh_run_bundle_js_1.TASKMAP_REFRESH_RUN_COMMIT_VERSION
    ...
    assertExactKeys(origin.sourceSnapshotProof, [
        "contractVersion",
    ...
    let reproduced;
    try {
        reproduced = (0, refresh_run_bundle_js_1.prepareTaskMapRefreshRunBundle)({
            plan: origin.plan,
            batch: origin.batch,
            connectorCheckpoints: origin.checkpoints.checkpoints.map((record) => record.checkpoint),
    ...
    if (reproduced.bundleId !== origin.bundleId
        || !canonicalEqual(reproduced.manifest, origin.manifest)
        || !canonicalEqual(reproduced.commitMarker, origin.commitMarker)
        || !canonicalEqual(JSON.parse(reproduced.members["checkpoints.json"]), origin.checkpoints)
        || !canonicalEqual(JSON.parse(reproduced.members["source-snapshot.json"]), origin.sourceSnapshotProof)) {
        fail("accepted origin immutable bundle closure is invalid");
    }
}
function assertPreviousAcceptedOrigin(origin, previous, predecessorRef) {
    ...
    assertExactKeys(origin, [
        ...
        "batch",
        "checkpoints",
    ], ["sourceSnapshotProof"], "previous accepted origin");
    const plan = (0, refresh_plan_js_1.assertTaskMapRefreshPlan)(origin.plan);
    const batch = (0, refresh_plan_js_1.assertTaskMapReadyBatch)(plan, origin.batch);
    if (!canonicalEqual(plan, origin.plan)
        || !canonicalEqual(batch, origin.batch)) {
    ...
    if (origin.bundleId !== accepted.bundleId
        || origin.manifest.bundleId !== accepted.bundleId
        || origin.commitMarker.bundleId !== accepted.bundleId
        || origin.plan.planId !== accepted.planId
        || origin.manifest.planId !== accepted.planId
        ...
        || previous.origin.replayClosureDigest
            !== (0, source_contracts_js_1.taskMapContractDigest)(previous.replayClosure)
        || previous.origin.acceptedOriginReplayDigest
            !== origin.plan.deterministicReplayDigest
        || predecessorRef.operation === "rollback"
        || previous.origin.replayClosureDigest
            !== previous.origin.acceptedOriginReplayDigest) {
        fail("previous sidecar does not close to the immediate predecessor bundle");
    }
}
function buildReplayClosure(ownerScopeDigest, sourceSnapshot, projection, projectionDigest, identityProofDigest, semanticRowsDigest) {
    return {
        contractVersion: exports.TASKMAP_IDENTITY_DEDUPE_REPLAY_CLOSURE_VERSION,
        ownerScopeDigest,
...
function envelopesById(snapshot) {
...
function envelopesByObjectKey(snapshot) {
    const result = new Map();
    for (const envelope of snapshot.envelopes) {
    ...
    for (const rows of result.values()) {
        rows.sort((left, right) => compareText(left.sourceIdentityDigest, right.sourceIdentityDigest));
    }
    return result;
}
function sourceDecisionFact(envelope) {
    return {
        ...
        contentDigest: envelope.contentDigest,
        bindingDigest: (0, source_contracts_js_1.taskMapContractDigest)(envelope.binding),
        authorityDigest: (0, source_contracts_js_1.taskMapContractDigest)(envelope.authority),
    };
}
function decisionProofDigest(reviewAttestationDigest, decisionKind, decision, referencedFacts) {
    return (0, source_contracts_js_1.taskMapContractDigest)({
        contractVersion: exports.TASKMAP_IDENTITY_DECISION_PROOF_VERSION,
        policyVersion: exports.TASKMAP_IDENTITY_DEDUPE_POLICY_VERSION,
        reviewAttestationDigest,
        decisionKind,
        decision,
        referencedFacts,
...
function deriveIdentityDecisionProofs(reviewAttestationDigest, sourceSnapshot, projection, input) {
    ...
    const taskRows = new Map(projection.tasks.map((row) => [row.id, row]));
    const rejectionRows = new Map(projection.rejections
        .filter((row) => row.kind === "task")
        .map((row) => [row.proposalId, row]));
    assertArrayBound(input.aliases, exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxAliases, "aliases");
    const aliasProofs = input.aliases.map((alias, index) => {
        ...
        assertDigest(alias.canonicalSourceObjectKeyDigest, `alias ${index} first member`);
        assertDigest(alias.aliasSourceObjectKeyDigest, `alias ${index} second member`);
        const memberSourceObjectKeyDigests = sortedUnique([
            alias.canonicalSourceObjectKeyDigest,
            alias.aliasSourceObjectKeyDigest,
        ], `alias ${index} members`);
        const referencedFacts = memberSourceObjectKeyDigests.flatMap((key) => ((byObjectKey.get(key) ?? []).map(sourceDecisionFact))).sort((left, right) => compareText(String(left.envelopeId), String(right.envelopeId)));
        ...
        return {
            identityKind: alias.identityKind,
            memberSourceObjectKeyDigests,
            proofDigest: decisionProofDigest(reviewAttestationDigest, "alias", decision, referencedFacts),
        };
    }).sort((left, right) => (compareText(left.identityKind, right.identityKind)
        || compareText(left.memberSourceObjectKeyDigests[0], right.memberSourceObjectKeyDigests[0])
        || compareText(left.memberSourceObjectKeyDigests[1], right.memberSourceObjectKeyDigests[1])));
    assertArrayBound(input.workBindings, exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxWorkBindings, "work bindings");
    const workBindingProofs = input.workBindings.map((binding, index) => {
        assertExactKeys(binding, ["projectionKind", "projectionId", "sourceEnvelopeId"], [], `work binding ${index}`);
        ...
        assertSafeId(binding.sourceEnvelopeId, `work binding ${index} envelope ID`);
        ...
        return {
            ...decision,
            proofDigest: decisionProofDigest(reviewAttestationDigest, "work_binding", decision, {
                source: sourceDecisionFact(envelope),
                projectionRowDigest: (0, source_contracts_js_1.taskMapContractDigest)(projectionRow),
            }),
        };
    }).sort((left, right) => (compareText(left.projectionKind, right.projectionKind)
        || compareText(left.projectionId, right.projectionId)
        || compareText(left.sourceEnvelopeId, right.sourceEnvelopeId)));
    assertArrayBound(input.sessionLineage, exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxSessionLineage, "session lineage");
    const sessionProofByEnvelopeId = new Map();
    const sessionProofs = input.sessionLineage.map((lineage, index) => {
        assertExactKeys(lineage, ["sessionEnvelopeId", "role"], ["workSourceObjectKeyDigest", "parentSessionEnvelopeId"], `session lineage ${index}`);
        assertSafeId(lineage.sessionEnvelopeId, `session lineage ${index} envelope ID`);
        ...
        const decision = {
            ...
            ...(lineage.workSourceObjectKeyDigest === undefined
                ? {}
                : {
                    workSourceObjectKeyDigest: lineage.workSourceObjectKeyDigest,
                }),
            ...(lineage.parentSessionEnvelopeId === undefined
                ? {}
                : {
                    parentSessionEnvelopeId: lineage.parentSessionEnvelopeId,
                }),
        };
        const proofDigest = decisionProofDigest(reviewAttestationDigest, "session_lineage", decision, {
            session: sourceDecisionFact(envelope),
            ...(lineage.parentSessionEnvelopeId === undefined
                ? {}
                : {
                    parentSession: byEnvelopeId.get(lineage.parentSessionEnvelopeId) === undefined
                        ? null
                        : sourceDecisionFact(byEnvelopeId.get(lineage.parentSessionEnvelopeId)),
                }),
            ...(lineage.workSourceObjectKeyDigest === undefined
                ? {}
                : {
                    workSources: (byObjectKey.get(lineage.workSourceObjectKeyDigest) ?? [])
    ...
    }).sort((left, right) => compareText(left.sessionEnvelopeId, right.sessionEnvelopeId));
    assertArrayBound(input.lifecycleAdjudications, exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxLifecycleAdjudications, "lifecycle adjudications");
    const lifecycleProofByCanonicalKey = new Map();
    const lifecycleProofs = input.lifecycleAdjudications.map((adjudication, index) => {
        const decision = {
            canonicalSourceObjectKeyDigest: adjudication.canonicalSourceObjectKeyDigest,
            ...
            ...(adjudication.previousSourceIdentityDigest === undefined
                ? {}
                : {
                    previousSourceIdentityDigest: adjudication.previousSourceIdentityDigest,
                }),
            currentSourceIdentityDigest: adjudication.currentSourceIdentityDigest,
            adjudicatedDelta: adjudication.adjudicatedDelta,
        };
        const currentEnvelope = sourceSnapshot.envelopes.find((envelope) => (envelope.sourceIdentityDigest
            === adjudication.currentSourceIdentityDigest));
        const proofDigest = decisionProofDigest(reviewAttestationDigest, "lifecycle", decision, {
            currentSource: currentEnvelope === undefined
                ? null
                : sourceDecisionFact(currentEnvelope),
            canonicalWorkSources: (byObjectKey.get(adjudication.canonicalSourceObjectKeyDigest) ?? []).map(sourceDecisionFact),
            projectionDigest: (0, source_contracts_js_1.taskMapContractDigest)(projection),
        });
        if (lifecycleProofByCanonicalKey.has(adjudication.canonicalSourceObjectKeyDigest)) {
            fail(`lifecycle adjudication ${index} repeats a proof identity`);
        }
        lifecycleProofByCanonicalKey.set(adjudication.canonicalSourceObjectKeyDigest, proofDigest);
        return { ...decision, adjudicationDigest: proofDigest };
    }).sort((left, right) => compareText(left.canonicalSourceObjectKeyDigest, right.canonicalSourceObjectKeyDigest));
    return {
        digest: (0, source_contracts_js_1.taskMapContractDigest)({
            contractVersion: exports.TASKMAP_IDENTITY_DECISION_PROOF_VERSION,
            policyVersion: exports.TASKMAP_IDENTITY_DEDUPE_POLICY_VERSION,
            reviewAttestationDigest,
            aliasProofs,
...
function normalizeAliases(values, byObjectKey) {
    assertArrayBound(values, exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxAliases, "aliases");
    const maps = {
        work: new Map(),
        session: new Map(),
    };
    ...
    for (const [index, alias] of values.entries()) {
        ...
        assertDigest(alias.canonicalSourceObjectKeyDigest, `alias ${index} canonical key`);
        assertDigest(alias.aliasSourceObjectKeyDigest, `alias ${index} alias key`);
        if (alias.canonicalSourceObjectKeyDigest
            === alias.aliasSourceObjectKeyDigest) {
            fail(`alias ${index} cannot point to itself`);
        }
        const canonicalRows = byObjectKey.get(alias.canonicalSourceObjectKeyDigest);
        ...
        if (members.has(alias.canonicalSourceObjectKeyDigest)
...
function canonicalKey(key, aliases) {
    return aliases.get(key) ?? key;
}
function normalizeWorkBindings(values, projection, byEnvelopeId, byObjectKey, workAliases) {
    assertArrayBound(values, exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxWorkBindings, "work bindings");
    const taskIds = new Set(projection.tasks.map((task) => task.id));
    const taskRejectionIds = new Set(projection.rejections
    ...
    for (const [index, binding] of values.entries()) {
        ...
        assertSafeId(binding.projectionId, `work binding ${index} projection ID`);
        assertSafeId(binding.sourceEnvelopeId, `work binding ${index} envelope ID`);
        const referenceKey = `${binding.projectionKind}:${binding.projectionId}`;
        ...
        const canonical = canonicalKey(envelope.sourceObjectKeyDigest, workAliases);
        const group = groups.get(canonical) ?? {
            sourceKeys: new Set(),
            sourceIdentityDigests: new Set(),
            envelopeIds: new Set(),
            projectionReferences: new Map(),
        };
        ...
        const projectionRow = binding.projectionKind === "task"
            ...
            : projection.rejections.find((rejection) => (rejection.kind === "task"
                && rejection.proposalId === binding.projectionId));
        const projectionRowDigest = (0, source_contracts_js_1.taskMapContractDigest)(projectionRow);
    ...
    if (taskIds.size !== boundTasks.size
...
function validateLifecycleAdjudication(value, index) {
    assertExactKeys(value, [
        "canonicalSourceObjectKeyDigest",
        "previousState",
        "currentState",
        ...
        "adjudicatedDelta",
    ], ["previousSourceIdentityDigest"], `lifecycle adjudication ${index}`);
    assertDigest(value.canonicalSourceObjectKeyDigest, `lifecycle adjudication ${index} work key`);
    assertDigest(value.currentSourceIdentityDigest, `lifecycle adjudication ${index} current source identity`);
    if (value.previousSourceIdentityDigest !== undefined) {
        assertDigest(value.previousSourceIdentityDigest, `lifecycle adjudication ${index} previous source identity`);
    }
    if (!["absent", "open", "resolved", "superseded", "rejected"]
    ...
    const mechanical = (value.adjudicatedDelta === "open"
        ...
        || (value.adjudicatedDelta === "no_op"
            ...
            && value.previousSourceIdentityDigest
                === value.currentSourceIdentityDigest);
    if (!mechanical) {
        fail(`lifecycle adjudication ${index} is not a mechanically closed delta`);
    }
}
function assertPreviousLifecycleClosure(previous, previousAcceptedOrigin, ownerScopeDigest, currentGeneration, generations, adjudications, lifecycleProofByCanonicalKey, currentWorkGroups, isExplicitGenesis) {
    if (previous === null) {
        if (previousAcceptedOrigin !== null
            || !isExplicitGenesis
            || adjudications.some((adjudication) => (adjudication.previousState !== "absent"
                || adjudication.previousSourceIdentityDigest !== undefined))) {
            fail("previous sidecar may be null only for an explicit absent-state genesis");
        }
        return;
    }
    if (previousAcceptedOrigin === null) {
    ...
    const previousIndex = generations.findIndex((candidate) => (candidate.generation === previous.origin.currentGeneration
    ...
    if (predecessorRef === undefined
        ...
        || generations.slice(previousIndex + 1, -1).some((candidate) => (candidate.operation !== "rollback"))) {
        fail("previous sidecar is not the latest semantic predecessor across "
            + "rollback observations");
    }
    assertPreviousAcceptedOrigin(previousAcceptedOrigin, previous, predecessorRef);
    const priorLifecycleByEventId = new Map(previous.events
        .filter((event) => (event.eventKind === "work_lifecycle"))
        .map((event) => [event.eventId, event]));
    const priorWorkByVariantKey = new Map();
    const priorWorkByVariantIdentity = new Map();
    ...
    const continuedPriorWorkIds = new Map();
    for (const adjudication of adjudications) {
        const currentGroup = currentWorkGroups.get(adjudication.canonicalSourceObjectKeyDigest);
        if (currentGroup === undefined) {
            fail("lifecycle adjudication has no normalized current work group");
        }
        const touchedPriorWorks = new Map();
        ...
        if (touchedPriorWorks.size > 1) {
            fail("current work merges distinct prior lifecycle identities without "
                + "a reviewed migration");
        }
        const priorWork = [...touchedPriorWorks.values()][0];
        if (priorWork === undefined) {
            if (adjudication.previousState !== "absent"
                || adjudication.previousSourceIdentityDigest !== undefined) {
        ...
        if (adjudication.canonicalSourceObjectKeyDigest
            !== priorWork.canonicalSourceObjectKeyDigest
            || !canonicalEqual(currentVariantKeys, priorWork.variantSourceObjectKeyDigests)) {
            fail("work alias membership cannot change without a reviewed migration");
        }
        ...
        if (priorContinuation !== undefined
            ...
            fail("a prior lifecycle identity cannot split into multiple current works");
        }
        continuedPriorWorkIds.set(priorWork.workId, adjudication.canonicalSourceObjectKeyDigest);
        const priorLifecycle = priorLifecycleByEventId.get(priorWork.lifecycleEventId);
        if (priorLifecycle === undefined) {
            fail("previous sidecar work has no closed lifecycle event");
        }
        const isExactReplay = adjudication.currentState === priorLifecycle.lifecycleState
            && adjudication.currentSourceIdentityDigest
                === priorLifecycle.sourceIdentityDigest
            && lifecycleProofByCanonicalKey.get(adjudication.canonicalSourceObjectKeyDigest)
                === priorLifecycle.adjudicationDigest;
        if (isExactReplay)
            continue;
        if (adjudication.previousState !== priorLifecycle.lifecycleState
...
function assertPreviousSessionAliasContinuity(previous, currentEvents) {
    if (previous === null)
        return;
    const previousSessions = previous.events.filter((event) => (event.eventKind === "session"));
    const currentSessions = currentEvents.filter((event) => (event.eventKind === "session"));
    ...
    for (const event of currentSessions) {
        ...
        for (const key of event.identitySourceObjectKeyDigests) {
            const prior = previousByIdentityKey.get(key);
            if (prior !== undefined)
                touched.set(prior.eventId, prior);
        }
        if (touched.size > 1) {
            fail("current session aliases merge prior identities without "
                + "a reviewed migration");
        }
        ...
        if (priorContinuation !== undefined
            ...
            fail("a prior session alias identity cannot split without "
                + "a reviewed migration");
        }
        if (event.canonicalSourceObjectKeyDigest
            !== prior.canonicalSourceObjectKeyDigest
            || event.corroboratesWorkId !== prior.corroboratesWorkId
            || !canonicalEqual(event.identitySourceObjectKeyDigests, prior.identitySourceObjectKeyDigests)) {
            fail("session alias membership or work binding cannot change without "
                + "a reviewed migration");
        }
    ...
    for (const prior of previousSessions) {
        if (!continued.has(prior.eventId)) {
            fail("a prior session alias identity cannot disappear without "
                + "a reviewed migration");
        }
    }
}
function sessionRoleDispositions(events, rejectedVariants, label) {
    const dispositions = new Map();
    const add = (sourceObjectKeyDigest, disposition) => {
    ...
    for (const event of events) {
        if (event.eventKind !== "session")
            continue;
        for (const sourceObjectKeyDigest of event.variantSourceObjectKeyDigests) {
            add(sourceObjectKeyDigest, {
                role: "accepted",
                eventId: event.eventId,
                canonicalSourceObjectKeyDigest: event.canonicalSourceObjectKeyDigest,
...
function assertPreviousSessionRoleContinuity(previous, currentEvents, currentRejectedVariants) {
    if (previous === null)
        return;
    const prior = sessionRoleDispositions(previous.events, previous.rejectedVariants, "previous sidecar");
    const current = sessionRoleDispositions(currentEvents, currentRejectedVariants, "current sidecar");
    for (const [sourceObjectKeyDigest, previousDisposition] of prior) {
        ...
        if (currentDisposition === undefined) {
            fail("a prior stable session role identity cannot disappear or rekey "
                + "without a reviewed migration");
        }
        if (currentDisposition.role !== previousDisposition.role) {
            fail("a stable session identity cannot change its accepted/rejected "
                + "role without a reviewed migration");
        }
        if (previousDisposition.role === "accepted"
            ...
            fail("an accepted session variant cannot change its canonical session "
                + "or work binding without a reviewed migration");
        }
        if (previousDisposition.role === "rejected"
            ...
            fail("a rejected session identity cannot change its stable rejection "
                + "without a reviewed migration");
        }
    }
}
function assertAdjacentSidecarLifecycleContinuity(previous, current) {
    assertPreviousSessionAliasContinuity(previous, current.events);
    assertPreviousSessionRoleContinuity(previous, current.events, current.rejectedVariants);
    const previousEvents = new Map(previous.events
        .filter((event) => (event.eventKind === "work_lifecycle"))
        .map((event) => [event.eventId, event]));
    const currentEvents = new Map(current.events
        .filter((event) => (event.eventKind === "work_lifecycle"))
        .map((event) => [event.eventId, event]));
    const previousWorkByVariantKey = new Map();
    const previousWorkByVariantIdentity = new Map();
    ...
    const continuedPreviousWorkIds = new Map();
    for (const currentWork of current.works) {
        const touchedPreviousWorks = new Map();
        ...
        if (previousWork === undefined) {
            if (currentDelta.previousState !== "absent"
                || currentDelta.previousSourceIdentityDigest !== undefined) {
                fail("stored new work identity does not begin from absent state");
            }
            continue;
        }
        if (currentWork.canonicalSourceObjectKeyDigest
            !== previousWork.canonicalSourceObjectKeyDigest
            || !canonicalEqual(currentWork.variantSourceObjectKeyDigests, previousWork.variantSourceObjectKeyDigests)) {
            fail("stored work alias membership changed without reviewed migration");
        }
        const priorContinuation = continuedPreviousWorkIds.get(previousWork.workId);
        if (priorContinuation !== undefined
        ...
        const previousEvent = previousEvents.get(previousWork.lifecycleEventId);
        const currentEvent = currentEvents.get(currentWork.lifecycleEventId);
        if (TERMINAL_STATES.has(previousEvent.lifecycleState)
            && currentEvent.lifecycleState === "open") {
            fail("stored sidecar continuity reopens a terminal work");
        }
        const exactReplay = (previousEvent.lifecycleState === currentEvent.lifecycleState
        ...
        if (exactReplay)
            continue;
        if (currentDelta.previousState !== previousEvent.lifecycleState
            || currentDelta.previousSourceIdentityDigest
                !== previousEvent.sourceIdentityDigest) {
            fail("stored sidecar lifecycle does not continue its adjacent predecessor");
        }
    }
...
function buildLifecycle(values, workGroups, lifecycleProofByCanonicalKey) {
    assertArrayBound(values, exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxLifecycleAdjudications, "lifecycle adjudications");
    ...
    for (const [index, adjudication] of values.entries()) {
        ...
        if (!group.sourceIdentityDigests.has(adjudication.currentSourceIdentityDigest)) {
            fail("current lifecycle identity is absent from the work variants");
        }
        const projectionKinds = new Set([...group.projectionReferences.values()].map((reference) => (reference.kind)));
        if (projectionKinds.size !== 1) {
            fail("canonical work has mixed task/rejection projection dispositions");
        }
        if (adjudication.currentState === "rejected"
        ...
        const adjudicationDigest = lifecycleProofByCanonicalKey.get(adjudication.canonicalSourceObjectKeyDigest);
...
function normalizeSessionLineage(values, snapshot, byEnvelopeId, workAliases, sessionAliases, workIdByCanonicalKey, sessionProofByEnvelopeId) {
    assertArrayBound(values, exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxSessionLineage, "session lineage");
    ...
    for (const [index, lineage] of values.entries()) {
        ...
        if (lineage.role === "root") {
            if (lineage.workSourceObjectKeyDigest === undefined
                || lineage.parentSessionEnvelopeId !== undefined) {
                fail("root sessions require only an explicit work corroboration link");
            }
            assertDigest(lineage.workSourceObjectKeyDigest, "root session work identity");
        }
        else if (lineage.role === "subagent") {
            if (lineage.parentSessionEnvelopeId === undefined
                || lineage.workSourceObjectKeyDigest !== undefined) {
                fail("subagents require only an explicit delegation parent");
            }
            assertSafeId(lineage.parentSessionEnvelopeId, "subagent parent session ID");
        }
        else if (lineage.workSourceObjectKeyDigest !== undefined
            || lineage.parentSessionEnvelopeId !== undefined) {
    ...
    const rejected = [];
    const groups = new Map();
    const rootFor = (declaration) => {
        let current = declaration;
        const seen = new Set();
        for (let depth = 0; depth <= exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxDelegationDepth; depth += 1) {
    ...
    for (const lineage of declarations.values()) {
        const envelope = byEnvelopeId.get(lineage.sessionEnvelopeId);
        const lineageProofDigest = sessionProofByEnvelopeId.get(lineage.sessionEnvelopeId);
        ...
        const canonicalSessionKey = canonicalKey(rootEnvelope.sourceObjectKeyDigest, sessionAliases);
        const canonicalWorkKey = canonicalKey(root.workSourceObjectKeyDigest, workAliases);
        ...
        const group = groups.get(canonicalSessionKey) ?? {
            ...
            identitySourceObjectKeyDigests: new Set(),
            variantSourceObjectKeyDigests: new Set(),
            variantEnvelopeIds: new Set(),
        };
        if (group.corroboratesWorkId !== workId) {
            fail("aliased root sessions corroborate different work identities");
        }
        group.identitySourceObjectKeyDigests.add(rootEnvelope.sourceObjectKeyDigest);
        group.variantSourceObjectKeyDigests.add(envelope.sourceObjectKeyDigest);
        group.variantSourceObjectKeyDigests.add(rootEnvelope.sourceObjectKeyDigest);
    ...
    const events = [...groups.values()].map((group) => {
        const core = {
            canonicalSourceObjectKeyDigest: group.canonicalSourceObjectKeyDigest,
        ...
        return {
            ...
            ...core,
            identitySourceObjectKeyDigests: sortedUnique([...group.identitySourceObjectKeyDigests], "session identity source-object-key variants"),
            variantSourceObjectKeyDigests: sortedUnique([...group.variantSourceObjectKeyDigests], "accepted session source-object-key variants"),
            variantEnvelopeIds: sortedUnique([...group.variantEnvelopeIds], "session variants"),
        };
    }).sort((left, right) => compareText(left.eventId, right.eventId));
    rejected.sort((left, right) => compareText(left.rejectionId, right.rejectionId));
    const eventIdsByWorkId = new Map();
    for (const event of events) {
...
function buildMeetingEvents(snapshot, byEnvelopeId) {
    return snapshot.canonicalMeetings.map((meeting) => {
        ...
        const variantEnvelopeIds = sourceVariantEnvelopeIds.map((envelopeId) => (stableId("tmvariant", { sourceEnvelopeId: envelopeId }))).sort(compareText);
        return {
            eventKind: "meeting",
...
function buildWorks(workGroups, lifecycleByWorkKey, sessionEventIdsByWorkId) {
    const works = [];
    for (const [canonicalSourceObjectKeyDigest, group] of workGroups) {
        ...
        const variants = sortedUnique([...new Set([
                canonicalSourceObjectKeyDigest,
                ...group.sourceKeys,
            ])], "work source-object-key variants");
        if (variants.length
            > exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxVariantsPerIdentity) {
        ...
        works.push({
            ...
            variantSourceIdentityDigests: sortedUnique([...group.sourceIdentityDigests], "work source-identity variants"),
            projectionReferences: [...group.projectionReferences.values()]
                .sort((left, right) => (compareText(left.kind, right.kind)
                || compareText(left.id, right.id))),
            corroboratingSessionEventIds: sessionEventIdsByWorkId.get(workId) ?? [],
...
function buildIdentityDedupeSemanticRows(sourceSnapshot, projection, supplied, decisionProofs) {
    const byEnvelopeId = envelopesById(sourceSnapshot);
    const byObjectKey = envelopesByObjectKey(sourceSnapshot);
    const aliases = normalizeAliases(supplied.aliases, byObjectKey);
    const workGroups = normalizeWorkBindings(supplied.workBindings, projection, byEnvelopeId, byObjectKey, aliases.work);
    const lifecycle = buildLifecycle(supplied.lifecycleAdjudications, workGroups, decisionProofs.lifecycleProofByCanonicalKey);
    const workIdByCanonicalKey = new Map();
    for (const [key, value] of lifecycle.byWorkKey) {
        workIdByCanonicalKey.set(key, value.event.workId);
    }
    const sessions = normalizeSessionLineage(supplied.sessionLineage, sourceSnapshot, byEnvelopeId, aliases.work, aliases.session, workIdByCanonicalKey, decisionProofs.sessionProofByEnvelopeId);
    const works = buildWorks(workGroups, lifecycle.byWorkKey, sessions.eventIdsByWorkId);
    const events = [
...
function identityDedupeSemanticRowsDigest(rows) {
    return (0, source_contracts_js_1.taskMapContractDigest)({
        contractVersion: exports.TASKMAP_IDENTITY_DEDUPE_PROJECTION_VERSION,
        policyVersion: exports.TASKMAP_IDENTITY_DEDUPE_POLICY_VERSION,
        works: rows.works,
        events: rows.events,
...
function sidecarDigest(sidecar) {
    return (0, source_contracts_js_1.taskMapContractDigest)(sidecar);
}
function buildDiff(previous, current) {
    ...
    for (const [key, row] of after) {
        ...
        if (prior === undefined) {
            ...
            const beforeDigest = (0, source_contracts_js_1.taskMapContractDigest)(prior.value);
            const afterDigest = (0, source_contracts_js_1.taskMapContractDigest)(row.value);
    ...
    for (const [key, row] of before) {
        if (!after.has(key)) {
            removed.push({
                kind: row.kind,
                id: row.id,
                beforeDigest: (0, source_contracts_js_1.taskMapContractDigest)(row.value),
            });
        }
    }
    const sort = (rows) => rows.sort((left, right) => (compareText(left.kind, right.kind) || compareText(left.id, right.id)));
    const core = {
    ...
    return {
        contractVersion: exports.TASKMAP_IDENTITY_DEDUPE_DIFF_VERSION,
...
function sidecarCore(value) {
    return value;
}
function assertSortedById(rows, id, label) {
...
function assertSortedStringArray(value, kind, limit, label, requireNonEmpty = false) {
    ...
    for (let index = 0; index < value.length; index += 1) {
        ...
        if (index > 0
            && compareText(value[index - 1], value[index]) >= 0) {
            fail(`${label} must be strictly sorted and duplicate-free`);
        }
    }
}
function assertOriginAndReplayClosure(value) {
    assertExactKeys(value.origin, [
        "currentRefId",
        "currentGeneration",
    ...
    assertHashedId(value.origin.currentRefId, HASHED_IDS.currentRef, "sidecar origin current ref ID");
    assertHashedId(value.origin.bundleId, HASHED_IDS.bundle, "sidecar origin bundle ID");
    assertHashedId(value.origin.planId, HASHED_IDS.plan, "sidecar origin plan ID");
    assertHashedId(value.origin.batchId, HASHED_IDS.batch, "sidecar origin batch ID");
    assertHashedId(value.origin.sourceSnapshotProofId, HASHED_IDS.sourceSnapshotProof, "sidecar origin source snapshot proof ID");
    ...
    assertExactKeys(value.replayClosure, [
        "contractVersion",
        "ownerScopeDigest",
        "sourceSnapshotDigest",
    ...
    if (value.replayClosure.contractVersion
        !== exports.TASKMAP_IDENTITY_DEDUPE_REPLAY_CLOSURE_VERSION) {
    ...
    assertHashedId(value.replayClosure.projectionRunId, HASHED_IDS.projectionRun, "sidecar replay closure projection run ID");
    if (value.origin.replayClosureDigest
        !== (0, source_contracts_js_1.taskMapContractDigest)(value.replayClosure)
...
function assertIdentityRows(value) {
    const worksById = new Map();
    const workIdByCanonicalKey = new Map();
    const workIdByVariantKey = new Map();
    const workIdByVariantIdentity = new Map();
    const workIdByProjectionReference = new Map();
    for (const [index, work] of value.works.entries()) {
        ...
        assertDigest(work.canonicalSourceObjectKeyDigest, `${label} canonical source-object-key digest`);
        if (work.workId !== stableId("tmwork", {
            policyVersion: exports.TASKMAP_IDENTITY_DEDUPE_POLICY_VERSION,
            canonicalSourceObjectKeyDigest: work.canonicalSourceObjectKeyDigest,
        })) {
            fail(`${label} ID is not derived from canonical work identity`);
        }
        assertSortedStringArray(work.variantSourceObjectKeyDigests, "digest", exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxVariantsPerIdentity, `${label} source-object-key variants`, true);
        assertSortedStringArray(work.variantSourceIdentityDigests, "digest", exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxVariantsPerIdentity, `${label} source-identity variants`, true);
        if (work.variantSourceObjectKeyDigests[0]
            !== work.canonicalSourceObjectKeyDigest) {
            fail(`${label} canonical work key is not its lexical representative`);
        }
        assertArrayBound(work.projectionReferences, exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxWorkBindings, `${label} projection references`);
        if (work.projectionReferences.length === 0) {
            fail(`${label} must retain at least one projection reference`);
        }
        for (let referenceIndex = 0; referenceIndex < work.projectionReferences.length; referenceIndex += 1) {
            const reference = work.projectionReferences[referenceIndex];
            assertExactKeys(reference, ["kind", "id", "projectionRowDigest"], [], `${label} projection reference ${referenceIndex}`);
            if (!["task", "rejection"].includes(reference.kind)) {
                fail(`${label} projection reference has an unsupported kind`);
            }
            assertHashedId(reference.id, HASHED_IDS.projectionReference, `${label} projection reference ${referenceIndex} ID`);
            assertDigest(reference.projectionRowDigest, `${label} projection reference ${referenceIndex} row digest`);
            if (reference.id !== stableId("tmprojectionref", {
                kind: reference.kind,
            ...
            if (referenceIndex > 0
                && (compareText(work.projectionReferences[referenceIndex - 1].kind, reference.kind)
                    || compareText(work.projectionReferences[referenceIndex - 1].id, reference.id)) >= 0) {
        ...
        assertSortedStringArray(work.corroboratingSessionEventIds, "id", exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxSessionLineage, `${label} corroborating session event IDs`);
        assertHashedId(work.lifecycleEventId, HASHED_IDS.event, `${label} lifecycle event ID`);
        if (!["open", "resolved", "superseded", "rejected"]
            .includes(work.lifecycleState)) {
            fail(`${label} has an unsupported lifecycle state`);
        }
        if (worksById.has(work.workId)
        ...
        workIdByCanonicalKey.set(work.canonicalSourceObjectKeyDigest, work.workId);
    ...
    const meetingByCanonicalId = new Map();
    const meetingByVariant = new Map();
    const sessionByCanonicalKey = new Map();
    const sessionByVariant = new Map();
    const sessionBySourceObjectKeyDigest = new Map();
    const lifecycleByWorkId = new Map();
    const sessionEventIdsByWorkId = new Map();
    for (const [index, event] of value.events.entries()) {
        ...
        if (event.eventKind === "meeting") {
            ...
            assertHashedId(event.canonicalMeetingId, HASHED_IDS.meeting, `${label} canonical meeting ID`);
            if (!["calendar_event", "bounded_fingerprint"].includes(event.identityMethod)) {
            ...
            assertSortedStringArray(event.variantEnvelopeIds, "id", exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxVariantsPerIdentity, `${label} meeting variants`, true);
            for (const variant of event.variantEnvelopeIds) {
                assertHashedId(variant, HASHED_IDS.variant, `${label} meeting variant`);
            }
            if (event.eventId !== stableId("tmmeetingevent", {
            ...
            for (const variant of event.variantEnvelopeIds) {
                if (meetingByVariant.has(variant)
                    || sessionByVariant.has(variant)) {
                    fail("an accepted variant belongs to more than one meeting/session identity");
                }
        ...
        if (event.eventKind === "session") {
            ...
            assertDigest(event.canonicalSourceObjectKeyDigest, `${label} canonical session digest`);
            assertHashedId(event.corroboratesWorkId, HASHED_IDS.work, `${label} work ID`);
            assertSortedStringArray(event.identitySourceObjectKeyDigests, "digest", exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxVariantsPerIdentity, `${label} session identity source-object-key variants`, true);
            if (event.identitySourceObjectKeyDigests[0]
                !== event.canonicalSourceObjectKeyDigest) {
                fail(`${label} canonical session key is not its lexical representative`);
            }
            assertSortedStringArray(event.variantSourceObjectKeyDigests, "digest", exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxVariantsPerIdentity, `${label} accepted session source-object-key variants`, true);
            if (event.identitySourceObjectKeyDigests.some((identity) => (!event.variantSourceObjectKeyDigests.includes(identity)))) {
                fail(`${label} accepted session variants omit a root identity member`);
            }
            assertSortedStringArray(event.variantEnvelopeIds, "id", exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxVariantsPerIdentity, `${label} session variants`, true);
            for (const variant of event.variantEnvelopeIds) {
                assertHashedId(variant, HASHED_IDS.variant, `${label} session variant`);
            }
            if (event.eventId !== stableId("tmsessionevent", {
                canonicalSourceObjectKeyDigest: event.canonicalSourceObjectKeyDigest,
                corroboratesWorkId: event.corroboratesWorkId,
            })) {
                fail(`${label} ID is not derived from canonical session identity`);
            }
            if (sessionByCanonicalKey.has(event.canonicalSourceObjectKeyDigest)) {
                fail("sidecar contains a duplicate canonical session");
            }
            sessionByCanonicalKey.set(event.canonicalSourceObjectKeyDigest, event.eventId);
            for (const sourceObjectKeyDigest of event.variantSourceObjectKeyDigests) {
                if (sessionBySourceObjectKeyDigest.has(sourceObjectKeyDigest)) {
                    fail("a stable accepted session variant belongs to more than one "
                        + "canonical session");
                }
                sessionBySourceObjectKeyDigest.set(sourceObjectKeyDigest, event.eventId);
            }
            for (const variant of event.variantEnvelopeIds) {
                if (sessionByVariant.has(variant)
        ...
        if (event.eventKind === "work_lifecycle") {
            ...
            assertDigest(event.sourceIdentityDigest, `${label} source identity digest`);
            assertDigest(event.adjudicationDigest, `${label} adjudication digest`);
            if (!["open", "resolved", "superseded", "rejected"]
    ...
    for (const [index, delta] of value.lifecycleDeltas.entries()) {
        ...
        assertExactKeys(delta, [
            ...
            "adjudicationDigest",
            "lifecycleEventId",
        ], ["previousSourceIdentityDigest"], label);
        assertHashedId(delta.deltaId, HASHED_IDS.lifecycleDelta, `${label} ID`);
        assertHashedId(delta.workId, HASHED_IDS.work, `${label} work ID`);
        assertHashedId(delta.lifecycleEventId, HASHED_IDS.event, `${label} lifecycle event ID`);
        ...
        validateLifecycleAdjudication({
            ...
            ...(delta.previousSourceIdentityDigest === undefined
                ? {}
                : {
                    previousSourceIdentityDigest: delta.previousSourceIdentityDigest,
                }),
    ...
    const rejectedEnvelopeIds = new Set();
    const rejectedSourceObjectKeyDigests = new Set();
    for (const [index, rejected] of value.rejectedVariants.entries()) {
        ...
        assertHashedId(rejected.rejectionId, HASHED_IDS.sessionRejection, `${label} ID`);
        assertHashedId(rejected.envelopeId, HASHED_IDS.variant, `${label} envelope ID`);
        assertDigest(rejected.sourceObjectKeyDigest, `${label} source-object-key digest`);
        ...
        if (rejectedSourceObjectKeyDigests.has(rejected.sourceObjectKeyDigest)) {
            fail("a stable rejected session identity is repeated");
        }
        if (sessionBySourceObjectKeyDigest.has(rejected.sourceObjectKeyDigest)) {
            fail("a stable session identity cannot be both accepted and rejected");
        }
        if (sessionByVariant.has(rejected.envelopeId)
        ...
        rejectedSourceObjectKeyDigests.add(rejected.sourceObjectKeyDigest);
    }
    for (const work of value.works) {
        ...
        if (lifecycleEvent === undefined
            ...
            || !work.variantSourceIdentityDigests.includes(lifecycleEvent.sourceIdentityDigest)) {
            fail("canonical work/lifecycle event/delta closure is inconsistent");
        }
        const expectedSessionIds = (sessionEventIdsByWorkId.get(work.workId) ?? []).sort(compareText);
        if (!canonicalEqual(work.corroboratingSessionEventIds, expectedSessionIds)) {
            fail("canonical work/session corroboration closure is inconsistent");
        }
    }
    if (lifecycleByWorkId.size !== worksById.size
        || deltasByWorkId.size !== worksById.size
        || [...sessionEventIdsByWorkId.keys()].some((workId) => !worksById.has(workId))) {
        fail("sidecar contains orphan lifecycle or session rows");
    }
}
function assertTaskMapIdentityDedupeProjection(value) {
    assertArrayPrototypeIntegrity();
    preflightJson(value, "identity sidecar", {
        nodes: 0,
        descriptors: 0,
        bytes: 0,
        byteLimit: exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxCanonicalArtifactBytes,
    });
    assertExactKeys(value, [
        "contractVersion",
        "sidecarId",
    ...
    if (value.contractVersion !== exports.TASKMAP_IDENTITY_DEDUPE_PROJECTION_VERSION
        || value.policyVersion !== exports.TASKMAP_IDENTITY_DEDUPE_POLICY_VERSION) {
    ...
    assertDigest(value.suppliedIdentityProofDigest, "sidecar supplied identity proof digest");
    assertHashedId(value.sourceSnapshotId, HASHED_IDS.sourceSnapshot, "sidecar snapshot ID");
    if (value.sourceSnapshotId
        !== `tmsnapshot_${(0, source_contracts_js_1.taskMapContractDigest)(value.sourceSnapshotDigest).slice(0, 16)}`) {
    ...
    assertHashedId(value.projectionRunId, HASHED_IDS.projectionRun, "sidecar projection run ID");
    ...
    assertArrayBound(value.works, exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxWorkBindings, "sidecar works");
    assertArrayBound(value.events, exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxArrayLength, "sidecar events");
    assertArrayBound(value.lifecycleDeltas, exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxLifecycleAdjudications, "sidecar lifecycle deltas");
    assertArrayBound(value.rejectedVariants, exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxSessionLineage, "sidecar rejected variants");
    if (1
        + value.works.length
        + value.events.length
        + value.lifecycleDeltas.length
        + value.rejectedVariants.length
        > exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxArrayLength) {
    ...
    assertSortedById(value.lifecycleDeltas, (row) => row.deltaId, "sidecar lifecycle deltas");
    assertSortedById(value.rejectedVariants, (row) => row.rejectionId, "sidecar rejected variants");
    assertIdentityRows(value);
    if (value.replayClosure.semanticRowsDigest
    ...
    const canonicalBytes = (0, source_contracts_js_1.taskMapContractCanonicalJson)(value);
    if (Buffer.byteLength(canonicalBytes, "utf8")
        > exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxCanonicalArtifactBytes) {
        fail("identity sidecar exceeds its canonical byte bound");
    }
    assertNoSensitiveOutput(value);
    return value;
}
function assertTaskMapIdentityDedupeDiff(value) {
    assertArrayPrototypeIntegrity();
    ...
    if (value.contractVersion !== exports.TASKMAP_IDENTITY_DEDUPE_DIFF_VERSION) {
    ...
    for (const [rows, label] of [
        ...
        assertArrayBound(rows, exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxArrayLength, `identity diff ${label}`);
        for (const [index, row] of rows.entries()) {
            assertExactKeys(row, ["kind", "id"], ["beforeDigest", "afterDigest"], `identity diff ${label} ${index}`);
            if (![
                "metadata",
                "work",
                "event",
            ...
            if (row.kind === "metadata") {
                ...
                assertHashedId(row.id, pattern, `identity diff ${label} ${index} ID`);
            }
            if (row.beforeDigest !== undefined) {
                assertDigest(row.beforeDigest, `identity diff ${label} ${index} before digest`);
            }
            if (row.afterDigest !== undefined) {
                assertDigest(row.afterDigest, `identity diff ${label} ${index} after digest`);
            }
            if (label === "added"
            ...
            if (index > 0
                && (compareText(rows[index - 1].kind, row.kind)
                    || compareText(rows[index - 1].id, row.id)) >= 0) {
                fail(`identity diff ${label} rows are not strictly sorted`);
            }
        }
    }
    if (value.previousSidecarDigest === value.currentSidecarDigest
        ? entryCount !== 0
        : entryCount === 0) {
...
function taskMapIdentityDedupeProjectionCanonicalBytes(value) {
    return (0, source_contracts_js_1.taskMapContractCanonicalJson)(assertTaskMapIdentityDedupeProjection(value));
}
function taskMapIdentityDedupeDiffCanonicalBytes(value) {
    return (0, source_contracts_js_1.taskMapContractCanonicalJson)(assertTaskMapIdentityDedupeDiff(value));
}
function taskMapIdentityDedupeReplayClosureDigest(sourceSnapshot, projection, suppliedProofs, reviewAttestationDigest) {
    assertArrayPrototypeIntegrity();
    ...
    const normalizedSnapshot = (0, source_contracts_js_1.assertTaskMapSourceSnapshot)(snapshot.sourceSnapshot);
    const projectionDiff = (0, source_contracts_js_1.diffTaskMapProjections)(null, snapshot.projection);
    const proofs = deriveIdentityDecisionProofs(snapshot.reviewAttestationDigest, normalizedSnapshot, snapshot.projection, snapshot.suppliedProofs);
    const semanticRows = buildIdentityDedupeSemanticRows(normalizedSnapshot, snapshot.projection, snapshot.suppliedProofs, proofs);
    const closure = buildReplayClosure(normalizedSnapshot.ownerScopeDigest, normalizedSnapshot, snapshot.projection, projectionDiff.currentProjectionDigest, proofs.digest, identityDedupeSemanticRowsDigest(semanticRows));
    return (0, source_contracts_js_1.taskMapContractDigest)(closure);
}
/**
 ...
 */
function unsafeBuildTaskMapIdentityDedupeProjectionFromVerifiedInputsForTest(input) {
    assertArrayPrototypeIntegrity();
    const snapshot = snapshotInput(input);
    const sourceSnapshot = (0, source_contracts_js_1.assertTaskMapSourceSnapshot)(snapshot.sourceSnapshot);
    ...
    if (current.head.operation === "rollback") {
        fail("rollback cannot accept caller-authored identity or lifecycle "
            + "decisions without a reviewed authority artifact");
    }
    assertAcceptedOrigin(snapshot.acceptedOrigin, current.accepted, current.head.ownerScopeDigest, sourceSnapshot);
    const decisionProofs = deriveIdentityDecisionProofs(snapshot.acceptedOrigin.plan.reviewedDigests.reviewAttestationDigest, sourceSnapshot, snapshot.projection, snapshot);
    const identityProofDigest = decisionProofs.digest;
    const semanticRows = buildIdentityDedupeSemanticRows(sourceSnapshot, snapshot.projection, snapshot, decisionProofs);
    const replayClosure = buildReplayClosure(current.head.ownerScopeDigest, sourceSnapshot, snapshot.projection, projectionDiff.currentProjectionDigest, identityProofDigest, identityDedupeSemanticRowsDigest(semanticRows));
    ...
    assertPreviousLifecycleClosure(previous, snapshot.previousAcceptedOrigin, current.head.ownerScopeDigest, current.head.generation, snapshot.currentSnapshot.generations, snapshot.lifecycleAdjudications, decisionProofs.lifecycleProofByCanonicalKey, semanticRows.workGroups, (snapshot.currentSnapshot.generations.slice(0, -1).every((generation) => generation.accepted === undefined)
    ...
    assertPreviousSessionRoleContinuity(previous, semanticRows.events, semanticRows.rejectedVariants);
    if (snapshot.acceptedOrigin.plan.deterministicReplayDigest
        !== replayClosureDigest) {
        fail("projection replay closure is not the active accepted origin replay");
    }
    const originProof = {
        ...
        sourceSnapshotProofId: snapshot.acceptedOrigin.sourceSnapshotProof.sourceSnapshotProofId,
        acceptedOriginReplayDigest: snapshot.acceptedOrigin.plan.deterministicReplayDigest,
        replayClosureDigest,
    };
    ...
    return {
        sidecar,
        diff,
        sidecarCanonicalBytes: taskMapIdentityDedupeProjectionCanonicalBytes(sidecar),
...
function storeEntryCore(value) {
    return value;
}
function assertTaskMapIdentityDedupeStoreEntry(value) {
    assertArrayPrototypeIntegrity();
    preflightJson(value, "sidecar store entry", {
        nodes: 0,
        descriptors: 0,
        bytes: 0,
        byteLimit: exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxStoreEntryBytes,
    });
    let snapshot;
    try {
        snapshot = structuredClone(value);
    ...
    assertExactKeys(snapshot, snapshot.entryKind === "projection"
        ? [
            ...commonKeys,
    ...
    if (snapshot.contractVersion
        !== exports.TASKMAP_IDENTITY_DEDUPE_STORE_ENTRY_VERSION) {
        fail("sidecar store entry has an unsupported contract version");
    }
    assertHashedId(snapshot.entryId, HASHED_IDS.storeEntry, "sidecar store entry ID");
    ...
    assertHashedId(snapshot.currentRefId, HASHED_IDS.currentRef, "sidecar store current ref");
    assertDigest(snapshot.currentRefDigest, "sidecar store current ref digest");
    assertExactKeys(snapshot.privacy, Object.keys(STORE_PRIVACY), [], "sidecar store privacy");
    ...
    if (snapshot.semanticHead !== null) {
        assertExactKeys(snapshot.semanticHead, [
            "generation",
            "currentRefId",
            "sidecarId",
            "sidecarDigest",
        ], [], "sidecar store semantic head");
        assertFixedGeneration(snapshot.semanticHead.generation, "sidecar store semantic-head generation");
        assertHashedId(snapshot.semanticHead.currentRefId, HASHED_IDS.currentRef, "sidecar store semantic-head current ref");
        assertHashedId(snapshot.semanticHead.sidecarId, HASHED_IDS.sidecar, "sidecar store semantic-head sidecar ID");
        assertDigest(snapshot.semanticHead.sidecarDigest, "sidecar store semantic-head sidecar digest");
        if (BigInt(snapshot.semanticHead.generation)
    ...
    if (snapshot.predecessor === null) {
        ...
        assertExactKeys(snapshot.predecessor, [
            "generation",
            "currentRefId",
            "entryId",
        ], [], "sidecar store predecessor");
        assertFixedGeneration(snapshot.predecessor.generation, "sidecar store predecessor generation");
        assertHashedId(snapshot.predecessor.currentRefId, HASHED_IDS.currentRef, "sidecar store predecessor current ref");
        assertHashedId(snapshot.predecessor.entryId, HASHED_IDS.storeEntry, "sidecar store predecessor entry ID");
        if (BigInt(snapshot.predecessor.generation) + 1n
            !== BigInt(snapshot.generation)) {
            fail("sidecar store predecessor is not the exact prior generation");
        }
    }
    if (snapshot.entryKind === "projection") {
        assertHashedId(snapshot.acceptedOriginBundleId, HASHED_IDS.bundle, "sidecar store accepted bundle");
        assertDigest(snapshot.acceptedOriginReplayDigest, "sidecar store accepted origin replay");
        assertDigest(snapshot.replayClosureDigest, "sidecar store replay closure");
        assertHashedId(snapshot.sidecarId, HASHED_IDS.sidecar, "sidecar store sidecar ID");
        assertDigest(snapshot.sidecarDigest, "sidecar store sidecar digest");
        assertHashedId(snapshot.diffId, HASHED_IDS.diff, "sidecar store diff ID");
        assertDigest(snapshot.diffDigest, "sidecar store diff digest");
        const sidecar = assertTaskMapIdentityDedupeProjection(snapshot.sidecar);
        const diff = assertTaskMapIdentityDedupeDiff(snapshot.diff);
        if (snapshot.sidecarId !== sidecar.sidecarId
            || snapshot.sidecarDigest !== (0, source_contracts_js_1.taskMapContractDigest)(sidecar)
            || snapshot.diffId !== diff.diffId
            || snapshot.diffDigest !== (0, source_contracts_js_1.taskMapContractDigest)(diff)
        ...
        if (snapshot.accepted !== null) {
            ...
            assertDigest(snapshot.accepted.acceptedStateDigest, "sidecar observation accepted-state digest");
            assertHashedId(snapshot.accepted.bundleId, HASHED_IDS.bundle, "sidecar observation accepted bundle");
            assertHashedId(snapshot.accepted.planId, HASHED_IDS.plan, "sidecar observation accepted plan");
            assertHashedId(snapshot.accepted.batchId, HASHED_IDS.batch, "sidecar observation accepted batch");
            assertFixedGeneration(snapshot.accepted.originGeneration, "sidecar observation accepted origin generation");
        }
        if (snapshot.rollback !== null) {
            assertExactKeys(snapshot.rollback, [
                "targetGeneration",
                "targetRefId",
                "targetAcceptedStateDigest",
            ], [], "sidecar observation rollback tuple");
            assertFixedGeneration(snapshot.rollback.targetGeneration, "sidecar observation rollback target generation");
            assertHashedId(snapshot.rollback.targetRefId, HASHED_IDS.currentRef, "sidecar observation rollback target ref");
            assertDigest(snapshot.rollback.targetAcceptedStateDigest, "sidecar observation rollback accepted-state digest");
        }
        const isNoAccepted = snapshot.entryKind === "no_accepted_origin"
        ...
        const isRollback = snapshot.entryKind === "rollback_observed"
    ...
    const canonicalBytes = (0, source_contracts_js_1.taskMapContractCanonicalJson)(snapshot);
    if (Buffer.byteLength(canonicalBytes, "utf8")
        > exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxStoreEntryBytes) {
        fail("sidecar store entry exceeds its canonical byte bound");
    }
    assertNoSensitiveOutput(snapshot);
    return snapshot;
}
function buildStoreEntry(built, predecessor, currentRef) {
    const sidecar = assertTaskMapIdentityDedupeProjection(built.sidecar);
    const diff = assertTaskMapIdentityDedupeDiff(built.diff);
    const predecessorProof = predecessor === null
        ? null
        : {
    ...
    const core = storeEntryCore({
        contractVersion: exports.TASKMAP_IDENTITY_DEDUPE_STORE_ENTRY_VERSION,
        entryKind: "projection",
        ownerScopeDigest: sidecar.ownerScopeDigest,
        generation: sidecar.origin.currentGeneration,
        currentRefId: sidecar.origin.currentRefId,
        currentRefDigest: (0, source_contracts_js_1.taskMapContractDigest)(currentRef),
...
function buildObservationStoreEntry(ref, predecessor) {
    const predecessorProof = predecessor === null
        ? null
        : {
            generation: predecessor.generation,
    ...
    const core = storeEntryCore({
        ...
        currentRefDigest: (0, source_contracts_js_1.taskMapContractDigest)(ref),
        predecessor: predecessorProof,
...
async function assertDisjointTaskMapStoreRoots(roots) {
    const candidates = [
    ...
    for (const [label, candidate] of candidates) {
        ...
        let stats;
        try {
            canonical = await (0, promises_1.realpath)(node_path_1.default.resolve(candidate));
            stats = await (0, promises_1.lstat)(canonical, { bigint: true });
        }
        catch {
    ...
    for (let leftIndex = 0; leftIndex < resolved.length; leftIndex += 1) {
        for (let rightIndex = leftIndex + 1; rightIndex < resolved.length; rightIndex += 1) {
            ...
            if (contains(left.canonical, right.canonical)
                || contains(right.canonical, left.canonical)
                || (left.dev === right.dev
                    && left.ino === right.ino)) {
                fail(`task-map store roots must be realpath-disjoint; `
...
async function revalidateDisjointTaskMapStoreRoots(roots, expected) {
    ...
    for (const label of [
        ...
        if (current[label].canonical !== expected[label].canonical
...
function sidecarErrnoCode(error) {
    if (error !== null
        && typeof error === "object"
        && "code" in error
...
function sidecarCurrentUid() {
    if (typeof process.getuid !== "function") {
        fail("sidecar store owner UID verification is unavailable");
    }
    return process.getuid();
}
function sidecarModeBits(stats) {
    return Number(stats.mode & 4095n);
}
async function checkedSidecarDirectory(directory, label) {
    if (!node_path_1.default.isAbsolute(directory)) {
        fail(`${label} must be an absolute path`);
    }
    const normalized = node_path_1.default.resolve(directory);
    let stats;
    let canonical;
    try {
        stats = await (0, promises_1.lstat)(normalized, { bigint: true });
        canonical = await (0, promises_1.realpath)(normalized);
    }
    catch {
...
async function revalidateSidecarDirectory(checked, label) {
    const current = await checkedSidecarDirectory(checked.directory, label);
    if (current.stats.dev !== checked.stats.dev
        || current.stats.ino !== checked.stats.ino) {
        fail(`${label} was replaced after validation`);
    }
}
function revalidateSidecarDirectorySync(checked, label) {
    let current;
    try {
        current = (0, node_fs_1.lstatSync)(checked.directory, { bigint: true });
    }
    catch {
        fail(`${label} is unavailable`);
    }
...
function observeSidecarDirectorySync(directory, label) {
    let current;
    try {
        current = (0, node_fs_1.lstatSync)(directory, { bigint: true });
    }
    catch {
        fail(`${label} is unavailable`);
    }
...
function openVerifiedSidecarParentSync(checked, label) {
    revalidateSidecarDirectorySync(checked, label);
    let descriptor;
    try {
        descriptor = (0, node_fs_1.openSync)(checked.directory, node_fs_1.constants.O_RDONLY
...
function checkedSidecarParentStillCurrentSync(descriptor, checked) {
    const opened = (0, node_fs_1.fstatSync)(descriptor, { bigint: true });
    let current;
    try {
        current = (0, node_fs_1.lstatSync)(checked.directory, { bigint: true });
...
function unlinkExactCreatedSidecarFileSync(filePath, expectedParent, expectedFile, expectedBytes, label) {
    ...
    let fileDescriptor;
    try {
        parentDescriptor = (0, node_fs_1.openSync)(node_path_1.default.dirname(filePath), node_fs_1.constants.O_RDONLY
            | node_fs_1.constants.O_NOFOLLOW
            | node_fs_1.constants.O_NONBLOCK);
        const openedParent = (0, node_fs_1.fstatSync)(parentDescriptor, { bigint: true });
        ...
        const observed = (0, node_fs_1.lstatSync)(filePath, { bigint: true });
        ...
        if (expectedBuffer.byteLength > 0) {
            fileDescriptor = (0, node_fs_1.openSync)(filePath, node_fs_1.constants.O_RDONLY
                | node_fs_1.constants.O_NOFOLLOW
                | node_fs_1.constants.O_NONBLOCK);
            const opened = (0, node_fs_1.fstatSync)(fileDescriptor, { bigint: true });
            ...
            const bytes = (0, node_fs_1.readFileSync)(fileDescriptor);
        ...
        const finalParent = (0, node_fs_1.lstatSync)(node_path_1.default.dirname(filePath), { bigint: true });
        const finalFile = (0, node_fs_1.lstatSync)(filePath, { bigint: true });
        if (finalParent.dev !== openedParent.dev
        ...
        (0, node_fs_1.fsyncSync)(parentDescriptor);
    }
    catch (error) {
        if (error instanceof Error
            && error.message.startsWith("P10.2 identity/dedupe projection:")) {
        ...
        if (parentDescriptor !== undefined) {
            try {
                (0, node_fs_1.closeSync)(parentDescriptor);
            }
            catch {
...
function removeExactCreatedSidecarDirectorySync(directory, expectedParent, expectedDirectory, label) {
    let parentDescriptor;
    try {
        parentDescriptor = (0, node_fs_1.openSync)(node_path_1.default.dirname(directory), node_fs_1.constants.O_RDONLY
            | node_fs_1.constants.O_NOFOLLOW
            | node_fs_1.constants.O_NONBLOCK);
        const openedParent = (0, node_fs_1.fstatSync)(parentDescriptor, { bigint: true });
        if (!openedParent.isDirectory()
        ...
        const observed = (0, node_fs_1.lstatSync)(directory, { bigint: true });
        ...
        if (!observed.isDirectory()
            ...
            || (0, node_fs_1.readdirSync)(directory).length !== 0) {
            fail(`${label} is not the exact empty created directory`);
        }
        const finalParent = (0, node_fs_1.lstatSync)(node_path_1.default.dirname(directory), { bigint: true });
        const finalDirectory = (0, node_fs_1.lstatSync)(directory, { bigint: true });
        if (finalParent.dev !== openedParent.dev
            || finalParent.ino !== openedParent.ino
...
function createSidecarDirectoryGuardedSync(parent, parentDescriptor, directory, label) {
    let created = false;
    let createdStats;
    let childDescriptor;
    if (!checkedSidecarParentStillCurrentSync(parentDescriptor, parent)) {
    ...
    try {
        try {
            ...
            if (existing.dev !== parent.stats.dev
        ...
        // boundary.
        createdStats = (0, node_fs_1.lstatSync)(directory, { bigint: true });
        childDescriptor = (0, node_fs_1.openSync)(directory, node_fs_1.constants.O_RDONLY
            | node_fs_1.constants.O_NOFOLLOW
            | node_fs_1.constants.O_NONBLOCK);
        const opened = (0, node_fs_1.fstatSync)(childDescriptor, { bigint: true });
        if (!opened.isDirectory()
            || opened.uid !== BigInt(sidecarCurrentUid())
        ...
        (0, node_fs_1.fchmodSync)(childDescriptor, DIRECTORY_MODE);
        createdStats = (0, node_fs_1.fstatSync)(childDescriptor, { bigint: true });
        ...
        (0, node_fs_1.fsyncSync)(childDescriptor);
        ...
        if (childDescriptor !== undefined) {
            try {
                (0, node_fs_1.closeSync)(childDescriptor);
            }
            catch {
        ...
        if (createdStats !== undefined
            && checkedSidecarParentStillCurrentSync(parentDescriptor, parent)) {
            try {
                removeExactCreatedSidecarDirectorySync(directory, parent.stats, createdStats, label);
            }
            catch (rollbackError) {
            ...
            rollbackErrors.push(new Error(`P10.2 identity/dedupe projection: ${label} has no exact `
                + "rollback receipt"));
        }
        if (rollbackErrors.length > 0) {
            throw new AggregateError([error, ...rollbackErrors], "P10.2 identity/dedupe projection: sidecar directory creation "
...
async function syncSidecarDirectory(checked, label) {
    let handle;
    try {
        handle = await (0, promises_1.open)(checked.directory, node_fs_1.constants.O_RDONLY
            | node_fs_1.constants.O_NOFOLLOW
            | node_fs_1.constants.O_NONBLOCK);
...
async function listSidecarDirectory(checked, label, maximum) {
    const names = [];
    let directory;
    try {
        directory = await (0, promises_1.opendir)(checked.directory);
...
async function openSidecarStore(sidecarRoot) {
    const root = await checkedSidecarDirectory(sidecarRoot, "sidecar store root");
    const rootEntries = await listSidecarDirectory(root, "sidecar store root", 3);
    ...
    const generations = await checkedSidecarDirectory(node_path_1.default.join(root.directory, STORE_GENERATIONS_DIRECTORY), "sidecar store generations directory");
    const staging = await checkedSidecarDirectory(node_path_1.default.join(root.directory, STORE_STAGING_DIRECTORY), "sidecar store staging directory");
    const claims = await checkedSidecarDirectory(node_path_1.default.join(root.directory, STORE_CLAIMS_DIRECTORY), "sidecar store claims directory");
    if (generations.stats.dev !== root.stats.dev
...
function revalidateOpenSidecarStoreSync(store, label) {
    ...
    for (const [name, child] of [
        ...
        const stats = revalidateSidecarDirectorySync(child, `${label} ${name} directory`);
        if (stats.dev !== store.root.stats.dev) {
            fail(`${label} ${name} directory left the store volume`);
        }
    }
}
async function assertSidecarFileStats(filePath, label, allowedLinks, expectedDev, minimumBytes = 1) {
    let stats;
    try {
    ...
    if (!stats.isFile()
        ...
        || stats.size
            > BigInt(exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxStoreEntryBytes)) {
        fail(`${label} must be a bounded same-owner 0600 regular file`);
    }
    return stats;
}
async function readSidecarEntryFile(filePath, label, allowedLinks, expectedDev) {
    const observed = await assertSidecarFileStats(filePath, label, allowedLinks, expectedDev);
    let handle;
    try {
        handle = await (0, promises_1.open)(filePath, node_fs_1.constants.O_RDONLY
            | node_fs_1.constants.O_NOFOLLOW
            | node_fs_1.constants.O_NONBLOCK);
        const stats = await handle.stat({ bigint: true });
        if (stats.dev !== observed.dev
        ...
        const entry = assertTaskMapIdentityDedupeStoreEntry(parsed);
        const canonicalBytes = (0, source_contracts_js_1.taskMapContractCanonicalJson)(entry);
...
function sidecarGenerationFileName(generation) {
    assertFixedGeneration(generation, "sidecar generation");
    return `${generation}.entry.json`;
}
async function initializeTaskMapIdentityDedupeProjectionStore(roots, writeOptions = {}) {
    const snapshot = snapshotStoreRoots(roots);
    const isolatedRoots = await assertDisjointTaskMapStoreRoots(snapshot);
    const root = await checkedSidecarDirectory(snapshot.sidecarRoot, "sidecar store root");
    const priorNames = await listSidecarDirectory(root, "sidecar store root", 3);
    for (const name of priorNames) {
        if (name !== STORE_GENERATIONS_DIRECTORY
            && name !== STORE_STAGING_DIRECTORY
            && name !== STORE_CLAIMS_DIRECTORY) {
            fail("sidecar store root contains unknown entries");
        }
    }
    await writeOptions.faultInjection?.("before_initialize_directory_create");
    await revalidateDisjointTaskMapStoreRoots(snapshot, isolatedRoots);
    const rootDescriptor = openVerifiedSidecarParentSync(root, "sidecar store root");
    const created = [];
    try {
        for (const name of [
            STORE_GENERATIONS_DIRECTORY,
            ...
            const receipt = createSidecarDirectoryGuardedSync(root, rootDescriptor, directory, `sidecar store ${name} directory`);
            if (receipt !== null)
        ...
        (0, node_fs_1.fsyncSync)(rootDescriptor);
    }
    catch (error) {
        const rollbackErrors = [];
        for (const receipt of [...created].reverse()) {
            if (!checkedSidecarParentStillCurrentSync(rootDescriptor, root)) {
                rollbackErrors.push(new Error("P10.2 identity/dedupe projection: sidecar store root "
                    + "changed before initialization rollback"));
                break;
            }
            try {
                removeExactCreatedSidecarDirectorySync(receipt.directory, root.stats, receipt.stats, `sidecar store ${node_path_1.default.basename(receipt.directory)} directory`);
            }
            catch (rollbackError) {
                rollbackErrors.push(rollbackError);
            }
        ...
        if (rollbackErrors.length > 0) {
            throw new AggregateError([error, ...rollbackErrors], "P10.2 identity/dedupe projection: sidecar store initialization "
        ...
        try {
            (0, node_fs_1.closeSync)(rootDescriptor);
        }
        catch {
...
async function readSidecarStoreFromOpen(store, allowKnownStaging = false, allowedLinkedGenerationInodes = new Set()) {
    await validateSidecarClaimDirectory(store);
    const stagingNames = await listSidecarDirectory(store.staging, "sidecar store staging directory", exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxStagingEntries);
    let stagingBytes = 0n;
    for (const name of stagingNames) {
        ...
        const stage = await assertRecoverableSidecarStage(node_path_1.default.join(store.staging.directory, name), `sidecar staged entry ${name}`, store.staging.stats.dev);
        stagingBytes += stage.size;
        if (stagingBytes
            > BigInt(exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxStagingBytes)) {
    ...
    const names = await listSidecarDirectory(store.generations, "sidecar store generations directory", exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxStoreEntries);
    const entries = [];
    let retainedHistoryBytes = 0n;
    let semanticSidecar = null;
    let semanticHead = null;
    for (let index = 0; index < names.length; index += 1) {
        const name = names[index];
        const match = STORE_GENERATION_FILE.exec(name);
        if (match === null || match[1] !== expectedGeneration(index)) {
            fail("sidecar store generations must be an exact contiguous prefix");
        }
        const generationPath = node_path_1.default.join(store.generations.directory, name);
        const generationStats = await assertSidecarFileStats(generationPath, `sidecar store generation ${match[1]}`, allowedLinkedGenerationInodes.size === 0 ? [1] : [1, 2], store.generations.stats.dev);
        if (generationStats.nlink === 2n
            && !allowedLinkedGenerationInodes.has(`${generationStats.dev}:${generationStats.ino}`)) {
            fail("sidecar store generation has an unbound extra hardlink");
        }
        retainedHistoryBytes += generationStats.size;
        if (retainedHistoryBytes
            > BigInt(exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxStoreHistoryBytes)) {
            fail("sidecar store history exceeds its aggregate byte bound");
        }
        const observed = await readSidecarEntryFile(generationPath, `sidecar store generation ${match[1]}`, allowedLinkedGenerationInodes.size === 0 ? [1] : [1, 2], store.generations.stats.dev);
        ...
        if (entry.entryKind === "projection") {
            ...
            if (semanticSidecar !== null) {
                assertAdjacentSidecarLifecycleContinuity(semanticSidecar, entry.sidecar);
            }
        ...
        if (entry.semanticHead !== null
            && semanticSidecar !== null
            && (entry.semanticHead.sidecarId !== semanticSidecar.sidecarId
                || entry.semanticHead.sidecarDigest
                    !== (0, source_contracts_js_1.taskMapContractDigest)(semanticSidecar))) {
            fail("sidecar semantic head does not resolve to stored semantics");
        }
        entries.push(entry);
    }
    await revalidateSidecarDirectory(store.generations, "sidecar store generations directory");
    await revalidateSidecarDirectory(store.staging, "sidecar store staging directory");
    await revalidateSidecarDirectory(store.root, "sidecar store root");
    await revalidateSidecarDirectory(store.claims, "sidecar store claims directory");
    await validateSidecarClaimDirectory(store);
    const canonicalByteLength = Number(retainedHistoryBytes);
    return {
        entries,
        canonicalByteLength,
        remainingByteCapacity: exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxStoreHistoryBytes
            - canonicalByteLength,
    };
}
async function readTaskMapIdentityDedupeProjectionStoreStructural(sidecarRoot) {
    assertArrayPrototypeIntegrity();
 ...
 */
async function unsafeReadTaskMapIdentityDedupeProjectionStoreStructuralForTest(sidecarRoot) {
 ...
 */
async function readTaskMapIdentityDedupeProjectionStore(roots) {
    assertArrayPrototypeIntegrity();
    const snapshot = snapshotStoreRoots(roots, "sidecar authenticated-read roots");
    const isolatedRoots = await assertDisjointTaskMapStoreRoots(snapshot);
    const currentSnapshot = await (0, refresh_current_ref_js_1.readTaskMapRefreshCurrent)(snapshot.currentRoot, snapshot.runRoot);
    assertHealthyCurrentHistory(currentSnapshot);
    const store = await readTaskMapIdentityDedupeProjectionStoreStructural(snapshot.sidecarRoot);
    const { currentEntry } = assertSidecarStoreAlignedWithCurrent(store, currentSnapshot);
    if (currentSnapshot.generations.length > 0
        && currentEntry === null) {
        fail("authenticated sidecar store is not caught up to the P10.1 head");
    }
    await verifySidecarStoreAcceptedOrigins(store, currentSnapshot, snapshot.runRoot);
    await revalidateDisjointTaskMapStoreRoots(snapshot, isolatedRoots);
    const latest = await (0, refresh_current_ref_js_1.readTaskMapRefreshCurrent)(snapshot.currentRoot, snapshot.runRoot);
...
async function sidecarPathExists(filePath) {
    try {
        await (0, promises_1.lstat)(filePath, { bigint: true });
        return true;
    }
    catch (error) {
        if (sidecarErrnoCode(error) === "ENOENT")
            return false;
        fail("sidecar claim path cannot be inspected");
    }
}
function writeExclusiveSidecarClaim(parent, claimPath, label, bytes = "") {
    const parentDescriptor = openVerifiedSidecarParentSync(parent, `${label} parent`);
    let descriptor;
    let createdStats;
    let rollbackBytes = Buffer.alloc(0);
    let rolledBack = false;
    try {
        descriptor = (0, node_fs_1.openSync)(claimPath, node_fs_1.constants.O_RDWR
        ...
        if (buffer.byteLength > 0) {
            const written = (0, node_fs_1.writeSync)(descriptor, buffer, 0, buffer.byteLength, 0);
        ...
        if (!checkedSidecarParentStillCurrentSync(parentDescriptor, parent)) {
            ...
            const currentParent = observeSidecarDirectorySync(parent.directory, `${label} replacement parent`);
            unlinkExactCreatedSidecarFileSync(claimPath, currentParent, createdStats, rollbackBytes, label);
            rolledBack = true;
        ...
        if (createdStats !== undefined
            ...
            unlinkExactCreatedSidecarFileSync(claimPath, parent.stats, createdStats, rollbackBytes, label);
            rolledBack = true;
        }
        if (error instanceof Error
            && error.message.startsWith("P10.2 identity/dedupe projection:")) {
...
async function assertRecoverableSidecarClaim(claimPath, label, expectedDev, maximumBytes = 128) {
    let observed;
    try {
        observed = await (0, promises_1.lstat)(claimPath, { bigint: true });
    }
    catch {
        fail(`${label} is unavailable`);
    }
...
async function bindSidecarWriterClaimToStage(store, claim, stagePath, stageStats) {
    const journal = {
        contractVersion: STORE_WRITER_JOURNAL_VERSION,
        writerToken: claim.token,
        stageFileName: node_path_1.default.basename(stagePath),
        stageDevice: stageStats.dev.toString(),
        stageInode: stageStats.ino.toString(),
    };
    const bytes = (0, source_contracts_js_1.taskMapContractCanonicalJson)(journal);
    if (Buffer.byteLength(bytes, "utf8") > MAX_WRITER_JOURNAL_BYTES) {
        fail("sidecar writer journal exceeds its byte bound");
    }
    const observed = await assertRecoverableSidecarClaim(claim.claimPath, "sidecar writer claim", store.claims.stats.dev, MAX_WRITER_JOURNAL_BYTES);
    if (observed.size !== 0n) {
        fail("sidecar writer claim was already bound to a stage");
    }
    const parentDescriptor = openVerifiedSidecarParentSync(store.claims, "sidecar writer claim parent");
    let descriptor;
    try {
        descriptor = (0, node_fs_1.openSync)(claim.claimPath, node_fs_1.constants.O_WRONLY
            | node_fs_1.constants.O_NOFOLLOW
            | node_fs_1.constants.O_NONBLOCK);
        const before = (0, node_fs_1.fstatSync)(descriptor, { bigint: true });
        if (before.dev !== observed.dev
        ...
        if (!checkedSidecarParentStillCurrentSync(parentDescriptor, store.claims)) {
    ...
    await syncSidecarDirectory(store.claims, "sidecar store claims directory");
    return journal;
}
async function readSidecarWriterJournal(claimPath, expectedDev) {
    const claimName = node_path_1.default.basename(claimPath);
    const match = STORE_WRITER_CLAIM.exec(claimName);
    if (match === null) {
        fail("sidecar writer journal has no canonical writer token");
    }
    const observed = await assertRecoverableSidecarClaim(claimPath, `sidecar writer claim ${claimName}`, expectedDev, MAX_WRITER_JOURNAL_BYTES);
    if (observed.size === 0n) {
        fail("sidecar writer claim has no durable stage binding");
    }
    let handle;
    try {
        handle = await (0, promises_1.open)(claimPath, node_fs_1.constants.O_RDONLY
            | node_fs_1.constants.O_NOFOLLOW
            | node_fs_1.constants.O_NONBLOCK);
        ...
        if (journal.contractVersion !== STORE_WRITER_JOURNAL_VERSION
            ...
            || (0, source_contracts_js_1.taskMapContractCanonicalJson)(journal) !== bytes) {
...
async function readSidecarRecoveryClaimToken(claimPath, expectedDev, expectedToken) {
    const observed = await assertRecoverableSidecarClaim(claimPath, "sidecar recovery claim", expectedDev);
    ...
    try {
        ...
        const token = decodeExactUtf8(await handle.readFile(), "sidecar recovery claim");
...
async function assertRecoverableSidecarStage(stagePath, label, expectedDev) {
    let observed;
    try {
        observed = await (0, promises_1.lstat)(stagePath, { bigint: true });
    }
    catch {
        fail(`${label} is unavailable`);
    }
...
async function readExactRecoverableSidecarStageBytes(stagePath, expectedStats, label) {
    let handle;
    try {
        handle = await (0, promises_1.open)(stagePath, node_fs_1.constants.O_RDONLY
            | node_fs_1.constants.O_NOFOLLOW
            | node_fs_1.constants.O_NONBLOCK);
        const opened = await handle.stat({ bigint: true });
        if (opened.dev !== expectedStats.dev
...
async function unlinkExactSidecarStage(store, staged, label) {
    ...
    const expectedBytesDigest = (0, node_crypto_1.createHash)("sha256")
    ...
    if (node_path_1.default.dirname(staged.stagePath) !== store.staging.directory
        || node_path_1.default.basename(staged.stagePath) !== expectedStageName
        || node_path_1.default.dirname(staged.writerClaim.claimPath)
            !== store.claims.directory
        || node_path_1.default.basename(staged.writerClaim.claimPath) !== expectedClaimName
        ...
        fail(`${label} has no exact writer/stage binding; explicit recovery is required`);
    }
    const claimStats = await assertRecoverableSidecarClaim(staged.writerClaim.claimPath, `${label} writer claim`, store.claims.stats.dev, MAX_WRITER_JOURNAL_BYTES);
    ...
    if (claimStats.size !== 0n) {
        const journal = await readSidecarWriterJournal(staged.writerClaim.claimPath, store.claims.stats.dev);
        if (journal.writerToken !== staged.writerClaim.token
            ...
            fail(`${label} writer journal no longer binds the stage; explicit recovery is required`);
        }
    }
    const parentDescriptor = openVerifiedSidecarParentSync(store.staging, `${label} staging parent`);
    let descriptor;
    let removed = false;
    try {
        const observed = (0, node_fs_1.lstatSync)(staged.stagePath, { bigint: true });
        if (!observed.isFile()
            || observed.isSymbolicLink()
            || observed.uid !== BigInt(sidecarCurrentUid())
            ...
            fail(`${label} inode changed before cleanup; explicit recovery is required`);
        }
        descriptor = (0, node_fs_1.openSync)(staged.stagePath, node_fs_1.constants.O_RDONLY
            | node_fs_1.constants.O_NOFOLLOW
            | node_fs_1.constants.O_NONBLOCK);
        const opened = (0, node_fs_1.fstatSync)(descriptor, { bigint: true });
        if (opened.dev !== observed.dev
            ...
            || opened.size !== observed.size) {
            fail(`${label} inode changed while opening for cleanup; explicit recovery is required`);
        }
        const buffer = (0, node_fs_1.readFileSync)(descriptor);
        if (BigInt(buffer.byteLength) !== opened.size) {
            fail(`${label} bytes changed while reading for cleanup; explicit recovery is required`);
        }
        if (!buffer.equals(expectedBuffer)
            || (0, node_crypto_1.createHash)("sha256").update(buffer).digest("hex")
                !== expectedBytesDigest) {
            fail(`${label} content changed before cleanup; explicit recovery is required`);
        }
        const finalObserved = (0, node_fs_1.lstatSync)(staged.stagePath, { bigint: true });
        if (finalObserved.dev !== opened.dev
            ...
            fail(`${label} pathname changed before cleanup; explicit recovery is required`);
        }
        if (!checkedSidecarParentStillCurrentSync(parentDescriptor, store.staging)) {
            fail(`${label} staging parent changed before cleanup; explicit recovery is required`);
        }
    ...
    await syncSidecarDirectory(store.staging, "sidecar store staging directory");
}
async function validateSidecarClaimDirectory(store) {
    const names = await listSidecarDirectory(store.claims, "sidecar store claims directory", exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxConcurrentWriters + 2);
    let writerCount = 0;
    for (const name of names) {
        if (name === STORE_RECOVERY_CLAIM) {
            await readSidecarRecoveryClaimToken(node_path_1.default.join(store.claims.directory, name), store.claims.stats.dev);
            continue;
        }
        if (name === STORE_WRITER_ADMISSION_CLAIM) {
            const admissionClaim = await assertRecoverableSidecarClaim(node_path_1.default.join(store.claims.directory, name), "sidecar writer admission claim", store.claims.stats.dev);
        ...
        writerCount += 1;
        const writerClaim = await assertRecoverableSidecarClaim(node_path_1.default.join(store.claims.directory, name), `sidecar writer claim ${name}`, store.claims.stats.dev, MAX_WRITER_JOURNAL_BYTES);
        if (writerClaim.size !== 0n) {
            await readSidecarWriterJournal(node_path_1.default.join(store.claims.directory, name), store.claims.stats.dev);
        }
    }
    if (writerCount
        > exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxConcurrentWriters) {
        fail("sidecar store writer claims exceed their reserved capacity");
    }
    return names;
}
async function createSidecarWriterClaim(store, options) {
    const recoveryPath = node_path_1.default.join(store.claims.directory, STORE_RECOVERY_CLAIM);
    if (await sidecarPathExists(recoveryPath)) {
        fail("sidecar writer refuses an active recovery claim");
    }
    const admissionPath = node_path_1.default.join(store.claims.directory, STORE_WRITER_ADMISSION_CLAIM);
    let admissionStats;
    try {
        await options.faultInjection?.("before_writer_admission_create");
        admissionStats = writeExclusiveSidecarClaim(store.claims, admissionPath, "sidecar writer admission claim");
        await syncSidecarDirectory(store.claims, "sidecar store claims directory");
    }
    catch (error) {
        if (sidecarErrnoCode(error) === "EEXIST") {
    ...
    const claimPath = node_path_1.default.join(store.claims.directory, `writer_${token}.claim`);
    ...
    try {
        ...
        const priorWriterCount = priorClaims.filter((name) => (STORE_WRITER_CLAIM.test(name))).length;
        if (priorWriterCount
            >= exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxConcurrentWriters) {
            fail("sidecar store has no reserved writer-claim capacity");
        }
        await options.faultInjection?.("before_writer_claim_create");
        writerStats = writeExclusiveSidecarClaim(store.claims, claimPath, "sidecar writer claim");
        writerCreated = true;
        await syncSidecarDirectory(store.claims, "sidecar store claims directory");
        await validateSidecarClaimDirectory(store);
        if (await sidecarPathExists(recoveryPath)) {
        ...
        await assertRecoverableSidecarClaim(claimPath, "sidecar writer claim", store.claims.stats.dev);
        if (admissionStats === undefined) {
            fail("sidecar writer admission claim lost its exact identity");
        }
        revalidateSidecarDirectorySync(store.claims, "sidecar store claims directory");
        unlinkExactCreatedSidecarFileSync(admissionPath, store.claims.stats, admissionStats, "", "sidecar writer admission claim");
        ...
        if (writerCreated
            ...
            && claimsStillCurrent) {
            try {
                unlinkExactCreatedSidecarFileSync(claimPath, store.claims.stats, writerStats, "", "sidecar writer claim");
            }
            catch (cleanupError) {
        ...
        if (cleanupErrors.length > 0) {
            throw new AggregateError([error, ...cleanupErrors], "P10.2 identity/dedupe projection: writer claim creation failed "
...
async function closeSidecarWriterClaim(store, claim) {
    revalidateSidecarDirectorySync(store.claims, "sidecar store claims directory");
    unlinkExactCreatedSidecarFileSync(claim.claimPath, store.claims.stats, claim.stats, claim.bytes, "sidecar writer claim");
    await syncSidecarDirectory(store.claims, "sidecar store claims directory");
}
async function acquireSidecarRecoveryClaim(store, options, writeOptions) {
    const { operationToken } = options;
    assertDigest(operationToken, "sidecar recovery operation token");
    const claimPath = node_path_1.default.join(store.claims.directory, STORE_RECOVERY_CLAIM);
    const createClaim = async () => {
        await writeOptions.faultInjection?.("before_recovery_claim_create");
        const stats = writeExclusiveSidecarClaim(store.claims, claimPath, "sidecar recovery claim", operationToken);
        try {
            await readSidecarRecoveryClaimToken(claimPath, store.claims.stats.dev, operationToken);
            await syncSidecarDirectory(store.claims, "sidecar store claims directory");
            ...
            if (claimsStillCurrent) {
                unlinkExactCreatedSidecarFileSync(claimPath, store.claims.stats, stats, operationToken, "sidecar recovery claim");
            }
            throw error;
        }
    };
    try {
        ...
        if (sidecarErrnoCode(error) !== "EEXIST")
            throw error;
        if (options.takeOverRecoveryClaim !== true
            || options.externalExclusivityAsserted !== true) {
            fail("sidecar recovery claim is already active");
        }
        const existingStats = await assertRecoverableSidecarClaim(claimPath, "sidecar recovery claim", store.claims.stats.dev);
        const existingToken = existingStats.size === 0n
            ? ""
            : await readSidecarRecoveryClaimToken(claimPath, store.claims.stats.dev);
        revalidateSidecarDirectorySync(store.claims, "sidecar store claims directory");
        unlinkExactCreatedSidecarFileSync(claimPath, store.claims.stats, existingStats, existingToken, "sidecar recovery claim takeover");
        await syncSidecarDirectory(store.claims, "sidecar store claims directory");
        try {
            return await createClaim();
        }
...
function snapshotRecoveryOptions(value) {
    if (value === null
        || typeof value !== "object"
        || node_util_1.types.isProxy(value)
    ...
    const snapshot = {
        operationToken: descriptors.get("operationToken").value,
        ...(descriptors.get("takeOverRecoveryClaim") === undefined
            ? {}
            : {
                takeOverRecoveryClaim: descriptors.get("takeOverRecoveryClaim").value,
            }),
        ...(descriptors.get("externalExclusivityAsserted") === undefined
            ? {}
            : {
                externalExclusivityAsserted: descriptors.get("externalExclusivityAsserted").value,
            }),
        ...(descriptors.get("abandonWriterClaims") === undefined
            ? {}
            : {
                abandonWriterClaims: descriptors.get("abandonWriterClaims").value,
            }),
    };
    ...
    if ((snapshot.takeOverRecoveryClaim === true
        || snapshot.abandonWriterClaims === true)
        && snapshot.externalExclusivityAsserted !== true) {
        fail("sidecar recovery takeover requires external exclusivity assertion");
    }
    return snapshot;
}
async function recoverTaskMapIdentityDedupeProjectionStore(roots, options, writeOptions = {}) {
    assertArrayPrototypeIntegrity();
    ...
    const recoveryClaim = await acquireSidecarRecoveryClaim(store, snapshot, writeOptions);
    let recoveryFailure;
    try {
        const claimNames = await listSidecarDirectory(store.claims, "sidecar store claims directory", exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxConcurrentWriters + 2);
        const writerClaims = new Map();
        let writerAdmissionPath;
        let writerAdmissionStats;
        for (const name of claimNames) {
            if (name === STORE_RECOVERY_CLAIM)
                continue;
            if (name === STORE_WRITER_ADMISSION_CLAIM) {
                writerAdmissionPath = node_path_1.default.join(store.claims.directory, name);
                const admission = await assertRecoverableSidecarClaim(writerAdmissionPath, "sidecar writer admission claim", store.claims.stats.dev);
            ...
            const writerPath = node_path_1.default.join(store.claims.directory, name);
            const writer = await assertRecoverableSidecarClaim(writerPath, `sidecar writer claim ${name}`, store.claims.stats.dev, MAX_WRITER_JOURNAL_BYTES);
            const journal = writer.size === 0n
                ? null
                : await readSidecarWriterJournal(writerPath, store.claims.stats.dev);
            writerClaims.set(match[1], {
                ...
                bytes: journal === null
                    ? ""
                    : (0, source_contracts_js_1.taskMapContractCanonicalJson)(journal),
                journal,
            });
        }
        if ((writerClaims.size > 0
            || writerAdmissionPath !== undefined)
            && snapshot.abandonWriterClaims !== true) {
            fail("sidecar store recovery refuses active writer claims or an "
                + "admission claim "
                + "without external exclusivity");
        }
        const names = await listSidecarDirectory(store.staging, "sidecar store staging directory", exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxStagingEntries);
        let stagedBytes = 0n;
        const stagedEntries = [];
        const linkedGenerationInodes = new Set();
        for (const name of names) {
            ...
            const stageStats = await assertRecoverableSidecarStage(stagePath, `sidecar staged entry ${name}`, store.staging.stats.dev);
            ...
            if (claim === undefined
                ...
                fail("sidecar staged residue has no exact durable writer-journal "
                    + "binding");
            }
            if (stageStats.nlink === 1n) {
                stagedEntries.push({
                    ...
                    stats: stageStats,
                    expectedNlink: 1n,
                    expectedBytes: await readExactRecoverableSidecarStageBytes(stagePath, stageStats, `sidecar staged entry ${name}`),
                    journalRequired: true,
                });
                continue;
            }
            const staged = await readSidecarEntryFile(stagePath, `sidecar staged entry ${name}`, [2], store.staging.stats.dev);
            const generationPath = node_path_1.default.join(store.generations.directory, sidecarGenerationFileName(staged.entry.generation));
            const committed = await readSidecarEntryFile(generationPath, `sidecar committed generation ${staged.entry.generation}`, [2], store.generations.stats.dev);
            if (staged.stats.dev !== stageStats.dev
            ...
            linkedGenerationInodes.add(`${committed.stats.dev}:${committed.stats.ino}`);
            stagedEntries.push({
                stagePath,
                writerClaim: {
                    token: stageMatch[1],
        ...
        await readSidecarStoreFromOpen(store, true, linkedGenerationInodes);
        await revalidateDisjointTaskMapStoreRoots(storeRoots, isolatedRoots);
        if (linkedGenerationInodes.size > 0) {
            await syncSidecarDirectory(store.generations, "sidecar store generations directory");
        }
        await writeOptions.faultInjection?.("before_recovery_stage_cleanup");
        for (const staged of stagedEntries) {
            await unlinkExactSidecarStage(store, staged, "sidecar recovery staged entry");
        }
        if (writerAdmissionPath !== undefined
            && writerAdmissionStats !== undefined) {
            revalidateSidecarDirectorySync(store.claims, "sidecar store claims directory");
            unlinkExactCreatedSidecarFileSync(writerAdmissionPath, store.claims.stats, writerAdmissionStats, "", "sidecar writer admission claim");
        }
        for (const claim of writerClaims.values()) {
            revalidateSidecarDirectorySync(store.claims, "sidecar store claims directory");
            unlinkExactCreatedSidecarFileSync(claim.path, store.claims.stats, claim.stats, claim.bytes, `sidecar writer claim ${claim.name}`);
        }
        if (writerAdmissionPath !== undefined
            || writerClaims.size > 0) {
            await syncSidecarDirectory(store.claims, "sidecar store claims directory");
    ...
    try {
        revalidateSidecarDirectorySync(store.claims, "sidecar store claims directory");
        unlinkExactCreatedSidecarFileSync(recoveryClaim.claimPath, store.claims.stats, recoveryClaim.stats, recoveryClaim.bytes, "sidecar recovery claim");
    }
    catch (error) {
...
function sameCurrentHead(left, right) {
    return (left.status === "healthy"
...
function assertSidecarStoreAlignedWithCurrent(sidecarStore, currentSnapshot) {
    const current = assertHealthyCurrentHistory(currentSnapshot);
    const generationIndex = Number(BigInt(current.head.generation) - 1n);
    if (sidecarStore.entries.length < generationIndex
        || sidecarStore.entries.length > generationIndex + 1) {
        fail("sidecar store requires reviewed sequential backfill from generation one");
    }
    for (let index = 0; index < sidecarStore.entries.length; index += 1) {
        ...
        if (ref === undefined
            ...
            || entry.currentRefDigest !== (0, source_contracts_js_1.taskMapContractDigest)(ref)
        ...
        else if (ref.operation !== "rollback"
            || accepted === undefined
            || ref.rollback === undefined
...
async function verifySidecarStoreAcceptedOrigins(sidecarStore, currentSnapshot, runRoot) {
    const indicesByBundleId = new Map();
    for (let index = 0; index < sidecarStore.entries.length; index += 1) {
        const entry = sidecarStore.entries[index];
        if (entry.entryKind !== "projection")
            continue;
    ...
    for (const [bundleId, indices] of indicesByBundleId) {
        const origin = await (0, refresh_run_bundle_js_1.verifyTaskMapRefreshRunBundle)(node_path_1.default.join(runRoot, bundleId), bundleId);
...
function resolveSemanticPredecessor(sidecarStore, predecessor) {
    const head = predecessor?.semanticHead ?? null;
    if (head === null)
        return null;
    const candidate = sidecarStore.entries.find((entry) => (entry.entryKind === "projection"
...
async function buildStoreBackedProjection(snapshot) {
    const isolatedRoots = await assertDisjointTaskMapStoreRoots(snapshot);
    const currentSnapshot = await (0, refresh_current_ref_js_1.readTaskMapRefreshCurrent)(snapshot.currentRoot, snapshot.runRoot);
    const current = assertHealthyCurrentSnapshot(currentSnapshot);
    if (current.head.operation === "rollback") {
        fail("rollback identity decisions require a reviewed authority artifact; "
            + "publish a rollback observation instead");
    }
    const sidecarStore = await readTaskMapIdentityDedupeProjectionStoreStructural(snapshot.sidecarRoot);
    const { predecessor, currentEntry } = assertSidecarStoreAlignedWithCurrent(sidecarStore, currentSnapshot);
    await verifySidecarStoreAcceptedOrigins(sidecarStore, currentSnapshot, snapshot.runRoot);
    const acceptedOrigin = await (0, refresh_run_bundle_js_1.verifyTaskMapRefreshRunBundle)(node_path_1.default.join(snapshot.runRoot, current.accepted.bundleId), current.accepted.bundleId);
    const semanticPredecessor = resolveSemanticPredecessor(sidecarStore, predecessor);
    let previousAcceptedOrigin = null;
    if (semanticPredecessor !== null) {
        const predecessorRef = currentSnapshot.generations[Number(BigInt(semanticPredecessor.generation) - 1n)];
        if (predecessorRef.accepted === undefined
        ...
        previousAcceptedOrigin = await (0, refresh_run_bundle_js_1.verifyTaskMapRefreshRunBundle)(node_path_1.default.join(snapshot.runRoot, predecessorRef.accepted.bundleId), predecessorRef.accepted.bundleId);
    }
    const { currentRoot: _currentRoot, runRoot: _runRoot, sidecarRoot: _sidecarRoot, ...verifiedInput } = snapshot;
    const built = unsafeBuildTaskMapIdentityDedupeProjectionFromVerifiedInputsForTest({
        ...verifiedInput,
        currentSnapshot,
        acceptedOrigin,
...
async function buildBackfillProjection(input) {
    const isolatedRoots = await assertDisjointTaskMapStoreRoots(input);
    const observedCurrentSnapshot = await (0, refresh_current_ref_js_1.readTaskMapRefreshCurrent)(input.currentRoot, input.runRoot);
    ...
    const targetSnapshot = {
        status: "healthy",
        head: targetRef,
        generations: observedCurrentSnapshot.generations.slice(0, targetIndex + 1),
        unknownGenerationNames: [],
        unknownObjectNames: [],
    };
    ...
    const sidecarStore = await readTaskMapIdentityDedupeProjectionStoreStructural(input.sidecarRoot);
    if (sidecarStore.entries.length !== targetIndex
        && sidecarStore.entries.length !== targetIndex + 1) {
        fail("backfill must append exactly the next missing sidecar generation");
    }
    const { predecessor, currentEntry } = assertSidecarStoreAlignedWithCurrent(sidecarStore, targetSnapshot);
    await verifySidecarStoreAcceptedOrigins(sidecarStore, targetSnapshot, input.runRoot);
    const acceptedOrigin = await (0, refresh_run_bundle_js_1.verifyTaskMapRefreshRunBundle)(node_path_1.default.join(input.runRoot, target.accepted.bundleId), target.accepted.bundleId);
    const semanticPredecessor = resolveSemanticPredecessor(sidecarStore, predecessor);
    let previousAcceptedOrigin = null;
    if (semanticPredecessor !== null) {
        const predecessorRef = targetSnapshot.generations[Number(BigInt(semanticPredecessor.generation) - 1n)];
        if (predecessorRef.accepted === undefined
            || predecessorRef.accepted.bundleId
                !== semanticPredecessor.acceptedOriginBundleId) {
            fail("backfill semantic predecessor has no accepted origin");
        }
        previousAcceptedOrigin = await (0, refresh_run_bundle_js_1.verifyTaskMapRefreshRunBundle)(node_path_1.default.join(input.runRoot, predecessorRef.accepted.bundleId), predecessorRef.accepted.bundleId);
    }
    const { targetGeneration: _targetGeneration, currentRoot: _currentRoot, runRoot: _runRoot, sidecarRoot: _sidecarRoot, ...verifiedInput } = input;
    const built = unsafeBuildTaskMapIdentityDedupeProjectionFromVerifiedInputsForTest({
        ...verifiedInput,
...
function historyStillContainsTarget(targetSnapshot, latestSnapshot) {
    if (targetSnapshot.status !== "healthy"
    ...
    const latestTarget = latestSnapshot.generations[targetSnapshot.generations.length - 1];
...
async function reopenExactRetryEntry(sidecarRoot, targetSnapshot, expectedEntry, label) {
    const latestStore = await readTaskMapIdentityDedupeProjectionStoreStructural(sidecarRoot);
    const aligned = assertSidecarStoreAlignedWithCurrent(latestStore, targetSnapshot);
    if (aligned.currentEntry === null
        || !canonicalEqual(aligned.currentEntry, expectedEntry)) {
        fail(`${label} changed before idempotent return`);
    }
    return aligned.currentEntry;
}
function assertTaskMapIdentityDedupeStoreAppendCapacity(committedBytes, candidateBytes, targetAlreadyExists = false, committedEntries = 0) {
    ...
    if (committedBytes + growth
        > exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxStoreHistoryBytes) {
        fail("sidecar store append exceeds the aggregate history byte bound");
    }
}
async function assertSidecarStoreAppendCapacity(store, generationFileName, candidateBytes) {
    const names = await listSidecarDirectory(store.generations, "sidecar store generations directory", exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxStoreEntries);
    ...
    for (const name of names) {
        ...
        const stats = await assertSidecarFileStats(node_path_1.default.join(store.generations.directory, name), `sidecar store generation ${name}`, [1, 2], store.generations.stats.dev);
    ...
    assertTaskMapIdentityDedupeStoreAppendCapacity(committedBytes, candidateBytes, targetAlreadyExists, names.length);
}
async function writeSidecarStage(store, entry, options) {
    const bytes = (0, source_contracts_js_1.taskMapContractCanonicalJson)(entry);
    const [firstChunk, secondChunk] = taskMapIdentityDedupeUtf8WriteChunks(bytes);
    ...
    const stagePath = node_path_1.default.join(store.staging.directory, `stage_${writerClaim.token}.json`);
    let descriptor;
    let stagedStats;
    let initialStageStats;
    let stageCreated = false;
    let journalBound = false;
    let injectedError;
    const inject = async (point) => {
        try {
    ...
    try {
        const existingStages = await listSidecarDirectory(store.staging, "sidecar store staging directory", exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxStagingEntries);
        ...
        if (existingStages.length
            >= exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxStagingEntries
            || existingStageBytes + BigInt(byteLength)
                > BigInt(exports.TASKMAP_IDENTITY_DEDUPE_LIMITS_V1.maxStagingBytes)) {
            fail("sidecar store has no transient staging capacity");
        }
        await inject("before_stage_create");
        const stagingParentDescriptor = openVerifiedSidecarParentSync(store.staging, "sidecar store staging directory");
        try {
            ...
            initialStageStats = (0, node_fs_1.fstatSync)(descriptor, { bigint: true });
            (0, node_fs_1.fchmodSync)(descriptor, FILE_MODE);
            initialStageStats = (0, node_fs_1.fstatSync)(descriptor, { bigint: true });
            if (!checkedSidecarParentStillCurrentSync(stagingParentDescriptor, store.staging)) {
                (0, node_fs_1.closeSync)(descriptor);
                descriptor = undefined;
                const currentParent = observeSidecarDirectorySync(store.staging.directory, "sidecar replacement staging directory");
                unlinkExactCreatedSidecarFileSync(stagePath, currentParent, initialStageStats, "", "sidecar staged entry");
                stageCreated = false;
            ...
            try {
                (0, node_fs_1.closeSync)(stagingParentDescriptor);
            }
            catch {
                // The staging parent was already checked around O_EXCL creation.
            }
        }
        if (!initialStageStats.isFile()
        ...
        await bindSidecarWriterClaimToStage(store, writerClaim, stagePath, initialStageStats);
        journalBound = true;
        await inject("after_stage_create");
        if ((0, node_fs_1.writeSync)(descriptor, firstChunk, 0, firstChunk.byteLength, 0) !== firstChunk.byteLength) {
            fail("sidecar staged entry first write was incomplete");
        }
        await inject("after_stage_partial_write");
        if ((0, node_fs_1.writeSync)(descriptor, secondChunk, 0, secondChunk.byteLength, firstChunk.byteLength) !== secondChunk.byteLength) {
            fail("sidecar staged entry second write was incomplete");
        }
        (0, node_fs_1.fsyncSync)(descriptor);
        stagedStats = (0, node_fs_1.fstatSync)(descriptor, { bigint: true });
        ...
        if (!journalBound) {
            ...
            if (stageCreated && initialStageStats !== undefined) {
                try {
                    ...
                    revalidateSidecarDirectorySync(store.staging, "sidecar store staging directory");
                    unlinkExactCreatedSidecarFileSync(stagePath, store.staging.stats, initialStageStats, "", "sidecar pre-journal staged entry");
                    stageCreated = false;
...
async function verifySidecarStageBeforeLink(staged, expectedEntry) {
    const observed = await readSidecarEntryFile(staged.stagePath, "sidecar staged entry before generation CAS", [1], staged.stats.dev);
...
function verifySidecarStageBeforeLinkSync(staged, expectedEntry) {
    let descriptor;
    try {
        const observed = (0, node_fs_1.lstatSync)(staged.stagePath, { bigint: true });
        if (!observed.isFile()
        ...
        descriptor = (0, node_fs_1.openSync)(staged.stagePath, node_fs_1.constants.O_RDONLY
            | node_fs_1.constants.O_NOFOLLOW
            | node_fs_1.constants.O_NONBLOCK);
        const current = (0, node_fs_1.fstatSync)(descriptor, { bigint: true });
        if (current.dev !== observed.dev
            || current.ino !== observed.ino
            || current.size !== observed.size
            || current.nlink !== 1n
        ...
        const bytes = decodeExactUtf8(buffer, "sidecar staged entry at generation CAS");
        let parsed;
        try {
            parsed = JSON.parse(bytes);
        }
        ...
        if (bytes !== staged.bytes
            || (0, source_contracts_js_1.taskMapContractCanonicalJson)(entry) !== bytes
...
function unlinkExactSidecarGenerationFromObservedParentSync(generationPath, expectedParent, staged, label) {
    let parentDescriptor;
    let entryDescriptor;
    try {
        parentDescriptor = (0, node_fs_1.openSync)(node_path_1.default.dirname(generationPath), node_fs_1.constants.O_RDONLY
            | node_fs_1.constants.O_NOFOLLOW
            | node_fs_1.constants.O_NONBLOCK);
        const openedParent = (0, node_fs_1.fstatSync)(parentDescriptor, { bigint: true });
        if (!openedParent.isDirectory()
        ...
        const observed = (0, node_fs_1.lstatSync)(generationPath, { bigint: true });
        if (!observed.isFile()
            || observed.isSymbolicLink()
            || observed.uid !== BigInt(sidecarCurrentUid())
            || sidecarModeBits(observed) !== FILE_MODE
        ...
        entryDescriptor = (0, node_fs_1.openSync)(generationPath, node_fs_1.constants.O_RDONLY
            | node_fs_1.constants.O_NOFOLLOW
            | node_fs_1.constants.O_NONBLOCK);
        const opened = (0, node_fs_1.fstatSync)(entryDescriptor, { bigint: true });
        if (opened.dev !== observed.dev
            || opened.ino !== observed.ino
            || opened.uid !== observed.uid
            || sidecarModeBits(opened) !== FILE_MODE
        ...
        const bytes = (0, node_fs_1.readFileSync)(entryDescriptor);
        if (BigInt(bytes.byteLength) !== opened.size
            || !bytes.equals(Buffer.from(staged.bytes, "utf8"))) {
            fail(`${label} bytes changed before exact rollback`);
        }
        const finalParent = (0, node_fs_1.lstatSync)(node_path_1.default.dirname(generationPath), { bigint: true });
        const finalEntry = (0, node_fs_1.lstatSync)(generationPath, { bigint: true });
        if (finalParent.dev !== openedParent.dev
            || finalParent.ino !== openedParent.ino
        ...
        (0, node_fs_1.unlinkSync)(generationPath);
        (0, node_fs_1.fsyncSync)(parentDescriptor);
    }
    catch (error) {
        if (error instanceof Error
        ...
        if (entryDescriptor !== undefined) {
            try {
                (0, node_fs_1.closeSync)(entryDescriptor);
            }
            catch {
                // Exact rollback success/failure was already decided above.
            }
...
async function verifyLinkedSidecarPeers(staged, generationPath, expectedEntry) {
    const stage = await readSidecarEntryFile(staged.stagePath, "linked sidecar staged entry", [2], staged.stats.dev);
    const generation = await readSidecarEntryFile(generationPath, `sidecar committed generation ${expectedEntry.generation}`, [2], staged.stats.dev);
    if (stage.stats.dev !== staged.stats.dev
...
async function publishSidecarEntry(store, entry, beforeCas, options = {}) {
    const generationPath = node_path_1.default.join(store.generations.directory, sidecarGenerationFileName(entry.generation));
    const candidateBytes = Buffer.byteLength((0, source_contracts_js_1.taskMapContractCanonicalJson)(entry), "utf8");
    await assertSidecarStoreAppendCapacity(store, node_path_1.default.basename(generationPath), candidateBytes);
    const staged = await writeSidecarStage(store, entry, options);
    let stageRemoved = false;
    let claimCloseAttempted = false;
    const closeRemovedStageClaim = async () => {
    ...
    try {
        ...
        await revalidateSidecarDirectory(store.staging, "sidecar store staging directory");
        try {
            await assertSidecarStoreAppendCapacity(store, node_path_1.default.basename(generationPath), Buffer.byteLength(staged.bytes, "utf8"));
        }
        catch (error) {
        ...
        let generationParentDescriptor;
        let stagingParentDescriptor;
        try {
            revalidateSidecarDirectorySync(store.generations, "sidecar store generations directory");
            await options.faultInjection?.("after_generation_parent_check");
            stagingParentDescriptor = openVerifiedSidecarParentSync(store.staging, "sidecar store staging directory");
            generationParentDescriptor = openVerifiedSidecarParentSync(store.generations, "sidecar store generations directory");
        }
        catch (error) {
        ...
        try {
            (0, node_fs_1.linkSync)(staged.stagePath, generationPath);
            generationLinked = true;
        }
        catch (error) {
            const generationParentCurrent = generationParentDescriptor !== undefined
                && checkedSidecarParentStillCurrentSync(generationParentDescriptor, store.generations);
            const stagingParentCurrent = stagingParentDescriptor !== undefined
                && checkedSidecarParentStillCurrentSync(stagingParentDescriptor, store.staging);
            if (generationParentDescriptor !== undefined) {
                (0, node_fs_1.closeSync)(generationParentDescriptor);
            ...
            revalidateOpenSidecarStoreSync(store, "sidecar already-published return");
            await unlinkExactSidecarStage(store, {
                ...staged,
                expectedNlink: 1n,
                expectedBytes: staged.bytes,
            ...
            stageRemoved = true;
            const existing = await readSidecarEntryFile(generationPath, `sidecar store generation ${entry.generation}`, [1, 2], store.generations.stats.dev);
        ...
        if (!linkedParentCurrent || !linkedStagingParentCurrent) {
            linkedParent = observeSidecarDirectorySync(store.generations.directory, "sidecar generations directory after generation CAS");
        }
        if (generationParentDescriptor !== undefined) {
            (0, node_fs_1.closeSync)(generationParentDescriptor);
            generationParentDescriptor = undefined;
        ...
        if (!linkedParentCurrent || !linkedStagingParentCurrent) {
            ...
            unlinkExactSidecarGenerationFromObservedParentSync(generationPath, linkedParent, staged, "sidecar generation in a replaced parent");
        ...
        await verifyLinkedSidecarPeers(staged, generationPath, entry);
        await options.faultInjection?.("after_generation_link");
        revalidateOpenSidecarStoreSync(store, "sidecar linked-generation postcheck");
        await verifyLinkedSidecarPeers(staged, generationPath, entry);
        await syncSidecarDirectory(store.generations, "sidecar store generations directory");
        revalidateOpenSidecarStoreSync(store, "sidecar linked-generation sync");
        await options.faultInjection?.("before_stage_cleanup");
        revalidateOpenSidecarStoreSync(store, "sidecar published return");
        await unlinkExactSidecarStage(store, {
            ...staged,
            expectedNlink: 2n,
            expectedBytes: staged.bytes,
 ...
 */
async function buildTaskMapIdentityDedupeProjection(input) {
    assertArrayPrototypeIntegrity();
    const snapshot = snapshotStoreInput(input);
    const context = await buildStoreBackedProjection(snapshot);
    await revalidateDisjointTaskMapStoreRoots(snapshot, context.isolatedRoots);
    const latest = await (0, refresh_current_ref_js_1.readTaskMapRefreshCurrent)(snapshot.currentRoot, snapshot.runRoot);
 ...
 */
async function buildAndPublishTaskMapIdentityDedupeProjection(input, options = {}) {
    assertArrayPrototypeIntegrity();
    const snapshot = snapshotStoreInput(input);
    const context = await buildStoreBackedProjection(snapshot);
    if (context.currentEntry !== null) {
        ...
        await revalidateDisjointTaskMapStoreRoots(snapshot, context.isolatedRoots);
        const exactEntry = await reopenExactRetryEntry(snapshot.sidecarRoot, context.currentSnapshot, context.currentEntry, "current sidecar generation");
        const latest = await (0, refresh_current_ref_js_1.readTaskMapRefreshCurrent)(snapshot.currentRoot, snapshot.runRoot);
        if (!sameCurrentHead(context.currentSnapshot, latest)) {
    ...
    const entry = buildStoreEntry(context.built, context.predecessor, context.currentSnapshot.head);
    await revalidateDisjointTaskMapStoreRoots(snapshot, context.isolatedRoots);
    const store = await openSidecarStore(snapshot.sidecarRoot);
    const publication = await publishSidecarEntry(store, entry, async () => {
        const latestSidecars = await readSidecarStoreFromOpen(store, true);
        const aligned = assertSidecarStoreAlignedWithCurrent(latestSidecars, context.currentSnapshot);
        if ((aligned.currentEntry !== null
    ...
    const latest = await (0, refresh_current_ref_js_1.readTaskMapRefreshCurrent)(snapshot.currentRoot, snapshot.runRoot);
    if (!sameCurrentHead(context.currentSnapshot, latest)) {
        fail("P10.1 current head advanced after committed sidecar publication; retry");
    }
    await revalidateDisjointTaskMapStoreRoots(snapshot, context.isolatedRoots);
 ...
 */
async function backfillTaskMapIdentityDedupeProjectionGeneration(input, options = {}) {
    assertArrayPrototypeIntegrity();
    ...
    if (context.currentEntry !== null) {
        await options.faultInjection?.("before_retry_revalidation");
        await revalidateDisjointTaskMapStoreRoots(snapshot, context.isolatedRoots);
        const exactEntry = await reopenExactRetryEntry(snapshot.sidecarRoot, context.currentSnapshot, context.currentEntry, "backfill sidecar generation");
        const latest = await (0, refresh_current_ref_js_1.readTaskMapRefreshCurrent)(snapshot.currentRoot, snapshot.runRoot);
...
async function publishTaskMapIdentityDedupeGenerationObservation(input, expectedKind, options) {
    assertArrayPrototypeIntegrity();
    const snapshot = snapshotObservationInput(input);
    const isolatedRoots = await assertDisjointTaskMapStoreRoots(snapshot);
    const observedCurrentSnapshot = await (0, refresh_current_ref_js_1.readTaskMapRefreshCurrent)(snapshot.currentRoot, snapshot.runRoot);
    ...
    if (sidecarStore.entries.length !== targetIndex
        && sidecarStore.entries.length !== targetIndex + 1) {
        fail("generation observation must append exactly the next missing "
            + "P10.2 generation");
    }
    const { predecessor, currentEntry } = assertSidecarStoreAlignedWithCurrent(sidecarStore, targetSnapshot);
    await verifySidecarStoreAcceptedOrigins(sidecarStore, targetSnapshot, snapshot.runRoot);
    ...
    if (currentEntry !== null) {
        ...
        await options.faultInjection?.("before_retry_revalidation");
        await revalidateDisjointTaskMapStoreRoots(snapshot, isolatedRoots);
        const exactEntry = await reopenExactRetryEntry(snapshot.sidecarRoot, targetSnapshot, currentEntry, "generation observation");
        const latest = await (0, refresh_current_ref_js_1.readTaskMapRefreshCurrent)(snapshot.currentRoot, snapshot.runRoot);
    ...
    const publication = await publishSidecarEntry(store, entry, async () => {
        const latestSidecars = await readSidecarStoreFromOpen(store, true);
        const aligned = assertSidecarStoreAlignedWithCurrent(latestSidecars, targetSnapshot);
        if ((aligned.currentEntry !== null
            && !canonicalEqual(aligned.currentEntry, entry))
            || (predecessor === null
                ? aligned.predecessor !== null
 ...
 * projection.
 */
async function backfillTaskMapIdentityDedupeNoAcceptedGeneration(input, options = {}) {
    return publishTaskMapIdentityDedupeGenerationObservation(input, "no_accepted_origin", options);
}
/**
 ...
 */
async function publishTaskMapIdentityDedupeRollbackObservation(input, options = {}) {
    return publishTaskMapIdentityDedupeGenerationObservation(input, "rollback_observed", options);
}
exports.backfillTaskMapIdentityDedupeRollbackObservation = publishTaskMapIdentityDedupeRollbackObservation;
