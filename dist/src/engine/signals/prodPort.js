...
exports.WUXING_ELEMENTS = void 0;
exports.extractIntradayFeatures = extractIntradayFeatures;
exports.updateRollingBaseline = updateRollingBaseline;
exports.normalizeForWuxing = normalizeForWuxing;
exports.inferYinYangState = inferYinYangState;
exports.inferWuxingStates = inferWuxingStates;
exports.updateBaselinesFromFeatures = updateBaselinesFromFeatures;
/** Insertion order used by wuxing.infer_states — load-bearing for max() tie-breaks. */
exports.WUXING_ELEMENTS = ["wood", "fire", "earth", "metal", "water"];
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
function extractHeartRateFeatures(samples) {
    if (samples.length === 0) {
        return {
            mean_hr: null,
            min_hr: null,
...
function extractHrvFeatures(samples) {
    if (samples.length === 0) {
        return {
            mean_hrv: null,
            min_hrv: null,
...
function extractActivityFeatures(stepsSamples, energySamples) {
    const features = {};
...
function extractRespiratoryFeatures(samples) {
    if (samples.length === 0) {
...
function computeMetricDeviation(currentValue, baselineMean, baselineStd) {
    if (baselineStd === 0)
        return 0.0;
    return (currentValue - baselineMean) / baselineStd;
}
function computeActivationScore(hrFeatures, hrvFeatures, activityFeatures, baselines, windowHours) {
    let score = 50.0;
    const hrBaseline = baselines["heart_rate"] ?? {};
    const hrMean = hrBaseline.baseline_mean ?? 70;
    const hrStd = hrBaseline.baseline_std ?? 0;
    const hrvBaseline = baselines["hrv"] ?? {};
    const hrvMean = hrvBaseline.baseline_mean ?? 50;
    const hrvStd = hrvBaseline.baseline_std ?? 0;
...
function extractIntradayFeatures(hk, baselines, windowHours) {
    const hrFeatures = extractHeartRateFeatures(hk.heart_rate ?? []);
    const hrvFeatures = extractHrvFeatures(hk.heart_rate_variability ?? []);
    const activityFeatures = extractActivityFeatures(hk.step_count ?? [], hk.active_energy_burned ?? []);
    const respFeatures = extractRespiratoryFeatures(hk.respiratory_rate ?? []);
    const activationScore = computeActivationScore(hrFeatures, hrvFeatures, activityFeatures, baselines, windowHours);
    const hasSensorData = hrFeatures.data_available ||
...
function updateRollingBaseline(mean, std, count, value) {
...
function normalizeForWuxing(features, baselines) {
...
function interpolateTudorLocke(steps) {
    if (steps <= 0)
        return 0.0;
...
function computeYinScore(hrvFeatures, hrFeatures, baselines) {
    let score = 50.0;
...
function computeYangScore(activityFeatures) {
    if (!activityFeatures.data_available) {
    ...
    const energyBoost = Math.min(((totalEnergy || 0) / ENERGY_DIVISOR) * ENERGY_BOOST_CAP, ENERGY_BOOST_CAP);
    const yang = tudorScore + energyBoost;
    return Math.max(0, Math.min(100, pythonRound(yang)));
}
function applySmoothing(newYin, newYang, prevYin, prevYang) {
...
function determineCategory(yinScore, yangScore) {
...
function interpretScore(score, bands) {
    const clamped = Math.max(0, Math.min(100, score));
    for (const [low, high, text] of bands) {
        if (low <= clamped && clamped <= high)
            return text;
    }
    return bands[bands.length - 1][2];
}
function yinYangInferState(hrvFeatures, hrFeatures, activityFeatures, baselines, previousState) {
 ...
 */
function inferYinYangState(features, baselines) {
    const yyf = features.yin_yang_features ?? {};
    return yinYangInferState(yyf.hrv_features ?? features.hrv ?? {}, yyf.hr_features ?? features.heart_rate ?? {}, yyf.activity_features ?? features.activity ?? {}, baselines, null);
}
// ---------------------------------------------------------------------------
// Wu Xing port (wuxing.py) — RAW scores only; eligibility gate NOT ported
// ---------------------------------------------------------------------------
function signalIntensity(value, baseline, inverted) {
 ...
 */
function inferWuxingStates(features, yinScore, yangScore) {
    ...
    const result = {};
    for (const element of exports.WUXING_ELEMENTS) {
 ...
 */
function updateBaselinesFromFeatures(baselines, features) {
    const metricMap = [
    ...
    for (const [name, value] of metricMap) {
        ...
        if (existing !== undefined) {
            [mean, std, count] = updateRollingBaseline(existing.baseline_mean, existing.baseline_std, existing.sample_count, value);
        }
        else {
            mean = value;
            std = 0.01;
