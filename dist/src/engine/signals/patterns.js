...
exports.UNMEASURABLE_PATTERNS = void 0;
exports.computePatternSignatures = computePatternSignatures;
exports.wornDominantDistribution = wornDominantDistribution;
const prodPort_js_1 = require("./prodPort.js");
/** wood→fire→earth→metal→water, aligned to WUXING_ELEMENTS insertion order. */
const ELEMENT_TO_PATTERN = {
    wood: "TENSION",
    fire: "OVERDRIVE",
    earth: "STAGNATION",
    metal: "CONSTRICTION",
...
exports.UNMEASURABLE_PATTERNS = new Set([
...
function measurabilityOf(dominant) {
    if (exports.UNMEASURABLE_PATTERNS.has(dominant))
        return "unmeasurable";
    if (HR_SEPARABLE.has(dominant))
        return "hr-separable";
    return "low";
}
/**
 ...
 * confident.
 */
function confidenceOf(sorted, dominant, coverage) {
    ...
    const capped = exports.UNMEASURABLE_PATTERNS.has(dominant)
        ? Math.min(base, 0.2)
        : base;
 ...
 */
function computePatternSignatures(samples, coverage, options = {}) {
    ...
    for (let ms = startMs; ms <= endMs; ms += DAY_MS) {
        ...
        const features = (0, prodPort_js_1.extractIntradayFeatures)(window, baselines, 12);
        const yy = (0, prodPort_js_1.inferYinYangState)(features, baselines);
        // WuXing scores read the wuxing-normalized features, not the raw
        // intraday dict (mirrors the reconstruction driver exactly).
        const wuxingFeatures = (0, prodPort_js_1.normalizeForWuxing)(features, baselines);
        const wuxing = (0, prodPort_js_1.inferWuxingStates)(wuxingFeatures, yy.yin_score, yy.yang_score);
        ...
        if (topScore <= 0)
            dominant = "BALANCED";
        const sortedScores = prodPort_js_1.WUXING_ELEMENTS.map((e) => wuxing[e].score).sort((a, b) => b - a);
        out.push({
            date,
            dominant,
            scores,
            confidence: dominant === "BALANCED" ? 0 : confidenceOf(sortedScores, dominant, cov),
            coverage: cov,
            measurability: dominant === "BALANCED" ? "low" : measurabilityOf(dominant),
            flaggedUnmeasurable: exports.UNMEASURABLE_PATTERNS.has(dominant),
            quality,
        });
        if (day.worn)
            (0, prodPort_js_1.updateBaselinesFromFeatures)(baselines, features);
    }
    return out;
}
function zeroScores() {
...
function wornDominantDistribution(signatures) {
    const dist = {};
    for (const sig of signatures) {
        if (sig.quality !== "real_inference")
            continue;
