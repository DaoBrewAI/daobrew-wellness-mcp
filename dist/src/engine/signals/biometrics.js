...
exports.DEFAULT_BIOMETRIC_METRICS = void 0;
exports.epochSecondsOf = epochSecondsOf;
exports.computeHttpBaselines = computeHttpBaselines;
exports.readBiometricSignals = readBiometricSignals;
...
exports.DEFAULT_BIOMETRIC_METRICS = [
    "heart_rate",
 ...
 */
function computeHttpBaselines(samplesByMetric) {
    const baselines = [];
    for (const [metric_type, samples] of samplesByMetric) {
        ...
        if (!Number.isFinite(baseline_avg) || !Number.isFinite(day_count))
            continue;
        baselines.push({ metric_type, baseline_avg, day_count });
    }
    return baselines;
}
async function readStateHistory(client, limit, startTs, endTs, chunkDays) {
...
async function readBiometricSignals(client, options = {}) {
    const metrics = options.metrics ?? exports.DEFAULT_BIOMETRIC_METRICS;
    ...
    const baselineHistories = await Promise.all(metrics.map((metric) => client.getHealthkitHistory(metric, "month"))).catch(() => []);
    ...
    const baselineStartTs = baselineEndTs === undefined ? undefined : baselineEndTs - BASELINE_WINDOW_DAYS * SECONDS_PER_DAY;
    const samplesByMetric = new Map();
    for (const history of baselineHistories) {
        const inWindow = history.samples.filter((sample) => {
            if (baselineEndTs === undefined)
                return true;
            const epochSec = epochSecondsOf(sample.timestamp);
            if (!Number.isFinite(epochSec))
                return false;
            return epochSec >= baselineStartTs && epochSec < baselineEndTs;
        });
        samplesByMetric.set(history.metric, inWindow.map((sample) => ({ value: sample.value, timestamp: sample.timestamp })));
    }
