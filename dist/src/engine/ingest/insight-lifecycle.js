...
exports.SUPPRESSED_INSIGHT_STRENGTH = exports.INSIGHT_LIFECYCLE_MAX_CLASSIFIED_ROWS = exports.INSIGHT_LIFECYCLE_NEIGHBOR_LIMIT = void 0;
exports.insightContentHash = insightContentHash;
exports.planInsightLifecycle = planInsightLifecycle;
...
exports.INSIGHT_LIFECYCLE_NEIGHBOR_LIMIT = 8;
exports.INSIGHT_LIFECYCLE_MAX_CLASSIFIED_ROWS = 24;
exports.SUPPRESSED_INSIGHT_STRENGTH = 0;
...
function lifecycleRowKey(source, sourceRef, contentHash) {
    return `${source}\u0000${sourceRef}\u0000${contentHash}`;
}
function decisionRecordId(userId, source, sourceRef, contentHash) {
...
function recentSameSourceSql(row) {
    return (`SELECT id, source_ref, insight_text AS text,\n` +
        `       COALESCE(occurred_at_ts, created_at_ts) AS ts,\n` +
        `       1.0 AS distance\n` +
        `  FROM user_insights\n` +
        ` WHERE user_id = ${user_scope_js_1.SCOPED_USER_ID_SQL}\n` +
        `   AND source = ${(0, graph_db_js_1.q)(row.source)}\n` +
        `   AND strength > 0\n` +
        ` ORDER BY created_at_ts DESC\n` +
        ` LIMIT ${exports.INSIGHT_LIFECYCLE_NEIGHBOR_LIMIT};`);
}
function buildLifecyclePrompt(rows) {
...
function validateDecisionPayload(raw, classified) {
    const payload = raw;
    ...
    for (const entry of payload.decisions) {
        ...
        const key = lifecycleRowKey(classified.find(({ work }) => work.row.source_ref === sourceRef && work.contentHash === contentHash)?.work.row.source ?? "", sourceRef, contentHash);
...
async function fetchStoredDecisions(query, rows) {
    if (rows.length === 0)
        return new Map();
    const tuples = rows
        .filter((work) => work.row.source_ref !== null)
        .map((work) => `(${(0, graph_db_js_1.q)(work.row.source)}, ${(0, graph_db_js_1.q)(work.row.source_ref)}, ${(0, graph_db_js_1.q)(work.contentHash)})`);
    if (tuples.length === 0)
        return new Map();
    const records = await query(`SELECT incoming_source, incoming_source_ref, incoming_content_hash, decision_kind,\n` +
...
async function fetchCandidateMetadata(query, ids) {
    if (ids.length === 0)
        return new Map();
    const rows = await query(`SELECT id, source, source_ref, insight_text, topics_json, importance, strength, occurred_at_ts, last_accessed_ts\n` +
        `  FROM user_insights\n` +
        ` WHERE user_id = ${user_scope_js_1.SCOPED_USER_ID_SQL}\n` +
        `   AND id IN (${ids.map((id) => (0, graph_db_js_1.q)(id)).join(", ")});`);
...
function buildDecisionSql(userId, nowTs, records) {
    return records.map((record) => `INSERT INTO insight_lifecycle_decisions(\n` +
        ...
        `)\n` +
        `VALUES (\n` +
        `  ${(0, graph_db_js_1.q)(decisionRecordId(userId, record.incomingRow.source, record.incomingRow.source_ref ?? "", record.incomingContentHash))},\n` +
        `  ${(0, graph_db_js_1.q)(userId)}, ${(0, graph_db_js_1.q)(record.incomingRow.source)}, ${(0, graph_db_js_1.q)(record.incomingRow.source_ref ?? "")}, ${(0, graph_db_js_1.q)(record.incomingContentHash)}, ${(0, graph_db_js_1.q)(record.incomingRow.id)},\n` +
        `  ${(0, graph_db_js_1.q)(record.decisionKind)}, ${litText(record.winnerInsightId)}, ${litText(record.winnerSource)}, ${litText(record.winnerSourceRef)},\n` +
        `  ${litText(record.loserInsightId)}, ${litText(record.loserSource)}, ${litText(record.loserSourceRef)},\n` +
        `  ${normalizeImportance(record.decidedImportance, record.incomingRow.importance)}, ${(0, graph_db_js_1.q)(record.rationale)}, ${nowTs}, ${nowTs}\n` +
        `)\n` +
...
function fallbackPlan(rows, warnings, geminiCallsUsed, decisionCounts, prefixRows) {
    return {
        rows: rows.map((row) => {
            if (!prefixRows || row.source_ref === null)
                return cloneRow(row);
...
async function planInsightLifecycle(options) {
    ...
    const storeKind = options.storeKind ?? graph_db_js_1.graphStoreKind;
    if (rows.length === 0) {
    ...
    try {
        ...
        const pending = [];
        const winnerUpdates = new Map();
        const loserSuppressions = new Set();
        for (const work of workRows) {
            ...
            if (replay.decision_kind === "noop_duplicate") {
                replayedRow.strength = exports.SUPPRESSED_INSIGHT_STRENGTH;
        ...
        const maxClassifiedRows = Math.max(0, options.maxClassifiedRows ?? exports.INSIGHT_LIFECYCLE_MAX_CLASSIFIED_ROWS);
        ...
        if (classified.length === 0) {
            const postWriteSql = [
                ...[...winnerUpdates.entries()].map(([id, update]) => `UPDATE user_insights SET importance = GREATEST(importance, ${update.importance}), strength = GREATEST(strength, ${update.strength}) WHERE user_id = ${(0, graph_db_js_1.q)(options.userId)} AND id = ${(0, graph_db_js_1.q)(id)};`),
                ...[...loserSuppressions].map((id) => `UPDATE user_insights SET strength = ${exports.SUPPRESSED_INSIGHT_STRENGTH} WHERE user_id = ${(0, graph_db_js_1.q)(options.userId)} AND id = ${(0, graph_db_js_1.q)(id)};`),
            ];
            return {
                rows: rows.map((row) => {
        ...
        for (let index = 0; index < classified.length; index++) {
            ...
            const annRows = await options.query((0, semantic_js_1.semanticStage1Sql)("user_insights", options.userId, vectors[index], {
                limit: exports.INSIGHT_LIFECYCLE_NEIGHBOR_LIMIT,
        ...
        for (const { work } of candidateBundles) {
            ...
            if (raw.decision_kind === "noop_duplicate") {
                ...
                nextRow.strength = exports.SUPPRESSED_INSIGHT_STRENGTH;
