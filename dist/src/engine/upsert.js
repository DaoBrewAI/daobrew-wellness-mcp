...
exports.GraphDeltaValidationError = void 0;
exports.generationLeaseSql = generationLeaseSql;
exports.mergeGhostProps = mergeGhostProps;
exports.validateGraphDelta = validateGraphDelta;
exports.upsertGraphDelta = upsertGraphDelta;
...
class GraphDeltaValidationError extends Error {
    issues;
    constructor(issues) {
...
exports.GraphDeltaValidationError = GraphDeltaValidationError;
const NODE_KINDS = new Set(schema_js_1.GRAPH_NODE_KINDS);
const EDGE_KINDS = new Set(schema_js_1.GRAPH_EDGE_KINDS);
 ...
 */
function generationLeaseSql(userId, generationId, leaseToken) {
    const user = (0, graph_db_js_1.q)(userId);
    const generation = (0, graph_db_js_1.q)(generationId);
...
function resolveGenerationLeaseSql(userId, options, isPostgres) {
...
function sqlValue(value) {
    ...
    return (0, graph_db_js_1.q)(value);
}
function sqlJson(value) {
    const literal = (0, graph_db_js_1.q)(JSON.stringify(value ?? {}));
    return (0, graph_db_js_1.graphStoreKind)() === "postgres" ? `${literal}::jsonb` : literal;
}
...
function mergeRootProps(node, delta) {
    return {
        ...(node.props_json ?? {}),
        status: "armed",
...
function deepMergeRecord(existing, incoming) {
    const merged = { ...existing };
    for (const [key, value] of Object.entries(incoming)) {
        if (isRecord(merged[key]) && isRecord(value)) {
            merged[key] = deepMergeRecord(merged[key], value);
        }
        else {
            merged[key] = value;
        }
...
function mergeGhostProps(existingDbProps = {}, incomingProps = {}) {
...
function requiredBriefFields(brief) {
    const missing = [];
    if (!brief.cause?.trim())
        missing.push("brief.cause");
    if (!Array.isArray(brief.evidence) || brief.evidence.length === 0)
        missing.push("brief.evidence");
    if (!brief.context?.trim())
        missing.push("brief.context");
    if (!brief.artifact_spec?.trim())
        missing.push("brief.artifact_spec");
    if (!brief.suggested_block?.title?.trim())
        missing.push("brief.suggested_block.title");
    if (!Number.isFinite(brief.suggested_block?.start_offset_min))
        missing.push("brief.suggested_block.start_offset_min");
    if (!Number.isFinite(brief.suggested_block?.duration_min))
        missing.push("brief.suggested_block.duration_min");
    return missing;
}
async function existingNodeIds(userId, query) {
    const rows = await query(`SELECT id FROM graph_nodes WHERE user_id = ${(0, user_scope_js_1.scopedUserIdExpr)(userId)};`);
    return new Set(rows.map((row) => row.id));
}
async function existingRootStatus(nodeId, userId, query) {
    const statusExpr = (0, graph_db_js_1.graphStoreKind)() === "postgres"
        ? "props_json ->> 'status'"
        : "json_extract(props_json, '$.status')";
    const rows = await query(`SELECT ${statusExpr} AS status
       FROM graph_nodes
      WHERE id = ${(0, graph_db_js_1.q)(nodeId)} AND user_id = ${(0, user_scope_js_1.scopedUserIdExpr)(userId)}
      LIMIT 1;`);
    return rows[0]?.status ?? null;
}
async function existingGhostStates(nodes, userId, query) {
    const ids = [...new Set(nodes
            .filter((node) => node.kind === "ghost")
            .map((node) => node.id))];
    if (ids.length === 0)
        return new Map();
    const rows = await query(`SELECT id, props_json, created_at_ts
       FROM graph_nodes
      WHERE user_id = ${(0, user_scope_js_1.scopedUserIdExpr)(userId)}
        AND id IN (${ids.map(graph_db_js_1.q).join(", ")});`);
...
async function validateGraphDelta(delta, options = {}) {
    const userId = (0, require_user_id_js_1.requireUserId)(options.userId ?? delta.user_id, "validateGraphDelta");
    ...
    for (const node of delta.nodes) {
        ...
        if (!NODE_KINDS.has(node.kind))
            issues.push(`invalid node kind ${node.kind}`);
        if (!node.title?.trim())
            issues.push(`node ${node.id} missing title`);
        if (!node.source?.trim())
            issues.push(`node ${node.id} missing source`);
        if (!node.source_ref?.trim())
            issues.push(`node ${node.id} missing source_ref`);
    ...
    if (!rootNode)
        issues.push("armed_root_cause.node_id is absent from nodes");
    ...
    for (const edge of delta.edges) {
        ...
        if (!EDGE_KINDS.has(edge.kind))
            issues.push(`invalid edge kind ${edge.kind}`);
        if (!allowedIds.has(edge.src_id))
            issues.push(`edge ${edge.id} missing src node ${edge.src_id}`);
        if (!allowedIds.has(edge.dst_id))
            issues.push(`edge ${edge.id} missing dst node ${edge.dst_id}`);
    }
    for (const id of duplicateEdgeIds)
        issues.push(`duplicate edge id ${id}`);
    if (issues.length > 0)
        throw new GraphDeltaValidationError(issues);
}
function resolvedNodeProps(node, nowTs, rootDelta, existingGhostStatesById = new Map()) {
    ...
    if (node.kind === "ghost" && isRecord(props) && props.status === "armed") {
        if (existingProps.status === "armed") {
            if (existingProps.armed_at_ts !== undefined)
                props = { ...props, armed_at_ts: existingProps.armed_at_ts };
            ...
            if (existingProps.offer_state !== undefined)
                props = { ...props, offer_state: existingProps.offer_state };
        }
        else {
...
function nodeSql(node, userId, nowTs, props) {
...
INSERT INTO graph_nodes(
  ...
  ${(0, graph_db_js_1.q)(node.id)}, ${(0, graph_db_js_1.q)(node.user_id ?? userId)}, ${(0, graph_db_js_1.q)(node.kind)}, ${(0, graph_db_js_1.q)(node.title)},
  ${sqlValue(node.subtitle)}, ${sqlValue(node.element)}, ${sqlValue(node.occurred_at_ts)},
  ${(0, graph_db_js_1.q)(node.source)}, ${(0, graph_db_js_1.q)(node.source_ref)}, ${sqlJson(props)}, ${sqlBlob(node.embedding)}, ${nowTs}
)
...
function edgeSql(edge, userId, nowTs) {
    if ((0, graph_db_js_1.graphStoreKind)() === "postgres") {
        return `
INSERT INTO graph_edges(
  id, user_id, src_id, dst_id, kind, label, weight, props_json, created_at_ts
) VALUES (
  ${(0, graph_db_js_1.q)(edge.id)}, ${(0, graph_db_js_1.q)(edge.user_id ?? userId)}, ${(0, graph_db_js_1.q)(edge.src_id)}, ${(0, graph_db_js_1.q)(edge.dst_id)},
  ${(0, graph_db_js_1.q)(edge.kind)}, ${sqlValue(edge.label)}, ${sqlValue(edge.weight ?? 1)},
...
function replaceInactiveGraphSql(delta, userId) {
    ...
    const edgeKeepClause = retainedEdgeIds.size
        ? `AND id NOT IN (${[...retainedEdgeIds].map(graph_db_js_1.q).join(", ")})`
        : "";
    const nodeKeepClause = retainedNodeIds.size
        ? `AND id NOT IN (${[...retainedNodeIds].map(graph_db_js_1.q).join(", ")})`
        : "";
    return `
DELETE FROM graph_edges
 WHERE user_id = ${(0, graph_db_js_1.q)(userId)}
 ${edgeKeepClause};
...
function bestEffortAssignmentSql(sql) {
...
async function upsertGraphDelta(delta, options = {}) {
    const userId = (0, require_user_id_js_1.requireUserId)(options.userId ?? delta.user_id, "upsertGraphDelta");
    ...
    const isPostgres = (0, graph_db_js_1.graphStoreKind)() === "postgres";
    ...
    if (isPostgres && shouldArmRoot && options.armedOffer) {
        const rootWrite = nodeWrites.find(({ node }) => node.id === delta.armed_root_cause.node_id);
        if (rootWrite?.node.kind === "ghost"
            && isRecord(rootWrite.props)
            && rootWrite.props.status === "armed") {
            const existing = existingGhostStatesById.get(rootWrite.node.id);
            armedOffer = (0, assignments_js_1.prepareArmedOffer)({
                userId,
            ...
            assignmentSql = bestEffortAssignmentSql((0, assignments_js_1.interventionAssignmentSql)({
                userId,
                ghostId: rootWrite.node.id,
    ...
    const disarmOtherGhostsSql = (0, graph_db_js_1.graphStoreKind)() === "postgres"
        ? `UPDATE graph_nodes
        SET props_json = jsonb_set(COALESCE(props_json, '{}'::jsonb), '{status}', '"suggested"'::jsonb, true)
      WHERE user_id = ${(0, graph_db_js_1.q)(userId)}
        AND kind = 'ghost'
        ${shouldArmRoot ? `AND id <> ${(0, graph_db_js_1.q)(delta.armed_root_cause.node_id)}` : ""}
