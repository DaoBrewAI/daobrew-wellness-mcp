...
import { scopedExec, scopedQuery, scopedUserIdExpr } from "../src/engine/user-scope.js";
import { execSql, queryJson, __resetGraphDbConfigCacheForTests } from "../src/graph-db.js";
...
describe("user-scope SQL shape (exec)", () => {
  it("composes ONE transaction: BEGIN, transaction-local set_config, delegated SQL, COMMIT", async () => {
    const captured: string[] = [];
    const exec = scopedExec("u1", async (sql) => {
      captured.push(sql);
    });
    ...
    assert.ok(
      sql.includes(`SELECT set_config('${GUC}', 'u1', true);`),
      `exec SQL must set the GUC transaction-locally (is_local = true), got: ${sql}`,
    );
    ...
    assert.ok(
      sql.includes("INSERT INTO graph_nodes (id, user_id) VALUES ('n1', 'u1')"),
      "delegated SQL must appear intact inside the transaction",
    );
    assert.ok(
      sql.indexOf("set_config") < sql.indexOf("INSERT INTO graph_nodes"),
      "set_config must run before the delegated SQL",
    );
  });

  it("statement-terminates the delegated SQL whether or not it ends with a semicolon", async () => {
    ...
    for (const delegated of ["SELECT 1", "SELECT 1;", "SELECT 1;;  "]) {
      ...
      assert.deepStrictEqual(
        lines,
        ["BEGIN;", `SELECT set_config('${GUC}', 'u1', true);`, "SELECT 1", ";", "COMMIT;"],
        `delegated ${JSON.stringify(delegated)} must compose a well-terminated script`,
      );
    }
  });

  it("keeps the terminator effective when the delegated SQL ends in a line comment", async () => {
    ...
    assert.deepStrictEqual(
      withoutComments,
      ["BEGIN;", `SELECT set_config('${GUC}', 'u1', true);`, "SELECT 1", ";", "COMMIT;"],
      `the delegated statement must stay terminated once comments are stripped, got: ${captured[0]}`,
    );
  });

  it("escapes a hostile userId so it stays inert inside a quoted literal", async () => {
    const captured: string[] = [];
    const exec = scopedExec(HOSTILE_USER_ID, async (sql) => {
      captured.push(sql);
    });
    await exec("SELECT 1;");
    ...
    const sql = captured[0];
    assert.ok(
      sql.includes(`set_config('${GUC}', ${HOSTILE_ESCAPED_LITERAL}, true)`),
      `hostile userId must be q()-escaped inside the literal, got: ${sql}`,
    );
    ...
    assert.ok(
      !outsideLiterals(sql).includes("DROP TABLE"),
      "DROP TABLE payload must stay confined to the quoted literal",
    );
  });
});

describe("user-scope SQL shape (query)", () => {
  it("composes a single SELECT with a lateral scope dependency (no statement prefix)", async () => {
    const captured: string[] = [];
    const rows = [{ id: "n1" }];
    const query = scopedQuery("u1", async (sql) => {
      captured.push(sql);
      return rows;
    });
    ...
    assert.ok(
      sql.startsWith(`SELECT __daobrew_scoped.* FROM (SELECT set_config('${GUC}', 'u1', true) AS uid) AS __daobrew_scope`),
      `query SQL must lead with the set_config scope subquery, got: ${sql}`,
    );
    assert.ok(sql.includes("CROSS JOIN LATERAL"), "body must be laterally joined after the scope row");
    assert.ok(
      sql.includes("WHERE __daobrew_scope.uid IS NOT NULL"),
      "lateral body must reference the scope row to pin evaluation order",
    );
    assert.ok(
      sql.includes("(SELECT id FROM graph_nodes ORDER BY id) AS __daobrew_body"),
      "original query must be embedded with its trailing semicolon stripped",
    );
  ...
  it("sets the GUC transaction-locally (is_local = true) — nothing persists on the pooled session", async () => {
    ...
    const sql = captured[0];
    assert.ok(
      sql.includes(`set_config('${GUC}', 'u1', true)`),
      `scoped query must set the GUC transaction-locally, got: ${sql}`,
    );
    assert.ok(
      !sql.includes(", false)"),
      `scoped query must not contain a session-level set_config, got: ${sql}`,
    );
  });

  it("escapes a hostile userId so it stays inert inside a quoted literal", async () => {
    const captured: string[] = [];
    const query = scopedQuery(HOSTILE_USER_ID, async (sql) => {
      captured.push(sql);
      return [];
    });
    ...
    assert.ok(
      !outsideLiterals(sql).includes(";"),
      `hostile userId must not smuggle a statement boundary outside a literal, got: ${sql}`,
    );
    assert.ok(
      !outsideLiterals(sql).includes("DROP TABLE"),
      "DROP TABLE payload must stay confined to the quoted literal",
...
// ---------------------------------------------------------------------------
// Entry-point wiring (plan Task 4): the REAL internal-server handlers must
// route job SQL through scopedExec/scopedQuery. Wave A proved scopedExec
// works in isolation; these tests prove the GUC actually flows on the real
// call path — HTTP request → handler → job → graph-db transport. The
// compiled CJS module resolves `graph_db_js_1.execSql` at call time, so
// swapping graph-db's exports for capturing fakes intercepts the genuine
// delegate chain (same seam scopedExec's default parameter reads).
// ---------------------------------------------------------------------------
...
async function withWiredServer(
  fn: (post: (path: string, body: Record<string, unknown>) => Promise<Response>, captured: CapturedSql) => Promise<void>,
): Promise<void> {
  ...
  try {
    const post = async (path: string, body: Record<string, unknown>) => {
      const response = await postInternal(path, body);
      return {
...
function assertAllScopedQueries(statements: string[], label: string): void {
  ...
  for (const sql of statements) {
    ...
    assert.ok(
      ...
      `${label}: every query must carry the lateral GUC scope for the request's userId, got: ${sql}`,
    );
    assert.strictEqual(
...
function assertAllScopedExecs(statements: string[], label: string): void {
  assert.ok(statements.length > 0, `${label}: at least one SQL statement must reach the transport`);
  for (const sql of statements) {
    ...
    assert.ok(
      sql.includes(`set_config('${GUC}', '${WIRING_USER}', true)`),
      `${label}: every exec must carry the transaction-local GUC for the request's userId, got: ${sql}`,
    );
    assert.strictEqual(
      occurrences(sql, "set_config"),
      1,
...
describe("entry-point wiring (internal-server handler path)", () => {
  it("/internal/layer2/nightly runs every statement through the request user's scope", async () => {
    await withWiredServer(async (post, captured) => {
      ...
      assert.ok(
        captured.execs.every((sql) =>
          sql.includes("pg_advisory_xact_lock(1144153669)")
          && sql.includes(`lease_token = '${leaseToken}'`)
          && sql.includes(`user_id = '${WIRING_USER}'`)
          && sql.includes("lease_expires_at_ts >")
          && sql.indexOf("causal execution lease is not live") < sql.lastIndexOf("COMMIT;")
        ),
        "every nightly write must prove the exact live lease inside its transaction",
      );
      ...
      assert.ok(
        captured.queries.some((sql) => sql.includes("user_id = __daobrew_scope.uid")),
        "nightly reads must filter user_id via the scope row",
      );
      assert.ok(
        !captured.queries.some((sql) => sql.includes(`user_id = '${WIRING_USER}'`)),
        "nightly reads must NOT compare user_id to a literal (One-Time-Filter fail-closed trap)",
      );
    });
  });

  it("/internal/layer2/nightly preserves trusted unleased maintenance but rejects malformed capabilities", async () => {
    await withWiredServer(async (post, captured) => {
      ...
      assert.ok(
        captured.execs.every((sql) => !sql.includes("daobrew_causal_lease_guard")),
        "trusted scheduled maintenance without a lease keeps its existing idempotent path",
      );
  ...
  it("/internal/layer2/snapshot runs every statement through the request user's scope", async () => {
    await withWiredServer(async (post, captured) => {
  ...
  it("/internal/transfer/priors stays UNSCOPED — cross-user aggregation by design", async () => {
    await withWiredServer(async (post, captured) => {
      ...
      assert.ok(
        captured.execs.some((sql) => /INSERT INTO pipeline_metrics/.test(sql)),
        "priors job must write its metrics row",
      );
      ...
      for (const sql of all) {
        assert.ok(
          ...
          `priors SQL must never carry the GUC prefix, got: ${sql}`,
        );
      }
    });
  });
...
function adminPsql(sql: string): void {
  execFileSync("docker", [
    "compose",
    "-p",
    process.env.DAOBREW_POSTGRES_COMPOSE_PROJECT || "daobrewai",
...
describe("user-scope live transport (postgres)", () => {
  it("GUC scope survives queryJson's JSON extraction and scopedExec's transaction wrapper", async (t) => {
    ...
    try {
      ...
      const scoped = await scopedQuery("u1")(
        `SELECT current_setting('${GUC}', true) AS uid`,
      );
      assert.deepStrictEqual(scoped, [{ uid: "u1" }]);

      // 2. Request scoping: a fresh unscoped session sees no GUC (NULL).
      const unscoped = await queryJson(`SELECT current_setting('${GUC}', true) AS uid`);
      ...
      await execSql("CREATE TABLE user_scope_probe (uid TEXT);");
      await scopedExec("u2")(
        `INSERT INTO user_scope_probe (uid) VALUES (current_setting('${GUC}', true));`,
      );
      const probe = await queryJson("SELECT uid FROM user_scope_probe");
...
describe("scopedUserIdExpr (store-aware user_id comparison)", () => {
  ...
  it("returns the lateral scope-row reference under postgres", () => {
    withGraphStore("postgres", () => {
      assert.strictEqual(scopedUserIdExpr("u1"), "__daobrew_scope.uid");
      ...
      assert.ok(!scopedUserIdExpr("u1").includes("'u1'"));
    });
  });

  it("returns the q()-escaped literal under sqlite (no scope row exists there)", () => {
    withGraphStore("sqlite", () => {
      assert.strictEqual(scopedUserIdExpr("u1"), "'u1'");
      assert.strictEqual(scopedUserIdExpr(HOSTILE_USER_ID), HOSTILE_ESCAPED_LITERAL);
    });
  });
});
