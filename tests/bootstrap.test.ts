...
import { runBootstrap, BOOTSTRAP_DEFAULT_DAYS } from "../src/engine/bootstrap.js";
import { IngestSink } from "../src/engine/ingest/types.js";
import { ensureSchema } from "../src/engine/schema.js";
import { execSql, q } from "../src/graph-db.js";
...
function fakeSink(calls: string[], counts = { events: 1, notes: 1, insights: 2 }): IngestSink {
  return {
...
function fakeQuery(calls: string[], verified = { insights: 2, notes: 1, events: 1 }) {
  return async (sql: string): Promise<Record<string, any>[]> => {
    calls.push("query");
    if (/FROM user_insights/.test(sql) && /count\(\*\)/.test(sql)) return [{ n: verified.insights }];
    if (/FROM meeting_notes/.test(sql) && /count\(\*\)/.test(sql)) return [{ n: verified.notes }];
    if (/FROM events/.test(sql) && /count\(\*\)/.test(sql)) return [{ n: verified.events }];
    return [{ bytes: 1, calls: 0, count: 0 }];
  };
}

/** Stubs for the phases wired in Task 4 — inert here, asserted there. */
function inertLatePhases(calls: string[]) {
  return {
    embedSweep: (async () => { calls.push("embed-sweep"); return { rowsEmbedded: 0, geminiCallsUsed: 0, warnings: [], alerts: [], tables: {} }; }) as any,
    readBiometrics: (async () => { calls.push("biometrics"); return { signals: { metrics: [], states: [] }, source: "none" }; }) as any,
    engine: (async () => { calls.push("engine"); return {
    ...
    nightly: (async () => { calls.push("layer2:nightly"); return { threadsUpserted: 0, evidenceInserted: 0, verdictsSettled: 0, threadsArchived: 0, transferRecordsEmitted: 0, profileVectorsWritten: 0, warnings: [], alerts: [] }; }) as any,
    snapshot: (async () => { calls.push("layer2:snapshot"); return { written: false, warnings: [], alerts: [] }; }) as any,
  };
}

describe("cold-start bootstrap — ingest phase", () => {
  it("runs memory, granola (rawNotes), calendar (rawEvents) through the sink and verifies per wizard criteria", async () => {
    const calls: string[] = [];
    ...
    assert.strictEqual(report.window.start_ts, 1_800_000_000 - BOOTSTRAP_DEFAULT_DAYS * 86400);
  ...
  it("skips calendar without rawEvents and granola without token/rawNotes, but still verifies pre-existing rows", async () => {
    ...
    try {
      ...
      if (savedToken !== undefined) process.env.GRANOLA_API_TOKEN = savedToken;
    }
  });

  it("a throwing ingest job records error on that source and does not abort the bootstrap", async () => {
    const calls: string[] = [];
...
describe("cold-start bootstrap — engine, coverage, layer 2", () => {
  function baseDeps(calls: string[]) {
    return {
      userId: "u1",
      nowTs: () => 1_800_000_000,
      sink: fakeSink(calls),
  ...
  it("runs embed sweep, coverage read, engine replay over the window, then nightly + snapshot — in order", async () => {
    const calls: string[] = [];
    const engineArgs: any[] = [];
    // 2 worn days (>=3 HR samples each) + 1 sparse day
    const hr = (date: string, n: number) => Array.from({ length: n }, (_, i) => ({
      metric: "heart_rate", value: 60 + i, timestamp: `${date}T${String(9 + i).padStart(2, "0")}:00:00-07:00`,
      graph_source_ref: `hk:${date}:${i}`,
    }));
    const report = await runBootstrap({
      ...baseDeps(calls),
      embedSweep: (async () => { calls.push("embed-sweep"); return { rowsEmbedded: 4, geminiCallsUsed: 1, warnings: [], alerts: [], tables: {} }; }) as any,
      readBiometrics: (async (options: any) => {
      ...
      snapshot: (async () => { calls.push("layer2:snapshot"); return { written: true, version: 1, warnings: [], alerts: [] }; }) as any,
    });
  ...
  it("an engine failure is reported but nightly + snapshot still fire (day-1 snapshot guarantee)", async () => {
    const calls: string[] = [];
    const report = await runBootstrap({
      ...baseDeps(calls),
      ...inertLatePhases(calls),
      engine: (async () => { throw new Error("reasoner exploded"); }) as any,
    });
  ...
  it("zero worn days reports biometrics not ok without failing the bootstrap", async () => {
    const calls: string[] = [];
    const report = await runBootstrap({ ...baseDeps(calls), ...inertLatePhases(calls) });
...
describe("bootstrap entry points", () => {
  it("POST /internal/bootstrap is gated by DAOBREW_INTERNAL_TOKEN", async () => {
    const savedToken = process.env.DAOBREW_INTERNAL_TOKEN;
    process.env.DAOBREW_INTERNAL_TOKEN = "secret-1";
    try {
      const response = await postInternal("/internal/bootstrap", "{}");
  ...
  it("POST /internal/bootstrap returns 400 (not 500) on a non-postgres store", async () => {
    const savedToken = process.env.DAOBREW_INTERNAL_TOKEN;
    ...
    try {
      const response = await postInternal("/internal/bootstrap", { userId: BOOTSTRAP_TEST_USER });
  ...
  it("--run bootstrap exits 1 with a clear error on a non-postgres store", () => {
    const env = { ...process.env };
    delete env.DAOBREW_GRAPH_STORE;
    delete env.DAOBREW_POSTGRES_URL;
    env.DAOBREW_INTERNAL_USER = BOOTSTRAP_TEST_USER;
    try {
      execFileSync(process.execPath, ["dist/src/engine/internal-server.js", "--run", "bootstrap"], {
...
describe("cold-start bootstrap — postgres end-to-end", () => {
  it("bootstraps a fresh user: ingest -> verify -> engine -> layer 2, one call", async (t) => {
    ...
    const root = mkdtempSync(join(tmpdir(), "daobrew-bootstrap-e2e-"));
    ...
    writeFileSync(join(claudeProj, "s1.jsonl"),
      JSON.stringify({ cwd: "/Users/alice/Work/alpha", type: "user", timestamp: new Date((nowSec - 86400) * 1000).toISOString(), message: { content: "Fix the login flow" } }) + "\n");
    ...
    try {
      ...
      const report = await runBootstrap({
        ...
        embedSweep: (async () => ({ rowsEmbedded: 0, geminiCallsUsed: 0, warnings: [], alerts: [], tables: {} })) as any,
      });
...
describe("bootstrap RLS scoping (F1 One-Time-Filter regression)", () => {
  it("verification counts filter user_id by the scope row, never by a literal", async () => {
    ...
    await runBootstrap({
      ...
      nowTs: () => 1_800_000_000,
      sink: fakeSink(calls),
      query: async (sql: string) => { reads.push(sql); return inner(sql); },
      memoryRows: [{
        id: "m1", source: "claude_project_session", source_ref: "s:1",
        insight_text: "Reviewable evidence trail.", topics: ["#memory"],
        importance: 0.8, strength: 1, occurred_at_ts: null, last_accessed_ts: null,
    ...
    for (const sql of verifyReads) {
      assert.ok(
        ...
        `verification read must filter user_id via the scope row, got: ${sql}`,
      );
      assert.ok(
        !sql.includes("user_id = 'u1'"),
        `verification read must NOT compare user_id to a literal (One-Time-Filter fail-closed trap), got: ${sql}`,
      );
    }
  });
});
