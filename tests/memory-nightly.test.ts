...
import { runNightlyThreadMaintenance } from "../src/engine/memory/nightly.js";
import { contributorHash, threadId, evidenceId, transferRecordId, verificationId } from "../src/engine/memory/keys.js";

const NOW = 1_800_000_000;
const USER = "local";
const GHOST_ID = "ghost_root_local";
const TID = threadId(USER, `root:${GHOST_ID}`);
...
async function runWith(
  fixtures: Fixture[],
  extraDeps: Partial<Parameters<typeof runNightlyThreadMaintenance>[0]> = {},
) {
  const calls: string[] = [];
...
describe("layer 2 nightly thread maintenance", () => {
  it("upserts a thread from a ghost node with mapped claim, copied recurrence, and pattern keys", async () => {
  ...
  it("inserts deterministic evidence ids with a monotone NULL-fill upsert", async () => {
  ...
  it("is idempotent: re-running over identical rows emits byte-identical write SQL", async () => {
    const first = await runWith(defaultFixtures());
    const second = await runWith(defaultFixtures());
    const writes = (calls: string[]) => calls.filter((sql) => (
      /INSERT INTO causal_memory_threads|INSERT INTO causal_thread_evidence|UPDATE causal_memory_threads/.test(sql)
    ));
  ...
  it("adds a weighted intervention_outcome evidence row for detonated (done) ghosts", async () => {
    const doneGhost = {
      id: "ghost_done",
  ...
  it("classifies plaud: and gdocs: refs as meeting-note evidence exactly like granola:", async () => {
    ...
    assert.ok(lookup.includes("'filep1'") && lookup.includes("'doc1'") && lookup.includes("'note1'"),
      `meeting_notes lookup must cover plaud/gdocs/granola bare refs, got: ${lookup}`);
    const evidenceSql = calls.find((sql) => /INSERT INTO causal_thread_evidence/.test(sql)) ?? "";
  ...
  it("skips unknown ref prefixes and reports them as unresolved_refs", async () => {
    const weirdGhost = {
      id: "ghost_weird",
      title: "Odd refs",
      subtitle: null,
  ...
  it("records a job_failed metrics row, posts the alert webhook, and rethrows on failure", async () => {
    const calls: string[] = [];
    const webhookCalls: string[] = [];
    await assert.rejects(() => runNightlyThreadMaintenance({
      userId: USER,
      exec: async (sql) => {
        if (/INSERT INTO causal_memory_threads/.test(sql)) throw new Error("neon down");
        calls.push(sql);
      },
  ...
  it("never writes transcript bodies into layer 2 SQL", async () => {
    const { calls } = await runWith(defaultFixtures());
...
const DONE_TID = threadId(USER, `root:${DONE_GHOST_ID}`);
const VERDICT_ID = verificationId({ threadId: DONE_TID, kind: "verdict", bucket: `vd:${HANDLED}` });

function doneGhostRow(props: Record<string, any> = {}) {
  return {
    id: DONE_GHOST_ID,
...
function settleFixtures(observationRows: Record<string, any>[], extra: Fixture[] = []): Fixture[] {
  return [
    ...extra,
...
function verdictInserts(calls: string[]): string[] {
...
function deterministicWrites(calls: string[]): string[] {
  return calls.filter((sql) => !/INSERT INTO pipeline_metrics/.test(sql));
}

describe("V5 settlement pass (verdicts)", () => {
  it("thread upsert never demotes claim_level on conflict (base claim written on INSERT only)", async () => {
    const { calls } = await runWith(defaultFixtures());
    const threadSql = calls.find((sql) => /INSERT INTO causal_memory_threads/.test(sql)) ?? "";
  ...
  it("writes pattern_recurred when a same_thread_trigger observation lands in the verify window", async () => {
  ...
  it("writes no_recurrence_observed on >=3 in-window engine_ran_no_trigger and no triggers", async () => {
    const { calls } = await runWith(settleFixtures([
  ...
  it("writes insufficient_observation when coverage is too thin to clear the thread", async () => {
    const { calls } = await runWith(settleFixtures([
      { observation: "engine_ran_no_trigger", observed_at_ts: HANDLED + 1 * 86_400 },
    ]));
    const verdicts = verdictInserts(calls);
  ...
  it("does not settle while the verify window is still open", async () => {
    const fixtures: Fixture[] = [
  ...
  it("falls back to the e_done_ edge timestamp when done_at_ts is absent", async () => {
    const ghost = doneGhostRow();
    delete (ghost.props_json as any).done_at_ts;
  ...
  it("settlement re-runs are byte-identical (one verdict per handled cycle)", async () => {
    const fixtures = (verdictExists: boolean) => settleFixtures(
      [{ observation: "engine_ran_no_trigger", observed_at_ts: HANDLED + 86_400 }],
...
describe("V5 claim pass (promotion/demotion)", () => {
  const claimRowFixture = (row: Record<string, any>): Fixture => (
    [/SELECT id, graph_root_id, claim_level, promotion_state_json FROM causal_memory_threads WHERE/, [row]]
  );
  const verdictRowsFixture = (rows: Record<string, any>[]): Fixture => (
    [/SELECT thread_id, verdict, details_json ->> 'arm' AS arm FROM thread_verifications WHERE/, rows]
  );
  const claimUpdates = (calls: string[]) => calls.filter((sql) => /UPDATE causal_memory_threads SET claim_level =/.test(sql));

  it("promotes attribution_candidate to causal_hypothesis on 2 confirmations", async () => {
    const fixtures: Fixture[] = [
      ...defaultFixtures(),
      verdictRowsFixture([
    ...
    assert.deepEqual(
      { from: state.history[0].from, to: state.history[0].to },
      { from: "attribution_candidate", to: "causal_hypothesis" },
    );
  });

  it("demotes on >=2 contradictions with a history entry appended", async () => {
    const fixtures: Fixture[] = [
      ...defaultFixtures(),
      verdictRowsFixture([
    ...
    assert.deepEqual(
      { from: state.history[1].from, to: state.history[1].to },
      { from: "causal_hypothesis", to: "attribution_candidate" },
    );
  });

  it("excludes control-arm verdicts from promotion counts (promotion stays treatment-evidence-based)", async () => {
    const fixtures: Fixture[] = [
      ...defaultFixtures(),
      verdictRowsFixture([
  ...
  it("control contradictions never demote either: mixed arms count only the treatment rows", async () => {
    const fixtures: Fixture[] = [
      ...defaultFixtures(),
      verdictRowsFixture([
  ...
  it("no level change appends no history and re-runs emit identical statements", async () => {
    const fixtures = (): Fixture[] => [
      ...defaultFixtures(),
...
const HELD_TID = threadId(USER, `root:${HELD_GHOST_ID}`);
const HELD_VERDICT_ID = verificationId({ threadId: HELD_TID, kind: "verdict", bucket: `vd:${HANDLED}` });

function heldGhostRow(props: Record<string, any> = {}) {
  return {
    id: HELD_GHOST_ID,
...
function heldFixtures(observationRows: Record<string, any>[], ghost: Record<string, any> = heldGhostRow()): Fixture[] {
  return [
    [/FROM graph_nodes WHERE user_id = __daobrew_scope\.uid AND kind = 'ghost'/, [ghost]],
    [/SELECT observation, observed_at_ts FROM thread_verifications WHERE/, observationRows],
    [/SELECT id, claim_level FROM causal_memory_threads WHERE/, []],
...
describe("MRT v1 held-episode settlement (control arm)", () => {
  it("settles a held episode past the horizon with the SAME verdict formula and arm:'control' details", async () => {
  ...
  it("quiet held windows settle no_recurrence_observed on >=3 in-window quiet runs", async () => {
  ...
  it("does not settle a held episode while its window is still open", async () => {
    const { calls } = await runWith(heldFixtures(
      [],
      heldGhostRow({ armed_at_ts: NOW - 5 * 86_400 }),
    ));
    assert.equal(verdictInserts(calls).length, 0);
  });

  it("legacy stampless held roots anchor on created_at_ts (armedEpisodeTs convention)", async () => {
    const ghost = heldGhostRow();
    delete (ghost.props_json as any).armed_at_ts;
    const { calls } = await runWith(heldFixtures(
      [{ observation: "engine_ran_no_trigger", observed_at_ts: HANDLED } ],
      ghost,
    ));
    const verdicts = verdictInserts(calls);
    assert.equal(verdicts.length, 1);
    const legacyId = verificationId({ threadId: HELD_TID, kind: "verdict", bucket: `vd:${HANDLED - 86_400}` });
    assert.ok(verdicts[0].includes(`'${legacyId}'`), "bucket keys on created_at_ts when armed_at_ts is absent");
  });

  it("held episodes never emit transfer records or intervention_outcome evidence (no intervention happened)", async () => {
    const { calls, result } = await runWith(
      ...
      ).concat([[/FROM transfer_consent WHERE/, [{ opted_in: true }]]]),
  ...
  it("held settlement re-runs are byte-identical", async () => {
...
describe("MRT v1 comparison metric (per-arm settled verdict counts)", () => {
  ...
  function metricInserts(calls: string[]): string[] {
    return calls.filter((sql) => /INSERT INTO pipeline_metrics/.test(sql) && /mrt_comparison/.test(sql));
  }

  it("writes one pipeline_metrics row summarizing settled verdicts by arm x verdict", async () => {
    const { calls, result } = await runWith([
      ...defaultFixtures(),
  ...
  it("writes the row with explicit zeros when no verdicts have settled yet", async () => {
    const { calls, result } = await runWith(defaultFixtures());
    const inserts = metricInserts(calls);
    assert.equal(inserts.length, 1);
...
describe("Q3 dormant-thread archival", () => {
  const GC_CUTOFF = NOW - 90 * 86_400;
  const gcFixture = (rows: Record<string, any>[]): Fixture => (
    [/SELECT id, graph_root_id FROM causal_memory_threads WHERE/, rows]
  );
  const archiveUpdates = (calls: string[]) => calls.filter((sql) => /UPDATE causal_memory_threads SET status = 'archived'/.test(sql));

  it("selects candidates with the full dormancy predicate (active/dormant/zero-strength/aged/no fresh evidence)", async () => {
  ...
  it("archives a rootless dormant victim (never DELETE) and reports it", async () => {
    const { calls, result } = await runWith([
      ...defaultFixtures(),
  ...
  it("archives a victim whose graph root vanished from graph_nodes", async () => {
    const { calls, result } = await runWith([
      ...defaultFixtures(),
  ...
  it("does NOT archive a dormant thread whose root is still live in the graph", async () => {
    const { calls, result } = await runWith([
      ...defaultFixtures(),
  ...
  it("emits no archive UPDATE when there are no candidates", async () => {
    const { calls, result } = await runWith(defaultFixtures());
    assert.equal(archiveUpdates(calls).length, 0);
    assert.equal(result.threadsArchived, 0);
  });
...
const SALT = "salt-a";
const CONTRIB = contributorHash(USER, SALT);
const TRANSFER_ID = transferRecordId({
...
function fakeTransferProvider(): { embed(texts: string[]): Promise<number[][]> } {
  return { embed: async (texts: string[]) => texts.map(() => new Array(768).fill(0.5)) };
}

function transferFixtures(
  observationRows: Record<string, any>[],
  opts: { consent?: boolean; verdictRows?: Record<string, any>[] } = {},
): Fixture[] {
  return settleFixtures(observationRows, [
    ...
    pattern.source.includes("kind = 'ghost'")
      ? [pattern, [doneGhostRow(transferGhostProps)]] as Fixture
      : [pattern, rows] as Fixture);
}

function transferInserts(calls: string[]): string[] {
...
describe("T5 transfer-record emission (5B)", () => {
  it("emits one anonymized record when a verdict settles for an opted-in user", async () => {
  ...
  it("never leaks identity, artifact paths, or brief prose into the record SQL", async () => {
  ...
  it("maps pattern_recurred to did_not_work and computes outcome_strength from verdict counts", async () => {
    const { calls } = await runWith(
      transferFixtures(
        [{ observation: "same_thread_trigger", observed_at_ts: HANDLED + 3 * 86_400 }],
        { verdictRows: [
  ...
  it("does not emit on insufficient_observation", async () => {
    const { calls, result } = await runWith(
      transferFixtures([{ observation: "engine_ran_no_trigger", observed_at_ts: HANDLED + 86_400 }]),
      { transferSalt: SALT, transferProvider: fakeTransferProvider() },
    );
  ...
  it("skips silently without consent and warns-without-emitting when the salt is unset", async () => {
  ...
  it("writes a NULL embedding with a warning when no provider is available", async () => {
    const { calls, result } = await runWith(transferFixtures(QUIET_RUNS), {
      transferSalt: SALT,
  ...
  it("carries provenance columns and coarsens created_at_ts to the week bucket (5B §6.3)", async () => {
    const { calls } = await runWith(transferFixtures(QUIET_RUNS), {
      transferSalt: SALT,
      transferProvider: fakeTransferProvider(),
    });
  ...
  it("re-runs are idempotent: same fixtures, byte-identical transfer INSERT", async () => {
...
function profileFixtures(userVectorRows: Record<string, any>[] = []): Fixture[] {
  return [
...
function profileInserts(calls: string[]): string[] {
  return calls.filter((sql) => /INSERT INTO user_vectors/.test(sql));
}

describe("T6 nightly user-profile vector (5B)", () => {
  it("writes one versioned profile vector from thread vocabulary, never snapshot prose", async () => {
    ...
    assert.ok(inserts[0].includes(
      "profile patterns: fire wood | threads: root:ghost_a root:ghost_b | ceiling: attribution_candidate",
    ));
    assert.match(inserts[0], /::halfvec/);
  ...
  it("bumps the version when a prior vector exists on an earlier day", async () => {
    const { calls } = await runWith(
      profileFixtures([{ version: 3, created_at_ts: NOW - 2 * 86_400 }]),
      { transferSalt: SALT, transferProvider: fakeTransferProvider() },
    );
    const inserts = profileInserts(calls);
    assert.equal(inserts.length, 1);
  ...
  it("is idempotent within a local day: same-day prior vector means no write", async () => {
    const { calls, result } = await runWith(
      profileFixtures([{ version: 3, created_at_ts: NOW - 60 }]),
      { transferSalt: SALT, transferProvider: fakeTransferProvider() },
    );
  ...
  it("skips with a warning when threads exist but no provider is available", async () => {
    const { calls, result } = await runWith(profileFixtures(), {
      transferSalt: SALT,
      transferProvider: null,
    });
  ...
  it("skips silently when the user has no active threads", async () => {
    const fixtures = profileFixtures().map(([pattern, rows]) =>
      pattern.source.includes("thread_key, pattern_keys_json")
        ? [pattern, []] as Fixture
        : [pattern, rows] as Fixture);
    const { calls, result } = await runWith(fixtures, {
      transferSalt: SALT,
...
describe("nightly RLS scoping (F1 One-Time-Filter regression)", () => {
  it("every nightly read filters user_id by the scope row, never by a literal", async () => {
...
describe("nightly accepts an arbitrary identity (bare device UUID)", () => {
  it("runs the full job for a bare device-UUID userId, writing it verbatim", async () => {
    ...
    assert.ok(
      threadInsert.includes(`'${deviceUuid}'`),
      `thread INSERT must write the bare device UUID as user_id, got: ${threadInsert}`,
    );
    ...
    for (const sql of userScopedReads) {
      ...
      assert.ok(
        !sql.includes(`user_id = '${deviceUuid}'`),
        `read must NOT compare user_id to the bare-UUID literal, got: ${sql}`,
      );
    }
  });
});
