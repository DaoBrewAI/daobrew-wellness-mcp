...
function stubFetch(calls: RecordedCall[], values: (callIndex: number) => number[]): typeof fetch {
...
describe("GeminiEmbeddingProvider (gemini-embedding-001 embedContent)", () => {
  it("defaults to gemini-embedding-001 and posts to :embedContent", async () => {
  ...
  it("respects DAOBREW_EMBED_MODEL when options.model is not given", async (t) => {
    const saved = process.env.DAOBREW_EMBED_MODEL;
    process.env.DAOBREW_EMBED_MODEL = "custom-model";
    t.after(() => {
      if (saved !== undefined) process.env.DAOBREW_EMBED_MODEL = saved;
      else delete process.env.DAOBREW_EMBED_MODEL;
    });
    const calls: RecordedCall[] = [];
    const provider = new GeminiEmbeddingProvider({
      apiKey: "test-key",
  ...
  it("falls back to the default model when DAOBREW_EMBED_MODEL is empty or whitespace", async (t) => {
    const saved = process.env.DAOBREW_EMBED_MODEL;
  ...
  it("options.model wins over DAOBREW_EMBED_MODEL", async (t) => {
    const saved = process.env.DAOBREW_EMBED_MODEL;
  ...
  it("sends outputDimensionality 768 and the text in content.parts", async () => {
    const calls: RecordedCall[] = [];
    const provider = new GeminiEmbeddingProvider({
      apiKey: "test-key",
      fetchImpl: stubFetch(calls, () => new Array(768).fill(0.5)),
  ...
  it("L2-normalizes non-normalized responses to unit norm", async () => {
    const calls: RecordedCall[] = [];
    const provider = new GeminiEmbeddingProvider({
      apiKey: "test-key",
  ...
  it("makes one fetch call per text and preserves input order", async () => {
    const calls: RecordedCall[] = [];
  ...
  it("rejects the whole embed() on a mid-sequence failure with no partial result", async () => {
    const calls: RecordedCall[] = [];
    ...
    const provider = new GeminiEmbeddingProvider({ apiKey: "test-key", fetchImpl });
    ...
    await assert.rejects(
      () => provider.embed(["first", "second", "third"]),
      /Gemini embedding request failed \(500\): boom/,
    );
    // Failed on text 2 of 3: the third request must never have been made.
    assert.equal(calls.length, 2);
  });

  it("throws when the response is not 768-dimensional", async () => {
    const calls: RecordedCall[] = [];
    const provider = new GeminiEmbeddingProvider({
      apiKey: "test-key",
  ...
  it("throws fail-closed on a zero-norm vector", async () => {
    const calls: RecordedCall[] = [];
    const provider = new GeminiEmbeddingProvider({
      apiKey: "test-key",
  ...
  it("keeps the error style on http failures", async () => {
    const provider = new GeminiEmbeddingProvider({
      apiKey: "test-key",
      fetchImpl: (async () => ({
        ok: false,
