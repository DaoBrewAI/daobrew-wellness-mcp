...
import { RLS_GUC, appRoleProvisionSql } from "../src/engine/postgres-rls.js";
import { scopedQuery } from "../src/engine/user-scope.js";
...
function composePsqlArgs(user: string, db: string, extraPsqlArgs: string[] = []): string[] {
  return [
    "compose",
    "-p",
    COMPOSE_PROJECT,
 ...
 */
function psqlAs(user: string, db: string, sql: string, extraPsqlArgs: string[] = []): string {
  return execFileSync("docker", composePsqlArgs(user, db, extraPsqlArgs), {
    cwd: process.cwd(),
    encoding: "utf-8",
    input: sql,
    timeout: 30_000,
...
function psqlAsExpectFailure(user: string, db: string, sql: string): string {
  try {
    psqlAs(user, db, sql);
  } catch (err: any) {
...
function postgresRuntimePresent(): boolean {
  try {
    execFileSync("docker", composePsqlArgs(OWNER_USER, ADMIN_DB, ["-c", "SELECT 1;"]), {
      cwd: process.cwd(),
      stdio: "ignore",
    });
    return true;
...
function alembicUpgradeHead(db: string): void {
...
describe("RLS live isolation matrix (postgres)", () => {
  it("enforces deny-by-default per-tenant isolation for the app role", async (t) => {
    ...
    const setGuc = (userId: string) => `SELECT set_config('${RLS_GUC}', '${userId}', false);`;
    const previousEnv = {
    ...
    try {
      ...
      psqlAs(OWNER_USER, scratchDb, appRoleProvisionSql(appRole), [
        "-v",
      ...
      assert.deepStrictEqual(
        lines(psqlAs(appRole, scratchDb, "SELECT current_user, (SELECT rolbypassrls FROM pg_roles WHERE rolname = current_user);")),
        [`${appRole}|f`],
        "matrix must run as the non-BYPASSRLS scratch role",
      );

      // --- 1. No GUC → deny-by-default: zero rows, no writes. --------------
      await t.test("1: unscoped app-role session sees nothing and writes nothing", () => {
        assert.deepStrictEqual(
          lines(psqlAs(appRole, scratchDb, "SELECT count(*) FROM graph_nodes;")),
          ["0"],
          "unscoped session must see zero graph_nodes",
        );
        assert.deepStrictEqual(
          lines(psqlAs(appRole, scratchDb, "SELECT count(*) FROM causal_memory_threads;")),
          ["0"],
          "unscoped session must see zero causal_memory_threads",
        );
        const err = psqlAsExpectFailure(
          appRole,
          scratchDb,
      ...
      await t.test("2: tenant_a-scoped session is isolated from tenant_b", () => {
        ...
        assert.deepStrictEqual(
          counts,
          ["tenant_a", "2", "1", "0", "node_a1", "node_a2"],
          "tenant_a session must see exactly the tenant_a fixtures and no tenant_b rows",
        );

        // Cross-tenant UPDATE: policy filters tenant_b rows out of the target
        // set entirely, so the statement reports UPDATE 0.
        const updateOut = psqlAs(
          appRole,
          scratchDb,
          `${setGuc("tenant_a")}
        ...
        assert.match(
          err,
          /new row violates row-level security policy/,
          `cross-tenant INSERT must be rejected by WITH CHECK, got: ${err}`,
        );
        ...
        assert.deepStrictEqual(
          lines(psqlAs(appRole, scratchDb, `${setGuc("tenant_a")} SELECT count(*) FROM graph_nodes;`)),
          ["tenant_a", "3"],
          "properly scoped INSERT must land (grants intact, isolation is policy-driven)",
        );
      });
      ...
      await t.test("3: transfer_records annex is readable/writable without a GUC", () => {
        assert.deepStrictEqual(
          lines(psqlAs(appRole, scratchDb, "SELECT count(*) FROM transfer_records;")),
          ["1"],
          "annex must be readable with no GUC set",
        );
        psqlAs(
          appRole,
          scratchDb,
        ...
        assert.deepStrictEqual(
          lines(psqlAs(appRole, scratchDb, "SELECT count(*) FROM transfer_records;")),
          ["2"],
          "annex must accept writes with no GUC set",
        );
      });
      ...
      await t.test("4: population_priors are readable without a GUC", () => {
        assert.deepStrictEqual(
          lines(psqlAs(appRole, scratchDb, "SELECT count(*) FROM population_priors;")),
          ["1"],
          "global priors must be readable with no GUC set",
        );
      });

      // --- 5. Owner bypass intact (ENABLE, not FORCE). ----------------------
      await t.test("5: owner connection still sees every tenant's rows", async () => {
        const ownerCounts = await queryJson(
          `SELECT
        ...
        assert.deepStrictEqual(
          ownerCounts,
          [{ total: 4, a: 3, b: 1 }],
          "owner must bypass RLS and see all rows of both tenants",
        );
        const untouched = await queryJson(
          "SELECT title FROM graph_nodes WHERE id = 'node_b1'",
        );
        assert.deepStrictEqual(
          untouched,
          [{ title: "tenant_b node one" }],
          "tenant_b's row must be untouched by the blocked cross-tenant UPDATE",
        );
      });
      ...
      await t.test("6: scopedQuery returns exactly the scoped tenant's rows with policies active", async () => {
        ...
        try {
          const who = await queryJson("SELECT current_user AS cu");
          assert.deepStrictEqual(who, [{ cu: appRole }], "transport must run as the scratch app role");

          const unscoped = await queryJson("SELECT id FROM graph_nodes ORDER BY id");
          assert.deepStrictEqual(unscoped, [], "unscoped queryJson as app role must see zero rows");

          const tenantA = await scopedQuery("tenant_a")(
            "SELECT id, user_id FROM graph_nodes ORDER BY id",
          );
          assert.deepStrictEqual(
            tenantA,
            [
          ...
          const tenantB = await scopedQuery("tenant_b")(
            "SELECT id, user_id FROM graph_nodes ORDER BY id",
          );
          assert.deepStrictEqual(
            tenantB,
            [{ id: "node_b1", user_id: "tenant_b" }],
            "scopedQuery('tenant_b') must return exactly tenant_b's rows",
          );
        } finally {
          if (previousEnv.DAOBREW_POSTGRES_USER === undefined) delete process.env.DAOBREW_POSTGRES_USER;
          else process.env.DAOBREW_POSTGRES_USER = previousEnv.DAOBREW_POSTGRES_USER;
        }
      });
    } finally {
