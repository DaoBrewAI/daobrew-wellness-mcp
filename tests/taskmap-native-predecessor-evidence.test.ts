...
import {
  assertTaskMapNativePredecessorEvidence,
  ...
  type BuildTaskMapNativePredecessorEvidenceInputV1,
  type TaskMapNativePredecessorCurrentnessV1,
  type TaskMapNativePredecessorEvidenceV1,
  type TaskMapNativePredecessorReplayV1,
...
function taskMapInput(
    ...
    pointers: [
      {
        ...
        canonicalUrl:
          "https://github.com/DaoBrewAI/DaoBrewStrategy/blob/09262e4637bd6d0e59a2fd3dba5fc7d5fc501c57/tasks/TASKS.md",
  ...
  if (!includeBodyContext) return base;
  base.pointers.push(
    {
      id: "meeting-work",
...
function causalInput(): RootCausalGateInput {
  return {
    rootProposalId: "root-task-map",
    bodyAxis: "composite_recovery",
    bodyCategory: "below_baseline",
...
): TaskMapNativePredecessorCurrentnessV1 {
  return {
...
function fixture(input: {
...
} = {}): BuildTaskMapNativePredecessorEvidenceInputV1 {
  const rawInput = taskMapInput(
    input.generatedAt,
    input.includeBodyContext ?? false,
  );
  ...
  const replay: TaskMapNativePredecessorReplayV1 = {
    previousProjection,
    causalInputs: input.causalInputs ?? [],
  };
  const projection = buildTaskMapProjection(rawInput, brain, {
    arm: "E4",
...
function createFixedHome(
  ...
  const home = realpathSync(mkdtempSync(
    path.join(tmpdir(), "taskmap-predecessor-home-"),
  ));
  tempHomes.push(home);
  const evidencePath = taskMapNativePredecessorEvidencePath(home);
  const directory = path.dirname(evidencePath);
  mkdirSync(directory, { recursive: true, mode: 0o700 });
  chmodSync(directory, 0o700);
  const projectionPath = path.join(directory, "taskmap-projection.v1.json");
  const currentnessPath = path.join(directory, "taskmap-currentness.v1.json");
  writeFileSync(projectionPath, input.projectionFileBytes, { mode: 0o600 });
  writeFileSync(currentnessPath, input.currentnessFileBytes, { mode: 0o600 });
  return {
    home,
    directory,
    evidencePath,
...
function context(input: BuildTaskMapNativePredecessorEvidenceInputV1) {
  return {
...
describe("Task Map native predecessor evidence", () => {
  it("writes and loads one fixed canonical 0600 replay bound to exact files", async () => {
    const input = fixture();
    const evidence = buildTaskMapNativePredecessorEvidence(input);
    const fixed = createFixedHome(input);
    const written = await writeTaskMapNativePredecessorEvidence({
      homeDirectory: fixed.home,
      evidence,
    });
    const loaded = await loadTaskMapNativePredecessorEvidence({
      homeDirectory: fixed.home,
    });
    ...
    assert.equal(statSync(fixed.directory).mode & 0o777, 0o700);
    assert.equal(statSync(fixed.evidencePath).mode & 0o777, 0o600);
    assert.equal(
      readFileSync(fixed.evidencePath, "utf8"),
      taskMapContractCanonicalJson(evidence),
    );
    assert.deepEqual(loaded, written);
    assert.deepEqual(loaded.taskMapInput, input.taskMapInput);
    assert.deepEqual(
      loaded.semanticBrainOutput,
      input.semanticBrainOutput,
    );
    assert.equal(
      loaded.binding.projectionFileDigest,
      digest(input.projectionFileBytes.toString("utf8")),
    );
    assert.equal(Object.isFrozen(loaded), true);
    assert.equal(Object.isFrozen(loaded.taskMapInput), true);
  });

  it("detects exact projection/currentness file digest drift", async () => {
    const input = fixture();
    const evidence = buildTaskMapNativePredecessorEvidence(input);
    const fixed = createFixedHome(input);
    ...
    writeFileSync(
      ...
      Buffer.concat([input.currentnessFileBytes, Buffer.from(" ")]),
      { mode: 0o600 },
    );
    ...
    writeFileSync(
      ...
      Buffer.concat([input.projectionFileBytes, Buffer.from(" ")]),
      { mode: 0o600 },
    );
    await assert.rejects(
      loadTaskMapNativePredecessorEvidence({
  ...
  it("rejects a tampered evidence payload before trusting embedded input", () => {
    const input = fixture();
    const evidence = structuredClone(
      buildTaskMapNativePredecessorEvidence(input),
    );
  ...
  it("rejects an accepted-looking projection that is not the harness replay", () => {
    const input = fixture();
  ...
  it("rejects currentness with drifted binding or incomplete classification", () => {
    const input = fixture();
    assert.throws(
      () => buildTaskMapNativePredecessorEvidence({
        ...input,
  ...
  it("rejects brain and input substitutions independently", () => {
    const input = fixture();
  ...
  it("requires the exact previous projection and causal replay dependencies", () => {
    ...
    assert.doesNotThrow(
      () => buildTaskMapNativePredecessorEvidence(withPredecessor),
    );
    assert.throws(
      () => buildTaskMapNativePredecessorEvidence({
        ...withPredecessor,
    ...
    assert.doesNotThrow(
      () => buildTaskMapNativePredecessorEvidence(bodyReplay),
    );
    assert.throws(
      () => buildTaskMapNativePredecessorEvidence({
        ...bodyReplay,
  ...
  it("rejects unknown schema fields and bounded-string overflow", () => {
    const input = fixture();
    const evidence = structuredClone(
      buildTaskMapNativePredecessorEvidence(input),
    ) as TaskMapNativePredecessorEvidenceV1 & { unexpected?: boolean };
  ...
  it("rejects non-0600 fixed files, non-0700 directories, and unsafe paths", async () => {
    const input = fixture();
    const evidence = buildTaskMapNativePredecessorEvidence(input);
    const fixed = createFixedHome(input);
    await writeTaskMapNativePredecessorEvidence({
    ...
    chmodSync(fixed.directory, 0o755);
    await assert.rejects(
      loadTaskMapNativePredecessorEvidence({
        homeDirectory: fixed.home,
      }),
    ...
    chmodSync(fixed.directory, 0o700);

    assert.throws(
      () => taskMapNativePredecessorEvidencePath("relative/home"),
