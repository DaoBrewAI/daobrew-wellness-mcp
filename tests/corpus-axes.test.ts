...
function sig(
  ...
  quality: PatternSignature["quality"] = "real_inference",
): PatternSignature {
  return {
    date,
    dominant,
...
function insight(date: string, topics: string[]) {
  return { topics, occurred_at_ts: ts(date) };
}

function meeting(date: string, title: string) {
...
test("reproduces the documented pricing control ratio (1.46x = 0.5/0.343)", () => {
  ...
  const axis = axes[0];
  assert.equal(axis.theme, "pricing_monetization");
  assert.equal(axis.targetHits, 3);
  assert.equal(axis.targetN, 6);
  assert.equal(axis.referenceN, 35);
  assert.ok(Math.abs(axis.referenceRate - 12 / 35) < 1e-9);
  const ratio = axis.targetHits / axis.targetN / axis.referenceRate;
  assert.ok(Math.abs(ratio - 1.4583) < 0.01, `ratio was ${ratio}`);
  assert.deepEqual(axis.citedDays, [
    { date: citedTargets[0], sources: ["granola", "memory"] },
    { date: citedTargets[1], sources: ["granola", "memory"] },
    { date: citedTargets[2], sources: ["memory"] },
  ]);
  assert.equal(axis.rtmSuspected, true);
});

test("axes are sorted primary-first (highest ratio) and skip themes absent on target days", () => {
  ...
  assert.equal(themes[0], "alpha_only_target");
  assert.ok(themes.includes("beta_both"));
  assert.ok(!themes.includes("gamma_ref_only"));
});

test("non-real_inference and unworn days are excluded from both classes", () => {
  const signatures = new Map<string, PatternSignature>();
  const realTargets = ["2026-03-01", "2026-03-02"];
  for (const d of realTargets) signatures.set(d, sig(d, "TENSION"));
  ...
  assert.equal(axes.length, 1);
  const axis = axes[0];
  assert.equal(axis.targetN, 2);
  assert.equal(axis.targetHits, 2);
  assert.equal(axis.referenceN, 2);
  assert.deepEqual(
    axis.citedDays.map((d) => d.date),
    realTargets,
  );
});

test("corroborating sources cannot mint cited days (review C1)", () => {
  ...
  assert.equal(axes.length, 1);
  const axis = axes[0];
  assert.equal(axis.targetHits, 1);
  assert.deepEqual(axis.citedDays, [
    { date: targetDays[0], sources: ["granola", "memory"] },
  ]);
});

test("theme matching requires whole words and skips stopword tokens (review C2)", () => {
  const signatures = new Map<string, PatternSignature>();
  ...
  assert.deepEqual(axesNoDeck[0].citedDays, [
    { date: "2026-03-01", sources: ["memory"] },
  ]);
});

test("empty-string topics mint nothing", () => {
  const signatures = new Map<string, PatternSignature>();
  signatures.set("2026-03-01", sig("2026-03-01", "TENSION"));
  signatures.set("2026-04-01", sig("2026-04-01", "BALANCED"));
