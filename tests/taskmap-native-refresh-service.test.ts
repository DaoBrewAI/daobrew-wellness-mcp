...
import { createInternalServer } from "../src/engine/internal-server.js";
import {
...
import {
  TASKMAP_AGENT_SESSION_EPISODE_IDENTITY_DOMAIN,
  TASKMAP_AGENT_SESSION_CONTEXT_WINDOW_MS,
  TASKMAP_AGENT_SESSION_IDENTITY_DOMAIN,
...
import {
  ...
  buildTaskMapPhysiologicalSourceSnapshot,
  serializeTaskMapPhysiologicalSourceSnapshot,
} from "../src/engine/taskmap/physiological-source-snapshot.js";
import {
  TASKMAP_BODY_SIGNAL_ASSESSMENT_FILENAME,
  TASKMAP_NATIVE_CURRENTNESS_GATE_VERSION,
  ...
  TASKMAP_NATIVE_REFRESH_CANDIDATE_VERSION,
  TASKMAP_NATIVE_REFRESH_STATE_VERSION,
  TASKMAP_NATIVE_REFRESH_STATUS_VERSION,
  TaskMapNativeRefreshService as ProductionTaskMapNativeRefreshService,
  ...
  boundedHistoricalGenerationIdsForRecovery,
  ...
  nativeTaskMapGenerationReferencePath,
  publishTaskMapNativeProjection,
  retainedSemanticGroupingMarker,
  taskMapBodyAssessmentPreservesAcceptedMembership,
  taskMapNativeReadyProofTargetsPath,
...
import {
  buildTaskMapNativePredecessorEvidence,
  loadTaskMapNativePredecessorEvidence,
  writeTaskMapNativePredecessorEvidence,
} from "../src/engine/taskmap/native-predecessor-evidence.js";
import {
...
const TEST_OWNER_SCOPE = createTaskMapOwnerScope(
  "14802294-BEED-480E-ABF6-7E3703FA25CD",
  "/tmp/taskmap-native-refresh-service-tests",
);

type TestRefreshServiceOptions = Omit<
  TaskMapNativeRefreshServiceOptions,
  "confirmedOwner" | "collectors"
> & {
  confirmedOwner?: ConfirmedTaskMapOwner;
  ownerUserId?: string;
  collectors?: TaskMapNativeRefreshServiceOptions["collectors"];
};

class TaskMapNativeRefreshService
  extends ProductionTaskMapNativeRefreshService {
  constructor(options: TestRefreshServiceOptions = {}) {
    const {
      ownerUserId,
      confirmedOwner = confirmedTestOwner(
        ownerUserId ?? TEST_OWNER_SCOPE.userId,
      ),
      collectors,
      ...rest
    } = options;
    const adaptedCollectors = ownerUserId === undefined
      ? collectors
      : Object.fromEntries(
          Object.entries(collectors ?? {}).map(([source, collect]) => [
            source,
            async () => {
        ...
        ) as TaskMapNativeRefreshServiceOptions["collectors"];
    super({
      ...rest,
      confirmedOwner,
      createAgentSessionExtractionStation:
        rest.createAgentSessionExtractionStation
        ?? testMentionExtractionStation,
      createCalendarExtractionStation:
        rest.createCalendarExtractionStation
        ?? testMentionExtractionStation,
...
async function testMentionExtractionStation(): Promise<LlmStation> {
  return {
    provider: {
    ...
    async run(request) {
      ...
      const body = request.promptText.split(
        `<<<BEGIN_UNTRUSTED_${delimiter}>>>`,
      )[1]?.split(`<<<END_UNTRUSTED_${delimiter}>>>`)[0]?.trim() ?? "";
...
function boundedFixtureTitle(value: string): string {
  return value.slice(0, 96) || "Extracted work";
}

const CURRENT_OURA_TOKEN: OuraToken = {
...
async function emptyLiveOuraContext(
...
async function classifiedLiveOuraContext(
  options: FetchOuraTaskMapContextOptions,
  targetScore = 60,
): Promise<OuraTaskMapContextDocument> {
...
async function repeatedPatternLiveOuraContext(
  options: FetchOuraTaskMapContextOptions,
): Promise<OuraTaskMapContextDocument> {
...
async function oneDayBodyInformedLiveOuraContext(
  options: FetchOuraTaskMapContextOptions,
): Promise<OuraTaskMapContextDocument> {
...
function legacyLocaleCanonicalJson(value: unknown): string {
  ...
  if (value && typeof value === "object") {
    const entries = Object.entries(value as Record<string, unknown>)
      ...
      .map(([key, item]) =>
        `${JSON.stringify(key)}:${legacyLocaleCanonicalJson(item)}`
      );
    return `{${entries.join(",")}}`;
  }
...
function meetingProducerBinding(): TaskMapSourceAuthorityBindingV1 {
  return {
...
function meetingProducerMeeting(
  documentId: string,
  occurredAt: string,
...
function writeMeetingProducerSnapshot(
  filePath: string,
  {
    userId = "owner-service-test",
    producedAt = "2026-07-29T12:00:00.000Z",
    meetings = [
      meetingProducerMeeting(
        "document-a",
  ...
  const snapshot = buildTaskMapMeetingProducerSnapshot({
    ownerScopeDigest: confirmedTestOwner(userId).ownerScopeDigest,
...
function loaderCompatibleProjection(): TaskMapProjectionV1 {
...
function projectionWithTaskCount(
  count: number,
  source = loaderCompatibleProjection(),
): TaskMapProjectionV1 {
...
function projectionWithDistinctAddedTask(
  source: TaskMapProjectionV1,
  taskId: string,
): TaskMapProjectionV1 {
  const projection = projectionWithTaskCount(
  ...
  assert.ok(root);
  root.taskIds = root.taskIds.map((id) =>
    id === generatedTaskId ? taskId : id
  );
  return projection;
}

function acceptedExternalSingletonProjection(
  source: TaskMapProjectionV1,
): TaskMapProjectionV1 {
  const projection = structuredClone(source);
  ...
  projection.roots = projection.tasks.map((task, index) => {
    const predecessorRoot = source.roots.find((root) =>
      root.id === task.rootId
    );
  ...
  projection.edges = [
    ...
    ...projection.tasks.flatMap((task, index) => {
      ...
      if (predecessorEdge === undefined) return [];
      return [{
...
function rankinglessPublicationCandidate(
  reviewCount = 0,
  projection = loaderCompatibleProjection(),
  ...
  return {
    ...
    currentness: {
      ...
      taskDispositions: projection.tasks.map((task, index) => ({
        taskId: task.id,
        disposition:
          index < reviewCount ? "needs_lifecycle_review" : "current",
      })),
    },
  };
}

function rankingCompatibleProjection(
  sourceProjection: TaskMapProjectionV1,
): TaskMapProjectionV1 {
  ...
  for (const task of projection.tasks) {
    task.citations = task.citations.map((citation) =>
      convertedSourceIds.has(citation.pointerId)
        ? { ...citation, sourceKind: "gemini_meet" as const }
        : citation
    );
  }
  return projection;
...
function publicationCandidate(
  reviewCount = 0,
  sourceProjection = loaderCompatibleProjection(),
  ...
  candidate.ranking = buildTaskMapTaskRankingPublication({
    projection,
    ownerScopeDigest,
...
function rankedPublicationCandidate(
  projection = loaderCompatibleProjection(),
  ownerScopeDigest = TEST_OWNER_SCOPE.ownerScopeDigest,
): TaskMapNativePublicationCandidate {
...
function rankedCurrentWorkPublicationCandidate(
  projection = loaderCompatibleProjection(),
): TaskMapNativePublicationCandidate {
  return rankedExecutablePublicationCandidate(projection);
}

function rankedExecutablePublicationCandidate(
  sourceProjection = loaderCompatibleProjection(),
): TaskMapNativePublicationCandidate {
  return publicationCandidate(0, sourceProjection);
}

function ownerBoundPublicationInput(
  candidate: TaskMapNativePublicationCandidate,
  expectedOwnerScopeDigest: string,
  label: string,
): TaskMapNativePublicationInput & { expectedOwnerScopeDigest: string } {
  return {
...
function currentWorkPath(projectionPath: string): string {
  return path.join(
...
function readyProofTargetFromCurrentWorkFixture(
  currentWork: Record<string, unknown>,
): TaskMapReadyFrontierProofTargetV1 {
  return {
    ...structuredClone(
      currentWork.nextTaskToProve as TaskMapReadyFrontierProofTargetV1,
    ),
    approvalPackage: {
...
function readyProofTargetsFixture(
  candidate: TaskMapNativePublicationCandidate,
  ...
  return buildTaskMapReadyProofTargets({
    ...
    proofTargets: taskIds.map((taskId) =>
      readyProofTargetFromCurrentWorkFixture(
        currentWorkForProjection(candidate.projection, taskId),
      )
    ),
  });
}

function expectedCurrentWorkPredecessors(
...
function currentWorkForProjection(
  projection: TaskMapProjectionV1,
  ...
  const task = projection.tasks.find((candidate) => {
    ...
    return (
      (preferredTaskId === undefined || candidate.id === preferredTaskId)
      &&
  ...
  const core = {
    ...
    nextTaskToProve: {
      ...
      predecessors: expectedCurrentWorkPredecessors(
        projection,
        task.id,
      ),
      doneDefinition: [
...
function successorProjection(
  predecessor: TaskMapProjectionV1,
): TaskMapProjectionV1 {
...
function writePublicationPredecessor(
  ...
  const currentTaskId = candidate.currentness.taskDispositions.find(
    (row) => row.disposition === "current",
  )?.taskId;
  const currentWork = currentWorkForProjection(
  ...
  writeFileSync(
    currentnessPath,
    `${JSON.stringify(candidate.currentness, null, 2)}\n`,
    { mode: 0o600 },
  );
  writeFileSync(
    currentWorkPath(projectionPath),
...
function writeReferencedPublicationPredecessor(
  projectionPath: string,
  currentnessPath: string,
  candidate: TaskMapNativePublicationCandidate,
  label: string,
  ...
  assert.ok(
    ranking,
    "owner generation fixtures require the same ranking member as Swift",
  );
  const generationId = digest(candidate);
  const root = path.dirname(projectionPath);
  ...
  const selectedTaskId = (
    currentWork.nextTaskToProve as Record<string, unknown>
  ).taskId as string;
  const readyProofTargets = readyProofTargetsFixture(
    candidate,
    readyTaskIds ?? [selectedTaskId],
  );
  const artifacts = [
    [path.basename(projectionPath), candidate.projection],
    [path.basename(currentnessPath), candidate.currentness],
  ...
  writeFileSync(
    path.join(generationDirectory, "taskmap-task-ranking.v1.json"),
    taskMapContractCanonicalJson(ranking),
    { mode: 0o600 },
  );
  writeFileSync(
    taskMapNativeReadyProofTargetsPath(path.join(
      generationDirectory,
  ...
  const receipt = (filename: string, value: unknown) => ({
    filename,
    sha256: digest(value),
  });
  const manifest = {
    ...
    requestedAtMs: 2_240,
    artifacts: {
      projection: receipt(path.basename(projectionPath), candidate.projection),
      currentness: receipt(
  ...
  writeFileSync(
    path.join(generationDirectory, TASKMAP_NATIVE_GENERATION_MANIFEST_FILENAME),
    taskMapContractCanonicalJson(manifest),
    { mode: 0o600 },
  );
  writeFileSync(
    path.join(root, TASKMAP_NATIVE_GENERATION_REFERENCE_FILENAME),
    taskMapContractCanonicalJson({
      contractVersion: TASKMAP_NATIVE_GENERATION_REFERENCE_VERSION,
      generationId,
      ownerScopeDigest: TEST_OWNER_SCOPE.ownerScopeDigest,
      manifestDigest: createHash("sha256")
        .update(taskMapContractCanonicalJson(manifest))
        .digest("hex"),
    }),
    { mode: 0o600 },
  );
...
function rewriteGenerationManifestWithLegacyLocaleOrder(
  projectionPath: string,
): void {
  ...
  writeFileSync(manifestPath, legacyBytes, { mode: 0o600 });
  reference.manifestDigest = createHash("sha256")
    .update(legacyBytes)
    .digest("hex");
  writeFileSync(
    referencePath,
    taskMapContractCanonicalJson(reference),
    { mode: 0o600 },
  );
}
...
function slice(
  ...
  revision = `${source}-r1`,
  identityDigest = `${source}-identity`,
): TaskMapOwnerCollectedSlice<TaskMapNativeSafeSlice> {
  return {
    ownerScopeDigest: TEST_OWNER_SCOPE.ownerScopeDigest,
    revision,
    sliceDigest: taskMapContractDigest({ source, revision, identityDigest }),
    value: {
...
function admittedAgentSessionSlice(
  summary: string,
  seed = "agent-session-admission",
  ownerScopeDigest = TEST_OWNER_SCOPE.ownerScopeDigest,
): TaskMapOwnerCollectedSlice<TaskMapNativeSafeSlice> {
  ...
  agent.sliceDigest = taskMapContractDigest(agent.value);
  return agent;
}
...
function task4AgentObservation(input: {
  ...
  root: string;
  route: string;
  turns: Array<{ id: string; text: string; at: string; outcome?: string }>;
...
function task6AgentProjection(
  admission: ReturnType<typeof task4AgentAdmission>,
  generatedAt: string,
  previousProjection?: TaskMapProjectionV1,
): TaskMapProjectionV1 {
  return buildAgentSessionOnlyProjection(
    admission,
    buildAgentSessionExtractionFixture(admission, generatedAt),
    generatedAt,
    previousProjection,
  );
}

describe("Task Map agent-session workstream projection", () => {
  it("collapses workstreams and proposal clusters with stable source-owned identities while suppressing latest noise", () => {
    ...
    const collapsed = task6AgentProjection(
      sameDirective,
      generatedAt,
    );
    assert.deepEqual(
      [collapsed.roots.length, collapsed.tasks.length, collapsed.edges.length],
      [1, 1, 1],
    );
    ...
    const threeDirectives = task6AgentProjection(
      task4AgentAdmission([
    ...
    assert.deepEqual(
      [threeDirectives.roots.length, threeDirectives.tasks.length, threeDirectives.edges.length],
      [1, 3, 3],
    );
    ...
    const twoWorkspaces = task6AgentProjection(
      task4AgentAdmission(workspaceObservations),
      generatedAt,
    );
    assert.deepEqual(
      [twoWorkspaces.roots.length, twoWorkspaces.tasks.length],
      [2, 2],
    );
    assert.equal(
      new Set(twoWorkspaces.roots.map((root) => root.id)).size,
      2,
      "source-owned workstream identities must disambiguate distinct roots",
    );
    const reversedWorkspaces = task6AgentProjection(
      task4AgentAdmission([...workspaceObservations].reverse()),
      generatedAt,
    );
    const rootIdentityView = (projection: TaskMapProjectionV1) =>
    ...
    assert.deepEqual(
      rootIdentityView(reversedWorkspaces),
      rootIdentityView(twoWorkspaces),
      "root identity and display titles must be permutation invariant",
    );
    for (const root of twoWorkspaces.roots) {
      const sourceOwnedWorkstreamRef = root.memberObjectRefs[0];
      assert.equal(
        root.id,
        `tmr_${createHash("sha256").update(sourceOwnedWorkstreamRef).digest("hex").slice(0, 16)}`,
        "display title must not participate in source-owned workstream identity",
      );
    }

    const crossProvider = task6AgentProjection(
      task4AgentAdmission([
    ...
    assert.deepEqual(
      [crossProvider.roots.length, crossProvider.tasks.length],
      [1, 1],
    );
    ...
    const duplicate = task6AgentProjection(
      duplicateAdmission,
      generatedAt,
      base,
    );
    assert.deepEqual(
      [duplicate.roots.length, duplicate.tasks.length, duplicate.edges.length],
      [1, 1, 1],
    );
  ...
  it("synthesizes a workstream theme from the highest-confidence mention with a digest tie-break", () => {
    const generatedAt = "2026-07-30T08:00:00.000Z";
    ...
    const fixture = buildAgentSessionExtractionFixture(admission, generatedAt);
    ...
    const extraction = {
      ...withoutDigest,
      reportDigest: taskMapContractDigest(withoutDigest),
    };
    ...
    assert.equal(
      projection.roots[0]?.title,
      "Workstream: Winning natural title",
    );
    assert.equal(
      projection.tasks.some((task) => task.title === projection.roots[0]?.title),
      false,
    );
  ...
  it("emits one task pointer per session episode when one episode supports many mentions", () => {
    const generatedAt = "2026-07-30T08:00:00.000Z";
    const admission = task4AgentAdmission([
    ...
    const clusters = fixture.clusters.map((cluster) => ({
      ...cluster,
      mentions: ["First mention title", "Second mention title"].map(
        (title, index) => ({
          ...cluster.mentions[0],
          text: title,
          title,
    ...
    assert.deepEqual(
      projection.tasks.map((task) => task.title).sort(),
      ["First mention title", "Second mention title"],
    );

    // Exactly one task pointer for the shared episode, plus the root pointer.
    const taskPointerIds = new Set(
      projection.tasks.flatMap((task) => task.originPointerIds),
    );
    ...
    assert.equal(
      new Set(projection.sources.map((source) => source.id)).size,
    ...
    const citedEventIds = new Set(
      projection.tasks.flatMap((task) =>
        task.citations.map((citation) => citation.eventId)
      ),
    );
    assert.equal(citedEventIds.size, 2);
  });

  it("retains only predecessor workstreams whose Station-1 clusters are pending", () => {
    const generatedAt = "2026-07-30T08:00:00.000Z";
    ...
    const fullExtraction = buildAgentSessionExtractionFixture(
      admission,
      generatedAt,
    );
    const pendingClusters = fullExtraction.clusters.filter((row) =>
      row.clusterIdentityDigest !== admission.clusters.find((cluster) =>
        cluster.userDirectiveSummary === "Publish extracted W1"
      )!.clusterIdentityDigest
    );
    const partialPayload = {
      ...fullExtraction,
      clusters: fullExtraction.clusters.map((row) =>
        pendingClusters.some((pending) =>
          pending.clusterIdentityDigest === row.clusterIdentityDigest
        )
          ? {
              ...row,
    ...
    const pendingWorkstreams = new Set(pendingClusters.map((row) =>
      row.workstreamIdentityDigest
    ));

    const partial = buildAgentSessionOnlyProjection(
      admission,
      {
        ...partialWithoutDigest,
        reportDigest: taskMapContractDigest(partialWithoutDigest),
      },
    ...
    assert.deepEqual(
      partial.roots.map((root) => root.title).sort(),
      [
  ...
  it("uses the newest representative outcome as the visible proposal summary without changing directive identity", () => {
    const generatedAt = "2026-07-30T08:00:00.000Z";
    const observations = [
      task4AgentObservation({
        root: "outcome-older",
    ...
    assert.equal(admission.clusters.length, 1);
    assert.equal(
      admission.clusters[0].assistantOutcomeSummary,
      "Verified the final release gates and owners.",
    );
    ...
    assert.equal(
      projection.tasks[0].summary,
      "Verified the final release gates and owners.",
    );
    const reversed = task6AgentProjection(
      task4AgentAdmission([...observations].reverse()),
      generatedAt,
    );
    assert.deepEqual(
      reversed.tasks.map(({ id, title, summary }) => ({ id, title, summary })),
      projection.tasks.map(({ id, title, summary }) => ({ id, title, summary })),
    );
  });
});
...
function nonMeetingCollectors(): Partial<
  TaskMapOwnerRefreshCollectors<TaskMapNativeSafeSlice>
> {
  return {
...
async function assertMeetingBarrierRaceFailsClosed(
  scenario: "disappears" | "changes",
): Promise<void> {
  const locations = roots(`taskmap-native-meeting-${scenario}-`);
  const ownerUserId = `synthetic-meeting-${scenario}-owner`;
  const ownerScopeDigest = confirmedTestOwner(ownerUserId).ownerScopeDigest;
  const meetingSnapshotPath = path.join(
    path.dirname(locations.runtimeRoot),
  ...
  writeMeetingProducerSnapshot(meetingSnapshotPath, {
    userId: ownerUserId,
    producedAt,
    meetings: [meetingProducerMeeting(
      "meeting-before-barrier-race",
  ...
  const baseline = new TaskMapNativeRefreshService({
    ...
    collectors: {
      agent_session: async () => admittedAgentSessionSlice(
        "Keep the accepted Meeting work current",
        `meeting-${scenario}-baseline-agent`,
        ownerScopeDigest,
      ),
      calendar: async () => slice("calendar"),
      body: async () => slice("body"),
  ...
  assert.equal(
    baselineResult.refreshStatus,
    "published",
    JSON.stringify(baselineResult),
  );
  const projectionBefore = readFileSync(locations.projectionPath);
  const currentnessBefore = readFileSync(locations.currentnessPath);
  const stateBefore = artifact(
    locations.runtimeRoot,
    "taskmap-refresh-state.v1.json",
  );
  const meetingSuccessBefore = (
    stateBefore.lastSourceSuccessAtMs as Record<string, number>
  ).meeting_notes;
  ...
  const racing = new TaskMapNativeRefreshService({
    ...
    collectors: {
      agent_session: async () => admittedAgentSessionSlice(
        "Review the changed owner work before publication",
        `meeting-${scenario}-changed-agent`,
        ownerScopeDigest,
      ),
      calendar: async () => slice("calendar"),
      body: async () => slice("body"),
    ...
    afterDefaultContextBarrierForTesting: async () => {
      ...
      writeMeetingProducerSnapshot(meetingSnapshotPath, {
        ...
        producedAt,
        meetings: [meetingProducerMeeting(
          "meeting-replaced-after-barrier",
          "2026-07-30T07:45:00.000Z",
        )],
      });
    },
  });
  ...
  assert.equal(
    result.publicationBlockReason,
    "semantic_provider_unavailable",
  );
  assert.equal(
    result.sourceStatuses.find(
      (status) => status.source === "meeting_notes",
    )?.state,
    "unavailable",
  );
  assert.deepEqual(readFileSync(locations.projectionPath), projectionBefore);
  assert.deepEqual(readFileSync(locations.currentnessPath), currentnessBefore);
  const stateAfter = artifact(
    locations.runtimeRoot,
    "taskmap-refresh-state.v1.json",
  );
  assert.equal(
    (
      stateAfter.lastSourceStatuses as Array<{
        source: string;
        disposition: string;
      }>
    ).find((status) => status.source === "meeting_notes")?.disposition,
    "unavailable",
  );
  assert.equal(
    (stateAfter.lastSourceSuccessAtMs as Record<string, number>)
      .meeting_notes,
    meetingSuccessBefore,
  );
}
...
function artifact(
  ...
  return JSON.parse(readFileSync(path.join(runtimeRoot, filename), "utf8"));
}
...
function committedGeneration(
  ...
  currentWork: Record<string, unknown> | null;
  ranking: TaskMapTaskRankingPublicationV1 | null;
  readyProofTargets: TaskMapReadyProofTargetsV1 | null;
} {
  const reference = JSON.parse(readFileSync(
    nativeTaskMapGenerationReferencePath(projectionPath),
    "utf8",
  ...
  const currentWorkArtifactPath = path.join(
    directory,
    "taskmap-current-work.v1.json",
  );
  const rankingPath = path.join(directory, "taskmap-task-ranking.v1.json");
  ...
  return {
    ...
    projection: JSON.parse(readFileSync(
      path.join(directory, "taskmap-projection.v1.json"),
      "utf8",
    )),
    currentness: JSON.parse(readFileSync(
      path.join(directory, "taskmap-currentness.v1.json"),
      "utf8",
    )),
    currentWork: existsSync(currentWorkArtifactPath)
      ? JSON.parse(readFileSync(currentWorkArtifactPath, "utf8"))
      : null,
    ranking: existsSync(rankingPath)
      ? JSON.parse(readFileSync(rankingPath, "utf8"))
      : null,
    readyProofTargets: existsSync(readyProofTargetsPath)
...
function roots(prefix: string): {
  ...
  const projectionDirectory = path.join(root, "taskmap");
  return {
...
function rawGranolaDegradationFixture(
  prefix: string,
  ownerLabel: string,
  bodies: readonly string[],
): ReturnType<typeof roots> & {
  ...
  const owner = confirmedTestOwner(ownerLabel);
  const producerPath = path.join(
    path.dirname(locations.runtimeRoot),
    "meeting-producer-snapshot.v1.json",
  );
  const rawPath = path.join(owner.sourceRoot, "granola-mcp-snapshot.json");
  const promptPath = path.join(
    path.dirname(locations.runtimeRoot),
    "mention-extraction-v1.md",
  );
  writeMeetingProducerSnapshot(producerPath, { userId: ownerLabel });
  mkdirSync(owner.taskMapRoot, { recursive: true, mode: 0o700 });
  mkdirSync(owner.sourceRoot, { recursive: true, mode: 0o700 });
  ...
  writeFileSync(rawPath, rawBytes, { mode: 0o600 });
  const granolaSuccessAt = "2026-07-29T12:59:00.000Z";
  utimesSync(rawPath, new Date(granolaSuccessAt), new Date(granolaSuccessAt));
  writeFileSync(
    path.join(owner.sourceRoot, "taskmap-resident-receipt.v1.json"),
    JSON.stringify({
  ...
  writeFileSync(promptPath, "Return strict JSON only.\n", { mode: 0o600 });
  return { ...locations, owner, producerPath, rawPath, promptPath };
}

async function killPublicationChild(
  ...
  const child = spawn(
    ...
    [
      path.join(
        __dirname,
        "support",
        "taskmap-publication-kill-child.js",
      ),
      childInput,
    ],
  ...
  assert.equal(
    ...
    true,
    `child did not reach ${killStage} crash boundary: ${childStderr}`,
  );
  child.kill("SIGKILL");
  await childClosed;
}

async function strategyFallbackFixture(
  prefix: string,
  {
    currentStrategyCount = 1,
    reviewStrategyCount = 0,
    currentOtherCount = 0,
    reviewOtherCount = 0,
    legacyStrategyEventCount = 0,
    bodyPatternReady = false,
    bodyInformedReady = false,
  }: {
    currentStrategyCount?: number;
    reviewStrategyCount?: number;
    currentOtherCount?: number;
    reviewOtherCount?: number;
    legacyStrategyEventCount?: number;
    bodyPatternReady?: boolean;
    bodyInformedReady?: boolean;
  } = {},
): Promise<{
  locations: {
    runtimeRoot: string;
  ...
  readAdapterInput: () => Promise<{
    ...
    repositoryProvider: {
      readImmutableRepositoryFile: () => Promise<{
  ...
  const taskMapDirectory = path.join(
    homeDirectory,
    "Library",
    "Application Support",
    "DaoBrew",
  ...
  const totalStrategyCount =
    currentStrategyCount + reviewStrategyCount;
  const strategyPointerIds = Array.from(
    { length: totalStrategyCount },
    (_, index) => `strategy-task-${index + 1}`,
  );
  const totalOtherCount = currentOtherCount + reviewOtherCount;
  const otherPointerIds = Array.from(
    { length: totalOtherCount },
    (_, index) => `manual-task-${index + 1}`,
  );
  const pointerIds = [...strategyPointerIds, ...otherPointerIds];
  const rowTexts = Array.from(
    { length: currentStrategyCount },
    (_, index) =>
      `| P${index} | Ship current Task Map task ${index + 1} | Ready |`,
  );
  ...
  const strategyPointers: TaskMapInput["pointers"] =
    strategyPointerIds.map(
    (pointerId, index) => ({
      id: pointerId,
      sourceKind: "strategy",
      ...
      canonicalUrl:
        `${remote}/blob/${revision}/${repositoryRelativePath}`,
  ...
  const otherPointers: TaskMapInput["pointers"] =
    otherPointerIds.map((pointerId) => ({
    id: pointerId,
    sourceKind: "manual",
  ...
  const bodyWorkPointers: TaskMapInput["pointers"] = bodyPatternReady
    ? [
        {
          id: "body-work-agent",
          sourceKind: "codex_session",
          sourceObjectId: "body-work-agent",
          sourceRefHash: taskMapContractDigest("body-work-agent"),
        ...
        },
        {
          ...
          sourceRefHash: taskMapContractDigest("body-work-meeting"),
  ...
  const authoritativeEvents: TaskMapInput["events"] = pointerIds.map(
    (pointerId, index) => ({
      id: `event-${pointerId}`,
      pointerId,
      ...
      activity: strategyPointerIds.includes(pointerId)
        ...
        : "task_created",
      occurredAt:
        `2026-07-${String(20 + (index % 9)).padStart(2, "0")}T17:00:00.000Z`,
  ...
  const legacyEvents: TaskMapInput["events"] = strategyPointerIds
    ...
    .map((pointerId, index) => ({
      ...
      occurredAt:
        `2026-07-${String(10 + index).padStart(2, "0")}T17:00:00.000Z`,
      observedAt: "2026-07-29T18:00:00.000Z",
      objectRefs: ["workstream:moonshot"],
  ...
  const bodyWorkEvents: TaskMapInput["events"] =
    bodyPatternReady || bodyInformedReady
    ? bodyWorkDates.flatMap((dayKey) =>
        bodyWorkPointers.map((pointer) => ({
    ...
    : [];
  const bodyCoverageEvents: TaskMapInput["events"] = bodyPatternReady
    ? bodyCoverageDates.flatMap((dayKey) =>
        bodyWorkPointers.map((pointer) => ({
          id: `coverage-${pointer.id}-${dayKey}`,
          pointerId: pointer.id,
          recordKind: "receipt" as const,
          activity: "receipt_observed" as const,
  ...
  const brain: SemanticBrainOutput = {
    ...
    promptHash: taskMapContractDigest("strategy-fallback-fixture"),
  ...
  const currentness: TaskMapNativePublicationCandidate["currentness"] = {
    contractVersion: TASKMAP_NATIVE_CURRENTNESS_GATE_VERSION,
    runId: projection.runId,
    inputDigest: projection.inputDigest,
    projectionDigest:
  ...
  const candidate: TaskMapNativePublicationCandidate = {
    ...
    ranking: {} as TaskMapTaskRankingPublicationV1,
  };
  writeReferencedPublicationPredecessor(
    locations.projectionPath,
    locations.currentnessPath,
    candidate,
    `${prefix}-strategy-predecessor`,
  );
  chmodSync(taskMapDirectory, 0o700);
  const projectionBytes = readFileSync(locations.projectionPath);
  const currentnessBytes = readFileSync(locations.currentnessPath);
  const evidence = buildTaskMapNativePredecessorEvidence({
    taskMapInput: input,
  ...
  await writeTaskMapNativePredecessorEvidence({
    homeDirectory,
    evidence,
  });
  ...
  const readAdapterInput = async () => ({
    ownerScopeDigest: taskMapContractDigest("strategy-owner"),
    binding: {
    ...
    projectionBytes,
    currentnessBytes,
    expectedProjectionFileDigest:
      createHash("sha256").update(projectionBytes).digest("hex"),
    expectedCurrentnessFileDigest:
      createHash("sha256").update(currentnessBytes).digest("hex"),
    rowBindings,
    repositoryProvider: {
      async readImmutableRepositoryFile() {
        return {
          ...
          contentDigest:
            createHash("sha256").update(repositoryText).digest("hex"),
        };
      },
    },
  });
...
describe("TaskMapNativeRefreshService", () => {
  it("allows safe partial Agent recovery but blocks one wholly unresolved singleton", () => {
    const allEight = new Set(Array.from({ length: 8 }, (_, index) =>
      `accepted-agent-${index}`
    ));
    assert.equal(
      acceptedAgentMigrationResultUnavailable(
        allEight,
        new Set(["accepted-agent-6", "accepted-agent-7"]),
        allEight,
      ),
      false,
      "six proven migrations may publish while two unproven tasks stay unchanged",
    );
    assert.equal(
      acceptedAgentMigrationResultUnavailable(
        new Set(["accepted-agent-only"]),
        new Set(["accepted-agent-only"]),
        new Set(["accepted-agent-only"]),
        true,
      ),
      true,
      "one unresolved Agent singleton must not publish as semantic success",
    );
    assert.equal(
      acceptedAgentMigrationResultUnavailable(
        new Set(["accepted-agent-only"]),
      ...
      false,
      "a legacy personal fork is not a failed migration without a multi-member semantic plan",
    );
    assert.equal(
      acceptedAgentMigrationResultUnavailable(
        new Set(["accepted-agent-a", "accepted-agent-b"]),
        new Set(["accepted-agent-a", "accepted-agent-b"]),
        new Set(["accepted-agent-a", "accepted-agent-b"]),
        false,
      ),
      true,
      "multiple unresolved receipt-backed singletons must fail closed when the semantic layer is unavailable",
    );
  });

  it("rejects all historical authority when the verified generation bound is exceeded", () => {
    const exactBound = Array.from({ length: 128 }, (_, index) =>
      index.toString(16).padStart(64, "0")
    ).reverse();
    assert.deepEqual(
      boundedHistoricalGenerationIdsForRecovery(exactBound),
      [...exactBound].sort(),
    );
    assert.equal(
      boundedHistoricalGenerationIdsForRecovery([
      ...
      null,
      "an unverified suffix must suppress the entire historical proof set",
    );
  });

  it("replaces only root membership while preserving accepted-task dependency edges", () => {
    const movedTaskIds = new Set(["accepted-task"]);
    assert.equal(
      acceptedAgentTopicMembershipEdgeShouldBeReplaced(
        { relation: "advances", to: "accepted-task" },
        movedTaskIds,
      ),
      true,
    );
    for (const relation of [
      ...
      assert.equal(
        acceptedAgentTopicMembershipEdgeShouldBeReplaced(
          { relation, to: "accepted-task" },
          movedTaskIds,
        ),
        false,
        `${relation} must survive topic reparenting`,
      );
    }
  });

  it("publishes two adopted candidates from one engine workstream as sibling accepted tasks", async () => {
    ...
    const createExtractionStation = async (): Promise<LlmStation> => {
      const station = await testMentionExtractionStation();
      return {
        ...station,
        async run(request) {
          const envelope = await station.run(request);
          const output = JSON.parse(envelope.outputJson) as {
    ...
    const admission = task4AgentAdmission([
      task4Work(
        "task10-group-a",
        "/repo/task10-grouped-adoption",
        "Repair proposal review ordering",
        "task10-group-turn-a",
      ),
      task4Work(
        "task10-group-b",
        "/repo/task10-grouped-adoption",
        "Restore community grouping fallback",
        "task10-group-turn-b",
      ),
      task4Work(
        "task10-group-long",
        "/repo/task10-grouped-adoption",
        longDirective,
        "task10-group-turn-long",
      ),
      task4Work(
        "task10-group-collapse-a",
        "/repo/task10-grouped-adoption",
        "Publish release notes",
        "task10-group-turn-collapse-a",
      ),
      task4Work(
        "task10-group-collapse-b",
        "/repo/task10-grouped-adoption",
        "Publish the release notes",
        "task10-group-turn-collapse-b",
      ),
    ], owner.ownerScopeDigest);
    assert.equal(admission.clusters.length, 5);
    assert.equal(
      new Set(admission.clusters.map((cluster) =>
        cluster.workstreamIdentityDigest
      )).size,
      1,
    );
    const collectedAgent = slice(
      "agent_session",
      admission.admissionDigest,
      admission.clusters[0]!.clusterIdentityDigest,
    );
    ...
    collectedAgent.sliceDigest = taskMapContractDigest(collectedAgent.value);
    const collectors = {
    ...
    assert.equal(
      (await proposalRefresh.requestRefresh("manual")).refreshStatus,
      "published",
    );
    const proposalGeneration = committedGeneration(locations.projectionPath);
    const proposalTasks = proposalGeneration.projection.tasks.filter((task) =>
      task.reviewState === "proposed"
    );
    assert.ok(
      proposalTasks.length < admission.clusters.length,
      "production projection must collapse at least the safely subsumed title pair",
    );
    assert.ok(
      proposalTasks.some((task) =>
        task.title.endsWith("…")
        && task.title.length < longExtractedTitle.length
      ),
      "production projection must exercise its bounded long-title path",
    );

    const extraction = await loadVerifiedTaskMapAgentSessionExtractionReport({
      admission,
    ...
    const candidateReview = buildTaskMapAgentSessionCandidateReview({
      admission,
      extraction,
      previous: null,
    ...
    for (const candidate of candidateReview.shelf.candidates) {
      const adopted = promoteTaskMapAgentSessionCandidate({
        admission,
        extraction,
        ...
        expectedAcceptanceHeadDigest:
          taskMapNativeCandidateAcceptanceHeadDigest(acceptanceStore),
        assessedAt,
    ...
    assert.equal(
      (await acceptedRefresh.requestRefresh("manual")).refreshStatus,
      "published",
    );
    const generation = committedGeneration(locations.projectionPath);
    const acceptedTasks = generation.projection.tasks.filter((task) =>
      task.reviewState === "accepted" && task.authority === "user"
    );
    assert.equal(acceptedTasks.length, 5);
    assert.equal(
      new Set(acceptedTasks.map((task) => task.rootId)).size,
      1,
      "engine topic membership must survive adoption",
    );
    const acceptedRootId = acceptedTasks[0]!.rootId;
    assert.equal(
      generation.projection.tasks.filter((task) =>
        task.rootId === acceptedRootId && task.reviewState === "proposed"
      ).length,
      0,
      "adopted leaves must replace their proposed shadows",
    );
    assert.ok(acceptedTasks.some((task) =>
      task.title === "Repair proposal review ordering"
    ));
    assert.ok(acceptedTasks.some((task) =>
      task.title === "Restore community grouping fallback"
    ));
    assert.ok(acceptedTasks.every((task) =>
      task.taskHomePointerId !== undefined
      && task.returnRoute.state === "personal_fork"
    ));
  });

  it("keeps adopted tasks in their semantic community after the current extraction rotates", async () => {
    ...
    const originalObservations = [
      task4Work(
        "task10-history-group-a",
        "/repo/task10-history-group-a",
        firstDirective,
        "task10-history-group-turn-a",
      ),
      task4Work(
        "task10-history-group-b",
        "/repo/task10-history-group-b",
        secondDirective,
        "task10-history-group-turn-b",
      ),
      task4Work(
        "task10-history-group-d",
        "/repo/task10-history-group-d",
        thirdDirective,
        "task10-history-group-turn-d",
      ),
      task4Work(
        "task10-history-group-e",
        "/repo/task10-history-group-e",
        fourthDirective,
        "task10-history-group-turn-e",
      ),
      task4Work(
        "task10-history-group-f",
        "/repo/task10-history-group-f",
        fifthDirective,
        "task10-history-group-turn-f",
      ),
    ];
    const laterObservations = [
      task4Work(
        "task10-history-group-c",
        "/repo/task10-history-group-c",
        laterDirective,
        "task10-history-group-turn-c",
      ),
    ];
    const createCommunityStation = async (): Promise<LlmStation> => ({
      provider: {
        transport: "claude-cli",
        executable: "/fixture/provider",
        args: [],
        ...
        if (request.stationId === "community-task-extraction-v1") {
          ...
          outputJson = JSON.stringify({
            mentions: rewrittenMentions
              .filter((mention) =>
                request.promptText.includes(mention.source)
              )
              .map((mention) => ({
                text: mention.text,
                title: mention.title,
    ...
    const proposalRefresh = new TaskMapNativeRefreshService({
      ...
      agentSessionGraphFeedForTesting: buildTaskMapAgentSessionGraphFeed({
        ownerScopeDigest: owner.ownerScopeDigest,
        producedAt: "2026-07-30T08:00:00.000Z",
    ...
    rmSync(acceptanceStorePath, { force: true });
    for (const candidate of candidateReview.shelf.candidates) {
      const adopted = promoteTaskMapAgentSessionCandidate({
        admission,
        extraction,
        ...
        idempotencyKeyDigest: digest(
          `task10-history-group-${candidate.candidateId}`,
        ),
        confirmedAt: assessedAt,
      });
      acceptanceStore = adopted.store;
    ...
    const firstAcceptedGeneration = committedGeneration(
      locations.projectionPath,
    );
    ...
    assert.equal(
      new Set(firstAcceptedTasks.map((task) => task.rootId)).size,
      1,
    );
    ...
    const singletonProjection = acceptedExternalSingletonProjection(
      firstAcceptedGeneration.projection,
    );
    const singletonCurrentness = currentnessForNativeProjection(
      singletonProjection,
      firstAcceptedGeneration.currentness,
    );
    const singletonCandidate: TaskMapNativePublicationCandidate = {
      contractVersion: TASKMAP_NATIVE_PUBLICATION_CANDIDATE_VERSION,
      projection: singletonProjection,
      currentness: singletonCurrentness,
      ranking: buildTaskMapTaskRankingPublication({
        projection: singletonProjection,
        ownerScopeDigest: owner.ownerScopeDigest,
        sourceStatuses: TASKMAP_OWNER_REFRESH_SOURCES.map((source) => ({
          source,
    ...
    await publishTaskMapNativeProjection(
      ...
      path.join(
        locations.runtimeRoot,
        "pre-proof-singleton-publication-journal.v1.json",
      ),
      {
        graphInputDigest: digest("pre-proof-singleton-graph"),
        candidateDigest: digest(singletonCandidate),
        candidate: singletonCandidate as unknown as Record<string, unknown>,
    ...
    assert.ok(singletonGeneration.projection.roots.every((root) =>
      root.taskIds.length === 1
      && root.memberObjectRefs.length === 1
      && root.memberObjectRefs[0]!.startsWith("external:")
    ));
    assert.deepEqual(
      singletonGeneration.currentWork?.agentSessionTaskProofs,
      [],
    );

    const laterAdmission = task4AgentAdmission(
      laterObservations,
      owner.ownerScopeDigest,
    );
    ...
    assert.equal(
      rotatedResult.refreshStatus,
      "published",
      JSON.stringify(rotatedResult),
    );
    assert.equal(
      (rotatedResult as unknown as {
        semanticGroupingRetention: unknown;
      }).semanticGroupingRetention,
      null,
      "a recovered semantic publication must clear predecessor retention truth",
    );
    const rotatedGeneration = committedGeneration(locations.projectionPath);
    const retainedAcceptedTasks = rotatedGeneration.projection.tasks.filter(
      (task) =>
        task.reviewState === "accepted" && task.authority === "user",
    );
    assert.equal(retainedAcceptedTasks.length, 5);
    assert.equal(
      new Set(retainedAcceptedTasks.map((task) => task.rootId)).size,
      1,
      "accepted tasks must not split back into singleton roots when the latest extraction rotates",
    );
    const retainedRoot = rotatedGeneration.projection.roots.find((root) =>
      root.id === retainedAcceptedTasks[0]!.rootId
    );
    assert.ok(retainedRoot);
    assert.equal(
      retainedRoot.memberObjectRefs.some((ref) => ref.startsWith("community:")),
      true,
      "retained accepted membership must remain engine-community-owned",
    );
    const laterTasks = rotatedGeneration.projection.tasks.filter((task) =>
      task.title === laterDirective && task.reviewState === "proposed"
    );
    assert.equal(
      rotatedGeneration.projection.tasks.filter((task) =>
        task.rootId === retainedAcceptedTasks[0]!.rootId
      ).length,
      5,
      "accepted work must win the five bounded topic slots",
    );
    assert.equal(laterTasks.length, 0);
    assert.equal(
      rotatedGeneration.projection.tasks.filter((task) =>
        (task.title === firstDirective || task.title === secondDirective)
        && task.reviewState === "proposed"
      ).length,
      0,
      "historical proposed shadows must be replaced by their adopted tasks",
    );
    const rotatedProofs = rotatedGeneration.currentWork
      ?.agentSessionTaskProofs;
    assert.ok(Array.isArray(rotatedProofs));
    assert.equal(
      rotatedProofs.length,
      5,
      "receipt-bound proofs must survive the first extraction rotation",
    );
    const conflictingProof = structuredClone(rotatedProofs[0]!);
    conflictingProof.supportIdentityDigest = digest(
      "conflicting-historical-support",
    );
    assert.equal(
      mergeAcceptedAgentSessionTaskProofHistory(
        [rotatedProofs[0]!],
        [conflictingProof],
      ).some((proof) => proof.taskId === rotatedProofs[0]!.taskId),
      false,
      "conflicting verified histories must remove all authority for that task",
    );
    ...
    assert.equal(
      relaunchedResult.refreshStatus,
      "no_op",
      JSON.stringify(relaunchedResult),
    );
    const relaunchedGeneration = committedGeneration(locations.projectionPath);
    const relaunchedAcceptedTasks = relaunchedGeneration.projection.tasks.filter(
      (task) =>
        task.reviewState === "accepted" && task.authority === "user",
    );
    assert.equal(relaunchedAcceptedTasks.length, 5);
    assert.equal(
      new Set(relaunchedAcceptedTasks.map((task) => task.rootId)).size,
      1,
      "source-bound grouping must survive a second refresh/relaunch",
    );
    const relaunchedProofs = relaunchedGeneration.currentWork
      ?.agentSessionTaskProofs;
    assert.ok(Array.isArray(relaunchedProofs));
    assert.equal(
      relaunchedProofs.length,
      5,
      "manual execution proofs must remain available after relaunch",
    );

    const tasklessSingletonProjection = acceptedExternalSingletonProjection(
      relaunchedGeneration.projection,
    );
    const tasklessSingletonCurrentness = currentnessForNativeProjection(
      tasklessSingletonProjection,
      relaunchedGeneration.currentness,
    );
    const tasklessSingletonCandidate: TaskMapNativePublicationCandidate = {
      contractVersion: TASKMAP_NATIVE_PUBLICATION_CANDIDATE_VERSION,
    ...
    await publishTaskMapNativeProjection(
      ...
      path.join(
        locations.runtimeRoot,
        "taskless-singleton-publication-journal.v1.json",
      ),
      {
        graphInputDigest: digest("taskless-singleton-graph"),
        candidateDigest: digest(tasklessSingletonCandidate),
        candidate:
          tasklessSingletonCandidate as unknown as Record<string, unknown>,
    ...
    const createEmptyAgentExtractionStation = async (): Promise<LlmStation> => ({
      provider: {
        transport: "claude-cli",
        executable: "/fixture/provider",
        args: [],
    ...
    const createTasklessCommunityStation = async (): Promise<LlmStation> => {
    ...
    assert.equal(
      tasklessResult.refreshStatus,
      "published",
      JSON.stringify(tasklessResult),
    );
    const tasklessExtractionReport = artifact(
      locations.runtimeRoot,
    ...
    assert.equal(
      tasklessExtractionReport.pendingCount,
      tasklessExtractionReport.clusters.length,
    );
    assert.ok(tasklessExtractionReport.clusters.every((cluster) =>
      cluster.status === "degraded"
      && cluster.degradationCode === "invalid_extraction_output"
      && cluster.mentions.length === 0
    ));
    const recoveredFromTaskless = committedGeneration(
      locations.projectionPath,
    );
    const tasklessAccepted = recoveredFromTaskless.projection.tasks.filter(
      (task) => task.reviewState === "accepted" && task.authority === "user",
    );
    ...
    for (const task of tasklessAccepted) {
      ...
      assert.deepEqual(
        payload,
        acceptedTaskPayloadById.get(task.id),
        "topic recovery may change only root membership; accepted citations and return routes are immutable",
      );
    }
    assert.equal(
      new Set(tasklessAccepted.map((task) => task.rootId)).size,
      1,
      "historical source lineage must recover membership even when every current bounded report leaf is empty",
    );
    assert.ok(recoveredFromTaskless.projection.roots.some((root) =>
      root.id === tasklessAccepted[0]!.rootId
      && root.memberObjectRefs.some((ref) => ref.startsWith("community:"))
    ));
    assert.equal(
      recoveredFromTaskless.projection.tasks.filter((task) =>
        task.reviewState === "proposed"
      ).length,
      0,
    );
    const recoveredTasklessProofs = (
      recoveredFromTaskless.currentWork as {
        agentSessionTaskProofs?: unknown[];
      } | null
    )?.agentSessionTaskProofs;
    assert.equal(
      recoveredTasklessProofs?.length,
      5,
    );
    assert.ok(recoveredFromTaskless.readyProofTargets);
    assert.ok(recoveredFromTaskless.readyProofTargets.proofTargets.some(
      (target) =>
        tasklessAccepted.some((task) => task.id === target.taskId)
        && target.approvalPackage.readyForLocalApproval
        && target.approvalPackage.authorizationScope
          === "prepare_local_package_only",
    ), "a migrated accepted task must remain eligible for a local approval package");

    const unavailableSingletonProjection = acceptedExternalSingletonProjection(
      recoveredFromTaskless.projection,
    );
    const unavailableSingletonCandidate: TaskMapNativePublicationCandidate = {
      contractVersion: TASKMAP_NATIVE_PUBLICATION_CANDIDATE_VERSION,
      projection: unavailableSingletonProjection,
      currentness: currentnessForNativeProjection(
        unavailableSingletonProjection,
        recoveredFromTaskless.currentness,
      ),
      ranking: buildTaskMapTaskRankingPublication({
        projection: unavailableSingletonProjection,
        ownerScopeDigest: owner.ownerScopeDigest,
    ...
    await publishTaskMapNativeProjection(
      ...
      path.join(
        locations.runtimeRoot,
        "unavailable-singleton-publication-journal.v1.json",
      ),
      {
        graphInputDigest: digest("unavailable-singleton-graph"),
        candidateDigest: digest(unavailableSingletonCandidate),
        candidate:
          unavailableSingletonCandidate as unknown as Record<string, unknown>,
    ...
    const generationDirectory = path.join(
      path.dirname(locations.projectionPath),
      "taskmap-generations",
    );
    for (const entry of readdirSync(generationDirectory, {
      ...
      if (!existsSync(currentWorkPath)) continue;
      const currentWork = JSON.parse(readFileSync(
        currentWorkPath,
        "utf8",
      )) as { agentSessionTaskProofs?: unknown[] };
      if ((currentWork.agentSessionTaskProofs?.length ?? 0) > 0) {
        rmSync(path.join(generationDirectory, entry.name), {
          recursive: true,
          force: true,
        });
      }
    }
    const retainedProjectionBefore = readFileSync(locations.projectionPath);
    ...
    const retainedProjectionDigest = createHash("sha256")
      .update(retainedProjectionBefore)
      .digest("hex");
    assert.deepEqual(
      retainedSemanticGroupingMarker(
        unavailableSingletonProjection,
        retainedProjectionDigest,
        acceptanceStore,
    ...
    const legitimateRoot = unavailableSingletonProjection.roots.find((root) =>
      root.id === legitimateTask.rootId
    )!;
    assert.equal(
      retainedSemanticGroupingMarker(
        {
          ...
          edges: unavailableSingletonProjection.edges.filter((edge) =>
            edge.from === legitimateTask.id || edge.to === legitimateTask.id
          ),
        },
        retainedProjectionDigest,
        acceptanceStore,
      ...
      null,
      "one legitimate receipt-backed singleton is a valid boundary, not a failed grouping",
    );
    ...
    assert.equal(
      unavailableResult.publicationBlockReason,
      "accepted_membership_migration_unavailable",
    );
    assert.deepEqual(
      (unavailableResult as unknown as {
        semanticGroupingRetention: unknown;
      }).semanticGroupingRetention,
      {
        state: "retained_predecessor",
    ...
    assert.deepEqual(
      readFileSync(locations.projectionPath),
      retainedProjectionBefore,
      "retention truth must not rewrite the accepted projection",
    );
    assert.deepEqual(
      committedGeneration(locations.projectionPath).projection.tasks
        .map((task) => task.id).sort(),
      retainedTaskIdsBefore,
      "retention truth must not fabricate or mutate accepted task identity",
    );
    assert.deepEqual(
      committedGeneration(locations.projectionPath).projection.roots
        .map((root) => root.id).sort(),
      retainedRootIdsBefore,
      "retention truth must not fabricate or mutate root identity",
    );
    const unavailableStatus = artifact(
      locations.runtimeRoot,
      "taskmap-refresh-status.v1.json",
    );
    assert.deepEqual(
      unavailableStatus.semanticGroupingRetention,
      (unavailableResult as unknown as {
        semanticGroupingRetention: unknown;
      }).semanticGroupingRetention,
      "the durable status and reported refresh outcome must agree",
    );
    assert.equal(
      committedGeneration(locations.projectionPath).generationId,
      digest(unavailableSingletonCandidate),
      "unavailable migration must not publish a new singleton success",
    );
  });

  it("publishes an adopted agent proposal as one receipt-backed current and ready manual task", async () => {
    ...
    const work = (
      ...
      root,
      "/repo/task10-agent-adoption",
      "Implement receipt-backed proposal adoption",
      turn,
    );
    const originalAdmission = task4AgentAdmission(
    ...
    const agentSlice = () => {
      const collected = slice(
        "agent_session",
        currentAdmission.admissionDigest,
        currentAdmission.clusters[0]?.clusterIdentityDigest
          ?? "task10-agent-empty",
      );
      ...
      collected.sliceDigest = taskMapContractDigest(collected.value);
      return collected;
    };
    ...
    assert.equal(
      proposalGeneration.currentness.taskDispositions[0]?.disposition,
      "needs_lifecycle_review",
    );
    assert.deepEqual(
      proposalGeneration.ranking?.rankedAcceptedOpen,
      [],
    );
    assert.deepEqual(
      proposalGeneration.readyProofTargets?.proofTargets,
      [],
    );

    const extraction = buildAgentSessionExtractionFixture(
      originalAdmission,
      assessedAt,
    );
    const candidateProjection = buildTaskMapAgentSessionCandidateReview({
    ...
    const durableStore = JSON.parse(readFileSync(storePath, "utf8")) as {
      receipts: Array<{ promotionDigest: string }>;
    };
    assert.equal(durableStore.receipts.length, 1);
    assert.equal(
      durableStore.receipts[0]?.promotionDigest,
      adopted.receipt.promotionDigest,
    );
    ...
    const acceptedTasks = acceptedGeneration.projection.tasks.filter(
      (task) =>
    ...
    assert.equal(
      acceptedTask.taskHomePointerId,
      adopted.receipt.promotionId,
      "accepted identity must be anchored in the durable promotion receipt",
    );
    const acceptedSource = acceptedGeneration.projection.sources.find(
      (source) => source.id === adopted.receipt.promotionId,
    );
    ...
    assert.ok(
      acceptedTask.originPointerIds.includes(adopted.receipt.promotionId),
    );
    const stillProposed = acceptedGeneration.projection.tasks.filter(
    ...
    assert.deepEqual(
      acceptedGeneration.currentness.taskDispositions,
      [
        {
    ...
    assert.deepEqual(
      acceptedGeneration.readyProofTargets?.proofTargets.map(
    ...
    assert.equal(
      (
        acceptedGeneration.currentWork?.nextTaskToProve as
          | { taskId?: string }
          | undefined
      )?.taskId,
      acceptedTask.id,
    ...
    assert.equal(
      (await duplicateSupportRefresh.requestRefresh("manual")).refreshStatus,
      "published",
    );
    const duplicateGeneration = committedGeneration(locations.projectionPath);
    assert.equal(
      duplicateGeneration.projection.tasks.filter(
    ...
    const referencePath = nativeTaskMapGenerationReferencePath(
      locations.projectionPath,
    );
    const referenceBeforeMismatch = readFileSync(referencePath);
    const projectionBeforeMismatch = readFileSync(locations.projectionPath);
    const currentnessBeforeMismatch = readFileSync(locations.currentnessPath);
    const mismatched = new TaskMapNativeRefreshService({
      ...locations,
      ownerUserId: ownerLabel,
      collectors: sourceCollectors,
      readCandidateAcceptanceHeadDigest: async () =>
        digest("task10-mismatched-receipt-head"),
      nowMs: () => assessedAtMs,
    });
    const mismatchResult = await mismatched.requestRefresh("manual");
    assert.equal(mismatchResult.refreshStatus, "unavailable");
    assert.deepEqual(readFileSync(referencePath), referenceBeforeMismatch);
    assert.deepEqual(
      readFileSync(locations.projectionPath),
      projectionBeforeMismatch,
    );
    assert.deepEqual(
      readFileSync(locations.currentnessPath),
      currentnessBeforeMismatch,
    );
  });

  it("keeps a derived milestone current only while every cited source task remains current", () => {
    ...
    predecessor.taskDispositions = predecessor.taskDispositions.map((row) => ({
      ...row,
      disposition:
        row.taskId === sourceA.id || row.taskId === sourceB.id
          ? "current"
          : "needs_lifecycle_review",
    }));

    const supported = currentnessForNativeProjection(
      projection,
      predecessor,
    );
    const supportedByTask = new Map(
      supported.taskDispositions.map((row) => [
        row.taskId,
        row.disposition,
      ]),
    );
    ...
    assert.equal(
      supportedByTask.get(meetingOnly.id),
      "needs_lifecycle_review",
    );

    const missingSource = structuredClone(supported);
    missingSource.taskDispositions = missingSource.taskDispositions.map(
      (row) => row.taskId === sourceB.id
        ? { ...row, disposition: "needs_lifecycle_review" }
        : row,
    );
    assert.equal(
      missingSource.taskDispositions.find(
        (row) => row.taskId === derived.id,
      )?.disposition,
      "current",
    );
    const unsupported = currentnessForNativeProjection(
      projection,
      missingSource,
    );
    assert.equal(
      unsupported.taskDispositions.find((row) => row.taskId === derived.id)
        ?.disposition,
      "needs_lifecycle_review",
    );
  });

  it("publishes a bounded owner-only agent-session producer artifact and binds its exact result digest as context only", async () => {
    const locations = roots("taskmap-native-agent-producer-");
    const ownerUserId = "synthetic-agent-owner";
    const ownerScopeDigest = confirmedTestOwner(ownerUserId).ownerScopeDigest;
    const ownerCandidate = publicationCandidate(
      0,
      loaderCompatibleProjection(),
      ownerScopeDigest,
    );
    ...
    const codexRoot = path.join(sourceRoot, "codex");
    ...
    const recentSessionPath = path.join(codexRoot, "session.jsonl");
    writeFileSync(
      recentSessionPath,
      rawSession,
      { mode: 0o600 },
    );
    const oldSessionPath = path.join(codexRoot, "old-session.jsonl");
    writeFileSync(oldSessionPath, rawSession, { mode: 0o600 });
    const recentTime = new Date(
      nowMs - TASKMAP_AGENT_SESSION_CONTEXT_WINDOW_MS,
    );
    const oldTime = new Date(
      nowMs - TASKMAP_AGENT_SESSION_CONTEXT_WINDOW_MS - 1_000,
    );
    utimesSync(recentSessionPath, recentTime, recentTime);
    utimesSync(oldSessionPath, oldTime, oldTime);
    const agentSnapshotPath = path.join(
      sourceRoot,
    ...
    assert.equal(
      result.sourceStatuses.find(
        (status) => status.source === "agent_session",
      )?.state,
      "current",
    );
    assert.equal(statSync(agentSnapshotPath).mode & 0o777, 0o600);
    const agentSource = graphInput?.sources.find(
      (source) => source.source === "agent_session",
    );
    assert.match(agentSource?.revision ?? "", /^[a-f0-9]{64}$/);
    assert.equal(
      agentSource?.value?.metadata.producerResultDigest,
      agentSource?.revision,
    );
    ...
    assert.equal(
      Object.hasOwn(agentSource?.value ?? {}, "semanticAdmissions"),
      false,
      "the native source slice must persist only the v2 semantic admission",
    );
    ...
    const boundedProducer = readFileSync(agentSnapshotPath, "utf8");
    ...
    const smallSnapshot = JSON.parse(boundedProducer) as {
      coverage: string;
  ...
  it("publishes one review-only task from a fresh owner-bound Agent Session admission alone", async () => {
    ...
    const projection = JSON.parse(
      readFileSync(locations.projectionPath, "utf8"),
    ) as TaskMapProjectionV1;
    ...
    assert.deepEqual(
      projection.tasks[0].citations.map((citation) => citation.sourceKind),
      ["codex_session"],
    );
    assert.equal(
      result.sourceStatuses.find(
  ...
  it("publishes one agent task when one envelope repeats a normalized mention identity", async () => {
    const locations = roots("taskmap-native-agent-duplicate-mention-");
    const directive = "Implement duplicate mention folding";
    const agent = admittedAgentSessionSlice(
      directive,
      "agent-duplicate-mention",
    );
    const service = new TaskMapNativeRefreshService({
      ...locations,
      collectors: {
  ...
  it("retains published agent proposals and reports pending extraction when Station-1 is unavailable", async () => {
    const locations = roots("taskmap-native-agent-station-down-");
    let agent = admittedAgentSessionSlice(
      "Keep the previously extracted task",
      "agent-station-up",
    );
    const sourceCollectors = {
      agent_session: async () => agent,
    ...
    agent = admittedAgentSessionSlice(
      "Do not publish this uncached task",
      "agent-station-down",
    );
    ...
    assert.deepEqual(
      committedGeneration(locations.projectionPath).projection.tasks.map(
        ({ id, title }) => ({ id, title }),
      ),
      predecessor.tasks.map(({ id, title }) => ({ id, title })),
    );
    assert.deepEqual(
      degraded.sourceStatuses.find(
        (status) => status.source === "agent_session",
      ),
      {
    ...
    agent = admittedAgentSessionSlice(
      "Keep pending after report-level extraction failure",
    ...
    assert.equal(
      reportFailure.sourceStatuses.find(
        (status) => status.source === "agent_session",
  ...
  it("runs Station-2 on projection work text, never source digests, and replays it", async () => {
    ...
    const serviceOptions = {
      ...
      createAgentSessionExtractionStation: async () => {
        ...
        return {
          ...extractionStation,
          async run(request: LlmStationRequest) {
      ...
      createIdentityAdjudicationStation: async (): Promise<LlmStation> => {
        stationCalls += 1;
        return {
          provider: {
    ...
    const first = await new TaskMapNativeRefreshService(serviceOptions)
      .requestRefresh("manual");
    assert.deepEqual(first.stationStatuses?.find(
      (status) => status.stationId === "identity-adjudication-v1",
    ), {
      stationId: "identity-adjudication-v1",
      state: "current",
      pendingCount: 0,
    ...
    assert.equal(
      embeddedTexts.some((text) => /(?:^|:)[a-f0-9]{64}$/.test(text)),
      false,
    );
    ...
    const artifactName = readdirSync(identityDirectory)[0]!;
    const artifactBytes = readFileSync(
      path.join(identityDirectory, artifactName),
      "utf8",
    );
    assert.equal(artifactBytes.includes('"aliasesWritten":false'), true);
    const projectionBytes = readFileSync(locations.projectionPath, "utf8");
    assert.equal(projectionBytes.includes("tmidentitypair_"), false);
    const identitySurface = await loadTaskMapLlmProposalSurface({
      taskMapRoot: path.dirname(locations.projectionPath),
      ownerScopeDigest: TEST_OWNER_SCOPE.ownerScopeDigest,
      projection: JSON.parse(projectionBytes) as TaskMapProjectionV1,
    });
    ...
    assert.equal(embeddingCalls, 1);
    assert.equal(stationCalls, 1);
    assert.equal(readFileSync(locations.projectionPath, "utf8"), projectionBytes);
  });

  it("shares one bounded remote request group across refresh stations and embedding batches", async () => {
    ...
    const remoteFetch = (async (
      ...
      const prompt = JSON.parse(String(body.prompt_text)) as {
        pairs?: Array<{ pairId: string }>;
  ...
  it("runs Station-3 after projection construction, persists proposal-only results, and replays them", async () => {
    const locations = roots("taskmap-native-decomposition-station-");
    writeReferencedPublicationPredecessor(
      locations.projectionPath,
      locations.currentnessPath,
      rankedExecutablePublicationCandidate(),
      "decomposition-station-predecessor",
    );
    const meetingSnapshotPath = path.join(
      path.dirname(locations.runtimeRoot),
      "meeting-producer-snapshot.v1.json",
    ...
    const serviceOptions = {
      ...
      createDecompositionStation: async (): Promise<LlmStation> => ({
        provider: {
          transport: "gemini-remote",
          executable: "",
          args: [],
    ...
    assert.deepEqual(first.stationStatuses?.find(
      (status) => status.stationId === "task-decomposition-v1",
    ), {
      stationId: "task-decomposition-v1",
      state: "current",
      pendingCount: 0,
    ...
    const artifactName = readdirSync(artifactDirectory)[0]!;
    const artifactBytes = readFileSync(
    ...
    const decompositionSurface = await loadTaskMapLlmProposalSurface({
      taskMapRoot: path.dirname(locations.projectionPath),
      ownerScopeDigest: TEST_OWNER_SCOPE.ownerScopeDigest,
      projection: JSON.parse(projectionBytes) as TaskMapProjectionV1,
    });
    ...
    assert.equal(replayed.stationStatuses?.find(
      (status) => status.stationId === "task-decomposition-v1",
    )?.state, "current");
    assert.equal(readFileSync(locations.projectionPath, "utf8"), projectionBytes);
  });

  for (const source of ["agent_session", "calendar"] as const) {
    for (const degradation of [
      ...
      it(`publishes zero new ${source} proposals on ${degradation.name} and backfills the pending unit after recovery`, async () => {
        const locations = roots(
          `taskmap-native-task9-${source}-${degradation.code}-`,
        );
        ...
        const agentSlice = source === "agent_session"
          ? (() => {
              const value = slice(
                "agent_session",
                `task9-agent-${degradation.code}-revision`,
              );
              ...
              value.sliceDigest = taskMapContractDigest(value.value);
              return value;
            })()
          : null;
        const calendarExportPath = path.join(
          path.dirname(locations.runtimeRoot),
          "calendar-export.json",
        );
        if (source === "calendar") {
          const eventIdentityDigest = taskMapContractDigest(
            `task9-calendar-event-${degradation.code}`,
          );
              ...
              buildTaskMapLocalCalendarExport({
                ...
                events: [{
                  ...
                  revisionDigest: taskMapCalendarFieldDigest(
                    TASKMAP_CALENDAR_EVENT_REVISION_DOMAIN,
                    [eventIdentityDigest, targetTitle, startAt, endAt],
                  ),
                  title: targetTitle,
                  startAt,
                  endAt,
        ...
        const degradedFactory = async (): Promise<LlmStation> => {
        ...
        const commonOptions: TestRefreshServiceOptions = {
          ...locations,
          meetingProducerSnapshotPath: meetingSnapshotPath,
        ...
        const degraded = await new TaskMapNativeRefreshService({
          ...
          ...(source === "agent_session"
            ...
            : degradation.code === "prompt_template_missing"
              ? {
                  calendarExtractionPromptTemplatePath: path.join(
                    locations.runtimeRoot,
                    "missing-calendar-prompt.md",
                  ),
        ...
        assert.equal(
          degradedFactoryCalls,
          degradation.code === "prompt_template_missing" ? 0 : 1,
        );
        assert.equal(
          degradedRunCalls,
          degradation.code === "no_provider"
            || degradation.code === "remote_consent_required"
            || degradation.code === "prompt_template_missing"
            ? 0
            : 1,
        );
        const degradedSourceStatus = degraded.sourceStatuses.find(
        ...
        assert.equal(
          degradedSourceStatus?.stationDegradationCode,
          degradation.code,
        );
        assert.equal(degradedSourceStatus?.stationPendingCount, 1);
        assert.equal(
          degradedSourceStatus?.lastSuccessAtMs,
          source === "agent_session"
            ? assessedAtMs
            : Date.parse("2026-08-03T20:04:00.000Z"),
        );
        assert.equal(
          degradedSourceStatus?.nextDueAtMs,
          (degradedSourceStatus?.lastSuccessAtMs ?? 0) + 4 * 60 * 60 * 1_000,
        );
        const meetingStatus = degraded.sourceStatuses.find(
          (status) => status.source === "meeting_notes",
        );
        ...
        const degradedProjection = committedGeneration(
          locations.projectionPath,
        ).projection;
        ...
        assert.ok(acceptedPredecessorIds.every((taskId) =>
          degradedProjection.tasks.some((task) => task.id === taskId)
        ));

        let recoveryFactoryCalls = 0;
        let recoveryRunCalls = 0;
        const recoveryFactory = async (): Promise<LlmStation> => {
        ...
        const recoveredShelf = source === "agent_session"
          ...
          : buildTaskMapCalendarCandidateReview({
              ...
              extraction: JSON.parse(readFileSync(path.join(
                locations.runtimeRoot,
                "taskmap-calendar-extraction-report.v1.json",
              ), "utf8")) as TaskMapCalendarExtractionReportV1,
              previous: null,
              expectedOwnerScopeDigest: TEST_OWNER_SCOPE.ownerScopeDigest,
              assessedAt: recoveredAssessedAt,
            }).shelf;
        ...
        assert.ok(acceptedPredecessorIds.every((taskId) =>
          recoveredProjection.tasks.some((task) => task.id === taskId)
        ));
        assert.equal(
          JSON.stringify(recoveredProjection).includes("Agent workstream"),
          false,
        );
      });
    }
  ...
  it("replays unchanged Agent Session and Calendar inputs without selecting or running Station-1", async () => {
    ...
    agent.value.semanticAdmission = buildTaskMapAgentSessionSemanticAdmission(
      buildTaskMapAgentSessionProducerSnapshot({
        ownerScopeDigest: TEST_OWNER_SCOPE.ownerScopeDigest,
        producedAt: "2026-08-03T20:04:00.000Z",
        observations: [task4AgentObservation({
    ...
    const eventIdentityDigest = taskMapContractDigest(
      "task9-station-replay-calendar-event",
    );
    const startAt = "2026-08-03T21:00:00.000Z";
    const endAt = "2026-08-03T21:30:00.000Z";
    writeFileSync(
      calendarExportPath,
      taskMapLocalCalendarExportCanonicalJson(buildTaskMapLocalCalendarExport({
        ownerScopeDigest: TEST_OWNER_SCOPE.ownerScopeDigest,
        producedAt: "2026-08-03T20:04:00.000Z",
        events: [{
          eventIdentityDigest,
          ...
          revisionDigest: taskMapCalendarFieldDigest(
            TASKMAP_CALENDAR_EVENT_REVISION_DOMAIN,
            [eventIdentityDigest, calendarTitle, startAt, endAt],
          ),
          title: calendarTitle,
          startAt,
          endAt,
    ...
    const firstFactory = async (): Promise<LlmStation> => {
    ...
    const firstStatus = artifact(
      locations.runtimeRoot,
      "taskmap-refresh-status.v1.json",
    );
    const firstProjection = readFileSync(locations.projectionPath);
    const secondCalls = { factory: 0, run: 0 };
    const secondFactory = async (): Promise<LlmStation> => {
    ...
    assert.deepEqual(readFileSync(locations.projectionPath), firstProjection);
  });

  it("publishes a valid Agent Session task when Meet notes are fresh-empty", async () => {
    ...
    const service = new TaskMapNativeRefreshService({
      ...
      collectors: {
        agent_session: async () => admittedAgentSessionSlice(
          "Review the quarterly customer research summary",
          "agent-with-empty-meet",
        ),
        calendar: async () => slice("calendar"),
        body: async () => slice("body"),
      },
    ...
    assert.deepEqual(
      generation.readyProofTargets?.proofTargets,
      [],
    );
  });

  it("rebuilds all latest-per-root Agent Session history after derived state is wiped", async () => {
    ...
    agent.value.semanticAdmission = task4AgentAdmission([
      task4Work(
        "agent-history-codex-root",
        "/repo/agent-history-codex",
        "Restore the Codex-side Task Map workflow",
        "agent-history-codex-turn",
      ),
      task4Work(
        "agent-history-claude-root",
        "/repo/agent-history-claude",
        "Restore the Claude-side Task Map workflow",
        "agent-history-claude-turn",
        "claude",
      ),
    ]);
    agent.sliceDigest = taskMapContractDigest(agent.value);
    ...
    assert.deepEqual(
      [...new Set(projection.sources.map((source) => source.sourceKind))]
        .sort(),
      ["claude_session", "codex_session"],
    );
    assert.ok(
      projection.tasks.every(
        (task) => task.reviewState === "proposed" && task.authority === "none",
      ),
    );
  });

  it("falls back to a valid Agent Session task when Meet notes have no eligible work", async () => {
    ...
    const service = new TaskMapNativeRefreshService({
      ...
      collectors: {
        agent_session: async () => admittedAgentSessionSlice(
          "Prepare the support handoff for the next release",
          "agent-with-ineligible-meet",
        ),
        calendar: async () => slice("calendar"),
        body: async () => slice("body"),
      },
  ...
  it("keeps the accepted predecessor when only a new Agent Session directive is available", async () => {
    ...
    const service = new TaskMapNativeRefreshService({
      ...
      collectors: {
        agent_session: async () => admittedAgentSessionSlice(
          "Draft the migration notice for affected customers",
          "agent-replaces-predecessor",
        ),
        calendar: async () => slice("calendar"),
        body: async () => slice("body"),
  ...
  it("replaces a proposed Agent Session predecessor with the latest admitted directive", async () => {
    ...
    const first = new TaskMapNativeRefreshService({
      ...
      collectors: {
        agent_session: async () => admittedAgentSessionSlice(
          "The following is obsolete continuation metadata",
          "agent-proposed-old",
        ),
        calendar: async () => slice("calendar"),
        body: async () => slice("body"),
    ...
    const currentAdmissionSlice = admittedAgentSessionSlice(
      "Repair the Task Map publication",
      "agent-proposed-current",
    );
    const second = new TaskMapNativeRefreshService({
      ...locations,
      meetingProducerSnapshotPath: snapshotPath,
    ...
    noise.sliceDigest = taskMapContractDigest(noise.value);
  ...
  it("publishes candidates from both eligible Meet notes and an admitted Agent Session", async () => {
    ...
    const service = new TaskMapNativeRefreshService({
      ...
      collectors: {
        agent_session: async () => admittedAgentSessionSlice(
          "Summarize the onboarding feedback for the product team",
          "agent-and-meet",
        ),
        calendar: async () => slice("calendar"),
        body: async () => slice("body"),
    ...
    assert.equal(
      projection.tasks.some(({ title }) =>
        title === "Summarize the onboarding feedback for the product team"
      ),
      true,
    );
    assert.equal(
    ...
    assert.equal(
      taskMapProjectionArtifactValidationReasons(projection).length,
      0,
    );
  });

  it("keeps Meeting roots and tasks when the Agent Plan2 filter is active", async () => {
    ...
    const owner = confirmedTestOwner(ownerUserId);
    ...
    const observations = [
      task4Work(
        "plan2-passthrough-root",
        "/repo/plan2-passthrough",
        directive,
        "plan2-passthrough-turn",
      ),
      task4Work(
        "plan2-passthrough-verify",
        "/repo/plan2-passthrough-verify",
        secondDirective,
        "plan2-passthrough-verify-turn",
      ),
    ];
    const communityStation: LlmStation = {
      provider: {
        transport: "claude-cli",
        executable: "/fixture/provider",
        args: [],
    ...
    assert.equal(
      projection.tasks.some(({ title }) => title === directive),
      true,
    );
    assert.equal(
      projection.roots.some(({ title }) =>
        title === "Plan2 passthrough workstream"
      ),
      true,
    );
    const meetingTask = projection.tasks.find(({ title }) =>
      title === "Ship the native semantic refresh"
    )!;
    assert.equal(
      projection.roots.some((root) =>
        root.id === meetingTask.rootId
        && root.title !== "Plan2 passthrough workstream"
      ),
      true,
    );
    assert.equal(
  ...
  it("does not publish an empty genesis when only context receipts are current", async () => {
    ...
    assert.equal(existsSync(locations.projectionPath), false);
    assert.equal(existsSync(locations.currentnessPath), false);
    assert.equal(
      existsSync(nativeTaskRankingPath(locations.projectionPath)),
      false,
    );
    assert.deepEqual(
      result.sourceStatuses.map((status) => [status.source, status.state]),
      [
  ...
  it("publishes extracted task topics when live community planning is unavailable", async () => {
    ...
    const observations = [
      task4Work(
        "taskless-semantic-genesis-root-a",
        "/repo/taskless-semantic-genesis",
        directive,
      ),
      task4Work(
        "taskless-semantic-genesis-root-b",
        "/repo/taskless-semantic-genesis",
        "Group connected work into semantic Roadmap topics",
      ),
      task4Work(
        "taskless-semantic-genesis-root-c",
        "/repo/taskless-semantic-genesis",
        "Keep the candidate list selectable on the main page",
      ),
      task4Work(
        "taskless-semantic-genesis-root-d",
        "/repo/taskless-semantic-genesis",
        "Prevent accepted empty Roadmap publications",
      ),
      task4Work(
        "taskless-semantic-genesis-root-e",
        "/repo/taskless-semantic-genesis",
        "Verify the connected Roadmap still contains work",
      ),
    ];
    const admission = task4AgentAdmission(
      observations,
    ...
    assert.equal(existsSync(locations.projectionPath), true);
    assert.equal(existsSync(locations.currentnessPath), true);
    const projection = JSON.parse(
      readFileSync(locations.projectionPath, "utf8"),
    ) as TaskMapProjectionV1;
    ...
    assert.ok(projection.roots.every((root) =>
      projection.tasks.some((task) => task.rootId === root.id)
    ));
    const assessedAt = "2026-07-30T08:01:00.000Z";
    const extraction = await loadVerifiedTaskMapAgentSessionExtractionReport({
      admission,
    ...
    assert.ok(
      candidateReview.shelf.candidates.length > 0,
      "candidate work remains available to the inline Roadmap surface",
    );
  });

  it("reads a huge compacted Codex root through a bounded head and tail while retaining the latest explicit work episode", async () => {
    ...
    const hugePath = path.join(codexRoot, "huge-session.jsonl");
    const head = [
      {
      ...
      },
      {
        ...
        payload: {
          ...
          content: [{
            type: "input_text",
            text:
              "Review /Users/private-owner/private/task.md using api_key=private-secret-value",
          }],
        },
      },
      {
      ...
      },
      {
        ...
        payload: {
          ...
          content: [{
            type: "input_text",
            text:
              "The conversation history was compacted. Repeat the old task.",
          }],
        },
      },
      {
      ...
      },
      {
        ...
        payload: {
          ...
          content: [{
            type: "input_text",
            text:
              "<recommended_plugins><plugin>continuity</plugin></recommended_plugins>\n让这个 leaf node 先批准再 run，完成后保留 artifact；如果没有兄弟 leaf，就拆掉母节点。",
          }],
        },
      },
      {
        ...
        payload: {
          ...
          content: [{
            type: "output_text",
            text:
              "Prepared /tmp/private/report.html with Bearer private-secret-token-value.",
          }],
        },
      },
      {
    ...
    const descriptor = openSync(hugePath, "w", 0o600);
    try {
      writeSync(descriptor, head, 0, "utf8");
      const sparseGapEnd = 617 * 1_024 * 1_024;
      ftruncateSync(descriptor, sparseGapEnd);
      let position = sparseGapEnd;
      position += writeSync(descriptor, "\n", position, "utf8");
      position += writeSync(
        descriptor,
        `${giantToolLine}\n`,
        position,
      ...
      writeSync(descriptor, tailRows, position, "utf8");
    } finally {
      closeSync(descriptor);
    }
    writeFileSync(
      path.join(codexRoot, "later-unrelated-session.jsonl"),
      [
        JSON.stringify({
    ...
    writeFileSync(
      path.join(codexRoot, "stale-complete-loop-session.jsonl"),
      [
        JSON.stringify({
          timestamp: "2026-07-30T07:00:00.000Z",
          type: "session_meta",
    ...
    assert.ok(
      statSync(hugePath).size
        > TASKMAP_AGENT_SESSION_PRODUCER_LIMITS_V1.maxTailScanBytes
    ...
    assert.equal(result.refreshStatus, "published");
    const serializedSnapshot = readFileSync(agentSnapshotPath, "utf8");
    assert.ok(
      Buffer.byteLength(serializedSnapshot, "utf8")
        <= TASKMAP_AGENT_SESSION_PRODUCER_LIMITS_V1.maxSnapshotBytes,
    );
    ...
    assert.deepEqual(
      snapshot.sessions.map((session) => session.userDirectiveSummary)
        .sort(),
      [
        "Complete the loop",
    ...
    const latest = [...snapshot.sessions].sort(
      (left, right) =>
        Date.parse(right.occurredAt) - Date.parse(left.occurredAt),
    )[0]!;
    assert.equal(
      latest.userDirectiveSummary,
      "Prepare the customer launch checklist",
    );
    const ordinaryDirective = snapshot.sessions.find(
      (episode) =>
        episode.userDirectiveSummary === "Prepare the customer launch checklist",
    )!;
    const completeLoop = snapshot.sessions.find(
      (episode) => episode.userDirectiveSummary.includes("artifact"),
    )!;
    const laterSameRootStatus = snapshot.sessions.find(
      (episode) =>
        episode.userDirectiveSummary === "Why is the current goal blocked?",
    )!;
    assert.equal(
      laterSameRootStatus.rootSessionIdentityDigest,
      completeLoop.rootSessionIdentityDigest,
    );
    assert.ok(
      Date.parse(laterSameRootStatus.occurredAt)
        > Date.parse(completeLoop.occurredAt),
    );
    assert.equal(
      completeLoop.assistantOutcomeSummary,
      "Prepared [local-path] with Bearer [credential]",
    );
    const expectedRootIdentityDigest = taskMapContractDigest({
      domain: TASKMAP_AGENT_SESSION_IDENTITY_DOMAIN,
      provider: "codex",
    ...
    assert.equal(
      completeLoop.episodeIdentityDigest,
      taskMapContractDigest({
        domain: TASKMAP_AGENT_SESSION_EPISODE_IDENTITY_DOMAIN,
        provider: "codex",
    ...
    for (const forbidden of [
      ...
      assert.equal(
        serializedSnapshot.includes(forbidden),
        false,
        forbidden,
      );
    }
    ...
    const selectedLatest = [...latestByRoot.values()].sort((left, right) =>
      left.episodeId.localeCompare(right.episodeId)
    );
    assert.equal(semanticClusters.length, 3);
    assert.equal(
      new Set(
        semanticClusters.flatMap(
          (cluster) => cluster.supports.map(
            (support) => support.rootSessionIdentityDigest,
          ),
        ),
      ).size,
    ...
    assert.deepEqual(
      semanticClusters.map(
        (cluster) => cluster.userDirectiveSummary,
      ).sort(),
      selectedLatest.map((episode) => episode.userDirectiveSummary).sort(),
    );
    const ordinaryAdmission = semanticClusters.find(
      (cluster) => cluster.supports.some(
        (support) =>
          support.episodeIdentityDigest
            === ordinaryDirective.episodeIdentityDigest,
      ),
    );
    assert.ok(ordinaryAdmission);
    assert.equal(
      ordinaryAdmission.userDirectiveSummary,
      ordinaryDirective.userDirectiveSummary,
    );
    assert.equal(ordinaryAdmission.routingKind, "repository");
    assert.equal(ordinaryAdmission.authority, "none");
    assert.equal(
      JSON.stringify(agentSource?.value).includes(
        ordinaryDirective.userDirectiveSummary,
      ),
      true,
    );
  ...
  it("fails closed when the authenticated agent-session producer changes after the source barrier", async () => {
    ...
    mkdirSync(codexRoot, { recursive: true, mode: 0o700 });
    writeFileSync(
      path.join(codexRoot, "session.jsonl"),
      [
        JSON.stringify({
          timestamp: "2026-07-30T06:00:00.000Z",
          type: "session_meta",
      ...
      collectors: {
        ...
        body: async () => {
          for (let index = 0; index < 200; index += 1) {
            if (existsSync(agentSnapshotPath)) break;
            await new Promise((resolve) => setTimeout(resolve, 1));
          }
          assert.equal(existsSync(agentSnapshotPath), true);
          writeFileSync(
            agentSnapshotPath,
            '{"swapped_after_collection":true}\n',
            { mode: 0o600 },
          );
          return slice("body");
        },
    ...
    assert.equal(
      (
        state.lastSourceStatuses as Array<{
          source: string;
          disposition: string;
        }>
      ).find((status) => status.source === "agent_session")?.disposition,
      "unavailable",
    );
    assert.equal(
      Object.hasOwn(
        state.lastSourceSuccessAtMs as Record<string, unknown>,
        "agent_session",
      ),
      false,
    );
  ...
  it("fails closed without advancing Meeting success when GDocs disappears after the source barrier", async () => {
    await assertMeetingBarrierRaceFailsClosed("disappears");
  });

  it("fails closed without advancing Meeting success when GDocs changes after the source barrier", async () => {
    await assertMeetingBarrierRaceFailsClosed("changes");
  });

  it("counts a current owner-receipted Granola export as Meeting context without granting semantic authority", async () => {
    const locations = roots("taskmap-native-granola-context-");
    const ownerUserId = "synthetic-granola-context-owner";
    const confirmedOwner = confirmedTestOwner(ownerUserId);
    const ownerScope = createTaskMapOwnerScope(
      confirmedOwner.userId,
      confirmedOwner.homeDirectory,
    );
    const successAt = "2026-08-03T20:00:00Z";
    const successAtMs = Date.parse(successAt);
    mkdirSync(ownerScope.sourceRoot, { recursive: true, mode: 0o700 });
    const granolaPath = path.join(
    ...
    writeFileSync(granolaPath, granolaBytes, { mode: 0o600 });
    utimesSync(
      granolaPath,
      new Date(successAtMs + 3_000),
      new Date(successAtMs + 3_000),
    );
    writeFileSync(
      path.join(ownerScope.sourceRoot, "taskmap-resident-receipt.v1.json"),
      JSON.stringify({
        contractVersion: "taskmap-resident-receipt.v1",
    ...
    const service = new TaskMapNativeRefreshService({
      ...
      collectors: {
        agent_session: async () => admittedAgentSessionSlice(
          "Keep the accepted owner work current",
          "granola-context-agent",
    ...
    assert.equal(result.refreshStatus, "published", JSON.stringify(result));
    const meetingStatus = result.sourceStatuses.find(
      (status) => status.source === "meeting_notes",
    );
    assert.equal(meetingStatus?.state, "current");
    ...
    const state = artifact(
      ...
      sources: Record<string, { value: TaskMapNativeSafeSlice }>;
    };
    assert.equal(
      state.sources.meeting_notes.value.metadata.granolaCurrent,
      true,
    );
    assert.equal(
      state.sources.meeting_notes.value.metadata.contextOnly,
      true,
    );
  });

  it("ignores a legacy global Granola snapshot even when an owner receipt names its digest", async () => {
    ...
    mkdirSync(path.dirname(legacyGlobalPath), {
      recursive: true,
      mode: 0o700,
    });
    writeFileSync(legacyGlobalPath, granolaBytes, { mode: 0o600 });
    utimesSync(
      legacyGlobalPath,
      new Date(successAtMs),
      new Date(successAtMs),
    );
    mkdirSync(ownerScope.sourceRoot, { recursive: true, mode: 0o700 });
    writeFileSync(
    ...
    const service = new TaskMapNativeRefreshService({
      ...
      collectors: {
        agent_session: async () => admittedAgentSessionSlice(
          "Keep the accepted owner work current",
          "granola-global-isolation-agent",
          ownerScope.ownerScopeDigest,
        ),
        calendar: async () => slice("calendar"),
        body: async () => slice("body"),
  ...
  it("excludes wrong-owner GDocs bytes and revisions while retaining valid owner-receipted Granola context", async () => {
    ...
    assert.equal(firstResult.refreshStatus, "published", JSON.stringify(
      firstResult,
    ));
    assert.equal(
      firstResult.sourceStatuses.find(
        (status) => status.source === "meeting_notes",
      )?.state,
      "current",
    );
    const firstState = artifact(
      locations.runtimeRoot,
      "taskmap-refresh-state.v1.json",
    ) as {
    ...
    writeMeetingProducerSnapshot(gdocsPath, {
      ...
      meetings: [
        meetingProducerMeeting(
          "wrong-owner-gdocs-b",
          "2026-08-03T19:20:00.000Z",
        ),
        meetingProducerMeeting(
          "wrong-owner-gdocs-c",
          "2026-08-03T19:25:00.000Z",
        ),
      ],
    });
    nowMs += 5 * 60 * 1_000;
    ...
    assert.equal(replayResult.refreshStatus, "no_op", JSON.stringify(
      replayResult,
    ));
    const replayState = artifact(
      locations.runtimeRoot,
      "taskmap-refresh-state.v1.json",
  ...
  it("does not let the legacy Granola warm-tier stamp authorize the promoted provider snapshot", async () => {
    ...
    writeFileSync(granolaPath, JSON.stringify({
      events: [],
      meeting_notes: [],
    }), { mode: 0o600 });
    utimesSync(granolaPath, new Date(successAtMs), new Date(successAtMs));
    writeFileSync(
      path.join(ownerScope.sourceRoot, "taskmap-resident-receipt.v1.json"),
      JSON.stringify({
        contractVersion: "taskmap-resident-receipt.v1",
  ...
  it("rejects Granola bytes that do not match the owner success receipt digest", async () => {
    ...
    writeFileSync(granolaPath, replacedBytes, { mode: 0o600 });
    utimesSync(granolaPath, new Date(successAtMs), new Date(successAtMs));
    writeFileSync(
      path.join(ownerScope.sourceRoot, "taskmap-resident-receipt.v1.json"),
      JSON.stringify({
  ...
  it("accepts the native app's current owner-scoped Local Calendar export without Google", async () => {
    ...
    mkdirSync(ownerScope.sourceRoot, { recursive: true, mode: 0o700 });
    writeFileSync(
      path.join(ownerScope.sourceRoot, "calendar-export.json"),
    ...
    const calendarExtractionPath = path.join(
      locations.runtimeRoot,
      "taskmap-calendar-extraction-report.v1.json",
    );
    assert.equal(existsSync(calendarExtractionPath), true, JSON.stringify(result));
    const calendarExtraction = JSON.parse(
      readFileSync(calendarExtractionPath, "utf8"),
    ) as { pendingCount: number; segments: Array<{ mentions: unknown[] }> };
    ...
    assert.equal(
      result.sourceStatuses.find(
        (status) => status.source === "calendar",
      )?.state,
      "current",
    );
    assert.deepEqual(
      (result.calendarProviderStatuses ?? []).map((status) => [
        status.provider,
        status.state,
      ]),
      [
    ...
    const changedRevision = taskMapCalendarFieldDigest(
      TASKMAP_CALENDAR_EVENT_REVISION_DOMAIN,
      [eventIdentityDigest, changedTitle, startAt, endAt],
    );
    writeFileSync(
      path.join(ownerScope.sourceRoot, "calendar-export.json"),
      taskMapLocalCalendarExportCanonicalJson(buildTaskMapLocalCalendarExport({
    ...
    assert.deepEqual(
      degraded.sourceStatuses.find((status) => status.source === "calendar"),
      {
        source: "calendar",
        disposition: "fresh",
        stationDegradationCode: "no_provider",
    ...
    assert.equal(
      JSON.stringify(committedGeneration(locations.projectionPath).projection)
        .includes(changedTitle),
      false,
    );
  });

  it("collects a four-hour logical Calendar source from local and Google providers without minting task authority", async () => {
    ...
    const localExportPath = path.join(sourceRoot, "local.json");
    const googleSnapshotPath = path.join(sourceRoot, "google.json");
    const producedAt = "2026-07-30T12:00:00.000Z";
    const crossProviderIdentityDigest = taskMapContractDigest(
      "shared-calendar-occurrence",
    );
    const row = (
      name: string,
      cross: string | null,
    ) => {
      const eventIdentityDigest =
        taskMapContractDigest(`event:${name}`);
    ...
    writeFileSync(
      localExportPath,
      taskMapLocalCalendarExportCanonicalJson(buildTaskMapLocalCalendarExport({
        ownerScopeDigest: confirmedTestOwner(ownerUserId).ownerScopeDigest,
        producedAt,
        events: [
    ...
    assert.equal(result.refreshStatus, "published");
    const calendarStatus = result.sourceStatuses.find(
      (status) => status.source === "calendar",
    );
    assert.equal(calendarStatus?.state, "current");
    assert.equal(
      calendarStatus?.lastSuccessAtMs,
      Date.parse(producedAt),
    );
    assert.deepEqual(
      result.calendarProviderStatuses,
      [
        {
    ...
    const calendarSlice = graphInput?.sources.find(
      (source) => source.source === "calendar",
    )?.value;
    assert.equal(calendarSlice?.recordCount, 2);
    assert.equal(
      calendarSlice?.metadata.localProviderFreshness,
      "current",
    );
    assert.equal(
      calendarSlice?.metadata.googleProviderFreshness,
      "current",
    );
    assert.equal(calendarSlice?.metadata.contextOnly, true);
    assert.equal(
      calendarSlice?.metadata.boundedSemanticTitleCount,
      2,
    );
    ...
    assert.deepEqual(
      boundary.calendarProviderStatuses?.map((row) => ({
    ...
    assert.deepEqual(
      stale.calendarProviderStatuses,
      [
        {
          provider: "local_calendar",
          state: "retained",
  ...
  it("retains Strategy predecessor state as unavailable instead of republishing stale coverage", async () => {
    const fixture = await strategyFallbackFixture(
      "taskmap-native-strategy-fallback-",
    );
    const service = new TaskMapNativeRefreshService({
      ...fixture.locations,
    ...
    assert.equal(existsSync(nativeTaskRankingPath(fixture.locations.projectionPath)), true);
    const strategyEvidencePath = path.join(
      path.dirname(fixture.locations.projectionPath),
      TASKMAP_STRATEGY_SOURCE_EVIDENCE_FILENAME,
    );
    const strategyEvidence = assertTaskMapSourceSnapshot(JSON.parse(
      readFileSync(strategyEvidencePath, "utf8"),
    ));
    assert.equal(strategyEvidence.envelopes.length, 1);
    assert.equal(strategyEvidence.envelopes[0]!.sourceKind, "strategy");
    const projection = JSON.parse(
      readFileSync(fixture.locations.projectionPath, "utf8"),
    ) as TaskMapProjectionV1;
    assert.equal(projection.tasks.length, 1);
    assert.ok(projection.roots.every((root) =>
      root.causalGrade === "C0_NO_DATA"
      || root.causalGrade === "C1_CORRELATION"
    ));
  });

  it("does not publish a body assessment through the retired Strategy fallback", async () => {
    const fixture = await strategyFallbackFixture(
      "taskmap-native-strategy-body-assessment-",
    );
    const projectionBefore = readFileSync(
      fixture.locations.projectionPath,
    );
    const currentnessBefore = readFileSync(
      fixture.locations.currentnessPath,
    );
    const physiologicalSnapshotPath = path.join(
      path.dirname(fixture.locations.runtimeRoot),
      "private",
    ...
    assert.equal(first.publicationVerified, false);
    assert.equal(
      first.publicationBlockReason,
      "predecessor_continuity_required",
    );
    assert.equal(existsSync(bodyAssessmentPath), false);
    assert.deepEqual(
      readFileSync(fixture.locations.projectionPath),
      projectionBefore,
    );
    assert.deepEqual(
      readFileSync(fixture.locations.currentnessPath),
      currentnessBefore,
    );
    ...
    assert.equal(second.publicationVerified, false);
    assert.equal(
      second.publicationBlockReason,
      "predecessor_continuity_required",
    );
    assert.equal(providerReads, 2);
    assert.equal(
      second.sourceStatuses.find((status) => status.source === "body")
        ?.state,
      "retained",
    );
    assert.equal(existsSync(bodyAssessmentPath), false);
  ...
  it("does not republish body-informed Strategy work without four-family ranking coverage", async () => {
    const fixture = await strategyFallbackFixture(
      "taskmap-native-strategy-body-informed-",
      { bodyInformedReady: true },
    );
    const projectionBefore = readFileSync(
      fixture.locations.projectionPath,
    );
    ...
    const service = new TaskMapNativeRefreshService({
      ...
      readPhysiologicalProviderContext:
        oneDayBodyInformedLiveOuraContext,
      strategyFallback: {
        homeDirectory: fixture.homeDirectory,
        readAdapterInput: fixture.readAdapterInput,
      },
  ...
  it("keeps an accepted assessment baseline eligible when only non-published task identities churn", async () => {
    const fixture = await strategyFallbackFixture(
      "taskmap-native-strategy-body-task-identity-",
      {
        currentStrategyCount: 2,
        reviewStrategyCount: 2,
      },
    );
    const fixed = JSON.parse(readFileSync(
      fixture.locations.projectionPath,
      "utf8",
    )) as TaskMapProjectionV1;
    ...
    for (const root of baseline.roots) {
      root.taskIds = root.taskIds.map((taskId) =>
        taskId === fixed.tasks[0]!.id ? replacementTaskId : taskId
      );
    }
    for (const edge of baseline.edges) {
      if (edge.from === fixed.tasks[0]!.id) edge.from = replacementTaskId;
      if (edge.to === fixed.tasks[0]!.id) edge.to = replacementTaskId;
    }
  ...
  it("does not reassess a Strategy predecessor as a new successful publication after restart", async () => {
    const fixture = await strategyFallbackFixture(
      "taskmap-native-strategy-body-restart-",
    );
    const physiologicalSnapshotPath = path.join(
      path.dirname(fixture.locations.runtimeRoot),
      "private",
    ...
    const options: FetchOuraTaskMapContextOptions = {
      startDate: "2026-05-01",
      endDate: "2026-07-29",
      now: new Date(now),
    };
    const snapshot = buildTaskMapPhysiologicalSourceSnapshot(
      await classifiedLiveOuraContext(options, 60),
      TEST_OWNER_SCOPE.ownerScopeDigest,
      now,
      now,
    );
    mkdirSync(path.dirname(physiologicalSnapshotPath), {
      recursive: true,
      mode: 0o700,
    });
    writeFileSync(
      physiologicalSnapshotPath,
      serializeTaskMapPhysiologicalSourceSnapshot(snapshot),
      { mode: 0o600 },
    );
    const projectionBefore = readFileSync(fixture.locations.projectionPath);
    const currentnessBefore = readFileSync(fixture.locations.currentnessPath);
    const snapshotBefore = readFileSync(physiologicalSnapshotPath);
    let providerReads = 0;
    const service = new TaskMapNativeRefreshService({
      ...fixture.locations,
      sourcePaths: { physiologicalSnapshotPath },
    ...
    assert.deepEqual(readFileSync(physiologicalSnapshotPath), snapshotBefore);
  ...
  it("does not publish repeated-pattern labels through the retired Strategy fallback", async () => {
    const fixture = await strategyFallbackFixture(
      "taskmap-native-strategy-body-repeat-",
      { bodyPatternReady: true },
    );
    const projectionBefore = readFileSync(
      fixture.locations.projectionPath,
    );
    ...
    const service = new TaskMapNativeRefreshService({
      ...
      readPhysiologicalProviderContext:
        repeatedPatternLiveOuraContext,
      strategyFallback: {
        homeDirectory: fixture.homeDirectory,
        readAdapterInput: fixture.readAdapterInput,
      },
  ...
  it("retains a mixed Strategy predecessor instead of attesting it as newly current", async () => {
    const fixture = await strategyFallbackFixture(
      "taskmap-native-strategy-owner-shape-",
      {
    ...
    const projectionBytesBefore = readFileSync(
      fixture.locations.projectionPath,
    );
    const currentnessBytesBefore = readFileSync(
      fixture.locations.currentnessPath,
    );
    const projectionBefore = JSON.parse(
      projectionBytesBefore.toString("utf8"),
    ) as TaskMapProjectionV1;
    const currentnessBefore = JSON.parse(
      currentnessBytesBefore.toString("utf8"),
    ) as TaskMapNativePublicationCandidate["currentness"];
    assert.equal(projectionBefore.tasks.length, 31);
    assert.equal(
      currentnessBefore.taskDispositions.filter(
        (row) => row.disposition === "current",
      ).length,
      13,
    );
    ...
    rmSync(currentWorkPath(fixture.locations.projectionPath));
    const [freshStrategy, predecessor] = await Promise.all([
      fixture.readAdapterInput().then((input) =>
    ...
    assert.equal(
      predecessor.taskMapInput.pointers.filter(
        (pointer) => pointer.sourceKind === "strategy",
      ).length,
      22,
    );
    ...
    assert.equal(
      freshStrategy.exactProvenance.projection.runId,
      predecessor.binding.runId,
    );
    assert.equal(
      freshStrategy.exactProvenance.projection.projectionFileDigest,
      predecessor.binding.projectionFileDigest,
    );
    assert.equal(
      freshStrategy.exactProvenance.projection.currentnessFileDigest,
      predecessor.binding.currentnessFileDigest,
    );
    const currentIds = new Set(
      currentnessBefore.taskDispositions
        .filter((row) => row.disposition === "current")
        .map((row) => row.taskId),
    );
    const sourceById = new Map(
      projectionBefore.sources.map((source) => [source.id, source]),
    );
    assert.equal(
      new Set(projectionBefore.tasks
        .filter((task) => currentIds.has(task.id))
        .map((task) => task.taskHomePointerId)
        .filter((pointerId): pointerId is string =>
          pointerId !== undefined
          && sourceById.get(pointerId)?.sourceKind === "strategy"
        )).size,
      9,
    );
    const dispositionByTask = new Map(
      currentnessBefore.taskDispositions.map((row) => [
        row.taskId,
        row.disposition,
      ]),
    );
    assert.deepEqual(
      currentnessBefore.taskDispositions,
      projectionBefore.tasks.map((task) => ({
        taskId: task.id,
    ...
    const predecessorPointerById = new Map(
      predecessor.taskMapInput.pointers.map((pointer) => [
        pointer.id,
        pointer,
      ]),
    );
    const predecessorEventById = new Map(
      predecessor.taskMapInput.events.map((event) => [event.id, event]),
    );
    for (const pointer of freshStrategy.taskMapInput.pointers) {
      ...
      assert.equal(
        prior.sourceVersion,
        freshStrategy.exactProvenance.repository.revision,
      );
      assert.equal(pointer.authority, prior.authority);
      assert.equal(pointer.syncMode, prior.syncMode);
      assert.deepEqual(
        [...pointer.capabilities].sort(),
        [...prior.capabilities].sort(),
      );
    }
    for (const event of freshStrategy.taskMapInput.events) {
      ...
      assert.equal(
        ...
        prior.extractionConfidence,
      );
      assert.equal(event.sourceStatus, prior.sourceStatus);
    }
    const projectedTaskById = new Map(
      projectionBefore.tasks.map((task) => [task.id, task]),
    );
    for (const proof of freshStrategy.exactProvenance.tasks) {
      ...
      assert.ok(task.citations.some((citation) =>
    ...
    const service = new TaskMapNativeRefreshService({
      ...
      publisher: async (input) => {
        ...
        assert.deepEqual(
          readFileSync(fixture.locations.projectionPath),
          projectionBytesBefore,
        );
        assert.deepEqual(
          readFileSync(fixture.locations.currentnessPath),
          currentnessBytesBefore,
        );
        const candidate =
          input.candidate as unknown as TaskMapNativePublicationCandidate;
        assert.deepEqual(candidate.projection, projectionBefore);
        assert.deepEqual(candidate.currentness, currentnessBefore);
        return publishTaskMapNativeProjection(
          fixture.locations.projectionPath,
          fixture.locations.currentnessPath,
          path.join(
            fixture.locations.runtimeRoot,
    ...
    assert.deepEqual(
      JSON.parse(readFileSync(fixture.locations.projectionPath, "utf8")),
      projectionBefore,
    );
    assert.deepEqual(
      JSON.parse(readFileSync(fixture.locations.currentnessPath, "utf8")),
      currentnessBefore,
    );
  });

  it("fails closed before any Strategy no-op publication or companion race", async () => {
    const fixture = await strategyFallbackFixture(
      "taskmap-native-strategy-no-op-pair-race-",
      {
        currentStrategyCount: 9,
        reviewStrategyCount: 13,
        currentOtherCount: 4,
    ...
    assert.equal(
      (await baseline.requestRefresh("manual")).refreshStatus,
      "unavailable",
    );
    assert.deepEqual(
      readFileSync(fixture.locations.projectionPath),
    ...
    let adapterReads = 0;
    let rewrittenCurrentness: Buffer | null = null;
      ...
      strategyFallback: {
        ...
        readAdapterInput: async () => {
          ...
          if (adapterReads === 2) {
            ...
            const reviewRow = currentness.taskDispositions.find(
              (row) => row.disposition === "needs_lifecycle_review",
            );
            assert.ok(reviewRow);
            ...
            rewrittenCurrentness = readFileSync(
              fixture.locations.currentnessPath,
            );
          }
  ...
  it("fails closed on a valid fixed-currentness rewrite outside the authenticated predecessor pair", async () => {
    const fixture = await strategyFallbackFixture(
      "taskmap-native-strategy-currentness-swap-",
      {
        currentStrategyCount: 2,
    ...
    const currentness = JSON.parse(readFileSync(
      fixture.locations.currentnessPath,
      "utf8",
    )) as TaskMapNativePublicationCandidate["currentness"];
    const reviewRow = currentness.taskDispositions.find(
    ...
    const rewrittenCurrentness = readFileSync(
      fixture.locations.currentnessPath,
    );
    const service = new TaskMapNativeRefreshService({
      ...fixture.locations,
  ...
  it("fails closed when Strategy evidence does not match the fixed predecessor", async () => {
    const fixture = await strategyFallbackFixture(
      "taskmap-native-strategy-mismatch-",
    );
    const projectionBefore = readFileSync(
      fixture.locations.projectionPath,
    );
  ...
  it("fails closed without a semantic builder and preserves fixed last-good bytes", async () => {
    const locations = roots("taskmap-native-blocked-");
    mkdirSync(path.dirname(locations.projectionPath), { mode: 0o700 });
    writeFileSync(
      locations.projectionPath,
      "{\"keep\":\"last-good\"}\n",
      { mode: 0o600 },
    ...
    assert.deepEqual(
      result.sourceStatuses.map((item) => item.source),
      TASKMAP_OWNER_REFRESH_SOURCES,
    );
    assert.equal(
      readFileSync(locations.projectionPath, "utf8"),
    ...
    assert.equal(state.contractVersion, TASKMAP_NATIVE_REFRESH_STATE_VERSION);
    assert.deepEqual(state.sources, {});
    assert.equal(status.contractVersion, TASKMAP_NATIVE_REFRESH_STATUS_VERSION);
    assert.equal(status.failureStage, "graph_builder");
    assert.equal(
      status.publicationBlockReason,
      "semantic_provider_unavailable",
    );
    assert.equal(statSync(locations.runtimeRoot).mode & 0o777, 0o700);
  });

  it("reports a direct Oura receipt before semantic failure and retains it after provider failure", async () => {
    ...
    assert.equal(
      firstResult.publicationBlockReason,
      "semantic_provider_unavailable",
    );
    assert.equal(providerReads, 1);
    const firstBody = firstResult.sourceStatuses.find(
      (status) => status.source === "body",
    );
    assert.deepEqual(firstBody, {
      ...
      nextDueAtMs:
        firstAtMs + TASKMAP_OWNER_REFRESH_DUE_INTERVAL_MS,
    ...
    assert.equal(statSync(physiologicalSnapshotPath).mode & 0o777, 0o600);
    const receiptBefore = readFileSync(physiologicalSnapshotPath);
    assert.deepEqual(readFileSync(locations.projectionPath), projectionBefore);
    ...
    assert.equal(providerReads, 2);
    assert.deepEqual(
      secondResult.sourceStatuses.find(
        (status) => status.source === "body",
      ),
      {
        source: "body",
    ...
    assert.deepEqual(readFileSync(physiologicalSnapshotPath), receiptBefore);
    assert.deepEqual(readFileSync(locations.projectionPath), projectionBefore);
    const retainedState = artifact(
      locations.runtimeRoot,
      "taskmap-refresh-state.v1.json",
    );
    assert.equal(
      (
        retainedState.lastSourceSuccessAtMs as Record<string, number>
      ).body,
      firstAtMs,
    );
  });

  it("wires the barrier-bound physiological snapshot into the default projection and rejects a mismatched snapshot", async () => {
    ...
    writeMeetingProducerSnapshot(snapshotPath, {
      ...
      meetings: [
        meetingProducerMeeting(
          "body-overlap-previous-document",
          "2026-07-27T09:00:00.000Z",
          "complete",
        ),
        meetingProducerMeeting(
          "body-overlap-document",
          "2026-07-28T09:00:00.000Z",
          "complete",
        ),
      ],
    ...
    const service = new TaskMapNativeRefreshService({
      ...
      readPhysiologicalProviderContext: (options) =>
        classifiedLiveOuraContext(options, 60),
      nowMs: () => nowMs,
    });
    ...
    const mismatchMeetingPath = path.join(
      path.dirname(mismatched.runtimeRoot),
      "meeting-producer-snapshot.v1.json",
    );
    const mismatchBodyPath = path.join(
      path.dirname(mismatched.runtimeRoot),
      "private",
      "taskmap-physiological-source-snapshot.v1.json",
    );
    writeMeetingProducerSnapshot(mismatchMeetingPath, {
      ...
      meetings: [
        meetingProducerMeeting(
          "body-mismatch-previous-document",
          "2026-07-27T09:00:00.000Z",
          "complete",
        ),
        meetingProducerMeeting(
          "body-mismatch-document",
          "2026-07-28T09:00:00.000Z",
          "complete",
        ),
      ],
    });
    const contextOptions: FetchOuraTaskMapContextOptions = {
      startDate: "2026-05-01",
      endDate: "2026-07-29",
      now: new Date(now),
    };
    const snapshotA = buildTaskMapPhysiologicalSourceSnapshot(
      await classifiedLiveOuraContext(contextOptions, 60),
      confirmedTestOwner(userId).ownerScopeDigest,
      now,
      now,
    );
    const snapshotB = buildTaskMapPhysiologicalSourceSnapshot(
      await classifiedLiveOuraContext(contextOptions, 95),
      confirmedTestOwner(userId).ownerScopeDigest,
      now,
      now,
    );
    mkdirSync(path.dirname(mismatchBodyPath), {
      recursive: true,
      mode: 0o700,
    });
    writeFileSync(
      mismatchBodyPath,
      serializeTaskMapPhysiologicalSourceSnapshot(snapshotB),
      { mode: 0o600 },
    );
    ...
    assert.equal(mismatchResult.refreshStatus, "unavailable");
    assert.equal(
      mismatchResult.publicationBlockReason,
      "semantic_provider_unavailable",
    );
    assert.equal(existsSync(mismatched.projectionPath), false);
    assert.equal(
      existsSync(path.join(
  ...
  it("uses the authenticated default producer, proposes review-only work, and replays as no-op", async () => {
    ...
    assert.equal(projection.tasks[0].authority, "none");
    const currentness = JSON.parse(
      readFileSync(locations.currentnessPath, "utf8"),
    ) as TaskMapNativePublicationCandidate["currentness"];
    assert.deepEqual(
  ...
  it("uses one fixed assessment time when the refresh clock advances during collection", async () => {
    ...
    const service = new TaskMapNativeRefreshService({
      ...
      nowMs: () =>
        requestedAtMs + (clockReads++ === 0 ? 0 : 96),
    });
    ...
    const agentSnapshot = JSON.parse(
      readFileSync(agentSnapshotPath, "utf8"),
    ) as { producedAt: string };
    assert.equal(
      agentSnapshot.producedAt,
      new Date(requestedAtMs).toISOString(),
    );
    assert.ok(clockReads >= 2);
  });

  it("runs verified raw Granola extraction in the production meeting path without blocking Google evidence", async () => {
    ...
    const rawPath = path.join(owner.sourceRoot, "granola-mcp-snapshot.json");
    const promptPath = path.join(path.dirname(locations.runtimeRoot), "mention-extraction-v1.md");
  ...
  it("emits and persists one station-wide unauthenticated code through a due no-op", async () => {
    const fixture = rawGranolaDegradationFixture(
      "taskmap-native-granola-auth-degradation-",
      "granola-auth-degradation-owner",
      ["I will ship A.", "I will ship B."],
    );
    ...
    const first = await service.requestRefresh("manual");
    const firstMeeting = first.sourceStatuses.find(
      (status) => status.source === "meeting_notes",
    );
    assert.equal(stationSelections, 1);
    assert.equal(
      firstMeeting?.extractionDegradationCode,
      "provider_unauthenticated",
    );
    assert.equal(first.publicationBlockReason, null);

    const persistedState = JSON.parse(readFileSync(path.join(
      fixture.runtimeRoot,
      "taskmap-refresh-state.v1.json",
    ), "utf8")) as {
      lastSourceStatuses: Array<Record<string, unknown>>;
    };
    assert.equal(
      persistedState.lastSourceStatuses.find(
        (status) => status.source === "meeting_notes",
  ...
  it("emits the same station-wide contract when no supported provider exists", async () => {
    const fixture = rawGranolaDegradationFixture(
      "taskmap-native-granola-no-provider-",
      "granola-no-provider-owner",
      ["I will ship the provider setup."],
    );
    const service = new TaskMapNativeRefreshService({
      ...fixture,
      confirmedOwner: fixture.owner,
  ...
  it("reports remote consent as partial for meeting extraction and retries after consent", async () => {
    const body = "I will ship after approving DaoBrew Cloud AI.";
    const fixture = rawGranolaDegradationFixture(
      "taskmap-native-granola-consent-required-",
      "granola-consent-required-owner",
      [body],
    );
    let nowMs = Date.parse("2026-07-29T13:00:00.000Z");
  ...
  it("clears a persisted station-wide code after the next partial-success report", async () => {
    const degradedBody = "I will ship the initially degraded note.";
    const extractedBody = "I will ship the later extracted note.";
    const fixture = rawGranolaDegradationFixture(
      "taskmap-native-granola-degradation-clear-",
      "granola-degradation-clear-owner",
      [degradedBody, extractedBody],
    );
    let nowMs = Date.parse("2026-07-29T13:00:00.000Z");
    let stationSelections = 0;
    const service = new TaskMapNativeRefreshService({
    ...
    const first = await service.requestRefresh("manual");
    assert.equal(
      first.sourceStatuses.find(
        (status) => status.source === "meeting_notes",
      )?.extractionDegradationCode,
      "provider_unauthenticated",
    );
    const firstPersistedStatus = JSON.parse(readFileSync(path.join(
      fixture.runtimeRoot,
      "taskmap-refresh-status.v1.json",
    ), "utf8")) as {
    ...
    assert.equal(
      firstPersistedStatus.sourceStatuses.find(
        (status) => status.source === "meeting_notes",
      )?.extractionDegradationCode,
      "provider_unauthenticated",
    );
    ...
    for (const [filename, key] of [
      ...
      const statuses = persisted[key] as Array<Record<string, unknown>>;
      assert.equal(
        statuses.find(
          (status) => status.source === "meeting_notes",
        )?.extractionDegradationCode,
  ...
  it("omits station-wide degradation when any attempted raw note extracts", async () => {
    const degradedBody = "I will ship the degraded note.";
    const extractedBody = "I will ship the extracted note.";
    const fixture = rawGranolaDegradationFixture(
      "taskmap-native-granola-partial-extraction-",
      "granola-partial-extraction-owner",
      [degradedBody, extractedBody],
    );
    let stationSelections = 0;
    const service = new TaskMapNativeRefreshService({
    ...
    const persistedStatus = JSON.parse(readFileSync(path.join(
      fixture.runtimeRoot,
      "taskmap-refresh-status.v1.json",
    ), "utf8")) as {
      sourceStatuses: Array<Record<string, unknown>>;
    ...
    assert.equal(
      persistedStatus.sourceStatuses.find(
        (status) => status.source === "meeting_notes",
      )?.extractionDegradationCode,
      undefined,
    );
  ...
  it("drops raw Granola when its owner receipt disappears at report point-of-use", async () => {
    const locations = roots("taskmap-native-granola-receipt-race-");
    const ownerLabel = "granola-receipt-race-owner";
    const owner = confirmedTestOwner(ownerLabel);
    const missingGooglePath = path.join(
      path.dirname(locations.runtimeRoot),
      "missing-google.json",
    );
    const rawPath = path.join(owner.sourceRoot, "granola-mcp-snapshot.json");
    ...
    if (result.refreshStatus === "published") {
      ...
      assert.ok(
        projection.sources.every((source) => source.sourceKind !== "granola"),
        "receiptless raw evidence must not reach publication",
      );
    }
  });

  it("degrades only raw Granola when durable report persistence fails", async () => {
    ...
    assert.ok(
      projection.sources.some((source) => source.sourceKind === "gemini_meet"),
    );
    assert.ok(
      projection.sources.every((source) => source.sourceKind !== "granola"),
    );
  ...
  it("rejects future raw note time before provider selection or envelope writes", async () => {
    ...
    const envelopeRoot = path.join(
      owner.taskMapRoot,
      "llm-envelopes",
      "mention-extraction-v1",
    );
    assert.equal(
      existsSync(envelopeRoot) ? readdirSync(envelopeRoot).length : 0,
      0,
    );
    const projection = JSON.parse(
      readFileSync(locations.projectionPath, "utf8"),
  ...
  it("revalidates the persisted raw report after a first-collector snapshot swap", async () => {
    ...
    const rawSnapshot = (id: string, body: string) => JSON.stringify({
      events: [],
      meeting_notes: ["a", "b"].map((suffix) => ({
        id: `${id}-${suffix}`,
        source: "granola",
    ...
    const service = new TaskMapNativeRefreshService({
      ...
      collectors: {
        agent_session: async () => {
          writeFileSync(
            rawPath,
            rawSnapshot("raw-after-collector", newBody),
            { mode: 0o600 },
          );
          swapped = true;
    ...
    assert.notEqual(
      ...
      taskMapContractDigest(readFileSync(rawPath, "utf8")),
      "the probe must leave the durable report bound to the old raw bytes",
    );
    const projection = JSON.parse(
      readFileSync(locations.projectionPath, "utf8"),
    ) as TaskMapProjectionV1;
    assert.equal(
      result.sourceStatuses.find((status) =>
        status.source === "meeting_notes"
      )?.disposition,
      "fresh",
      "the deterministic fresh-empty Google family must remain available",
    );
    assert.ok(
      projection.sources.every((source) => source.sourceKind !== "granola"),
      "a stale raw report must not reach semantic projection",
    );
  });

  it("runs the raw-only production path when Google meeting evidence is unavailable", async () => {
    const locations = roots("taskmap-native-granola-only-");
    const owner = confirmedTestOwner("granola-only-refresh-owner");
    const rawPath = path.join(owner.sourceRoot, "granola-mcp-snapshot.json");
    const missingGooglePath = path.join(path.dirname(locations.runtimeRoot), "missing-google.json");
    const promptPath = path.join(path.dirname(locations.runtimeRoot), "mention-extraction-v1.md");
    ...
    const projection = JSON.parse(readFileSync(locations.projectionPath, "utf8")) as TaskMapProjectionV1;
    assert.ok(projection.sources.every((source) => source.sourceKind !== "gemini_meet"));
  });

  it("rehydrates an accepted receipt after restart when every meeting source is unavailable", async () => {
    const locations = roots("taskmap-native-receipt-only-");
    const owner = confirmedTestOwner("receipt-only-refresh-owner");
    ...
    const producer = assessTaskMapMeetingProducerSnapshot(snapshot, assessedAt);
    ...
    const promoted = promoteTaskMapNativeCandidate({
      result: producer,
      overlay,
      previousStore: null,
      expectedOwnerScopeDigest: owner.ownerScopeDigest,
    ...
    mkdirSync(owner.ownerRoot, { recursive: true, mode: 0o700 });
    await writeTaskMapNativeCandidateAcceptanceStore({
      storePath: path.join(
        owner.taskMapRoot,
        "native-candidate-acceptance.v1.json",
    ...
    const missingGoogle = path.join(
      path.dirname(locations.runtimeRoot),
      "missing-meeting-producer.json",
    );
    const options = {
      ...locations,
      confirmedOwner: owner,
    ...
    assert.deepEqual(
      firstProjection.tasks[0]?.citations.map((citation) => citation.sourceKind),
      ["manual"],
    );
    ...
    const restartedProjection = JSON.parse(
      readFileSync(locations.projectionPath, "utf8"),
    ) as TaskMapProjectionV1;
    assert.deepEqual(
      taskMapProjectionArtifactValidationReasons(restartedProjection),
      [],
    );
    assert.deepEqual(restartedProjection.tasks, firstProjection.tasks);
  });

  it("preserves accepted mixed-source predecessor membership when adding an unrelated receipt", async () => {
    const locations = roots("taskmap-native-receipt-mixed-predecessor-");
    const owner = confirmedTestOwner("receipt-mixed-predecessor-owner");
    const assessedAt = "2026-08-03T12:00:00.000Z";
    const predecessor = rankinglessPublicationCandidate(
      0,
      loaderCompatibleProjection(),
    );
    const deterministicSource = predecessor.projection.sources.find(
      (source) => source.sourceKind === "google_calendar",
    )!;
    for (const task of predecessor.projection.tasks) {
      if (task.reviewState !== "accepted") continue;
      task.citations = [{
        ...task.citations[0]!,
    ...
    predecessor.currentness = currentnessForNativeProjection(
      predecessor.projection,
      null,
    );
    predecessor.ranking = buildTaskMapTaskRankingPublication({
    ...
    await publishTaskMapNativeProjection(
      ...
      locations.currentnessPath,
      journalPath,
      ownerBoundPublicationInput(
        predecessor,
        owner.ownerScopeDigest,
        "receipt-mixed-predecessor",
      ),
    );
    rmSync(journalPath);
    const snapshot = buildTaskMapMeetingProducerSnapshot({
    ...
    const ownerCalendarSlice = (revision: string) => {
      ...
      current.sliceDigest = taskMapContractDigest(current.value);
      return current;
    };
    ...
    const serviceOptions = {
      ...locations,
      confirmedOwner: owner,
      meetingProducerSnapshotPath: path.join(
        path.dirname(locations.runtimeRoot),
        "missing-mixed-meeting.json",
      ),
      sourcePaths: { meetingSnapshotPaths: [] },
      collectors: {
    ...
    assert.equal(result.refreshStatus, "published", JSON.stringify(result));
    assert.equal(
      result.sourceStatuses.find((row) => row.source === "agent_session")
        ?.disposition,
      "unavailable",
    );
    assert.equal(
      result.sourceStatuses.find((row) => row.source === "meeting_notes")
        ?.state,
      "unavailable",
    );
    assert.equal(
      result.sourceStatuses.find((row) => row.source === "calendar")
        ?.state,
      "current",
    );
    const projection = JSON.parse(
    ...
    assert.ok(projection.sources.some((source) =>
      source.sourceKind === "codex_session"
      || source.sourceKind === "claude_session"
    ));
    ...
    assert.deepEqual(
      restartedProjection.tasks.map((task) => task.id).sort(),
      projection.tasks.map((task) => task.id).sort(),
    );
    assert.ok(restartedProjection.tasks.every((task) =>
      task.reviewState === "accepted"
    ));
    assert.ok(staleProposalIds.every((taskId) =>
      !restartedProjection.tasks.some((task) => task.id === taskId)
    ));
    assert.equal(
      restartedProjection.tasks.filter((task) =>
        task.reviewState === "accepted" && task.authority === "user"
      ).length,
      1,
    );
  ...
  it("drops unrelated rejection audit rows from an accepted membership predecessor subset", () => {
    ...
    for (const root of predecessor.roots) root.taskIds = [];

    const retained = acceptedMembershipPredecessorProjection(predecessor);

    assert.ok(retained);
    assert.deepEqual(retained.rejections, []);
    assert.deepEqual(
      retained.tasks.map((task) => task.id).sort(),
      acceptedIds.sort(),
    );
    assert.ok(retained.tasks.every((task) =>
      task.reviewState === "accepted" && task.authority !== "none"
    ));
    assert.deepEqual(taskMapProjectionArtifactValidationReasons(retained), []);
  });

  it("reassesses default freshness and owner scope before every shortcut", async () => {
    for (const scenario of ["expired", "owner_changed"] as const) {
      ...
      if (scenario === "expired") {
        assert.equal(
          firstResult.nextDueAtMs,
          Date.parse("2026-07-29T16:00:00.000Z"),
        );
      }
      const baselineState = artifact(
        locations.runtimeRoot,
        "taskmap-refresh-state.v1.json",
      );
      const projectionBefore = readFileSync(locations.projectionPath);
      const currentnessBefore = readFileSync(locations.currentnessPath);
      const candidateBefore = readFileSync(path.join(
        locations.runtimeRoot,
        "taskmap-refresh-candidate.v1.json",
      ));
      ...
      const failedAtMs =
        scenario === "expired"
          ...
          : firstAt + 10 * 60 * 1_000;
      const expectedBlockReason =
        scenario === "owner_changed"
          ? "publication_failed"
          : "semantic_provider_unavailable";
      const second = new TaskMapNativeRefreshService({
        ...locations,
      ...
      const secondResult = await second.requestRefresh(
        scenario === "expired" ? "manual" : "timer",
      );

      assert.equal(secondResult.refreshStatus, "unavailable");
      assert.equal(secondResult.publicationVerified, false);
      assert.equal(
        secondResult.publicationBlockReason,
        expectedBlockReason,
      );
      ...
      const failedState = artifact(
        locations.runtimeRoot,
        "taskmap-refresh-state.v1.json",
      );
      assert.equal(failedState.lastAttemptAtMs, failedAtMs);
      assert.equal(
        failedState.lastSuccessfulRefreshAtMs,
        scenario === "expired"
      ...
      assert.equal(
        failedState.lastPublicationBlockReason,
        expectedBlockReason,
      );
      for (const digestName of [
        ...
        assert.equal(
          failedState[digestName],
          scenario === "expired" ? baselineState[digestName] : null,
        );
      }
      assert.deepEqual(
        failedState.sources,
        scenario === "expired" ? baselineState.sources : {},
      );
      assert.equal(
        (
          failedState.lastSourceStatuses as Array<{
            source: string;
            disposition: string;
          }>
        ).find((status) => status.source === "meeting_notes")?.disposition,
  ...
  it("persists unavailable truth when authenticated evidence changes during a coordinator no-op", async () => {
    ...
    assert.equal(
      (await first.requestRefresh("manual")).refreshStatus,
      "published",
    );
    const baselineState = artifact(
      locations.runtimeRoot,
    ...
    const retainedMeetingSlice = (
      baselineState.sources as Record<
        string,
        TaskMapOwnerCollectedSlice<TaskMapNativeSafeSlice>
      >
    ).meeting_notes;
  ...
  it("accepts a predecessor generation written with the legacy locale manifest order", async () => {
    ...
    writeReferencedPublicationPredecessor(
      ...
      locations.currentnessPath,
      predecessor,
      "legacy-generation-order-predecessor",
    );
    rewriteGenerationManifestWithLegacyLocaleOrder(
      locations.projectionPath,
    );
    const service = new TaskMapNativeRefreshService({
      ...locations,
  ...
  it("preserves the exact accepted predecessor when work sources are unavailable", async () => {
    ...
    writeReferencedPublicationPredecessor(
      ...
      locations.currentnessPath,
      predecessor,
      "context-retirement-predecessor",
    );
    assert.ok(predecessor.projection.tasks.length > 0);
    assert.equal(existsSync(currentWorkPath(locations.projectionPath)), true);
    ...
    noise.sliceDigest = taskMapContractDigest(noise.value);
    const contextOnlyCollectors: TaskMapNativeRefreshServiceOptions["collectors"] = {
      agent_session: async () => noise,
      meeting_notes: async () => {
        throw new Error("no current meeting note");
      },
    ...
    assert.equal(existsSync(currentWorkPath(locations.projectionPath)), true);
    const preservedCurrentness = JSON.parse(
      readFileSync(locations.currentnessPath, "utf8"),
    ) as TaskMapNativePublicationCandidate["currentness"];
    ...
    const preservedRanking = JSON.parse(readFileSync(
      nativeTaskRankingPath(locations.projectionPath),
      "utf8",
    )) as TaskMapTaskRankingPublicationV1;
  ...
  it("retains predecessor on publication failure then recovers the preserved generation", async () => {
    const locations = roots("taskmap-native-context-retirement-crash-");
    const predecessor = rankedExecutablePublicationCandidate();
    const predecessorCurrentWork = writeReferencedPublicationPredecessor(
      locations.projectionPath,
      locations.currentnessPath,
      predecessor,
      "context-retirement-crash-predecessor",
    );
    const rankingPath = nativeTaskRankingPath(locations.projectionPath);
    const service = new TaskMapNativeRefreshService({
      ...locations,
      collectors: {
        agent_session: async () => {
      ...
      publisher: (input) => publishTaskMapNativeProjection(
        locations.projectionPath,
        locations.currentnessPath,
        path.join(
          locations.runtimeRoot,
    ...
    assert.deepEqual(
      (JSON.parse(readFileSync(
        locations.projectionPath,
        "utf8",
      )) as TaskMapProjectionV1).tasks,
      predecessor.projection.tasks,
    );
    assert.equal(existsSync(rankingPath), true);
    assert.equal(existsSync(path.join(
      locations.runtimeRoot,
      "taskmap-publication-journal.v1.json",
    )), true);
  ...
  it("publishes fresh-empty genesis coverage without inventing work", async () => {
    ...
    const emptyMeetingSlice = (
      state.sources as Record<
        string,
        TaskMapOwnerCollectedSlice<TaskMapNativeSafeSlice>
      >
    ).meeting_notes;
    ...
    assert.equal(projection.tasks.length, 1);
    const changedState = artifact(
      locations.runtimeRoot,
      "taskmap-refresh-state.v1.json",
    );
    assert.equal(changedState.lastSuccessfulRefreshAtMs, changedAtMs);
    const changedMeetingSlice = (
      changedState.sources as Record<
        string,
        TaskMapOwnerCollectedSlice<TaskMapNativeSafeSlice>
      >
    ).meeting_notes;
    ...
    assert.equal(
      changedMeetingSlice.sliceDigest,
      emptyMeetingSlice.sliceDigest,
    );
  });

  it("retires a valid fixed predecessor without requiring its retired source evidence", async () => {
    const locations = roots("taskmap-native-default-predecessor-");
    const predecessor = publicationCandidate(
      18,
      projectionWithTaskCount(27),
    );
    mkdirSync(path.dirname(locations.projectionPath), {
      recursive: true,
      mode: 0o700,
    });
    writeFileSync(
      locations.projectionPath,
      `${JSON.stringify(predecessor.projection)}\n`,
      { mode: 0o600 },
    );
    writeFileSync(
      locations.currentnessPath,
      `${JSON.stringify(predecessor.currentness)}\n`,
      { mode: 0o600 },
    );
    const projectionBefore = readFileSync(locations.projectionPath);
    const currentnessBefore = readFileSync(locations.currentnessPath);
    ...
    const retiredCurrentness = JSON.parse(
      readFileSync(locations.currentnessPath, "utf8"),
    ) as TaskMapNativePublicationCandidate["currentness"];
    assert.deepEqual(retiredCurrentness.taskDispositions, []);
  });

  it("preserves non-Agent work when a fresh Meeting producer changes to empty", async () => {
    for (const trigger of ["timer", "launch"] as const) {
      ...
      const projectionBefore = readFileSync(locations.projectionPath);
      const currentnessBefore = readFileSync(locations.currentnessPath);
      const rankingBefore = readFileSync(
        nativeTaskRankingPath(locations.projectionPath),
      );
      const candidatePath = path.join(
        locations.runtimeRoot,
      ...
      assert.deepEqual(
        readFileSync(nativeTaskRankingPath(locations.projectionPath)),
        rankingBefore,
      );
      assert.notDeepEqual(readFileSync(candidatePath), candidateBefore);
      const preservedProjection = JSON.parse(
        readFileSync(locations.projectionPath, "utf8"),
      ) as TaskMapProjectionV1;
      assert.ok(preservedProjection.tasks.length > 0);
      assert.equal(
        preservedProjection.sources.every(
          (source) => source.sourceKind === "gemini_meet",
        ),
        true,
      );
    }
  ...
  it("preserves the accepted pair for stale or wrong-owner default evidence", async () => {
    for (const scenario of ["stale", "wrong_owner"] as const) {
  ...
  it("runs one barrier, publishes a loader-compatible gated projection, and records verified digests", async () => {
    ...
    const service = new TaskMapNativeRefreshService({
      ...
      collectors: collectors(undefined, {
        meeting_notes: async () =>
          slice("meeting_notes", "meeting-r1", duplicateIdentity),
        calendar: async () =>
          slice("calendar", "calendar-r1", duplicateIdentity),
      }),
      graphBuilder: async (barrier) => {
        graphRecordCount = barrier.graphInput.sources.reduce(
          (total, source) =>
            total + (source.value?.records.length ?? 0),
          0,
        );
        return {
          candidateDigest: digest(candidate),
    ...
    assert.deepEqual(
      JSON.parse(readFileSync(locations.projectionPath, "utf8")),
      candidate.projection,
    );
    ...
    assert.equal(
      statSync(locations.projectionPath).mode & 0o777,
      0o600,
    );
    assert.equal(
      existsSync(path.join(
  ...
  it("publishes a deterministic current-work successor without changing its work semantics", async () => {
    ...
    const predecessorCurrentWork = writeReferencedPublicationPredecessor(
      ...
      locations.currentnessPath,
      predecessor,
      "current-work-success-predecessor",
    );
    const episodeAdmission = {
      admission: "authenticated_fresh_agent_session" as const,
      directive: "user_directive" as const,
    ...
    assert.equal(result.publicationVerified, true);
    assert.equal(result.publicationBlockReason, null);
    const bytes = readFileSync(currentWorkPath(locations.projectionPath));
    const currentWork = JSON.parse(bytes.toString("utf8")) as
      Record<string, unknown>;
    assert.equal(
      bytes.toString("utf8"),
      taskMapContractCanonicalJson(currentWork),
    ...
    const successorInput = (
      (successorSemantics.nextTaskToProve as Record<string, unknown>)
        .input as Record<string, unknown>
    );
    assert.deepEqual(
      successorInput.agentSessionEpisode,
      episodeAdmission,
    );
    ...
    assert.equal(
      currentWork.artifactDigest,
      taskMapContractDigest(digestCore),
    );
    assert.equal(
      statSync(currentWorkPath(locations.projectionPath)).mode & 0o777,
      0o600,
    );

    const withoutDirective = publicationCandidate(
      0,
      successorProjection(candidate.projection),
    );
    const nextRefresh = new TaskMapNativeRefreshService({
      ...locations,
      collectors: collectors(undefined, {
        agent_session: async () =>
          slice("agent_session", "agent-session-without-directive-r2"),
      }),
    ...
    const withoutAdmission = JSON.parse(readFileSync(
    ...
    assert.equal(
      Object.prototype.hasOwnProperty.call(
        withoutAdmission.nextTaskToProve.input,
        "agentSessionEpisode",
      ),
      false,
    );
  ...
  it("derives every structurally eligible proof-backed leaf from a legacy singleton companion", async () => {
    const locations = roots("taskmap-native-ready-proof-targets-");
    const predecessor = publicationCandidate();
    const firstCurrentWork = currentWorkForProjection(
      predecessor.projection,
    );
    const firstTaskId = (
      firstCurrentWork.nextTaskToProve as Record<string, unknown>
    ).taskId as string;
    writeReferencedPublicationPredecessor(
      locations.projectionPath,
      locations.currentnessPath,
      predecessor,
      "multiple-ready-predecessor",
    );
    const predecessorProofTargets = validateTaskMapReadyProofTargets(
    ...
    assert.deepEqual(
      predecessorProofTargets.proofTargets.map((target) => target.taskId),
      [firstTaskId],
    );
    const candidate = publicationCandidate(
      0,
    ...
    const independentlyEligibleTaskIds = candidate.projection.tasks
      .filter((task) =>
        ...
        && task.reviewState === "accepted"
        && task.openState === "open"
        && candidate.currentness.taskDispositions.some((row) =>
          row.taskId === task.id && row.disposition === "current"
        )
    ...
    const verified = validateTaskMapReadyProofTargets(
      generation.readyProofTargets,
      candidate.projection,
      candidate.currentness,
    );
    assert.deepEqual(
      verified.proofTargets.map((target) => target.taskId),
      independentlyEligibleTaskIds,
    );
    assert.deepEqual(
      verified.proofTargets.find((target) => target.taskId === firstTaskId),
      readyProofTargetFromCurrentWorkFixture(firstCurrentWork),
    );
    assert.ok(generation.ranking);
    assert.deepEqual(
      generation.ranking.rankedAcceptedOpen
        .map((row) => row.taskId)
        .sort(),
      independentlyEligibleTaskIds,
    );
    const fixed = validateTaskMapReadyProofTargets(
      JSON.parse(readFileSync(
        taskMapNativeReadyProofTargetsPath(locations.projectionPath),
        "utf8",
      )),
      candidate.projection,
      candidate.currentness,
    ...
    const manifest = JSON.parse(readFileSync(path.join(
      ...
      TASKMAP_NATIVE_GENERATION_MANIFEST_FILENAME,
    ), "utf8")) as { artifacts: Record<string, unknown> };
    assert.deepEqual(
      Object.keys(manifest.artifacts).sort(),
      ["currentWork", "currentness", "projection", "ranking"],
    );
  });

  it("publishes zero ready leaves when the predecessor generation companion is missing", async () => {
  ...
  it("publishes zero ready leaves when a generation companion has a valid seal for the wrong binding", async () => {
    ...
    const predecessorCurrentWork = writeReferencedPublicationPredecessor(
      ...
      locations.currentnessPath,
      predecessor,
      "mismatched-ready-predecessor",
    );
    const taskId = (
      predecessorCurrentWork.nextTaskToProve as Record<string, unknown>
    ).taskId as string;
    const candidate = publicationCandidate(
      0,
      successorProjection(predecessor.projection),
    ...
    writeFileSync(
      ...
      taskMapContractCanonicalJson(wrongBinding),
      { mode: 0o600 },
    );
    const service = new TaskMapNativeRefreshService({
      ...locations,
  ...
  it("seeds first publication with the complete eligible frontier instead of a singleton fallback", async () => {
    ...
    const nextTaskToProve = generation.currentWork.nextTaskToProve as
      Record<string, unknown>;
    assert.equal(nextTaskToProve.taskId, firstRanked.taskId);
    const selectedTask = candidate.projection.tasks.find(
      (task) => task.id === firstRanked.taskId,
    );
    assert.ok(selectedTask);
    const selectedRoot = candidate.projection.roots.find(
      (root) => root.id === selectedTask.rootId,
    );
    ...
    assert.equal(
      (nextTaskToProve.input as Record<string, unknown>).summary,
    ...
    assert.deepEqual(
      verified.proofTargets.map((target) => target.taskId).sort(),
      independentlyEligibleTaskIds,
    );
    assert.deepEqual(
      verified.proofTargets.find((target) => target.taskId === firstRanked.taskId),
      readyProofTargetFromCurrentWorkFixture(generation.currentWork),
    );
    const fixed = validateTaskMapReadyProofTargets(
      JSON.parse(readFileSync(
        taskMapNativeReadyProofTargetsPath(locations.projectionPath),
  ...
  it("preserves the exact last-good trio when the current-work target disappears", async () => {
    ...
    const predecessorCurrentWork = writeReferencedPublicationPredecessor(
      ...
      locations.currentnessPath,
      predecessor,
      "current-work-missing-predecessor",
    );
    const targetTaskId = (
      predecessorCurrentWork.nextTaskToProve as Record<string, unknown>
    ).taskId as string;
    ...
    projection.edges = projection.edges.filter((edge) =>
      edge.from !== targetTaskId && edge.to !== targetTaskId
    );
    const candidate = publicationCandidate(0, projection);
    const before = {
      projection: readFileSync(locations.projectionPath),
      currentness: readFileSync(locations.currentnessPath),
      currentWork: readFileSync(
        currentWorkPath(locations.projectionPath),
      ),
    };
    const service = new TaskMapNativeRefreshService({
    ...
    assert.deepEqual(
      readFileSync(currentWorkPath(locations.projectionPath)),
      before.currentWork,
    );
  });

  it("retains the predecessor trio on failure then recovers the successor generation", async () => {
    ...
    writeReferencedPublicationPredecessor(
      ...
      locations.currentnessPath,
      predecessor,
      "current-work-write-predecessor",
    );
    const candidate = publicationCandidate(
      0,
      successorProjection(predecessor.projection),
    ...
    assert.equal(result.refreshStatus, "unavailable");
    assert.equal(result.publicationVerified, false);
    assert.deepEqual(readFileSync(locations.projectionPath), before.projection);
    assert.deepEqual(readFileSync(locations.currentnessPath), before.currentness);
    assert.deepEqual(
      readFileSync(currentWorkPath(locations.projectionPath)),
      before.currentWork,
    );
    ...
    assert.equal(await restarted.recoverPendingPublication(), true);
    assert.deepEqual(
      committedGeneration(locations.projectionPath).projection,
      candidate.projection,
    );
    assert.equal(
      existsSync(path.join(
  ...
  it("retains the verified predecessor when a successor ranking write fails", async () => {
    const locations = roots("taskmap-native-ranking-write-");
    const baseline = rankedPublicationCandidate();
    await publishTaskMapNativeProjection(
      locations.projectionPath,
      locations.currentnessPath,
      path.join(locations.runtimeRoot, "baseline-ranking-journal.json"),
      {
        graphInputDigest: digest("baseline-ranking-graph"),
        candidateDigest: digest(baseline),
        candidate: baseline as unknown as Record<string, unknown>,
        requestedAtMs: 2_225,
        expectedOwnerScopeDigest: TEST_OWNER_SCOPE.ownerScopeDigest,
      },
    );
    ...
    const before = {
      projection: readFileSync(locations.projectionPath),
      currentness: readFileSync(locations.currentnessPath),
      ranking: readFileSync(rankingPath),
      reference: readFileSync(
        nativeTaskMapGenerationReferencePath(locations.projectionPath),
      ),
    };
    const candidate = rankedPublicationCandidate(
      successorProjection(baseline.projection),
    );
    const failedJournal = path.join(
      locations.runtimeRoot,
      "failed-ranking-journal.json",
    );
    ...
    assert.deepEqual(readFileSync(locations.projectionPath), before.projection);
    assert.deepEqual(readFileSync(locations.currentnessPath), before.currentness);
    assert.deepEqual(readFileSync(rankingPath), before.ranking);
    assert.deepEqual(
      readFileSync(nativeTaskMapGenerationReferencePath(locations.projectionPath)),
      before.reference,
    );
    assert.deepEqual(committedGeneration(locations.projectionPath).projection, baseline.projection);
    assert.equal(existsSync(failedJournal), true);
    rmSync(failedJournal);
  });

  it("retains one verified generation across every transient publication fault point", async () => {
    const stages = [
      "currentness",
      "current-work",
      "ready-proof-targets",
    ...
    for (const stage of stages) {
      ...
      const baseline = ranked
        ...
        : publicationCandidate();
      const baselineJournal = path.join(
        locations.runtimeRoot,
        `baseline-${stage}.json`,
      );
      if (ranked) {
      ...
      const before = new Map(
        companionPaths.map((filePath) => [filePath, readFileSync(filePath)]),
      );
      const referencePath = nativeTaskMapGenerationReferencePath(
        locations.projectionPath,
      );
      const referenceBefore = existsSync(referencePath)
        ? readFileSync(referencePath)
        : null;
      const successorProjectionValue = successorProjection(
        baseline.projection,
      );
      ...
      const dependencies: Record<
        typeof stage,
        TaskMapNativePublicationDependencies
      > = {
      ...
      await assert.rejects(
        ...
        new RegExp(`injected ${stage} write failure`),
      );
      ...
      for (const filePath of companionPaths) {
        assert.deepEqual(
          ...
          before.get(filePath),
          `${stage} failure must preserve every predecessor compatibility file`,
        );
      }
      assert.deepEqual(
        existsSync(referencePath) ? readFileSync(referencePath) : null,
        referenceBefore,
      );
      assert.equal(existsSync(failedJournal), true);
      rmSync(failedJournal);
  ...
  it("rejects a rankingless successor without mutating a ranked predecessor bundle", async () => {
    const locations = roots("taskmap-native-ranked-to-unranked-");
    const baseline = rankedPublicationCandidate();
    await publishTaskMapNativeProjection(
      locations.projectionPath,
      locations.currentnessPath,
      path.join(locations.runtimeRoot, "baseline-ranked-journal.json"),
      ownerBoundPublicationInput(
        baseline,
        TEST_OWNER_SCOPE.ownerScopeDigest,
        "baseline-ranked",
    ...
    const successor = rankinglessPublicationCandidate(
      0,
      successorProjection(baseline.projection),
    );
    const journalPath = path.join(
      locations.runtimeRoot,
      "rankingless-successor-journal.json",
    );

    await assert.rejects(() => publishTaskMapNativeProjection(
      locations.projectionPath,
      locations.currentnessPath,
      journalPath,
      ownerBoundPublicationInput(
    ...
    const rankedSuccessor = rankedPublicationCandidate(
      successorProjection(baseline.projection),
    );
    await publishTaskMapNativeProjection(
      locations.projectionPath,
      locations.currentnessPath,
      path.join(locations.runtimeRoot, "recovered-ranked-journal.json"),
      ownerBoundPublicationInput(
        rankedSuccessor,
        TEST_OWNER_SCOPE.ownerScopeDigest,
        "recovered-ranked-successor",
      ),
    );
    assert.deepEqual(
      JSON.parse(readFileSync(locations.projectionPath, "utf8")),
      rankedSuccessor.projection,
    );
    const recoveredCurrentness = JSON.parse(
      readFileSync(locations.currentnessPath, "utf8"),
    ) as TaskMapNativePublicationCandidate["currentness"];
    assert.deepEqual(
      {
        ...recoveredCurrentness,
        taskDispositions: [...recoveredCurrentness.taskDispositions].sort(
    ...
    assert.deepEqual(
      JSON.parse(readFileSync(rankingPath, "utf8")),
      rankedSuccessor.ranking,
    );
  });

  it("never references a first owner generation without ranking", async () => {
    const locations = roots("taskmap-native-first-ranking-required-");
    const candidate = rankinglessPublicationCandidate();
    const journalPath = path.join(
      locations.runtimeRoot,
      "first-rankingless-journal.json",
    );
    ...
    await assert.rejects(() => publishTaskMapNativeProjection(
      ...
      ownerBoundPublicationInput(
        candidate,
        TEST_OWNER_SCOPE.ownerScopeDigest,
        "first-rankingless-owner-generation",
      ),
    ), /loader_incompatible/);
    assert.equal(
      existsSync(nativeTaskMapGenerationReferencePath(locations.projectionPath)),
      false,
    );
    assert.equal(existsSync(journalPath), false);
  });

  it("never recovers a rankingless journal into an owner generation", async () => {
    ...
    mkdirSync(locations.runtimeRoot, { recursive: true, mode: 0o700 });
    writeFileSync(
      journalPath,
      `${JSON.stringify({
        ...
        candidateDigest: digest(candidate),
        requestedAtMs: 2_240,
        candidateProjectionDigest: diffTaskMapProjections(
          null,
          candidate.projection,
    ...
    await assert.rejects(
      () => service.recoverPendingPublication(),
      /ranking|publication|loader/i,
    );
    assert.equal(
      existsSync(nativeTaskMapGenerationReferencePath(locations.projectionPath)),
      false,
  ...
  it("ignores a missing compatibility ranking mirror when the generation is verified", async () => {
    ...
    const stateAfterPublish = artifact(
      locations.runtimeRoot,
      "taskmap-refresh-state.v1.json",
    );
    assert.equal(
      stateAfterPublish.verifiedRankingDigest,
      candidate.ranking!.artifactDigest,
    );
    rmSync(nativeTaskRankingPath(locations.projectionPath));
    now += 1;
  ...
  it("ignores hostile compatibility ranking mirrors and advances from the verified generation", async () => {
    for (const variant of ["pretty", "duplicate"] as const) {
      const locations = roots(`taskmap-native-ranking-${variant}-`);
      const baseline = rankedPublicationCandidate();
      await publishTaskMapNativeProjection(
        locations.projectionPath,
        locations.currentnessPath,
        path.join(locations.runtimeRoot, `baseline-${variant}.json`),
        {
      ...
      const rankingPath = nativeTaskRankingPath(locations.projectionPath);
      const canonical = taskMapContractCanonicalJson(baseline.ranking!);
      const hostileBytes = variant === "pretty"
        ? `${JSON.stringify(baseline.ranking, null, 2)}\n`
        : canonical.replace(
            "{",
            '{"contractVersion":"taskmap-task-ranking.hostile",',
          );
      writeFileSync(rankingPath, hostileBytes, { mode: 0o600 });
      const candidate = rankedPublicationCandidate(
        successorProjection(baseline.projection),
      );
      await publishTaskMapNativeProjection(
        locations.projectionPath,
        locations.currentnessPath,
        path.join(locations.runtimeRoot, `candidate-${variant}.json`),
        {
  ...
  it("rejects a ranking-bearing candidate whose owner differs from the publisher authority", async () => {
    const locations = roots("taskmap-native-ranking-candidate-owner-");
    const ownerA = confirmedTestOwner("ranking-candidate-owner-a")
      .ownerScopeDigest;
    const ownerB = confirmedTestOwner("ranking-candidate-owner-b")
      .ownerScopeDigest;
    const candidate = rankedPublicationCandidate(undefined, ownerA);
    await assert.rejects(() => publishTaskMapNativeProjection(
      locations.projectionPath,
      locations.currentnessPath,
      path.join(locations.runtimeRoot, "wrong-owner-candidate.json"),
      ownerBoundPublicationInput(candidate, ownerB, "wrong-owner-candidate"),
    ), /loader_incompatible/);
    assert.equal(existsSync(locations.projectionPath), false);
  });

  it("rejects a referenced predecessor owned by another confirmed installation", async () => {
    const locations = roots("taskmap-native-ranking-predecessor-owner-");
    const ownerA = confirmedTestOwner("ranking-predecessor-owner-a")
      .ownerScopeDigest;
    const ownerB = confirmedTestOwner("ranking-predecessor-owner-b")
      .ownerScopeDigest;
    const baseline = rankedPublicationCandidate(undefined, ownerA);
    await publishTaskMapNativeProjection(
      locations.projectionPath,
      locations.currentnessPath,
      path.join(locations.runtimeRoot, "owner-a-baseline.json"),
      ownerBoundPublicationInput(baseline, ownerA, "owner-a-baseline"),
    );
    const candidate = rankedPublicationCandidate(
      successorProjection(baseline.projection),
      ownerB,
    ...
    await assert.rejects(() => publishTaskMapNativeProjection(
      locations.projectionPath,
      locations.currentnessPath,
      path.join(locations.runtimeRoot, "owner-b-candidate.json"),
      ownerBoundPublicationInput(candidate, ownerB, "owner-b-candidate"),
    ), /publication_failed/);
  });

  it("does not recover an owner-A ranking journal under owner-B authority", async () => {
    const locations = roots("taskmap-native-ranking-recovery-owner-");
    const ownerA = confirmedTestOwner("ranking-recovery-owner-a");
    const ownerB = confirmedTestOwner("ranking-recovery-owner-b");
    const candidate = rankedPublicationCandidate(
      undefined,
      ownerA.ownerScopeDigest,
    );
    const journalPath = path.join(
      locations.runtimeRoot,
      "taskmap-publication-journal.v1.json",
    ...
    writeFileSync(journalPath, `${JSON.stringify({
      ...
      candidateProjectionDigest:
        diffTaskMapProjections(null, candidate.projection)
          .currentProjectionDigest,
      candidate,
      previousCurrentness: null,
      candidateCurrentWork: null,
    ...
    const unavailableCollectors = Object.fromEntries(
      TASKMAP_OWNER_REFRESH_SOURCES.map((source) => [
        source,
  ...
  it("does not verify an owner-A ranking during owner-B due-state replay", async () => {
    const locations = roots("taskmap-native-ranking-due-owner-");
    const ownerA = confirmedTestOwner("ranking-due-owner-a");
    const ownerB = confirmedTestOwner("ranking-due-owner-b");
    const candidateA = rankedPublicationCandidate(
      undefined,
      ownerA.ownerScopeDigest,
    );
    let now = 2_242;
    ...
    assert.equal(
      (await serviceA.requestRefresh("manual")).refreshStatus,
      "published",
    );
    ...
    const ownerBState = JSON.parse(readFileSync(statePath, "utf8"));
    ownerBState.ownerScopeDigest = ownerB.ownerScopeDigest;
    writeFileSync(
      statePath,
      `${JSON.stringify(ownerBState, null, 2)}\n`,
      { mode: 0o600 },
    );
    now += 1;
    const serviceB = new TaskMapNativeRefreshService({
      ...
      confirmedOwner: ownerB,
      collectors: collectors(),
      graphBuilder: graphBuilder(rankedPublicationCandidate(
        undefined,
        ownerB.ownerScopeDigest,
      )),
      nowMs: () => now,
    });
  ...
  it("reconciles a committed pair into durable state after a crash-before-state boundary", async () => {
    ...
    assert.equal(existsSync(journalPath), false);
    const recoveredCandidate = artifact(
      locations.runtimeRoot,
      "taskmap-refresh-candidate.v1.json",
    );
    assert.equal(
      recoveredCandidate.contractVersion,
      TASKMAP_NATIVE_REFRESH_CANDIDATE_VERSION,
    );
    ...
    assert.equal(
      ...
      createHash("sha256")
        .update(readFileSync(locations.projectionPath))
        .digest("hex"),
    );
  });

  it("does not swallow a promotion appended after a crash-before-state journal", async () => {
    ...
    assert.equal(
      await service.recoverPendingPublication(),
      false,
      "a journal bound to the predecessor receipt head must not recover",
    );
    assert.equal(
      existsSync(path.join(
        locations.runtimeRoot,
      ...
      false,
      "the stale journal must be durably discarded so a current rebuild can proceed",
    );
    const result = await service.requestRefresh("timer");
  ...
  it("recovers a same-owner v4 journal only while the acceptance head is empty", async () => {
    for (const receiptState of ["empty", "nonempty"] as const) {
      ...
      if (receiptState === "empty") {
        assert.equal(
          artifact(
            locations.runtimeRoot,
            "taskmap-refresh-state.v1.json",
          ).processedPromotionReceiptHeadDigest,
          currentHead,
        );
        const sourceCalls = new Map<TaskMapOwnerRefreshSource, number>();
        let graphBuilds = 0;
  ...
  it("rolls a journaled generation forward after its publisher is killed between companions and projection", async () => {
    const locations = roots("taskmap-native-killed-publication-");
    const baseline = rankedPublicationCandidate();
    const baselineJournal = path.join(
      locations.runtimeRoot,
      "baseline-publication-journal.json",
    );
    await publishTaskMapNativeProjection(
      locations.projectionPath,
      locations.currentnessPath,
    ...
    rmSync(baselineJournal);

    const successor = rankedPublicationCandidate(
      successorProjection(baseline.projection),
    );
    const journalPath = path.join(
      locations.runtimeRoot,
    ...
    const markerPath = path.join(
      locations.runtimeRoot,
      "companions-durable.marker",
    );
    await killPublicationChild(
      locations,
      journalPath,
      markerPath,
      "projection",
    ...
    assert.deepEqual(
      JSON.parse(readFileSync(locations.projectionPath, "utf8")),
      baseline.projection,
      "the kill boundary must precede the successor projection write",
    );
    const restarted = new TaskMapNativeRefreshService({
      ...locations,
      nowMs: () => 2_255,
    ...
    assert.deepEqual(
      [...recoveredCurrentness.taskDispositions].sort(
        (left, right) => left.taskId.localeCompare(right.taskId),
      ),
      [...successor.currentness.taskDispositions].sort(
        (left, right) => left.taskId.localeCompare(right.taskId),
      ),
    );
    assert.equal(
      recoveredCurrentness.projectionDigest,
      successor.currentness.projectionDigest,
    );
    assert.deepEqual(
      JSON.parse(readFileSync(
        nativeTaskRankingPath(locations.projectionPath),
  ...
  it("fails closed then recovers when a publisher is killed after each ranked companion mutation", async () => {
    for (const killStage of ["currentness", "ranking"] as const) {
      const locations = roots(`taskmap-native-killed-${killStage}-`);
      const baseline = rankedPublicationCandidate();
      const baselineJournal = path.join(
        locations.runtimeRoot,
        `baseline-${killStage}.json`,
      );
      await publishTaskMapNativeProjection(
        locations.projectionPath,
        locations.currentnessPath,
      ...
      await killPublicationChild(
        ...
        ownerBoundPublicationInput(
          successor,
          TEST_OWNER_SCOPE.ownerScopeDigest,
          `killed-${killStage}-successor`,
        ),
      );

      const fixedProjection = JSON.parse(
        readFileSync(locations.projectionPath, "utf8"),
      ) as TaskMapProjectionV1;
      const tornCurrentness = JSON.parse(
        readFileSync(locations.currentnessPath, "utf8"),
      ) as TaskMapNativePublicationCandidate["currentness"];
      assert.deepEqual(fixedProjection, baseline.projection);
      assert.equal(
        tornCurrentness.projectionDigest,
        baseline.currentness.projectionDigest,
        "the visible predecessor must remain complete before the generation commit",
      );
      assert.deepEqual(
        JSON.parse(readFileSync(
          nativeTaskRankingPath(locations.projectionPath),
        ...
        baseline.ranking,
        "ranking must remain in the same visible predecessor generation",
      );
  ...
  it("keeps every concurrent reader on one complete predecessor until successor commit", async () => {
    const locations = roots("taskmap-native-concurrent-reader-");
    const baseline = rankedPublicationCandidate();
    const baselineJournal = path.join(
      locations.runtimeRoot,
      "baseline-concurrent-reader.json",
    );
    await publishTaskMapNativeProjection(
      locations.projectionPath,
      locations.currentnessPath,
    ...
    const publication = publishTaskMapNativeProjection(
      locations.projectionPath,
      locations.currentnessPath,
      path.join(locations.runtimeRoot, "concurrent-reader-journal.json"),
      ownerBoundPublicationInput(
        successor,
        TEST_OWNER_SCOPE.ownerScopeDigest,
        "concurrent-reader-successor",
      ),
      {
    ...
    try {
      for (let observation = 0; observation < 100; observation += 1) {
        ...
        assert.equal(
          visible.currentness.projectionDigest,
          baseline.currentness.projectionDigest,
        );
        assert.equal(
          visible.ranking?.artifactDigest,
    ...
    assert.equal(
      visible.ranking?.artifactDigest,
      successor.ranking?.artifactDigest,
    );
  });

  it("keeps the production resolver available across atomic reference nlink 1-to-0 replacement", async () => {
    const locations = roots("taskmap-native-reference-unlink-reader-");
    const baseline = rankedExecutablePublicationCandidate(
      projectionWithTaskCount(2, loaderCompatibleProjection()),
    );
    let now = 2_240;
    ...
    assert.equal(
      (await publisher.requestRefresh("manual")).refreshStatus,
      "published",
    );
    const referencePath = nativeTaskMapGenerationReferencePath(
      locations.projectionPath,
    );
    const baselineReferenceBytes = readFileSync(referencePath);
    ...
    writeReferencedPublicationPredecessor(
      ...
      locations.currentnessPath,
      successor,
      "reference-unlink-successor",
    );
    const successorReferenceBytes = readFileSync(referencePath);
    const successorGenerationId = digest(successor);
    writeFileSync(referencePath, baselineReferenceBytes, { mode: 0o600 });
    ...
    fileHandlePrototype.readFile = async function(
      ...
      if (armedGenerationId !== null && replacementPath !== null) {
        try {
          ...
          if (
            parsed.contractVersion === TASKMAP_NATIVE_GENERATION_REFERENCE_VERSION
            && parsed.generationId === armedGenerationId
          ) {
            renameSync(replacementPath, referencePath);
    ...
    try {
      for (let observation = 0; observation < 24; observation += 1) {
        ...
        const targetGenerationId = sourceIsBaseline
          ? successorGenerationId
          : baselineGenerationId;
        writeFileSync(referencePath, sourceBytes, { mode: 0o600 });
        replacementPath = `${referencePath}.replacement-${observation}`;
        writeFileSync(replacementPath, targetBytes, { mode: 0o600 });
        ...
        assert.equal(
          ...
          "no_op",
          "an opened complete predecessor or successor must not become unavailable",
        );
        ...
        assert.ok(
          selected.generationId === baselineGenerationId
            || selected.generationId === successorGenerationId,
        );
        assert.deepEqual(selected.projection, baseline.projection);
        assert.equal(
          selected.ranking?.artifactDigest,
          baseline.ranking?.artifactDigest,
        );
      }
    } finally {
      fileHandlePrototype.readFile = originalReadFile;
      if (replacementPath !== null) rmSync(replacementPath, { force: true });
    }
  });

  it("never rewrites an older immutable generation when its candidate digest reappears", async () => {
    const locations = roots("taskmap-native-immutable-generation-");
    const baseline = rankedPublicationCandidate();
    await publishTaskMapNativeProjection(
      locations.projectionPath,
      locations.currentnessPath,
      path.join(locations.runtimeRoot, "immutable-baseline.json"),
      ownerBoundPublicationInput(
        baseline,
        TEST_OWNER_SCOPE.ownerScopeDigest,
        "immutable-baseline",
    ...
    const baselineBytes = new Map(
      readdirSync(baselineDirectory).map((filename) => [
        filename,
        readFileSync(path.join(baselineDirectory, filename)),
      ]),
    );
    ...
    await publishTaskMapNativeProjection(
      locations.projectionPath,
      locations.currentnessPath,
      path.join(locations.runtimeRoot, "immutable-successor.json"),
      ownerBoundPublicationInput(
        successor,
        TEST_OWNER_SCOPE.ownerScopeDigest,
        "immutable-successor",
    ...
    await assert.rejects(() => publishTaskMapNativeProjection(
      locations.projectionPath,
      locations.currentnessPath,
      path.join(locations.runtimeRoot, "immutable-reappearance.json"),
      ownerBoundPublicationInput(
        baseline,
        TEST_OWNER_SCOPE.ownerScopeDigest,
        "immutable-reappearance",
      ),
    ), /publication_failed/);
    ...
    for (const [filename, bytes] of baselineBytes) {
      assert.deepEqual(
        readFileSync(path.join(baselineDirectory, filename)),
        bytes,
        `${filename} in the prior generation must remain immutable`,
      );
    }
  });

  it("keeps a complete predecessor or successor when killed at the generation commit boundary", async () => {
    for (const killStage of ["manifest", "reference"] as const) {
      const locations = roots(`taskmap-native-killed-${killStage}-`);
      ...
      assert.notEqual(
        committedGeneration(locations.projectionPath).currentWork,
        null,
        "the referenced predecessor must include current-work before the kill",
      );
      const successor = rankedCurrentWorkPublicationCandidate(
        successorProjection(baseline.projection),
      );
      const publication = ownerBoundPublicationInput(
        successor,
        TEST_OWNER_SCOPE.ownerScopeDigest,
        `${killStage}-successor`,
      );
      ...
      assert.equal(
        existsSync(path.join(
          successorDirectory,
          "taskmap-generation-manifest.v1.json",
        )),
        true,
        "the child must reach the real manifest/reference boundary",
      );
      const visibleAfterKill = committedGeneration(locations.projectionPath);
      assert.equal(
        visibleAfterKill.generationId,
        killStage === "manifest"
          ? digest(baseline)
          : publication.candidateDigest,
      );
      assert.deepEqual(
        visibleAfterKill.projection,
        killStage === "manifest"
          ? baseline.projection
          : successor.projection,
      );
      assert.equal(
        (visibleAfterKill.currentWork as {
          projection: { projectionDigest: string };
        } | null)?.projection.projectionDigest,
        visibleAfterKill.currentness.projectionDigest,
        "the selected generation must include bound current-work",
      );
      assert.equal(
        visibleAfterKill.ranking?.artifactDigest,
        killStage === "manifest"
          ? baseline.ranking?.artifactDigest
          : successor.ranking?.artifactDigest,
        "the selected generation must include bound ranking",
      );
      ...
      assert.equal(
        (recovered.currentWork as {
          projection: { projectionDigest: string };
        } | null)?.projection.projectionDigest,
        recovered.currentness.projectionDigest,
      );
      assert.equal(
        recovered.ranking?.artifactDigest,
        successor.ranking?.artifactDigest,
      );
      assert.equal(existsSync(journalPath), false);
    }
  ...
  it("fails closed then recovers when a publisher is killed after current-work mutation", async () => {
    ...
    writeReferencedPublicationPredecessor(
      ...
      locations.currentnessPath,
      baseline,
      "killed-current-work-baseline",
    );
    assert.equal(
      existsSync(nativeTaskMapGenerationReferencePath(locations.projectionPath)),
      true,
      "the current-work kill predecessor must be reference-bound",
    );
    const successor = rankedCurrentWorkPublicationCandidate(
      successorProjection(baseline.projection),
    );
    ...
    assert.equal(
      visibleAfterKill.currentness.projectionDigest,
      baseline.currentness.projectionDigest,
      "current-work publication must not tear the visible predecessor",
    );
    assert.deepEqual(
      visibleAfterKill.currentWork,
      currentWorkForProjection(baseline.projection),
    );
    assert.equal(
      visibleAfterKill.ranking?.artifactDigest,
      baseline.ranking?.artifactDigest,
  ...
  it("rejects dependency drift when the current task is also semantically reparented", async () => {
    ...
    const currentTask = projection.tasks.find((task) =>
      task.id === currentTaskId
    );
    assert.ok(currentTask);
    const oldRoot = projection.roots.find((root) =>
      root.id === currentTask.rootId
    );
    assert.ok(oldRoot);
    ...
    const dependency = projection.tasks.find((task) =>
      task.id !== currentTask.id && task.reviewState === "accepted"
    );
    ...
    await assert.rejects(
      publishTaskMapNativeProjection(
        ...
        path.join(
          locations.runtimeRoot,
          "reparent-dependency-drift-journal.v1.json",
        ),
        ownerBoundPublicationInput(
          candidate,
          TEST_OWNER_SCOPE.ownerScopeDigest,
          "reparent-dependency-drift",
        ),
      ),
      /predecessor_continuity_required/,
    );
  ...
  it("serializes concurrent exported publishers against one fixed pair", async () => {
    ...
    const projectionA = projectionWithDistinctAddedTask(
      baseline.projection,
      "tmt_a000000000000000",
    );
    const projectionB = projectionWithDistinctAddedTask(
      baseline.projection,
      "tmt_b000000000000000",
    );
    ...
    const outcomes = await Promise.allSettled([
      publishTaskMapNativeProjection(
        locations.projectionPath,
        locations.currentnessPath,
        path.join(locations.runtimeRoot, "publisher-a-journal.v1.json"),
        inputs[0],
      ),
      publishTaskMapNativeProjection(
        locations.projectionPath,
        locations.currentnessPath,
        path.join(locations.runtimeRoot, "publisher-b-journal.v1.json"),
        inputs[1],
      ),
    ]);
    ...
    const fixedCurrentness = JSON.parse(
      readFileSync(locations.currentnessPath, "utf8"),
    ) as TaskMapNativePublicationCandidate["currentness"];
    assert.deepEqual(
      fixedProjection.tasks.map((task) => task.id).sort(),
      expected.projection.tasks.map((task) => task.id).sort(),
    );
    assert.deepEqual(
      fixedCurrentness.taskDispositions.map((item) => item.taskId).sort(),
      expected.projection.tasks.map((task) => task.id).sort(),
    );
    assert.equal(
      fixedCurrentness.projectionDigest,
      diffTaskMapProjections(
        null,
        fixedProjection,
      ).currentProjectionDigest,
  ...
  it("rejects an oversized fixed projection before journal or pair mutation", async () => {
    ...
    assert.equal(
      (await service.requestRefresh("manual")).refreshStatus,
      "published",
    );
    const candidatePath = path.join(
      locations.runtimeRoot,
    ...
    assert.ok(
      Buffer.byteLength(
        `${JSON.stringify(oversizedProjection, null, 2)}\n`,
        "utf8",
      ) > 2 * 1_024 * 1_024,
    );
    const oversizedCandidate = publicationCandidate(
      0,
      oversizedProjection,
    );
    const oversizedJournalPath = path.join(
      locations.runtimeRoot,
      "oversized-publication-journal.v1.json",
    );
    ...
    await assert.rejects(
      publishTaskMapNativeProjection(
        locations.projectionPath,
        locations.currentnessPath,
        oversizedJournalPath,
        {
          graphInputDigest: "9".repeat(64),
          candidateDigest: digest(oversizedCandidate),
          candidate:
            oversizedCandidate as unknown as Record<string, unknown>,
          requestedAtMs: 2_351,
        },
      ),
      /loader_incompatible/,
    ...
    assert.deepEqual(readFileSync(candidatePath), candidateBefore);
    assert.equal(existsSync(oversizedJournalPath), false);
    assert.equal(
      existsSync(path.join(
        path.dirname(locations.projectionPath),
        ".taskmap-native-publication.lock",
  ...
  it("publishes 9 current plus 18 review-only rows only with an exhaustive bound companion", async () => {
    ...
    writeFileSync(locations.projectionPath, prior, { mode: 0o600 });
    ...
    assert.equal(companion.contractVersion, TASKMAP_NATIVE_CURRENTNESS_GATE_VERSION);
    ...
    assert.equal(
      companion.projectionDigest,
      diffTaskMapProjections(null, candidate.projection).currentProjectionDigest,
    );
    assert.equal(
      companion.taskDispositions.filter(
        (item) => item.disposition === "current",
      ).length,
      9,
    );
    assert.equal(
      companion.taskDispositions.filter(
        (item) => item.disposition === "needs_lifecycle_review",
      ).length,
      18,
    );
    assert.equal(
      new Set(companion.taskDispositions.map((item) => item.taskId)).size,
      27,
    );
    assert.equal(statSync(locations.currentnessPath).mode & 0o777, 0o600);
  });

  it("retains the exact last-good pair when currentness is partial or mismatched", async () => {
    ...
    const priorProjection = readFileSync(locations.projectionPath);
    const priorCurrentness = readFileSync(locations.currentnessPath);
    ...
    const second = new TaskMapNativeRefreshService({
      ...
      collectors: collectors(undefined, {
        agent_session: async () =>
          slice("agent_session", "agent-session-r2"),
      }),
  ...
  it("rejects a 27-to-9 task disappearance before the fixed projection commit", async () => {
    ...
    const currentOnlyProjection = projectionWithTaskCount(
      9,
      predecessorProjection,
    );
  ...
  it("fails predecessor continuity before persisting any candidate artifact", async () => {
    ...
    const projectionBefore = readFileSync(locations.projectionPath);
    const currentnessBefore = readFileSync(locations.currentnessPath);
    const currentOnly = publicationCandidate(
      0,
      projectionWithTaskCount(9, predecessorProjection),
    );
    const service = new TaskMapNativeRefreshService({
      ...locations,
      collectors: collectors(),
  ...
  it("fails closed when either referenced predecessor companion changes before staging or publication", async () => {
    for (const phase of ["before_preflight", "inside_publisher"] as const) {
      for (const mutation of ["delete", "corrupt"] as const) {
        const locations = roots(
          `taskmap-native-pair-${phase}-${mutation}-`,
        );
        const predecessor = publicationCandidate();
        writeReferencedPublicationPredecessor(
          locations.projectionPath,
          locations.currentnessPath,
          predecessor,
          `pair-${phase}-${mutation}-predecessor`,
        );
        const projectionBefore = readFileSync(locations.projectionPath);
        const referencedCurrentnessPath = path.join(
          path.dirname(locations.projectionPath),
          TASKMAP_NATIVE_GENERATIONS_DIRECTORY,
          digest(predecessor),
          path.basename(locations.currentnessPath),
        );
        const mutateCurrentness = (): void => {
          if (mutation === "delete") {
            rmSync(referencedCurrentnessPath);
          } else {
            writeFileSync(referencedCurrentnessPath, "{}\n", { mode: 0o600 });
          }
        };
        let publisherCalls = 0;
        const service = new TaskMapNativeRefreshService({
          ...
          collectors: collectors(),
          graphBuilder: async () => {
            if (phase === "before_preflight") mutateCurrentness();
            return graphBuilder(predecessor)();
          },
          publisher: async (input) => {
            publisherCalls += 1;
            if (phase === "inside_publisher") mutateCurrentness();
            return publishTaskMapNativeProjection(
              locations.projectionPath,
              locations.currentnessPath,
              path.join(
  ...
  it("keeps restart no-op deterministic but rebuilds when source disposition changes", async () => {
    ...
    assert.equal(
      (await first.requestRefresh("timer")).refreshStatus,
      "published",
    );
    ...
    const second = new TaskMapNativeRefreshService({
      ...
      nowMs: () =>
        3_000 + TASKMAP_OWNER_REFRESH_DUE_INTERVAL_MS,
    });
    ...
    const third = new TaskMapNativeRefreshService({
      ...
      nowMs: () =>
        3_000 + 2 * TASKMAP_OWNER_REFRESH_DUE_INTERVAL_MS,
    });
    ...
    assert.equal(
      result.sourceStatuses.find(
        (item) => item.source === "meeting_notes",
      )?.disposition,
      "retained_last_good",
    );
  });

  it("reverifies the referenced generation after a compatibility-mirror race", async () => {
    ...
    const replay = new TaskMapNativeRefreshService({
      ...
      collectors: collectors(undefined, {
        agent_session: async () => {
          writeFileSync(locations.currentnessPath, "{}\n", { mode: 0o600 });
          return slice("agent_session");
        },
      }),
      graphBuilder: async () => {
    ...
    assert.deepEqual(readFileSync(locations.projectionPath), projectionBefore);
    assert.deepEqual(readFileSync(candidatePath), candidateBefore);
    assert.equal(readFileSync(locations.currentnessPath, "utf8"), "{}\n");
    const verifiedState = artifact(
      locations.runtimeRoot,
      "taskmap-refresh-state.v1.json",
    );
    assert.equal(verifiedState.lastAttemptAtMs, replayAtMs);
    assert.equal(
      verifiedState.lastSuccessfulRefreshAtMs,
      replayAtMs,
    );
  ...
  it("persists the four-hour due gate while manual refresh bypasses it", async () => {
    ...
    assert.equal(
      (await service.requestRefresh("timer")).refreshStatus,
      "published",
    );
    ...
    assert.equal(
      (await service.requestRefresh("launch")).refreshStatus,
      "no_op",
    );
    assert.equal([...calls.values()].reduce((a, b) => a + b, 0), 4);
  ...
  it("invalidates the four-hour shortcut when the authenticated promotion receipt head changes", async () => {
  ...
  it("does not record a no-eligible-work no-op when the promotion receipt head changes", async () => {
  ...
  it("reports a due no-op from the verified generation despite a torn compatibility mirror", async () => {
  ...
  it("fails closed when only fixed compatibility mirrors remain owner-bound", async () => {
    ...
    rmSync(nativeTaskMapGenerationReferencePath(locations.projectionPath));
    assert.equal(existsSync(locations.projectionPath), true);
    assert.equal(existsSync(locations.currentnessPath), true);
  ...
  it("keeps a failed first attempt immediately due and retries on the next timer", async () => {
    ...
    assert.equal(
      retried.nextDueAtMs,
      retriedAtMs + TASKMAP_OWNER_REFRESH_DUE_INTERVAL_MS,
    );
  });

  it("does not move an existing success boundary after a failed due attempt", async () => {
    ...
    const dueAtMs =
      succeededAtMs + TASKMAP_OWNER_REFRESH_DUE_INTERVAL_MS;
    ...
    assert.equal(
      retryResult.nextDueAtMs,
      retryAtMs + TASKMAP_OWNER_REFRESH_DUE_INTERVAL_MS,
    );
  });

  it("serializes two processes and re-evaluates timer due state after the owner exits", async () => {
    ...
    assert.equal([...calls.values()].reduce((a, b) => a + b, 0), 4);
    assert.equal(
      existsSync(path.join(locations.runtimeRoot, "taskmap-refresh.lock")),
      false,
    );
  });

  it("keeps lock-timeout receipts immediately due without racing the active owner", async () => {
    ...
    writeFileSync(
      path.join(lockPath, `owner.${generation}.json`),
      `${JSON.stringify({
    ...
    assert.deepEqual(readFileSync(statusPath), statusBefore);
    rmSync(lockPath, { recursive: true });
  });

  it("reclaims a live PID lock whose process-start marker no longer matches", async () => {
    ...
    const service = new TaskMapNativeRefreshService({
      ...
      nowMs: () => 7_300,
      lockWaitMs: 250,
      readProcessStartMarker: async (pid) =>
        pid === process.pid ? "current-process-start" : null,
    });
  ...
  it("never removes a newer lock generation after deciding an older owner is stale", async () => {
    ...
    let permitStaleDecision!: () => void;
    const staleDecisionMayContinue = new Promise<void>((resolve) => {
      permitStaleDecision = resolve;
    });
    let staleReceiptChecked!: () => void;
    const staleReceiptWasChecked = new Promise<void>((resolve) => {
    ...
    const liveOwner = JSON.parse(
      readFileSync(
        path.join(lockPath, `owner.${generationB}.json`),
        "utf8",
      ),
    ) as { processStartMarker: string };
    assert.equal(liveOwner.processStartMarker, "current-generation-b");
    rmSync(lockPath, { recursive: true });
  });

  it("does not let a contender replace the lock during generation cleanup", async () => {
    ...
    const cleanupReceiptWasClaimed = new Promise<void>((resolve) => {
      cleanupClaimed = resolve;
    });
    let contenderMissed!: () => void;
    const contenderSawExistingLock = new Promise<void>((resolve) => {
    ...
    assert.deepEqual(
      readdirSync(locations.runtimeRoot).filter((name) =>
        name.includes("taskmap-refresh.lock")
      ),
      [],
    );
  });

  it("recovers an empty lock only from its provably stale staged owner", async () => {
    ...
    mkdirSync(lockPath, { recursive: true, mode: 0o700 });
    writeFileSync(
      `${lockPath}.candidate.${generation}.json`,
      `${JSON.stringify({
        contractVersion: "taskmap-native-refresh-lock.v2",
        generation,
        pid: process.pid,
    ...
    assert.equal(existsSync(lockPath), false);
    assert.equal(
      existsSync(`${lockPath}.candidate.${generation}.json`),
      false,
    );
  });

  it("does not remove a recreated initializer lock after a concurrent reclaimer wins", async () => {
    ...
    mkdirSync(lockPath, { recursive: true, mode: 0o700 });
    writeFileSync(
      `${lockPath}.candidate.${generationA}.json`,
      `${JSON.stringify({
        contractVersion: "taskmap-native-refresh-lock.v2",
    ...
    mkdirSync(lockPath, { mode: 0o700 });
    writeFileSync(
      `${lockPath}.candidate.${generationB}.json`,
      `${JSON.stringify({
        contractVersion: "taskmap-native-refresh-lock.v2",
    ...
    assert.equal(existsSync(lockPath), true);
    assert.equal(
      existsSync(`${lockPath}.candidate.${generationB}.json`),
      true,
    );
    rmSync(lockPath, { recursive: true });
    rmSync(`${lockPath}.candidate.${generationB}.json`);
  });

  it("reacquires for an overlapping manual request instead of swallowing it as not due", async () => {
...
describe("DEBUG-only POST /internal/taskmap/refresh scaffold", () => {
  const internalTestUser = "14802294-BEED-480E-ABF6-7E3703FA25CD";
  it("is absent by default", async () => {
    const server = createInternalServer({ expectedUserId: internalTestUser });
    servers.push(server);
    ...
    const response = await fetch(
      `http://127.0.0.1:${address.port}/internal/taskmap/refresh`,
      {
        method: "POST",
  ...
  it("requires an explicit injected test dependency", async () => {
    const server = createInternalServer({
      ...
      taskMapRefresh: async () => ({
        ...
        requestedAtMs: 6_000,
        nextDueAtMs:
          6_000 + TASKMAP_OWNER_REFRESH_DUE_INTERVAL_MS,
