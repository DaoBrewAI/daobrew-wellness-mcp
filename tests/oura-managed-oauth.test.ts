...
function fixture(): { root: string; tokenFile: string; configFile: string; owner: ReturnType<typeof confirmedOwner> } {
  const root = mkdtempSync(join(tmpdir(), "daobrew-managed-oura-"));
  roots.push(root);
  const prefs = join(root, ".daobrew");
  mkdirSync(prefs, { mode: 0o700 });
  const configFile = join(prefs, "config.json");
  ...
  return { root, tokenFile: ouraTokenFileForConfirmedOwner(owner), configFile, owner };
}

function confirmedOwner(root: string) {
  const plan = loadConfirmedTaskMapOwnerSync(root);
  if (!plan.ok) throw new Error(plan.reason);
  mkdirSync(plan.owner.sourceRoot, { recursive: true, mode: 0o700 });
  return plan.owner;
}
...
function startEnvelope(overrides: Record<string, unknown> = {}): Record<string, unknown> {
  return {
    success: true,
    data: {
...
function tokenEnvelope(overrides: Record<string, unknown> = {}): Record<string, unknown> {
  return {
    success: true,
    data: {
      token: {
...
function managedConfig() {
  return resolveManagedOuraClientConfig(fixture().owner);
}

describe("managed Oura backend protocol", () => {
  it("starts with bearer-only device auth, POST, and no request body", async () => {
    ...
    const result = await startManagedOuraAuthorization(managedConfig(), fetchImpl);
  ...
  it("exchanges only code/state and refreshes only refresh_token", async () => {
    const calls: Array<{ url: string; init?: RequestInit }> = [];
    const fetchImpl = (async (input: URL | RequestInfo, init?: RequestInit) => {
      calls.push({ url: String(input), init });
    ...
    await exchangeManagedOuraCode(config, "one-time-code", STATE, fetchImpl);
    await refreshManagedOuraToken(config, "refresh-old", fetchImpl);
  ...
  it("rejects unsafe or contract-divergent authorization starts before opening", async () => {
    const invalidStarts = [
    ...
    for (const payload of invalidStarts) {
      ...
      await assert.rejects(
        startManagedOuraAuthorization(managedConfig(), fetchImpl),
        /managed Oura/i,
      );
    }
  });

  it("does not reflect backend OAuth error bodies or secrets", async () => {
    const secret = "secret-refresh-token-from-upstream";
    const fetchImpl = (async () => jsonResponse({ error: secret }, 502)) as typeof fetch;
    await assert.rejects(
      refreshManagedOuraToken(managedConfig(), "refresh-old", fetchImpl),
      (error: any) => {
  ...
  it("rejects non-renewable or unbounded token payloads", async () => {
    for (const override of [
      ...
      const fetchImpl = (async () => jsonResponse(tokenEnvelope(override))) as typeof fetch;
      await assert.rejects(
        refreshManagedOuraToken(managedConfig(), "refresh-old", fetchImpl),
        /managed Oura/i,
      );
...
describe("managed Oura refresh persistence", () => {
  function oldManagedToken(ownerScopeDigest: string): OuraToken {
    return {
  ...
  it("refreshes via backend and preserves mode, grant generation, and authorization time", async () => {
    const { tokenFile, owner } = fixture();
    const old = oldManagedToken(owner.ownerScopeDigest);
    writeFileSync(tokenFile, JSON.stringify(old), { mode: 0o600 });
    const fetchImpl = (async () => jsonResponse(tokenEnvelope({
      access_token: "access-new",
      refresh_token: "refresh-new",
    }))) as typeof fetch;

    const refreshed = await refreshAccessToken(old, {
      fetchImpl,
    ...
    assert.deepStrictEqual(JSON.parse(readFileSync(tokenFile, "utf8")), refreshed);
  });

  it("rejects an in-flight refresh when device enrollment changes", async () => {
    ...
    await assert.rejects(
      refreshAccessToken(old, {
        fetchImpl,
      }),
      /enrollment changed while Oura (?:authorization|refresh) was in progress/i,
    );
    assert.deepStrictEqual(JSON.parse(readFileSync(tokenFile, "utf8")), old);
  });

  it("keeps missing-mode legacy tokens on the personal-app refresh path", async () => {
    const { tokenFile, owner } = fixture();
    const legacy: OuraToken = {
