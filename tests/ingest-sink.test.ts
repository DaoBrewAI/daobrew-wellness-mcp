...
import { PostgresIngestSink } from "../src/engine/ingest/sink.js";
import {
  buildDiscoveredMemoryRows,
  buildMemoryRows,
  discoverMemoryProjects,
} from "../src/engine/sources/claudeMemory.js";

describe("PostgresIngestSink", () => {
  it("runs a read-only schema readiness probe before ingesting", async () => {
    ...
    const sink = new PostgresIngestSink({
      ...
      query: async (sql) => {
        queries.push(sql);
        if (/SELECT 1 FROM events LIMIT 0/.test(sql)) return [];
  ...
  it("schema readiness failures rethrow and are retryable (no cached rejection)", async () => {
  ...
  it("user-scopes row ids so two identities never collide on the bare PK", async () => {
    const statements: string[] = [];
    const sink = new PostgresIngestSink({
      exec: async (sql) => { statements.push(sql); },
  ...
  it("upserts events idempotently and reports dedup skips", async () => {
    const statements: string[] = [];
    const sink = new PostgresIngestSink({
      exec: async (sql) => { statements.push(sql); },
      query: async () => [{ inserted: 1, updated: 0 }],
  ...
  it("writes embedding null for meeting notes and insights", async () => {
    const statements: string[] = [];
    const sink = new PostgresIngestSink({
      exec: async (sql) => { statements.push(sql); },
      query: async () => [{ inserted: 1, updated: 0 }],
  ...
  it("refreshes a changed-file session insight via id conflict and clears embedding", async () => {
  ...
  it("prunes historical duplicate rows in a separate statement BEFORE the survivor UPDATE", async () => {
    ...
    assert.doesNotMatch(
      statements[0].slice(updateIdx),
      /DELETE FROM/,
      "survivor UPDATE must not embed the prune in a CTE (Postgres runs unreferenced modifying CTEs after the main statement)",
    );
    assert.strictEqual(statements.some((sql) => sql.includes("INSERT INTO user_insights")), false);
  });

  it("keeps embedding on a metadata-only insight change", async () => {
    const statements: string[] = [];
    const sink = new PostgresIngestSink({
      exec: async (sql) => { statements.push(sql); },
      query: async () => [{
  ...
  it("still dedup-skips an unchanged insight", async () => {
    const statements: string[] = [];
    const sink = new PostgresIngestSink({
      exec: async (sql) => { statements.push(sql); },
      query: async () => [{
  ...
  it("preserves lifecycle-owned importance and strength on an unchanged re-ingest", async () => {
    const statements: string[] = [];
    const sink = new PostgresIngestSink({
      exec: async (sql) => { statements.push(sql); },
      query: async () => [{
  ...
  it("persists lifecycle-owned fields for changed existing ADD, NOOP, and SUPERSEDES rows atomically", async () => {
    for (const decisionKind of ["add", "noop_duplicate", "supersedes"] as const) {
      const statements: string[] = [];
      let llmCalls = 0;
      ...
      const sink = new PostgresIngestSink({
        ...
        query: async (sql) => {
          ...
          if (/ORDER BY created_at_ts DESC/.test(sql) && !/id IN/.test(sql)) return [];
          if (/id IN/.test(sql)) {
            return targetId === null ? [] : [{
              id: targetId,
              source,
  ...
  it("submits the row mutation and durable decision in one rollback-capable scoped exec", async () => {
    const statements: string[] = [];
    let llmCalls = 0;
    const row = {
      id: "incoming-atomic",
    ...
    const sink = new PostgresIngestSink({
      ...
      query: async (sql) => {
        if (/SELECT 1 FROM events LIMIT 0/.test(sql)) return [];
        if (/FROM insight_lifecycle_decisions/.test(sql)) return [];
        if (/ORDER BY embedding <=>|ORDER BY created_at_ts DESC/.test(sql)) return [];
        if (/FROM user_insights/.test(sql)) return [{
          source: row.source,
...
describe("memory ingest project attribution (topics churn regression)", () => {
  function writeSession(root: string, dir: string, name: string, cwd: string, task: string): string {
    const projectDir = join(root, dir);
    mkdirSync(projectDir, { recursive: true });
    const file = join(projectDir, name);
    writeFileSync(file, JSON.stringify({ cwd, type: "user", message: { content: task } }) + "\n");
    return file;
  }

  function withSessions<T>(run: (files: { s1: string; s2: string; s3: string }) => T): T {
    const root = mkdtempSync(join(tmpdir(), "daobrew-attribution-"));
    try {
      return run({
        ...
        s3: writeSession(
          root,
          "-Users-alice-Work-app--claude-worktrees-fix-1",
          "s3.jsonl",
          "/Users/alice/Work/app/.claude/worktrees/fix-1",
          "Patch the flaky retry",
        ),
      });
    } finally {
  ...
  it("a parent-path project does not claim other projects' session files", () => {
    withSessions(({ s1, s2, s3 }) => {
      const rows = buildMemoryRows({ projectPath: "/Users/alice", codexFiles: [], claudeFiles: [s1, s2, s3] });
      assert.deepEqual(rows.map((row) => row.source_ref), [s1]);
      assert.deepEqual(rows[0].topics, ["#claude-session", "#alice"]);
    });
  });

  it("a project does not claim its worktrees' session files (and vice versa)", () => {
    withSessions(({ s1, s2, s3 }) => {
      const appRows = buildMemoryRows({
        projectPath: "/Users/alice/Work/app", codexFiles: [], claudeFiles: [s1, s2, s3],
      });
      assert.deepEqual(appRows.map((row) => row.source_ref), [s2]);
      const worktreeRows = buildMemoryRows({
  ...
  it("overlapping configured projects never produce the same insight key with different topics", () => {
    withSessions(({ s1, s2, s3 }) => {
      const claudeFiles = [s1, s2, s3];
      const byProject = ["/Users/alice", "/Users/alice/Work/app", "/Users/alice/Work/app/.claude/worktrees/fix-1"]
        .map((projectPath) => buildMemoryRows({ projectPath, codexFiles: [], claudeFiles }));
  ...
  it("streams every discovery file once while preserving legacy rows and order", async () => {
    const root = mkdtempSync(join(tmpdir(), "daobrew-single-pass-"));
    try {
      const projectA = join(root, "Work", "alpha");
      const projectB = join(root, "Work", "beta");
      const claudeRoot = join(root, "home", ".claude", "projects");
      const codexRoot = join(root, "home", ".codex", "sessions", "2026", "08");
      const claudeDir = join(
        claudeRoot,
        projectA.replace(/[^A-Za-z0-9]/g, "-"),
      );
      mkdirSync(claudeDir, { recursive: true });
      mkdirSync(codexRoot, { recursive: true });
      const claudeFile = join(claudeDir, "claude.jsonl");
      const codexFile = join(codexRoot, "codex.jsonl");
      ...
      const roots = {
        ...
        codexSessionsRoot: join(root, "home", ".codex", "sessions"),
      };
      const projectPaths = discoverMemoryProjects(roots);
      const expected = projectPaths.flatMap((projectPath) => buildMemoryRows({
        projectPath,
        codexFiles: [codexFile],
        claudeFiles: [claudeFile],
      }));
      const reads = new Map<string, number>();
      const actual = await buildDiscoveredMemoryRows({
        ...roots,
        codexFiles: [codexFile],
        claudeFiles: [claudeFile],
...
describe("PostgresIngestSink RLS scoping", () => {
   ...
   */
  const execScopeOpen = (userId: string) => `BEGIN;\nSELECT set_config('${GUC}', '${userId}', true);`;

  function occurrences(haystack: string, needle: string): number {
    return haystack.split(needle).length - 1;
  }

  function makeSink(statements: string[], queries: string[], existing: Record<string, any>[] = []) {
    return new PostgresIngestSink({
      exec: async (sql) => { statements.push(sql); },
  ...
  function fetchQueries(queries: string[]): string[] {
  ...
  it("scopes every event INSERT with the batch user's GUC exactly once", async () => {
    const statements: string[] = [];
    const queries: string[] = [];
    ...
    for (const sql of inserts) {
      assert.ok(
        sql.startsWith(execScopeOpen("u1")),
        `event INSERT must start with the scoped BEGIN + transaction-local set_config, got: ${sql}`,
      );
  ...
  it("scopes the dedup read-back query with the batch user (single-statement lateral form)", async () => {
    const statements: string[] = [];
    const queries: string[] = [];
    const sink = makeSink(statements, queries);
    ...
    assert.ok(
      ...
      `read-back query must carry the GUC set_config, got: ${sql}`,
    );
  ...
  it("dedup read-back filters user_id by the scope row, never by a literal (fail-closed regression)", async () => {
    ...
    for (const sql of dataQueries) {
      assert.ok(
        ...
        `read-back must filter user_id via the scope row, got: ${sql}`,
      );
      assert.strictEqual(
        sql.includes("user_id = 'u1'"),
        false,
        "read-back must NOT compare user_id to a literal (creates the One-Time Filter fail-closed trap)",
      );
    }
  });

  it("scopes meeting-note and insight INSERTs with the batch user's GUC exactly once", async () => {
    const statements: string[] = [];
  ...
  it("scopes each batch with its own user across sequential batches on one sink", async () => {
  ...
  it("leaves the read-only schema readiness probe unscoped", async () => {
    ...
    assert.strictEqual(queries[0], "SELECT 1 FROM events LIMIT 0;");
    assert.strictEqual(
      occurrences(queries[0], "set_config"),
      0,
      "readiness probe must not carry the GUC",
    );
