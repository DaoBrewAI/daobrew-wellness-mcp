...
type DevelopmentMockModule = typeof import("./mock.js");

function developmentMockModule(): DevelopmentMockModule {
  return require("./mock.js") as DevelopmentMockModule;
}
...
export async function handleToolCall(
  ...
  if (isMock) developmentMockModule();
...
export function toolUserId(): string {
  ...
  if (env) return env;
  const config = readLocalConfig() as { user_id?: unknown };
    ...
    const sql = graphStoreKind() === "postgres"
      ? `UPDATE graph_nodes
         ...
         INSERT INTO graph_edges(id, user_id, src_id, dst_id, kind, label, created_at_ts)
           SELECT 'e_done_' || ${q(causeId)}, user_id, id, ${q(causeId)}, 'caused', 'detonated', ${now}
      ...
      : `UPDATE graph_nodes SET props_json = json_set(props_json, '$.status', 'done', '$.artifact_ref', ${q(artifactRef)}, '$.done_at_ts', ${now}${acceptancePairSqlite}) WHERE id = ${q(causeId)};
       INSERT OR IGNORE INTO graph_edges(id, src_id, dst_id, kind, label, created_at_ts)
         SELECT 'e_done_' || ${q(causeId)}, id, ${q(causeId)}, 'caused', 'detonated', ${now}
         FROM graph_nodes WHERE kind = 'pattern' ORDER BY created_at_ts LIMIT 1;`;
    await exec(sql);
