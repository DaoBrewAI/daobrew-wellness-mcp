...
export function renderDashboardPNG(data: DashboardData): Buffer {
...
function drawHeader(ctx: SKRSContext2D, data: DashboardData): void {
...
function drawPatternCard(ctx: SKRSContext2D, data: DashboardData): void {
  const copy = STRESS_PATTERN_COPY[data.stressPattern];
  const pX = 32;
  const pY = 88;
  const pW = 380;
  const pH = 140;
  ...
  const deltaPart =
    Math.abs(data.stressScoreDelta) >= 1
...
function drawHRCard(ctx: SKRSContext2D, data: DashboardData): void {
  const hX = 432;
  const hY = 88;
  const hW = 536;
  const hH = 140;
  ...
  if (!stale && firstReal != null && data.currentHR != null) {
    ...
    ctx.font = "13px -apple-system, sans-serif";
    ctx.fillText(
      `${arrow} ${Math.abs(pct).toFixed(0)}%  (was ${Math.round(firstReal)})`,
      hX + 20,
      hY + 124,
    );
  } else {
...
function drawSparkline(
  ...
  const samples = data.hrHistory.filter((v): v is number => v != null);
  ...
  const allPoints: Array<{ x: number; y: number; v: number | null }> = data.hrHistory.map((v, i) => ({
    x: sxStart + (i / Math.max(1, data.hrHistory.length - 1)) * sw,
    y: v == null ? syEnd : syEnd - ((v - minV) / range) * sh,
    v,
  }));

  const accent = stale ? "#6B7280" : STRESS_PATTERN_COPY[data.stressPattern].color;
  ...
  if (tail.length >= 2) {
    ...
    for (const p of tail.slice(1)) ctx.lineTo(p.x, p.y);
...
function drawBreathCard(ctx: SKRSContext2D, data: DashboardData): void {
  const bX = 32;
  const bY = 252;
  const bW = 936;
  const bH = 220;
  ...
  ctx.font = "12px -apple-system, sans-serif";
  ctx.fillText(
    `BREATH  · cycle ${data.cycleNum} / ${data.totalCycles}   ·   ${data.breathBPM.toFixed(1)} BPM   ·   ratio ${data.inhaleRatio.toFixed(2)}`,
    bX + 24,
    bY + 28,
  );
...
function drawBottomBar(ctx: SKRSContext2D, data: DashboardData): void {
  const sX = 32;
  const sY = 496;
  const sW = 936;
  const sH = 92;
