...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    // New shape: handler returned `{ __mcp_content__: true, content: [...] }`
    // We forward `content` verbatim. Inject mock warning into the leading text block.
    if (result && typeof result === "object" && (result as any).__mcp_content__) {
      const blocks = (result as any).content as Array<any>;
      if (isMock && blocks.length > 0 && blocks[0].type === "text") {
        blocks[0] = {
          ...blocks[0],
    ...
    // Legacy shape: stringify the JSON result.
    ...
    case "breathe":
      return {
        ...
        messages: [
          {
            role: "user" as const,
            content: { type: "text" as const, text: "I need a breathing break. Call daobrew_check first to identify my top stress pattern, then daobrew_breathe with that pattern. After it returns, poll daobrew_check every 2 seconds and show me the dashboard image until I say stop or the session ends." },
          },
        ],
      };
    case "stress":
