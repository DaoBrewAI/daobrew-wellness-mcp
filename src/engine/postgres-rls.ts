...
import { POSTGRES_TABLES } from "./postgres-schema.js";

/** Session GUC carrying the tenant id; policies read it with `current_setting(..., true)`. */
export const RLS_GUC = "app.daobrew_user_id";
...
export const PERMISSIVE_TABLES = [
  "pipeline_metrics",
  "transfer_records",
  "population_priors",
] as const;

// Compile-time guard: both lists may only name real schema tables.
type PostgresTable = (typeof POSTGRES_TABLES)[number];
const _userScopedAreTables: readonly PostgresTable[] = USER_SCOPED_TABLES;
const _permissiveAreTables: readonly PostgresTable[] = PERMISSIVE_TABLES;
void _userScopedAreTables;
void _permissiveAreTables;
 ...
 */
export function appRoleProvisionSql(roleName: string): string {
  if (!/^[a-z_][a-z0-9_]*$/.test(roleName)) {
    throw new Error(`invalid role name: ${JSON.stringify(roleName)} (expected a simple lowercase SQL identifier)`);
  }
  return `CREATE ROLE ${roleName} LOGIN NOSUPERUSER NOBYPASSRLS PASSWORD :'app_password';
