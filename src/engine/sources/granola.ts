...
function toTsOrNull(value: unknown): number | null {
...
const MEETING_KINDS: MeetingRow["kind"][] = ["meeting", "call", "interview", "one_on_one", "standup", "other"];
...
function toSpans(raw: unknown): TranscriptSpan[] {
  if (!Array.isArray(raw)) return [];
  return raw
    .map((entry: any, idx: number): TranscriptSpan => {
      if (typeof entry === "string") return { idx, speaker: null, ts_offset_sec: null, text: entry };
 ...
 */
export async function fetchGranolaNotes(options: FetchGranolaOptions): Promise<any[]> {
  const token = options.token ?? resolveGranolaToken() ?? undefined;
  if (!token) {
    throw new Error(
      "Granola reader requires GRANOLA_API_TOKEN, granola_api_token in ~/.daobrew/config.json, or rawNotes in the ingest payload",
    );
  }
...
async function fetchPersonalV1(token: string, options: FetchGranolaOptions): Promise<any[]> {
...
async function fetchWorkspaceV2(token: string, options: FetchGranolaOptions): Promise<any[]> {
  ...
  const docsPayload: any = await docsResponse.json();
  ...
  for (const doc of docs) {
    if (!doc?.id) continue;
    let transcript: Array<{ speaker: string | null; text: string }> = [];
    try {
      ...
      if (transcriptResponse.ok) {
        const transcriptPayload: any = await transcriptResponse.json();
        ...
        transcript = segments
          ...
          .filter((segment: { text: string }) => segment.text.trim().length > 0);
      }
    } catch {
...
export function normalizeGranolaNotes(rawNotes: any[]): MeetingRow[] {
  return rawNotes.map((note) => {
    ...
    return {
      ...
      topics: Array.isArray(note.topics) ? note.topics.map((topic: unknown) => String(topic)) : [],
    };
  });
}
