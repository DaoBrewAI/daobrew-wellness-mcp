import {
  ...
  TASKMAP_AGENT_SESSION_PRODUCER_VERSION,
  ...
  type TaskMapAgentSessionGraphEpisodeSelectionDraftV1,
  type TaskMapAgentSessionGraphEpisodeSelectionV1,
  type TaskMapAgentSessionDirectiveDispositionV2,
  type TaskMapAgentSessionProducerSnapshotV1,
  type TaskMapAgentSessionProvider,
...
export const TASKMAP_AGENT_SESSION_SEMANTIC_ADMISSION_VERSION =
  "taskmap-agent-session-semantic-admission.v2" as const;
export const TASKMAP_AGENT_SESSION_WORKSTREAM_IDENTITY_DOMAIN =
  "taskmap-agent-session-workstream-identity.2" as const;
export const TASKMAP_AGENT_SESSION_PROPOSAL_CLUSTER_IDENTITY_DOMAIN =
  "taskmap-agent-session-proposal-cluster-identity.2" as const;
export const TASKMAP_AGENT_SESSION_GRAPH_FEED_VERSION =
  "taskmap-agent-session-graph-feed.v1" as const;
export const TASKMAP_AGENT_SESSION_GRAPH_EPISODE_IDENTITY_DOMAIN =
  "taskmap-agent-session-graph-episode-identity.1" as const;
export const TASKMAP_AGENT_SESSION_GRAPH_EPISODE_REVISION_DOMAIN =
  "taskmap-agent-session-graph-episode-revision.1" as const;

export const TASKMAP_AGENT_SESSION_SEMANTIC_ADMISSION_LIMITS_V2 =
  Object.freeze({
...
const DISPOSITIONS: readonly TaskMapAgentSessionDirectiveDispositionV2[] = [
...
export interface TaskMapAgentSessionSemanticAdmissionV2 {
  ...
  sourceSnapshotContractVersion:
    typeof TASKMAP_AGENT_SESSION_PRODUCER_SNAPSHOT_VERSION;
...
export interface TaskMapAgentSessionGraphFeedEpisodeV1
  extends TaskMapAgentSessionWorkEpisodeV1 {
...
interface RoutedEpisode {
  episode: TaskMapAgentSessionWorkEpisodeV1;
  routingKind: TaskMapAgentSessionRoutingKindV2;
  providerNeutralRoutingDigest: string;
  workstreamIdentityDigest: string;
}
...
function routeEpisode(
  ownerScopeDigest: string,
  episode: TaskMapAgentSessionWorkEpisodeV1,
): RoutedEpisode | null {
  const repository =
    episode.routing.providerNeutralRepositoryIdentityDigests[0];
  const project = episode.routing.providerNeutralProjectIdentityDigests[0];
  const routingKind: TaskMapAgentSessionRoutingKindV2 | null =
  ...
  const workstreamIdentityDigest = taskMapContractDigest({
    domain: TASKMAP_AGENT_SESSION_WORKSTREAM_IDENTITY_DOMAIN,
    ownerScopeDigest,
    routingKind,
...
export function taskMapAgentSessionIsoWeek(timestamp: string): string {
  ...
  const week = Math.floor(
    (date.getTime() - yearStart.getTime()) / (7 * 86_400_000),
  ) + 1;
  return `${year}-W${String(week).padStart(2, "0")}`;
}

function compareGraphEpisodeRecency(
  left: TaskMapAgentSessionWorkEpisodeV1,
  right: TaskMapAgentSessionWorkEpisodeV1,
): number {
  const occurred = Date.parse(right.occurredAt) - Date.parse(left.occurredAt);
...
function graphFeedPayload(
  value: Omit<TaskMapAgentSessionGraphFeedV1, "feedDigest" | "feedId">,
): Omit<TaskMapAgentSessionGraphFeedV1, "feedDigest" | "feedId"> {
  return value;
}
 ...
 */
export function buildTaskMapAgentSessionGraphFeed(
  input: TaskMapAgentSessionGraphEpisodeSelectionDraftV1,
): TaskMapAgentSessionGraphFeedV1 {
  const candidates = selectTaskMapAgentSessionGraphEpisodeCandidates(input);
 ...
 */
export function buildTaskMapAgentSessionGraphFeedFromSnapshot(
  snapshot: TaskMapAgentSessionProducerSnapshotV1,
): TaskMapAgentSessionGraphFeedV1 {
  assertTaskMapAgentSessionProducerSnapshot(snapshot);
  const recurrenceEpisodes = snapshot.sessions.filter((episode) =>
...
function buildTaskMapAgentSessionGraphFeedFromSelection(
  candidates: TaskMapAgentSessionGraphEpisodeSelectionV1,
): TaskMapAgentSessionGraphFeedV1 {
  const routed = candidates.episodes
    .map((episode) => routeEpisode(candidates.ownerScopeDigest, episode))
    .filter((row): row is RoutedEpisode => row !== null);
  ...
  const recurrenceByDirective = new Map<
    string,
    { count: number; firstSeenAt: string }
  >();
  ...
  const grouped = new Map<string, RoutedEpisode[]>();
  ...
  const summaries: TaskMapAgentSessionGraphFeedEpisodeV1[] = [];
  for (const members of grouped.values()) {
    members.sort((left, right) =>
      compareGraphEpisodeRecency(left.episode, right.episode)
    );
    ...
    const week = taskMapAgentSessionIsoWeek(
      representative.episode.occurredAt,
    );
    const base = {
      ...
      providerNeutralRoutingDigest:
        representative.providerNeutralRoutingDigest,
      isoWeek: week,
      recurrenceCount: recurrence.count,
      firstSeenAt: recurrence.firstSeenAt,
    };
    const graphEpisodeDigest = taskMapContractDigest({
      domain: TASKMAP_AGENT_SESSION_GRAPH_EPISODE_IDENTITY_DOMAIN,
    ...
    const graphEpisodeRevisionDigest = taskMapContractDigest({
      domain: TASKMAP_AGENT_SESSION_GRAPH_EPISODE_REVISION_DOMAIN,
      graphEpisodeDigest,
      representativeEpisodeRevisionDigest:
        representative.episode.episodeRevisionDigest,
      recurrenceCount: recurrence.count,
      firstSeenAt: recurrence.firstSeenAt,
    });
    summaries.push({
  ...
  const canonicalBuckets = [...byBucket.values()].sort((left, right) => {
    const workstream = compareCodePoint(
      left.workstreamIdentityDigest,
      right.workstreamIdentityDigest,
    );
    return workstream !== 0
      ? workstream
  ...
  const buckets = canonicalBuckets.map((bucket, bucketIndex) => {
    ...
    const fairEpisodes: TaskMapAgentSessionGraphFeedEpisodeV1[] = [];
    for (let round = 0; ; round += 1) {
      let added = false;
      for (let step = 0; step < providerQueues.length; step += 1) {
        const queue = providerQueues[
          (bucketIndex + round + step) % providerQueues.length
        ];
  ...
  for (const bucket of buckets) {
    const workstreamBuckets =
      bucketsByWorkstream.get(bucket.workstreamIdentityDigest) ?? [];
    workstreamBuckets.push(bucket);
    bucketsByWorkstream.set(
      bucket.workstreamIdentityDigest,
      workstreamBuckets,
    );
  }
  const workstreamQueues = [...bucketsByWorkstream.entries()]
    .map(([workstreamIdentityDigest, weekBuckets]) => ({
      workstreamIdentityDigest,
      weekBuckets: weekBuckets.sort((left, right) =>
        compareCodePoint(right.isoWeek, left.isoWeek)
      ),
    }))
  ...
  const selected: TaskMapAgentSessionGraphFeedEpisodeV1[] = [];
  selection: for (let episodeRound = 0; ; episodeRound += 1) {
    ...
    for (let weekRound = 0; ; weekRound += 1) {
      ...
      for (const workstream of workstreamQueues) {
        ...
        weekRoundAdded = true;
        if (
          selected.length === TASKMAP_GRAPH_BRAIN_LIMITS_V1.maxEpisodesGlobal
        ) break selection;
      }
      if (!weekRoundAdded) break;
    }
    if (!episodeRoundAdded) break;
  }
  const episodes = selected.sort((left, right) =>
    compareCodePoint(left.graphEpisodeId, right.graphEpisodeId)
  );
  const base = graphFeedPayload({
    ...
    authority: "none",
    acceptedMembershipAuthority: false,
    limits: TASKMAP_GRAPH_BRAIN_LIMITS_V1,
    counts: {
...
function supportFromEpisode(
  episode: TaskMapAgentSessionWorkEpisodeV1,
): TaskMapAgentSessionProposalSupportV2 {
  return {
...
function compareClusterRecency(
  left: TaskMapAgentSessionProposalClusterV2,
  right: TaskMapAgentSessionProposalClusterV2,
): number {
  const occurred = Date.parse(right.occurredAt) - Date.parse(left.occurredAt);
  if (occurred !== 0) return occurred;
  const observed = Date.parse(right.observedAt) - Date.parse(left.observedAt);
...
export function buildTaskMapAgentSessionSemanticAdmission(
  snapshot: TaskMapAgentSessionProducerSnapshotV1,
): TaskMapAgentSessionSemanticAdmissionV2 {
  assertTaskMapAgentSessionProducerSnapshot(snapshot);
  const latest = selectLatestTaskMapAgentSessionWorkEpisodesByRoot(
    snapshot.sessions,
  );
  const dispositionCount = new Map<
    TaskMapAgentSessionDirectiveDispositionV2,
    number
  >(DISPOSITIONS.map((disposition) => [disposition, 0]));
  for (const episode of latest) {
    dispositionCount.set(
      episode.disposition,
      (dispositionCount.get(episode.disposition) ?? 0) + 1,
    );
  }
  const work = latest.filter(
    (episode) => episode.disposition === "work_candidate",
  );
  const routed: RoutedEpisode[] = [];
  let unrouted = 0;
  ...
  const byLineage = new Map<string, RoutedEpisode>();
  let duplicateLineage = 0;
  for (const row of routed.sort((left, right) =>
    compareEpisodeRecency(left.episode, right.episode)
  )) {
  ...
  const unboundedClusters: TaskMapAgentSessionProposalClusterV2[] = [];
  for (const members of grouped.values()) {
    members.sort((left, right) =>
      compareEpisodeRecency(left.episode, right.episode)
    );
    ...
    const supports = members
      ...
      .sort((left, right) =>
        left.supportIdentityDigest.localeCompare(right.supportIdentityDigest)
      );
    supportOverflow += Math.max(
      0,
      supports.length
        - TASKMAP_AGENT_SESSION_SEMANTIC_ADMISSION_LIMITS_V2
          .maxSupportKeysPerCluster,
    );
    const boundedSupports = supports.slice(
      0,
      TASKMAP_AGENT_SESSION_SEMANTIC_ADMISSION_LIMITS_V2
        .maxSupportKeysPerCluster,
    );
    const clusterIdentityDigest = taskMapContractDigest({
      domain: TASKMAP_AGENT_SESSION_PROPOSAL_CLUSTER_IDENTITY_DOMAIN,
      ownerScopeDigest: snapshot.ownerScopeDigest,
      workstreamIdentityDigest: representative.workstreamIdentityDigest,
      directiveSemanticDigest:
        representative.episode.directiveSemanticDigest,
    ...
    unboundedClusters.push({
      ...
      assistantOutcomeSummary:
        representative.episode.assistantOutcomeSummary,
  ...
  const workstreamQueues = [...byWorkstream.entries()].map(
    ([workstreamIdentityDigest, clusters]) => {
      clusters.sort(compareClusterRecency);
      clusterOverflow += Math.max(
        0,
        clusters.length
          - TASKMAP_AGENT_SESSION_SEMANTIC_ADMISSION_LIMITS_V2
    ...
    const recency = compareClusterRecency(
      left.clusters[0],
      right.clusters[0],
    );
    return recency !== 0
      ? recency
      : left.workstreamIdentityDigest.localeCompare(
        right.workstreamIdentityDigest,
      );
  });

  const selected: TaskMapAgentSessionProposalClusterV2[] = [];
  for (
    let round = 0;
    selected.length
      < TASKMAP_AGENT_SESSION_SEMANTIC_ADMISSION_LIMITS_V2.maxClustersGlobal;
    round += 1
  ) {
    let added = false;
  ...
  const perWorkstreamRetained = workstreamQueues.reduce(
    (sum, queue) => sum + queue.clusters.length,
    0,
  );
  clusterOverflow += perWorkstreamRetained - selected.length;
  const clusters = selected.sort((left, right) => {
    const workstream = left.workstreamIdentityDigest.localeCompare(
      right.workstreamIdentityDigest,
    );
    return workstream !== 0
      ? workstream
  ...
  const base = admissionPayload({
    contractVersion: TASKMAP_AGENT_SESSION_SEMANTIC_ADMISSION_VERSION,
    ...
    authority: "none",
    acceptedMembershipAuthority: false,
    limits: TASKMAP_AGENT_SESSION_SEMANTIC_ADMISSION_LIMITS_V2,
    counts: {
  ...
  const admissionDigest = taskMapContractDigest(base);
  const admission: TaskMapAgentSessionSemanticAdmissionV2 = {
    ...base,
...
function assertSupport(
  value: unknown,
  label: string,
): asserts value is TaskMapAgentSessionProposalSupportV2 {
...
function assertCluster(
  ...
  ownerScopeDigest: string,
  label: string,
): asserts value is TaskMapAgentSessionProposalClusterV2 {
  assertPlainObject(value, label);
  assertClosedKeys(value, [
  ...
  assertDigest(
    value.workstreamIdentityDigest,
    `${label}.workstreamIdentityDigest`,
  );
  assertDigest(
    value.providerNeutralRoutingDigest,
    `${label}.providerNeutralRoutingDigest`,
  );
  assertDigest(
  ...
  value.supports.forEach((support, index) =>
    assertSupport(support, `${label}.supports[${index}]`)
  );
  const canonicalSupports = [...value.supports].sort((left, right) =>
    left.supportIdentityDigest.localeCompare(right.supportIdentityDigest)
  );
  if (
    new Set(value.supports.map((support) => support.supportIdentityDigest)).size
      !== value.supports.length
    || value.supports.some(
      (support, index) => support !== canonicalSupports[index],
    )
  ) {
...
export function assertTaskMapAgentSessionSemanticAdmission(
  value: unknown,
): asserts value is TaskMapAgentSessionSemanticAdmissionV2 {
  ...
  if (
    value.contractVersion !== TASKMAP_AGENT_SESSION_SEMANTIC_ADMISSION_VERSION
  ...
  if (
    value.limits.maxClustersGlobal
      !== TASKMAP_AGENT_SESSION_SEMANTIC_ADMISSION_LIMITS_V2.maxClustersGlobal
  ...
  if (
    value.dispositionCounts.length !== DISPOSITIONS.length
  ...
  const counts = value.counts as unknown as
    TaskMapAgentSessionSemanticAdmissionV2["counts"];
  const canonicalDispositionCounts = value.dispositionCounts as unknown as
    TaskMapAgentSessionSemanticAdmissionV2["dispositionCounts"];
  const dispositionTotal = canonicalDispositionCounts.reduce(
    (sum, row) => sum + row.count,
    0,
  );
  const workCount = canonicalDispositionCounts.find(
    (row) => row.disposition === "work_candidate",
  )?.count ?? 0;
  const nonWorkCount = dispositionTotal - workCount;
  const routedLineages = workCount - counts.unrouted;
  const uniqueAdmittedLineages =
    routedLineages - counts.duplicateLineage;
  if (
    dispositionTotal !== counts.latestTurns
  ...
  value.clusters.forEach((cluster, index) =>
    assertCluster(cluster, ownerScopeDigest, `clusters[${index}]`)
  );
  const visibleSupportCount = value.clusters.reduce(
    (sum, cluster) => sum + cluster.supports.length,
    0,
  );
  const representedClusterCount =
    value.clusters.length + counts.clusterOverflow;
  const hiddenRetained =
    uniqueAdmittedLineages - visibleSupportCount - counts.supportOverflow;
  const maximumHiddenRetained =
    counts.clusterOverflow
    * TASKMAP_AGENT_SESSION_SEMANTIC_ADMISSION_LIMITS_V2.maxSupportKeysPerCluster;
  if (
    counts.supportOverflow > uniqueAdmittedLineages
  ...
  if (
    new Set(value.clusters.map((cluster) => cluster.clusterIdentityDigest)).size
      !== value.clusters.length
    || value.clusters.some(
      (cluster, index) => cluster !== canonicalClusters[index],
    )
  ) {
  ...
  if ([...workstreamCounts.values()].some(
    (count) =>
      count
        > TASKMAP_AGENT_SESSION_SEMANTIC_ADMISSION_LIMITS_V2
          .maxClustersPerWorkstream,
  ...
  const { admissionId, admissionDigest, ...base } =
    value as unknown as TaskMapAgentSessionSemanticAdmissionV2;
  if (
    admissionDigest !== taskMapContractDigest(base)
    || admissionId !== stableId("tmaadmission", admissionDigest)
  ) {
