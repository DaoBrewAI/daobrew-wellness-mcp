 ...
 */
export const TASKMAP_AGENT_SESSION_PRODUCER_SNAPSHOT_VERSION =
  "taskmap-agent-session-producer-snapshot.v2" as const;
export const TASKMAP_AGENT_SESSION_PRODUCER_RESULT_VERSION =
  "taskmap-agent-session-producer-result.v2" as const;
export const TASKMAP_AGENT_SESSION_PRODUCER_VERSION =
  "taskmap-agent-session-producer.2" as const;
export const TASKMAP_AGENT_SESSION_PRODUCER_MAX_AGE_MS =
  14_400_000 as const;
export const TASKMAP_AGENT_SESSION_CONTEXT_WINDOW_MS =
  30 * 24 * 60 * 60 * 1_000;
export const TASKMAP_AGENT_SESSION_OWNER_SCOPE_DOMAIN =
  "taskmap-agent-session-owner-local.1" as const;
export const TASKMAP_AGENT_SESSION_IDENTITY_DOMAIN =
  "taskmap-agent-session-identity.1" as const;
export const TASKMAP_AGENT_SESSION_EPISODE_IDENTITY_DOMAIN =
  "taskmap-agent-session-work-episode-identity.2" as const;
export const TASKMAP_AGENT_SESSION_TURN_LINEAGE_IDENTITY_DOMAIN =
  "taskmap-agent-session-turn-lineage-identity.2" as const;
export const TASKMAP_AGENT_SESSION_DIRECTIVE_SEMANTIC_DOMAIN =
  "taskmap-agent-session-directive-semantic.2" as const;
export const TASKMAP_AGENT_SESSION_PROJECT_ROUTING_DOMAIN =
  "taskmap-agent-session-project-routing.1" as const;
export const TASKMAP_AGENT_SESSION_REPOSITORY_ROUTING_DOMAIN =
  "taskmap-agent-session-repository-routing.1" as const;
export const TASKMAP_AGENT_SESSION_PROVIDER_NEUTRAL_PROJECT_ROUTING_DOMAIN =
  "taskmap-agent-session-provider-neutral-project-routing.2" as const;
export const TASKMAP_AGENT_SESSION_PROVIDER_NEUTRAL_REPOSITORY_ROUTING_DOMAIN =
  "taskmap-agent-session-provider-neutral-repository-routing.2" as const;

export const TASKMAP_AGENT_SESSION_PRODUCER_LIMITS_V1 = Object.freeze({
  maxObservations: 128,
...
export const TASKMAP_AGENT_SESSION_PRODUCER_LIMITS_V2 =
...
const OWNER_LOCAL_PATH =
  /(?:^|[\s("'`])(?:~\/|\/(?:Users|home|private|var|tmp|Volumes|Applications|Library|opt|etc)\/|[A-Za-z]:[\\/]|\\\\)[^\s<>"')\]]+/g;
const GENERIC_ABSOLUTE_POSIX_PATH =
  /(?:^|[\s("'`:=])\/(?!\/)(?!(?:quit|exit)(?:\(\))?(?=$|[\s.,;:!?]))[^\s<>"')\],;]+/g;
const PRIVATE_KEY =
  /-----BEGIN [A-Z ]*PRIVATE KEY-----[\s\S]*?-----END [A-Z ]*PRIVATE KEY-----/gi;
const CREDENTIAL_ASSIGNMENT =
  /\b(api[_-]?key|access[_-]?token|refresh[_-]?token|client[_-]?secret|password|passwd|secret|token)\s*[:=]\s*["']?[^\s"',;]{4,}/gi;
const BEARER_SECRET =
  /\bbearer\s+[A-Za-z0-9._~+/=-]{8,}/gi;
const PROVIDER_TOKEN =
  /\b(?:sk-(?:proj-)?[A-Za-z0-9_-]{16,}|(?:ghp|gho|ghu|ghs|ghr)_[A-Za-z0-9]{16,}|grn_[A-Za-z0-9_-]{12,}|dbk_[A-Za-z0-9_-]{12,}|xox[baprs]-[A-Za-z0-9-]{12,}|AKIA[0-9A-Z]{16})\b/g;
const PROVIDER_TOKEN_PREFIX =
  /\b(?:sk-(?:proj-)?|ghp_|gho_|ghu_|ghs_|ghr_|grn_|dbk_|xox[baprs]-)/gi;
const JWT_SECRET =
  /\b[A-Za-z0-9_-]{8,}\.[A-Za-z0-9_-]{8,}\.[A-Za-z0-9_-]{8,}\b/g;
const LONG_OPAQUE_TOKEN =
  /\b[A-Za-z0-9_+=-]{32,}\b/g;
...
export type TaskMapAgentSessionDirectiveDispositionV2 =
...
export type TaskMapAgentSessionGraphDirectiveClassificationV1 =
  | TaskMapAgentSessionDirectiveDispositionV2
  | "execution_receipt";
...
export type TaskMapAgentSessionFreshnessDecision =
  | "fresh"
  | "boundary_due"
  | "stale"
  | "missing"
...
export type TaskMapAgentSessionObservationV2 =
  TaskMapAgentSessionObservationV1;
export type TaskMapAgentSessionProducerSnapshotDraftV2 =
  TaskMapAgentSessionProducerSnapshotDraftV1;
export type TaskMapAgentSessionWorkEpisodeRoutingV2 =
  TaskMapAgentSessionWorkEpisodeRoutingV1;
export type TaskMapAgentSessionWorkEpisodeV2 =
  TaskMapAgentSessionWorkEpisodeV1;
export type TaskMapAgentSessionProducerRejectionsV2 =
  TaskMapAgentSessionProducerRejectionsV1;
export type TaskMapAgentSessionProducerSnapshotV2 =
  TaskMapAgentSessionProducerSnapshotV1;
export type TaskMapAgentSessionProducerResultV2 =
  TaskMapAgentSessionProducerResultV1;
export type TaskMapAgentSessionProducerLoadInputV2 =
  TaskMapAgentSessionProducerLoadInputV1;
export type TaskMapAgentSessionProducerWriteInputV2 =
  TaskMapAgentSessionProducerWriteInputV1;
...
interface RejectedObservation {
  status: "rejected";
  reason: RejectionReason;
}

type ParsedObservation = AcceptedObservation | RejectedObservation;
...
function privacySanitizedText(value: string): string {
...
function sanitizedSummary(value: string, maximum: number): string {
  return boundedUtf16(privacySanitizedText(value), maximum);
}

function directiveSemanticDigest(value: string): string {
  ...
  return taskMapContractDigest({
    domain: TASKMAP_AGENT_SESSION_DIRECTIVE_SEMANTIC_DOMAIN,
    canonicalDirective,
  });
}

function assertPersistedSummary(
  value: unknown,
  maximum: number,
  label: string,
  nullable = false,
...
const CODEX_IMAGE_PLACEHOLDER =
  /\[Image:\s*[^\]\r\n]+\]\s*\uFFFC?/giu;

const TASKMAP_AGENT_SESSION_COMPACTED_MARKER =
  "taskmap_agent_session_compacted_v2" as const;
...
interface AnalyzedUserDirective {
  disposition: TaskMapAgentSessionDirectiveDispositionV2;
  directiveSemanticDigest: string;
  userDirectiveSummary: string;
}

function compactedDirectiveMetadata(
  item: Record<string, unknown>,
): AnalyzedUserDirective | null {
  ...
  const message = messageRoleAndContent(
    item,
    item.type === "response_item" ? "codex" : "claude",
  );
  const summary = sanitizedSummary(
    message === null ? "" : contentText(message.content),
    TASKMAP_AGENT_SESSION_PRODUCER_LIMITS_V1.maxUserDirectiveCharacters,
  );
  if (
    !TASKMAP_AGENT_SESSION_DISPOSITIONS.has(
      disposition as TaskMapAgentSessionDirectiveDispositionV2,
    )
  ...
  return {
    disposition: disposition as TaskMapAgentSessionDirectiveDispositionV2,
...
function classifyDirective(
  ...
  if (/^\d{1,2}$/.test(canonical)) return "option_only";
  return "work_candidate";
}

function isTaskMapExecutionReceiptDirective(value: string): boolean {
  const canonical = toWellFormedText(value)
    .normalize("NFKC")
    .replace(/\s+/g, " ")
    .trim();
  return TASKMAP_EXECUTION_RECEIPT_DIRECTIVES.some(
    (pattern) => pattern.test(canonical),
  );
}

export function classifyTaskMapAgentSessionDirectiveForGraph(
  value: string,
): TaskMapAgentSessionGraphDirectiveClassificationV1 {
 ...
 */
export function isTaskMapAgentSessionDiscoveryPathEligible(
  provider: TaskMapAgentSessionProvider,
...
function analyzeUserDirective(
  ...
  const explicitContinuity = CONTINUITY_ONLY_USER_TEXT.some(
  ...
  if (!privacySafeDirective) return null;
  const disposition = continuityOnly
    ? "continuity_only"
    : classifyDirective(privacySafeDirective);
  return {
    disposition,
    directiveSemanticDigest: directiveSemanticDigest(
      continuityOnly ? "" : privacySafeDirective,
    ),
    userDirectiveSummary: boundedUtf16(
      privacySafeDirective,
      TASKMAP_AGENT_SESSION_PRODUCER_LIMITS_V1.maxUserDirectiveCharacters,
    ),
  };
}
...
function opaqueIdentity(
  ...
  if (
    ...
    || Buffer.byteLength(value, "utf8")
      > TASKMAP_AGENT_SESSION_PRODUCER_LIMITS_V1.maxNativeIdentityBytes
  ) {
    return null;
  }
  return value;
...
function routingIdentityDigest(
  domain:
    | typeof TASKMAP_AGENT_SESSION_PROJECT_ROUTING_DOMAIN
    | typeof TASKMAP_AGENT_SESSION_REPOSITORY_ROUTING_DOMAIN,
  provider: TaskMapAgentSessionProvider,
  nativeIdentity: string,
): string {
  return taskMapContractDigest({ domain, provider, nativeIdentity });
}

function providerNeutralRoutingIdentityDigest(
  domain:
    | typeof TASKMAP_AGENT_SESSION_PROVIDER_NEUTRAL_PROJECT_ROUTING_DOMAIN
    | typeof TASKMAP_AGENT_SESSION_PROVIDER_NEUTRAL_REPOSITORY_ROUTING_DOMAIN,
  ownerScopeDigest: string,
  nativeIdentity: string,
): string {
  return taskMapContractDigest({
...
function boundedRoutingIdentities(values: unknown[]): string[] {
  const unique = new Set<string>();
  for (const value of values) {
    const bounded = opaqueIdentity(value, "routing identity");
    if (bounded !== null) unique.add(bounded);
  }
  return [...unique]
...
function routingIdentities(
  ...
  providerNeutralProject: string[] | null;
  providerNeutralRepository: string[] | null;
} {
  const payload = isPlainObject(item.payload) ? item.payload : null;
  ...
  const projectRoutingPresent = projectKeys.some(
    (key) => hasOwn(item, key) || hasOwn(payload, key),
  );
  const repositoryRoutingPresent = repositoryKeys.some(
    (key) => hasOwn(item, key) || hasOwn(payload, key),
  ) || hasOwn(payload, "workspace_roots")
  ...
  return {
    project: !projectRoutingPresent
      ...
      : projects.map((value) =>
        routingIdentityDigest(
          TASKMAP_AGENT_SESSION_PROJECT_ROUTING_DOMAIN,
          provider,
          value,
        )
      ).sort((left, right) => left.localeCompare(right)),
    repository: !repositoryRoutingPresent
      ...
      : repositories.map((value) =>
        routingIdentityDigest(
          TASKMAP_AGENT_SESSION_REPOSITORY_ROUTING_DOMAIN,
          provider,
          value,
        )
      ).sort((left, right) => left.localeCompare(right)),
    providerNeutralProject: !projectRoutingPresent
      ? null
      : projects.map((value) =>
        providerNeutralRoutingIdentityDigest(
...
function nativeUserTurnIdentity(
  item: Record<string, unknown>,
): string | null {
  const payload = isPlainObject(item.payload) ? item.payload : null;
  const message = isPlainObject(item.message) ? item.message : null;
...
function messageRoleAndContent(
  item: Record<string, unknown>,
  provider: TaskMapAgentSessionProvider,
): { role: unknown; content: unknown } | null {
  if (
    provider === "codex"
...
function compactedTimestamp(item: Record<string, unknown>): string | null {
  try {
...
function compactedRoutingFields(
  ...
  for (const key of COMPACTED_ROUTING_SCALAR_KEYS) {
    ...
    if (bounded !== null) fields[key] = bounded;
  }
  for (const key of ["workspace_roots", "workspaceRoots"] as const) {
...
function compactedClaudeSessionFields(
  item: Record<string, unknown>,
): Record<string, string> {
  const fields: Record<string, string> = {};
  const sessionIdentity = opaqueIdentity(
    item.sessionId ?? item.session_id,
    "Claude session identity",
  );
  const parentIdentity = opaqueIdentity(
    item.parentSessionId ?? item.parent_session_id,
    "Claude parent session identity",
  );
  if (sessionIdentity !== null) fields.sessionId = sessionIdentity;
  if (parentIdentity !== null) fields.parentSessionId = parentIdentity;
  return fields;
}
 ...
 */
export function compactTaskMapAgentSessionJsonlLine(
  provider: TaskMapAgentSessionProvider,
  rawLine: string,
): TaskMapAgentSessionCompactedJsonlLineV1 | null {
  let parsed: unknown;
  try {
  ...
  if (provider === "codex" && item.type === "session_meta") {
    ...
    if (timestamp !== null) compacted.timestamp = timestamp;
    return {
      kind: "identity",
      jsonlLine: taskMapContractCanonicalJson(compacted),
    };
  }
  ...
  if (message === null) {
    if (!routingPresent) return null;
    const compacted: Record<string, unknown> = provider === "codex"
      ? {
        ...topLevelRouting,
        type: "turn_context",
  ...
  let summary: string;
  let kind: "user" | "assistant";
  let analyzedDirective: AnalyzedUserDirective | null = null;
  if (message.role === "user") {
    analyzedDirective = analyzeUserDirective(
      item,
      contentText(message.content),
    );
    if (analyzedDirective === null) return null;
...
function freezeWorkEpisode(
  draft: MutableWorkEpisode,
): TaskMapAgentSessionWorkEpisodeV1 {
  const base = {
    ...
    semanticUnit: "bounded_user_turn_work_episode" as const,
    recordKind: "work_context" as const,
    proposalDisposition: "candidate_or_context_only" as const,
  ...
  const episodeRevisionDigest = taskMapContractDigest(base);
  return {
    ...base,
...
function compareEpisodeRecency(
  left: TaskMapAgentSessionWorkEpisodeV1,
  right: TaskMapAgentSessionWorkEpisodeV1,
): number {
...
function parseObservation(
  ...
  limits: {
    maxRawBytesPerObservation: number;
    maxEpisodesPerRootSession: number | null;
    excludeExecutionReceipts: boolean;
  },
): ParsedObservation {
  if (
  ...
  let nativeSessionIdentity: string | null = null;
  let nativeParentIdentity: string | null = null;
  const parsedLines: ParsedSessionLine[] = [];
  ...
  for (const line of lines) {
    ...
    if (observation.provider === "claude") {
      const candidate = opaqueIdentity(
        item.sessionId ?? item.session_id,
        "Claude session identity",
      );
      if (
  ...
  const rootSessionIdentityDigest = identityDigest(
  ...
  let projectIdentityDigests: string[] = [];
  let repositoryIdentityDigests: string[] = [];
  let providerNeutralProjectIdentityDigests: string[] = [];
  let providerNeutralRepositoryIdentityDigests: string[] = [];
  let current: MutableWorkEpisode | null = null;
  const workEpisodes: TaskMapAgentSessionWorkEpisodeV1[] = [];
  let userOrdinal = 0;
  ...
  for (const { item, timestamp } of parsedLines) {
    const routing = routingIdentities(
      item,
      observation.provider,
      ownerScopeDigest,
    );
    ...
    if (message.role === "user") {
      ...
      if (analyzedDirective === null || timestamp === null) continue;
      finishCurrent();
      ...
      const fallbackIdentity = nativeTurnIdentity === null
        ...
        : null;
      const episodeIdentityDigest = taskMapContractDigest({
      ...
      const turnLineageIdentityDigest = nativeTurnIdentity === null
        ? analyzedDirective.directiveSemanticDigest
        : taskMapContractDigest({
          domain: TASKMAP_AGENT_SESSION_TURN_LINEAGE_IDENTITY_DOMAIN,
          provider: observation.provider,
    ...
    const assistantOutcomeSummary = sanitizedSummary(
      text,
      TASKMAP_AGENT_SESSION_PRODUCER_LIMITS_V1
        .maxAssistantOutcomeCharacters,
    );
    if (!assistantOutcomeSummary) continue;
  ...
  const contextCutoffMs =
    Date.parse(producedAt) - TASKMAP_AGENT_SESSION_CONTEXT_WINDOW_MS;
...
export function buildTaskMapAgentSessionProducerSnapshot(
  input: TaskMapAgentSessionProducerSnapshotDraftV1,
): TaskMapAgentSessionProducerSnapshotV1 {
  assertPlainObject(input, "agent session producer draft");
  assertClosedKeys(
    input,
    ["ownerScopeDigest", "observations", "producedAt"],
    "agent session producer draft",
  );
  ...
  const rejections: TaskMapAgentSessionProducerRejectionsV1 = {
    malformed: 0,
    oversize: 0,
    missingIdentity: 0,
  ...
  const acceptedByIdentity =
    new Map<string, TaskMapAgentSessionWorkEpisodeV1>();
  let chargedBytes = 0;
  ...
  for (const observation of input.observations) {
    ...
    if (
      ...
      chargedBytes += Math.min(
        Buffer.byteLength(observation.rawJsonl, "utf8"),
    ...
    for (const episode of parsed.sessions) {
      const previous = acceptedByIdentity.get(
  ...
  const byRoot = new Map<string, TaskMapAgentSessionWorkEpisodeV1[]>();
  ...
  const globallyBounded = globallyNewest.slice(
    0,
  ...
  const sessions = globallyBounded.sort((left, right) =>
    left.episodeId.localeCompare(right.episodeId)
  );
  const rejectedCount = Object.values(rejections).reduce(
    (sum, count) => sum + count,
    0,
  );
  const coverage = input.observations.length === 0
    ? "fresh_empty" as const
  ...
  const base = snapshotPayload({
    contractVersion: TASKMAP_AGENT_SESSION_PRODUCER_SNAPSHOT_VERSION,
    producerVersion: TASKMAP_AGENT_SESSION_PRODUCER_VERSION,
    ownerScopeDigest: input.ownerScopeDigest,
    producedAt,
    validThrough,
    maxAgeMs: TASKMAP_AGENT_SESSION_PRODUCER_MAX_AGE_MS,
    coverage,
    observedCount: input.observations.length,
    rejections,
    watermark: {
      kind: "episode_revision",
      valueDigest: taskMapContractDigest(watermarkRows),
      observedThrough,
    },
    sessions,
    privacy: PRIVACY,
...
function compareGraphEpisodeChronology(
  left: TaskMapAgentSessionWorkEpisodeV1,
  right: TaskMapAgentSessionWorkEpisodeV1,
): number {
 ...
 */
export function selectTaskMapAgentSessionGraphEpisodeCandidates(
  input: TaskMapAgentSessionGraphEpisodeSelectionDraftV1,
): TaskMapAgentSessionGraphEpisodeSelectionV1 {
  assertPlainObject(input, "agent session graph episode selection draft");
  assertClosedKeys(
    input,
    ["ownerScopeDigest", "observations", "producedAt"],
    "agent session graph episode selection draft",
  );
  assertDigest(input.ownerScopeDigest, "ownerScopeDigest");
  const producedAt = canonicalTimestamp(input.producedAt, "producedAt");
  if (
    !Array.isArray(input.observations)
    || input.observations.length > TASKMAP_GRAPH_BRAIN_LIMITS_V1.maxObservations
  ) {
  ...
  input.observations.forEach((observation, index) => {
    if (
      ...
      const rawBytes = Buffer.byteLength(observation.rawJsonl, "utf8");
      if (rawBytes > TASKMAP_GRAPH_BRAIN_LIMITS_V1.maxRawBytesPerObservation) {
        throw new Error(`graph observation ${index} exceeds its byte limit`);
      }
      totalRawBytes += rawBytes;
      if (totalRawBytes > TASKMAP_GRAPH_BRAIN_LIMITS_V1.maxRawBytesGlobal) {
  ...
  input.observations.forEach((observation, index) => {
    const parsed = parseObservation(
      ...
      input.ownerScopeDigest,
      producedAt,
      TASKMAP_GRAPH_BRAIN_LIMITS_V1.maxRawBytesGlobal - chargedBytes,
      {
        maxRawBytesPerObservation:
    ...
    if (parsed.status === "rejected") {
      throw new Error(
        `graph observation ${index} is not eligible: ${parsed.reason}`,
      );
    }
  ...
  const episodes = [...boundaryEpisodes.values()].sort((left, right) =>
    compareCodePoint(left.episodeId, right.episodeId)
  );
  episodes.forEach((episode, index) =>
    assertSession(episode, `graph episodes[${index}]`)
  );
  return deepFreeze({
    ownerScopeDigest: input.ownerScopeDigest,
    producedAt,
...
function assertRouting(
  value: unknown,
  label: string,
): asserts value is TaskMapAgentSessionWorkEpisodeRoutingV1 {
  ...
  for (const key of [
    ...
    digests.forEach((digest, index) =>
      assertDigest(digest, `${label}.${key}[${index}]`)
    );
    const expected = [...digests].sort((left, right) =>
...
function assertSession(
  value: unknown,
  label: string,
): asserts value is TaskMapAgentSessionWorkEpisodeV1 {
  assertPlainObject(value, label);
  assertClosedKeys(value, [
  ...
  assertDigest(
    value.episodeIdentityDigest,
    `${label}.episodeIdentityDigest`,
  );
  assertDigest(
    value.turnLineageIdentityDigest,
    `${label}.turnLineageIdentityDigest`,
  );
  assertDigest(
    value.directiveSemanticDigest,
    `${label}.directiveSemanticDigest`,
  );
  assertDigest(
    value.rootSessionIdentityDigest,
    `${label}.rootSessionIdentityDigest`,
  );
  if (value.parentRootSessionIdentityDigest !== null) {
    assertDigest(
      value.parentRootSessionIdentityDigest,
      `${label}.parentRootSessionIdentityDigest`,
    );
  }
  assertDigest(
    value.episodeRevisionDigest,
    `${label}.episodeRevisionDigest`,
  );
  if (value.provider !== "codex" && value.provider !== "claude") {
    throw new Error(`${label}.provider is invalid`);
  }
  if (!TASKMAP_AGENT_SESSION_DISPOSITIONS.has(value.disposition as TaskMapAgentSessionDirectiveDispositionV2)) {
  ...
  assertPersistedSummary(
    value.userDirectiveSummary,
    TASKMAP_AGENT_SESSION_PRODUCER_LIMITS_V1.maxUserDirectiveCharacters,
    `${label}.userDirectiveSummary`,
  );
  assertPersistedSummary(
    value.assistantOutcomeSummary,
    TASKMAP_AGENT_SESSION_PRODUCER_LIMITS_V1
      .maxAssistantOutcomeCharacters,
    `${label}.assistantOutcomeSummary`,
    true,
  );
  ...
  const {
    ...
    episodeRevisionDigest,
    ...base
  } = value as unknown as TaskMapAgentSessionWorkEpisodeV1;
  if (
    episodeId !== stableId("tmaepisode", value.episodeIdentityDigest)
    || episodeRevisionDigest !== taskMapContractDigest(base)
  ) {
    throw new Error(`${label} identity or revision is inconsistent`);
  }
}

export function assertTaskMapAgentSessionProducerSnapshot(
  value: unknown,
): asserts value is TaskMapAgentSessionProducerSnapshotV1 {
  ...
  if (
    value.contractVersion !== TASKMAP_AGENT_SESSION_PRODUCER_SNAPSHOT_VERSION
    || value.producerVersion !== TASKMAP_AGENT_SESSION_PRODUCER_VERSION
  ) {
  ...
  if (
    value.maxAgeMs !== TASKMAP_AGENT_SESSION_PRODUCER_MAX_AGE_MS
  ...
  if (
    ...
    || (value.observedCount as number) < 0
    || (value.observedCount as number)
      > TASKMAP_AGENT_SESSION_PRODUCER_LIMITS_V1.maxObservations
  ) {
    throw new Error("observedCount is invalid");
  }
  const observedCount = value.observedCount as number;
  ...
  value.sessions.forEach((session, index) => {
    ...
    rootCounts.set(
      session.rootSessionIdentityDigest,
      (rootCounts.get(session.rootSessionIdentityDigest) ?? 0) + 1,
    );
  });
  if (
    observedCount < rejectedObservationCount
    || [...rootCounts.values()].some(
      (count) =>
        count
          > TASKMAP_AGENT_SESSION_PRODUCER_LIMITS_V1
            .maxEpisodesPerRootSession,
  ...
  if (
    value.sessions.some(
      (episode, index) => episode.episodeId !== canonicalEpisodeIds[index],
    )
  ) {
  ...
  assertClosedKeys(
    value.watermark,
    ["kind", "observedThrough", "valueDigest"],
    "watermark",
  );
  ...
  const observedThrough = canonicalTimestamp(
    value.watermark.observedThrough,
  ...
  if (
    observedThrough !== expectedObservedThrough
    || value.watermark.valueDigest !== taskMapContractDigest(
      value.sessions.map((episode) => ({
        episodeIdentityDigest: episode.episodeIdentityDigest,
        episodeRevisionDigest: episode.episodeRevisionDigest,
      })),
    )
  ...
  if (
    Object.values(value.privacy).some((stored) => stored !== false)
  ) {
  ...
  const {
    ...
    snapshotDigest,
    ...base
  } = value as unknown as TaskMapAgentSessionProducerSnapshotV1;
  if (
    snapshotDigest !== taskMapContractDigest(base)
    || snapshotId !== stableId("tmassnapshot", snapshotDigest)
 ...
 */
export function selectLatestTaskMapAgentSessionWorkEpisodesByRoot(
  sessions: readonly TaskMapAgentSessionWorkEpisodeV1[],
): TaskMapAgentSessionWorkEpisodeV1[] {
  if (
...
function buildResult(input: {
  ...
  const base = {
    contractVersion: TASKMAP_AGENT_SESSION_PRODUCER_RESULT_VERSION,
    producerVersion: TASKMAP_AGENT_SESSION_PRODUCER_VERSION,
...
export function assessTaskMapAgentSessionProducerSnapshot(
  value: unknown,
...
export function taskMapAgentSessionOwnerScopeDigest(userId: string): string {
  return taskMapOwnerScopeDigest(userId);
}

export function taskMapAgentSessionProducerSnapshotPath(
  ...
  return path.join(
    ...
    ".daobrew",
    "taskmap",
    "agent-session-producer-snapshot.v2.json",
  );
}

function assertAbsoluteSnapshotPath(snapshotPath: unknown): asserts snapshotPath is string {
  if (
 ...
 */
export async function writeTaskMapAgentSessionProducerSnapshot(
  input: TaskMapAgentSessionProducerWriteInputV1,
): Promise<void> {
  ...
  const parent = path.dirname(input.snapshotPath);
  await mkdir(parent, { recursive: true, mode: 0o700 });
  await chmod(parent, 0o700);
  const temporary = path.join(
    parent,
    `.${path.basename(input.snapshotPath)}.${randomUUID()}.tmp`,
  );
  try {
    const bytes = `${taskMapContractCanonicalJson(input.snapshot)}\n`;
    ...
    await rename(temporary, input.snapshotPath);
    await chmod(input.snapshotPath, 0o600);
  } finally {
...
function unavailableResult(
  decision: Extract<
    TaskMapAgentSessionFreshnessDecision,
    "missing" | "malformed"
  >,
  assessedAt: string,
  detailCode: string,
 ...
 */
export async function loadTaskMapAgentSessionProducerResult(
  input: TaskMapAgentSessionProducerLoadInputV1,
): Promise<TaskMapAgentSessionProducerResultV1> {
  assertPlainObject(input, "agent session producer load input");
  assertClosedKeys(
    input,
    ["assessedAt", "expectedOwnerScopeDigest", "snapshotPath"],
    "load input",
  );
  ...
  try {
    ...
    if (
      ...
      || before.size < 2n
      || before.size
        > BigInt(TASKMAP_AGENT_SESSION_PRODUCER_LIMITS_V1.maxSnapshotBytes)
    ) {
      return unavailableResult(
        "malformed",
        assessedAt,
    ...
    return unavailableResult(
      errorCode(error) === "ENOENT" ? "missing" : "malformed",
      assessedAt,
