...
import {
  ...
  TaskMapAblationReport,
  TaskMapCausalGateResult,
  TaskMapCitation,
  TaskMapEdge,
  TaskMapEvent,
  TaskMapExperimentArm,
  TaskMapHarnessOptions,
  TaskMapInput,
  TaskMapProjectionV1,
  TaskMapProjectionSource,
  TaskMapRejection,
  TaskMapReviewState,
  TaskMapRoot,
  TaskMapRootScoreBreakdown,
  TaskMapScoreBreakdown,
...
export const TASKMAP_ALGORITHM_POLICY = {
  version: TASKMAP_ALGORITHM_POLICY_VERSION,
  ...
  causal: {
    ...
    minCorpusSourcesPerDay: DEFAULT_ENRICHMENT_THRESHOLDS.minSourcesPerDay,
    rtmSuspected: "true-when-any-extreme-target-hit-is-cited",
    minNeutralReferenceDays: DEFAULT_ENRICHMENT_THRESHOLDS.minBackedDays,
    neutralReferenceRationale: "require at least the same three-day floor as target backed-day evidence",
    enrichmentThresholds: DEFAULT_ENRICHMENT_THRESHOLDS,
  },
  scoreWeights: {
    sourcePriority: 0.25,
...
const VALID_ARMS = new Set<TaskMapExperimentArm>(["E0", "E1", "E2", "E3", "E4"]);
...
function stableSourceIdentity(pointer: TaskMapSourcePointer | undefined): string | undefined {
  return pointer ? `${pointer.sourceKind}:${pointer.sourceRefHash}` : undefined;
}

function hasReturnCapability(pointer: TaskMapSourcePointer): boolean {
  return pointer.syncMode !== "reference_only"
    && pointer.capabilities.some((capability) => RETURN_CAPABILITIES.has(capability));
}

function hasSourceOwnedRoute(pointer: TaskMapSourcePointer): boolean {
...
const ALLOWED_CAPABILITIES_BY_SYNC_MODE: Record<string, ReadonlySet<string>> = {
...
const TEXT_LEAK_PATTERNS: Array<[string, RegExp]> = [
...
export const TASKMAP_ALGORITHM_POLICY_DIGEST = digest(TASKMAP_ALGORITHM_POLICY);

export function taskMapInputDigest(input: TaskMapInput): string {
  return digest(input);
}
 ...
 */
export function taskMapSemanticInput(input: TaskMapInput): TaskMapInput {
...
export function taskMapSemanticInputDigest(input: TaskMapInput): string {
...
function unknownFieldReasons(
  ...
  return Object.keys(value as Record<string, unknown>).some((key) => !allowed.has(key))
    ? [`${label} contains unknown fields`]
    : [];
}

function isStrictRfc3339(value: string | undefined): boolean {
...
function decodedTextVariants(text: string): string[] {
  const variants = [text];
  let current = text;
...
function rawBiometricLeakReasons(text: string): string[] {
  if (!BIOMETRIC_TERM.test(text)) return [];
  let residual = text.replace(
    /\b(?:\d+(?:\.\d+)?|zero|one|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve|thirteen|fourteen|fifteen|sixteen|seventeen|eighteen|nineteen|twenty|thirty|forty|fifty|sixty|seventy|eighty|ninety|hundred|thousand)\s+(?:windows?|days?|events?|records?|sources?|matches?|signals?)\b/gi,
    "",
  );
  residual = residual.replace(
    /\b(?:\d+(?:\.\d+)?|zero|one|two|three|four|five|six|seven|eight|nine|ten)\s+[^.\n]{0,32}\bwindows?\b/gi,
    (match) => BIOMETRIC_UNIT.test(match) ? match : "",
  );
  return NUMBER_TOKEN.test(residual) ? ["privacy leak: raw_biometric"] : [];
}

function textLeakReasons(text: string): string[] {
...
function boundedTextReasons(title: string, summary: string): string[] {
  const reasons = [
    ...textLeakReasons(title),
    ...textLeakReasons(summary),
  ];
  if (!title.trim()) reasons.push("title is empty");
  if (title.length > MAX_TITLE) reasons.push(`title exceeds ${MAX_TITLE} characters`);
  if (summary.length > MAX_SUMMARY) reasons.push(`summary exceeds ${MAX_SUMMARY} characters`);
  return unique(reasons);
}

export function taskMapProjectionPrivacyLeakReasons(value: unknown): string[] {
  if (typeof value === "string") return textLeakReasons(value);
  if (Array.isArray(value)) {
    return unique(value.flatMap(taskMapProjectionPrivacyLeakReasons));
  }
  if (value && typeof value === "object") {
    return unique(Object.values(value as Record<string, unknown>)
      .flatMap(taskMapProjectionPrivacyLeakReasons));
  }
  return [];
}

function safeIdReasons(label: string, value: string, pattern = SAFE_PUBLIC_ID): string[] {
  const reasons = textLeakReasons(value);
  if (!pattern.test(value)) reasons.push(`${label} is not a safe opaque id`);
  return unique(reasons);
}
...
function latestAuthoritativeEvents(input: TaskMapInput): Map<string, TaskMapEvent> {
  ...
  for (const event of validResolvedEvents(input)) {
    ...
    if (order > 0) latest.set(event.pointerId, event);
  }
  return latest;
}
...
function citations(
  ids: readonly string[],
  events: Map<string, TaskMapEvent>,
  pointers: Map<string, TaskMapSourcePointer>,
): TaskMapCitation[] {
...
function nonBodyEvidence(
  ids: readonly string[],
  events: ReadonlyMap<string, TaskMapEvent>,
): TaskMapEvent[] {
  return unique(ids)
    .map((id) => events.get(id))
    .filter((event): event is TaskMapEvent => Boolean(event && event.recordKind !== "body_context"));
}

function isAgentSessionReviewOnlyEvidence(
  evidence: readonly TaskMapEvent[],
...
function validateInput(input: TaskMapInput): TaskMapRejection[] {
  const rejections: TaskMapRejection[] = [];
  ...
  const invalidPointerIds = new Set<string>();
  for (const pointer of input.pointers) {
    const reasons = [
    ...
    if ((pointerCounts.get(pointer.id) ?? 0) > 1) reasons.push("duplicate pointer id");
    if ((sourceIdentityCounts.get(`${pointer.sourceKind}:${pointer.sourceRefHash}`) ?? 0) > 1) {
      reasons.push("duplicate source identity");
    }
    if (!VALID_SOURCE_KINDS.has(pointer.sourceKind)) reasons.push("source kind is invalid");
    if (!VALID_AUTHORITIES.has(pointer.authority)) reasons.push("source authority is invalid");
    if (!VALID_SYNC_MODES.has(pointer.syncMode)) reasons.push("source sync mode is invalid");
    ...
    if (pointer.canonicalUrl) {
      try {
        ...
        if (!["https:", "http:"].includes(url.protocol)) reasons.push("canonical URL must be http(s)");
        if (url.username || url.password) reasons.push("canonical URL must not contain credentials");
        ...
        for (const key of url.searchParams.keys()) {
          if (
            /(?:token|secret|api[_-]?key|auth|code|pwd|pass(?:word)?|sig(?:nature)?|credential|access[_-]?key|session(?:id)?)/i
              .test(key)
          ) {
  ...
  for (const event of input.events) {
    ...
    if ((eventCounts.get(event.id) ?? 0) > 1) reasons.push("duplicate event id");
    if (pointerIds.has(event.id)) reasons.push("event id collides with pointer id");
    if (!pointerIds.has(event.pointerId)) reasons.push("pointer does not exist");
    if (invalidPointerIds.has(event.pointerId)) reasons.push("pointer was rejected");
    if (!isStrictRfc3339(event.occurredAt) || !isStrictRfc3339(event.observedAt)) {
      ...
      if (
        isStrictRfc3339(input.generatedAt)
    ...
    if (!VALID_RECORD_KINDS.has(event.recordKind)) reasons.push("record kind is invalid");
    if (!VALID_ACTIVITIES.has(event.activity)) reasons.push("activity is invalid");
    ...
    if (event.recordKind === "body_context") {
      if (!isRealDayKey(event.dayKey)) reasons.push("body context requires an explicit source-local dayKey");
      if (!event.bodyCategory) reasons.push("body context must have a relative category");
      ...
      if (event.bodyCategory !== undefined) reasons.push("body category is only valid on body context");
      if (event.bodyAxis !== undefined) reasons.push("body axis is only valid on body context");
    }
    ...
    if (event.recordKind === "authoritative_task" && pointer) {
      if (pointer.authority === "none") reasons.push("authoritative task pointer must own status");
    ...
    if (event.supersedesEventId) {
      if (!eventIds.has(event.supersedesEventId)) reasons.push("superseded event does not exist");
      if (event.supersedesEventId === event.id) reasons.push("event cannot supersede itself");
      const target = input.events.find((candidate) => candidate.id === event.supersedesEventId);
      if (target) {
        if (target.pointerId !== event.pointerId) reasons.push("supersedes must stay within one source pointer");
        if (target.recordKind !== event.recordKind) reasons.push("supersedes must preserve record kind");
        ...
        if (!later) reasons.push("superseding event must be observed after its target");
      }
    }
    if (event.retractsEventId) {
      if (!eventIds.has(event.retractsEventId)) reasons.push("retracted event does not exist");
      if (event.retractsEventId === event.id) reasons.push("event cannot retract itself");
      const target = input.events.find((candidate) => candidate.id === event.retractsEventId);
      if (target) {
        if (target.pointerId !== event.pointerId) reasons.push("retracts must stay within one source pointer");
        if (target.recordKind !== event.recordKind) reasons.push("retracts must preserve record kind");
        const later = Date.parse(event.observedAt) > Date.parse(target.observedAt)
          || (
            Date.parse(event.observedAt) === Date.parse(target.observedAt)
            && Date.parse(event.occurredAt) > Date.parse(target.occurredAt)
          );
        if (!later) reasons.push("retracting event must be observed after its target");
      }
    }
    if (reasons.length) rejections.push({ proposalId: event.id, kind: "input", reasons: unique(reasons) });
  }
  return rejections;
}

/** Read-only validation of the complete Task Map input contract. */
export function taskMapInputValidationReasons(input: TaskMapInput): string[] {
  return unique(validateInput(input).flatMap((rejection) =>
    rejection.reasons
  ));
}

function validResolvedEvents(input: TaskMapInput): TaskMapEvent[] {
  const eventIds = new Set(input.events.map((event) => event.id));
...
function validateRoot(
  ...
  events: Map<string, TaskMapEvent>,
  semanticEventIds: ReadonlySet<string>,
): string[] {
  const reasons = [
  ...
  if (root.evidenceEventIds.some((id) => !events.has(id))) reasons.push("evidence event does not exist");
  ...
  if (evidence.length === 0) reasons.push("body context cannot create a root");
  ...
  if (!(root.confidence >= 0 && root.confidence <= 1)) reasons.push("confidence must be in [0,1]");
  return unique(reasons);
}

function validateTask(
  task: BrainTaskProposal,
  acceptedRoots: Map<string, BrainRootProposal>,
  events: Map<string, TaskMapEvent>,
  latestAuthoritative: Map<string, TaskMapEvent>,
  semanticEventIds: ReadonlySet<string>,
  pointers: Map<string, TaskMapSourcePointer>,
): string[] {
  const reasons = [
  ...
  if (!root) reasons.push("task root was rejected or is missing");
  if (task.evidenceEventIds.some((id) => !events.has(id))) reasons.push("evidence event does not exist");
  ...
  if (evidence.length === 0) reasons.push("body context cannot create a task");
  ...
  if (!VALID_OPEN_STATES.has(task.openState)) reasons.push("open state is invalid");
  if (!(task.confidence >= 0 && task.confidence <= 1)) reasons.push("confidence must be in [0,1]");

  let authoritative: TaskMapEvent | undefined;
  ...
  if (task.openState === "completed") {
    if (!sourceCompleted) reasons.push("completion cannot be inferred without a source completion event");
  }
  if (isAgentSessionReviewOnlyEvidence(evidence, pointers)) {
    const workItemRefs = unique(evidence.flatMap((event) =>
      event.objectRefs.filter((ref) => ref.startsWith("work-item:"))
    ));
    ...
    if (workItemRefs.length === 1 && evidence.some(
      (event) => !event.objectRefs.includes(workItemRefs[0]),
...
function validateEdge(
  edge: BrainEdgeProposal,
  acceptedProposals: Map<string, {
  ...
  if (edge.fromProposalId === edge.toProposalId) reasons.push("self edges are not allowed");
  if (!VALID_RELATIONS.has(edge.relation)) reasons.push("edge relation is invalid");
  ...
  if (edge.evidenceEventIds.some((id) => !events.has(id))) reasons.push("evidence event does not exist");
  ...
  if (evidence.length === 0) reasons.push("edge needs non-body evidence");
  ...
  if (!(edge.confidence >= 0 && edge.confidence <= 1)) reasons.push("confidence must be in [0,1]");
  return unique(reasons);
}

function cyclicExecutionEdgeIds(edges: readonly BrainEdgeProposal[]): Set<string> {
  const ordered = edges.filter((edge) => (
    ["depends_on", "blocks", "supersedes"].includes(edge.relation)
  ));
...
function persistentRootIds(
  roots: readonly BrainRootProposal[],
  previous: TaskMapProjectionV1 | undefined,
  events: ReadonlyMap<string, TaskMapEvent>,
  pointers: ReadonlyMap<string, TaskMapSourcePointer>,
): Map<string, string> {
  ...
  if (orderedRoots.length > 0 && orderedPrior.length > 0) {
    ...
    const costs = orderedRoots.map((root) => [
      ...orderedPrior.map((prior) => {
        ...
        return overlap >= TASKMAP_ALGORITHM_POLICY.rootReuseJaccard ? 1 - overlap : ineligibleCost;
      }),
    ...
    for (let column = 1; column <= realColumnCount; column += 1) {
      ...
      if (
        jaccard(unique(root.memberObjectRefs), unique(prior.memberObjectRefs))
        >= TASKMAP_ALGORITHM_POLICY.rootReuseJaccard
      ) {
  ...
  for (const root of orderedRoots) {
    if (ids.has(root.proposalId)) continue;
    const members = unique(root.memberObjectRefs).sort();
    const sourceOwnedWorkstreamRefs = members.filter((ref) =>
      SOURCE_OWNED_WORKSTREAM_REF.test(ref)
    );
    const agentSessionEvidence = isAgentSessionReviewOnlyEvidence(
...
function taskIdentity(
  ...
  events: Map<string, TaskMapEvent>,
  pointers: Map<string, TaskMapSourcePointer>,
  root: BrainRootProposal | undefined,
): string {
  if (task.authoritativeTaskEventId) {
    const event = events.get(task.authoritativeTaskEventId);
    const pointer = event ? pointers.get(event.pointerId) : undefined;
    if (pointer) return stableId("tmt", `${pointer.sourceKind}:${pointer.sourceRefHash}`);
  }
  const evidence = nonBodyEvidence(task.evidenceEventIds, events);
  const sourceOwnedWorkItemRefs = isAgentSessionReviewOnlyEvidence(
    evidence,
    pointers,
  ) ? unique(evidence.flatMap((event) =>
    event.objectRefs.filter((ref) => SOURCE_OWNED_WORK_ITEM_REF.test(ref))
  )) : [];
  if (sourceOwnedWorkItemRefs.length === 1) return stableId("tmc", sourceOwnedWorkItemRefs[0]);
  const normalizedText = (value: string): string => semanticTokens(value).join(" ");
...
function reusePreviousDiscoveredTaskIds(
  tasks: readonly BrainTaskProposal[],
  ...
  for (const task of candidates) {
    ...
    const matches = priorTasks
      ...
      .map((prior) => ({
        prior,
        evidenceOverlap: jaccard(
          evidenceSourceIdentities,
          prior.citations.map((citation) => `${citation.sourceKind}:${citation.sourceRefHash}`),
        ),
      ...
      .sort((a, b) => (
        b.evidenceOverlap - a.evidenceOverlap
...
function causalGrade(input: RootCausalGateInput | undefined): {
  grade: CausalGrade;
  result?: TaskMapCausalGateResult;
} {
  if (!input) return { grade: "C0_NO_DATA" };
  const gateResult = enrichmentGate(input.enrichment);
  const rawResult = (
    gateResult.verdict === "attribution_candidate"
    && input.enrichment.referenceN < TASKMAP_ALGORITHM_POLICY.causal.minNeutralReferenceDays
  ) ? {
      ...gateResult,
      verdict: "insufficient_power" as const,
      reasons: [
        `only ${input.enrichment.referenceN} neutral reference day(s); need >= ${TASKMAP_ALGORITHM_POLICY.causal.minNeutralReferenceDays}`,
      ],
    } : gateResult;
  const result: TaskMapCausalGateResult = {
    ...rawResult,
...
function bodyBonusFor(grade: CausalGrade): number {
  switch (grade) {
    case "C2_ATTRIBUTION_CANDIDATE": return TASKMAP_ALGORITHM_POLICY.bodyBonus.C2_ATTRIBUTION_CANDIDATE;
    case "C3_CAUSAL_HYPOTHESIS": return TASKMAP_ALGORITHM_POLICY.bodyBonus.C3_CAUSAL_HYPOTHESIS;
    case "C4_VALIDATED_PATTERN": return TASKMAP_ALGORITHM_POLICY.bodyBonus.C4_VALIDATED_PATTERN;
    default: return 0;
  }
}

function deadlinePressure(deadlineAt: string | undefined, nowMs: number): number {
  if (!deadlineAt || !isStrictRfc3339(deadlineAt)) return 0;
  const days = (Date.parse(deadlineAt) - nowMs) / (24 * 60 * 60 * 1_000);
  if (days < 0) return 1;
  for (const bucket of TASKMAP_ALGORITHM_POLICY.deadlinePressure) {
    if (days <= bucket.maxDays) return bucket.score;
  }
  return 0;
}

function staleOpenScore(evidence: readonly TaskMapEvent[], nowMs: number): number {
  ...
  return clamp01(days / TASKMAP_ALGORITHM_POLICY.staleOpenDays);
}
...
function recurrenceScore(
  ...
  const days = new Set(evidence.map((event) => (
    isRealDayKey(event.dayKey) ? event.dayKey : event.occurredAt.slice(0, 10)
  ))).size;
  const sources = new Set(evidence
    .map((event) => stableSourceIdentity(pointers.get(event.pointerId)))
    .filter((identity): identity is string => Boolean(identity))).size;
  return clamp01((
    Math.min(days, TASKMAP_ALGORITHM_POLICY.recurrence.dayCap)
    + Math.min(sources, TASKMAP_ALGORITHM_POLICY.recurrence.pointerCap)
  ) / TASKMAP_ALGORITHM_POLICY.recurrence.divisor);
}

function scoreTask(
  task: BrainTaskProposal,
  evidence: TaskMapEvent[],
  ...
  const total = (
    sourcePriority * TASKMAP_ALGORITHM_POLICY.scoreWeights.sourcePriority
    + deadline * TASKMAP_ALGORITHM_POLICY.scoreWeights.deadlinePressure
    + clamp01(dependencyImpact) * TASKMAP_ALGORITHM_POLICY.scoreWeights.dependencyImpact
    + recurrence * TASKMAP_ALGORITHM_POLICY.scoreWeights.recurrence
    + staleOpen * TASKMAP_ALGORITHM_POLICY.scoreWeights.staleOpen
    + evidenceStrength * TASKMAP_ALGORITHM_POLICY.scoreWeights.evidenceStrength
    + Math.min(TASKMAP_ALGORITHM_POLICY.bodyBonus.C4_VALIDATED_PATTERN, Math.max(0, bodyBonus))
  );
  return {
    ...
    bodyBonus: round(Math.min(
      TASKMAP_ALGORITHM_POLICY.bodyBonus.C4_VALIDATED_PATTERN,
      Math.max(0, bodyBonus),
    )),
    total: round(total),
  };
}

function whyNow(score: TaskMapScoreBreakdown, grade: CausalGrade): string[] {
  const reasons: Array<[number, string]> = [
    [score.deadlinePressure * TASKMAP_ALGORITHM_POLICY.scoreWeights.deadlinePressure, "deadline pressure"],
    [score.sourcePriority * TASKMAP_ALGORITHM_POLICY.scoreWeights.sourcePriority, "source priority"],
    [score.dependencyImpact * TASKMAP_ALGORITHM_POLICY.scoreWeights.dependencyImpact, "blocks downstream work"],
    [score.recurrence * TASKMAP_ALGORITHM_POLICY.scoreWeights.recurrence, "recurs across work context"],
    [score.staleOpen * TASKMAP_ALGORITHM_POLICY.scoreWeights.staleOpen, "old but still open"],
    [score.evidenceStrength * TASKMAP_ALGORITHM_POLICY.scoreWeights.evidenceStrength, "source-backed evidence"],
  ];
  if (score.bodyBonus > 0) reasons.push([score.bodyBonus, `body context (${grade}; not causal)`]);
  return reasons
...
function scoreRoot(
  ...
  joinedBodyCount: number,
  grade: CausalGrade,
  pointers: ReadonlyMap<string, TaskMapSourcePointer>,
): TaskMapRootScoreBreakdown {
  const actionableTasks = tasks.filter((task) => (
    !["source_complete", "superseded"].includes(task.reviewState)
  ...
  const maxChildActionability = clamp01(Math.max(
    0,
    ...actionableTasks.map((task) => task.score.total - task.score.bodyBonus),
  ));
  ...
  const sourceBreadth = clamp01(new Set(evidence
    ...
    .filter(Boolean)).size / TASKMAP_ALGORITHM_POLICY.rootScoreCaps.sourceKinds);
  const actionableLoad = clamp01(
    actionableTasks.length / TASKMAP_ALGORITHM_POLICY.rootScoreCaps.actionableTasks,
  );
  const dependencyBreadth = clamp01(
    dependencyEdgeCount / TASKMAP_ALGORITHM_POLICY.rootScoreCaps.dependencyEdges,
  );
  const bodyBonus = joinedBodyCount > 0
    ? Math.min(TASKMAP_ALGORITHM_POLICY.rootScoreCaps.bodyBonus, bodyBonusFor(grade))
    : 0;
  const total = clamp01(
    maxChildActionability * TASKMAP_ALGORITHM_POLICY.rootScoreWeights.maxChildActionability
    + rootRecurrence * TASKMAP_ALGORITHM_POLICY.rootScoreWeights.rootRecurrence
    + evidenceStrength * TASKMAP_ALGORITHM_POLICY.rootScoreWeights.evidenceStrength
    + sourceBreadth * TASKMAP_ALGORITHM_POLICY.rootScoreWeights.sourceBreadth
    + actionableLoad * TASKMAP_ALGORITHM_POLICY.rootScoreWeights.actionableLoad
    + dependencyBreadth * TASKMAP_ALGORITHM_POLICY.rootScoreWeights.dependencyBreadth
    + bodyBonus
  );
  return {
...
function rootWhyNow(score: TaskMapRootScoreBreakdown, grade: CausalGrade): string[] {
  const reasons: Array<[number, string]> = [
    [
      score.maxChildActionability
        * TASKMAP_ALGORITHM_POLICY.rootScoreWeights.maxChildActionability,
      "top actionable task",
    ],
    [
      score.rootRecurrence * TASKMAP_ALGORITHM_POLICY.rootScoreWeights.rootRecurrence,
      "recurs across work context",
    ],
    [
      score.evidenceStrength * TASKMAP_ALGORITHM_POLICY.rootScoreWeights.evidenceStrength,
      "source-backed root evidence",
    ],
    [
      score.sourceBreadth * TASKMAP_ALGORITHM_POLICY.rootScoreWeights.sourceBreadth,
      "spans multiple sources",
    ],
    [
      score.actionableLoad * TASKMAP_ALGORITHM_POLICY.rootScoreWeights.actionableLoad,
      "multiple open tasks",
    ],
    [
      score.dependencyBreadth * TASKMAP_ALGORITHM_POLICY.rootScoreWeights.dependencyBreadth,
...
function bodyContextCount(
  ...
  for (const body of bodyEvents) {
    ...
    if (
      joinableEvidence.some((event) => (
        Math.abs(Date.parse(event.occurredAt) - bodyMs)
...
function sourceOpenState(event: TaskMapEvent | undefined): BrainTaskProposal["openState"] | undefined {
  ...
  if (event.activity === "task_reopened" || event.activity === "task_created") return "open";
  return "unknown";
}

function explicitOnlyBrain(input: TaskMapInput): SemanticBrainOutput {
  const authoritative = [...latestAuthoritativeEvents(input).values()]
    .sort((a, b) => a.pointerId.localeCompare(b.pointerId));
  const roots: BrainRootProposal[] = authoritative.map((event) => ({
  ...
  const tasks: BrainTaskProposal[] = authoritative.map((event) => ({
...
function effectiveBrain(
  input: TaskMapInput,
  brain: SemanticBrainOutput | null,
  arm: TaskMapExperimentArm,
): SemanticBrainOutput | null {
  if (arm === "E0") return explicitOnlyBrain(input);
  return brain;
}
...
function validateBrain(
  ...
  if (!isStrictRfc3339(brain.generatedAt)) {
    ...
    if (
      isStrictRfc3339(inputGeneratedAt)
    ...
    if (
      isStrictRfc3339(runGeneratedAt)
...
function validatePreviousProjection(
  ...
  if (previous.contractVersion !== TASKMAP_CONTRACT_VERSION) {
  ...
  if (
    previous.algorithmPolicyVersion !== TASKMAP_ALGORITHM_POLICY_VERSION
    || previous.algorithmPolicyDigest !== TASKMAP_ALGORITHM_POLICY_DIGEST
  ) {
  ...
  if (!isStrictRfc3339(previous.generatedAt)) {
    ...
    isStrictRfc3339(currentGeneratedAt)
  ...
  if (!Array.isArray(previous.roots)) reasons.push("previous projection roots must be an array");
  if (!Array.isArray(previous.tasks)) reasons.push("previous projection tasks must be an array");
  if (!Array.isArray(previous.edges)) reasons.push("previous projection edges must be an array");
  if (!Array.isArray(previous.rejections)) reasons.push("previous projection rejections must be an array");

  const rootIds = new Set<string>();
  const nodeIds = new Set<string>();
  for (const root of previousRoots) {
    reasons.push(...unknownFieldReasons(root, PROJECTION_ROOT_FIELDS, "previous root"));
    reasons.push(...unknownFieldReasons(
      root.scoreBreakdown,
      ROOT_SCORE_FIELDS,
      "previous root score breakdown",
    ));
    if (!/^tmr_[a-f0-9]{16}$/.test(root.id)) reasons.push("previous root id is invalid");
    if (nodeIds.has(root.id)) reasons.push("previous root ids must be globally unique");
    rootIds.add(root.id);
    nodeIds.add(root.id);
    if (!Array.isArray(root.memberObjectRefs) || root.memberObjectRefs.length === 0) {
  ...
  for (const task of previousTasks) {
    ...
    if (!/^tm[ct]_[a-f0-9]{16}$/.test(task.id)) reasons.push("previous task id is invalid");
    if (nodeIds.has(task.id)) reasons.push("previous task ids must be globally unique");
    nodeIds.add(task.id);
  ...
  const taskMembershipCount = new Map<string, number>();
  for (const root of previousRoots) {
  ...
  for (const edge of previousEdges) {
    ...
    if (!/^tme_[a-f0-9]{16}$/.test(edge.id)) reasons.push("previous edge id is invalid");
...
export function taskMapProjectionArtifactValidationReasons(
...
function deriveCausalEnrichment(
  input: RootCausalGateInput,
  root: BrainRootProposal | undefined,
  events: Map<string, TaskMapEvent>,
  bodyEvents: readonly TaskMapEvent[],
  pointers: Map<string, TaskMapSourcePointer>,
  ...
  for (const event of rootEvents) {
    ...
    const sources = workSourcesByDate.get(date) ?? new Set<string>();
  ...
  const requiredRootSourceKinds = new Set(rootEvents
    ...
    .filter((
      sourceKind,
    ): sourceKind is TaskMapSourcePointer["sourceKind"] => sourceKind !== undefined));
  ...
  const hasComparableCoverage = (date: string): boolean => {
...
function validateCausalInput(
  input: RootCausalGateInput,
  root: BrainRootProposal | undefined,
  events: Map<string, TaskMapEvent>,
  bodyEvents: readonly TaskMapEvent[],
  pointers: Map<string, TaskMapSourcePointer>,
): { reasons: string[]; derived: EnrichmentGateInput } {
  const reasons = [
  ...
  if (!Array.isArray(enrichment.citedDays)) {
    ...
    for (const day of enrichment.citedDays) {
      ...
      if (dates.has(day.date)) reasons.push("causal cited days must be unique");
      dates.add(day.date);
      if (typeof day.backed !== "boolean") reasons.push("causal cited day backed must be boolean");
  ...
  if (
    !Number.isFinite(enrichment.referenceRate)
...
export function buildTaskMapProjection(
  input: TaskMapInput,
  brain: SemanticBrainOutput | null,
  options: TaskMapHarnessOptions,
): TaskMapProjectionV1 {
  ...
  const rootMembershipCounts = new Map<string, number>();
  ...
  const acceptedRoots: BrainRootProposal[] = [];
  if (canAcceptSemantic && semantic) {
    for (const root of semantic.roots) {
      ...
      if (root.evidenceEventIds.some((id) => invalidInputIds.has(id))) reasons.push("root cites rejected input");
      if (reasons.length) {
  ...
  const acceptedTasksBeforeIdentity: BrainTaskProposal[] = [];
  if (canAcceptSemantic && semantic) {
    for (const task of semantic.tasks) {
      const reasons = validateTask(
        task,
        acceptedRootsByProposal,
        events,
        latestAuthoritative,
        semanticEventIds,
        pointers,
      ...
      if (task.evidenceEventIds.some((id) => invalidInputIds.has(id))) reasons.push("task cites rejected input");
      if (reasons.length) {
  ...
  const taskIdByProposal = new Map<string, string>();
  const taskIdentityCounts = new Map<string, number>();
  ...
  const acceptedTasks: BrainTaskProposal[] = [];
  for (const task of acceptedTasksBeforeIdentity) {
    const identity = taskIdentity(task, events, pointers, acceptedRootsByProposal.get(task.rootProposalId));
  ...
  if (canAcceptSemantic && options.arm !== "E0") {
    for (const authoritative of latestAuthoritative.values()) {
      ...
      if (state === "completed" || state === "superseded") continue;
      const coverage = acceptedTasks.filter((task) => (
        task.authoritativeTaskEventId === authoritative.id
      )).length;
      if (coverage !== 1) {
        rejections.push({
  ...
  const acceptedProposals = new Map<string, {
    evidenceEventIds: string[];
    kind: "root" | "task";
    task?: BrainTaskProposal;
  }>([
    ...acceptedRoots.map((root): [string, {
      evidenceEventIds: string[];
      kind: "root";
    }] => [
      root.proposalId,
  ...
  let acceptedEdges: BrainEdgeProposal[] = [];
  ...
  if (cyclicEdgeIds.size > 0) {
    for (const edge of acceptedEdges) {
      if (!cyclicEdgeIds.has(edge.proposalId)) continue;
      rejections.push({
  ...
  const rootIdByProposal = persistentRootIds(
    acceptedRoots,
    ["E3", "E4"].includes(options.arm) && previousReasons.length === 0
      ? options.previousProjection
      : undefined,
    events,
    pointers,
  );
  reusePreviousDiscoveredTaskIds(
    acceptedTasks,
    taskIdByProposal,
    rootIdByProposal,
    ["E3", "E4"].includes(options.arm) && previousReasons.length === 0
  ...
  const bodyEvents = options.arm === "E4"
    ? allBodyEvents.filter((event) => (
      TASKMAP_ALGORITHM_POLICY.actionableBodyCategories.includes(
        event.bodyCategory as "below_baseline" | "above_baseline",
      )
      && nowMs - Date.parse(event.occurredAt) >= 0
      && nowMs - Date.parse(event.occurredAt) <= TASKMAP_ALGORITHM_POLICY.bodyFreshnessWindowMs
    ))
    : [];
  const causalByRoot = new Map<string, RootCausalGateInput>();
  if (options.arm === "E4") {
    for (const item of options.causalInputs ?? []) {
      const validation = validateCausalInput(
        item,
        acceptedRootsByProposal.get(item.rootProposalId),
        events,
        allBodyEvents,
        pointers,
      );
      const reasons = [...validation.reasons];
      if (!acceptedRootProposalIds.has(item.rootProposalId)) reasons.push("causal root is not accepted");
      if (causalByRoot.has(item.rootProposalId)) reasons.push("duplicate causal root input");
      if (reasons.length) {
        rejections.push({
  ...
  const dependencyByTask = new Map<string, number>();
  for (const edge of acceptedEdges) {
  ...
  const taskRows: TaskMapTask[] = acceptedTasks.flatMap((task) => {
    ...
    const eligibleReturnOrigins = originPointerIds
      ...
      .filter((pointer): pointer is TaskMapSourcePointer => (
        pointer !== undefined && hasReturnCapability(pointer)
      ));
    const personalForkPointer = originPointerIds
      .map((pointerId) => pointers.get(pointerId))
      .find((pointer) => pointer?.sourceKind === "manual" && pointer.syncMode === "personal_fork");
    const returnRoute: TaskMapTask["returnRoute"] = taskHome
      ? (
    ...
    const joinedBodyEvents = causalInput
      ? bodyEvents.filter((event) => (
        event.bodyAxis === causalInput.bodyAxis
    ...
    const rootCausal = joinedBodyCount > 0
      ...
      : { grade: "C0_NO_DATA" as CausalGrade };
    const score = scoreTask(
      { ...task, openState: effectiveOpenState },
      evidence,
      clamp01((
        dependencyByTask.get(task.proposalId) ?? 0
      ) / TASKMAP_ALGORITHM_POLICY.dependencyCountCap),
      joinedBodyCount > 0 ? bodyBonusFor(rootCausal.grade) : 0,
      nowMs,
      pointers,
    );
    const reviewState: TaskMapReviewState = authoritativeEvent
      ? (
    ...
    return [{
      ...
      discoveredBy: unique(evidence.map((event) => pointers.get(event.pointerId)?.sourceKind).filter(Boolean)) as string[],
      bodyContextCount: joinedBodyCount,
    }];
  }).sort((a, b) => b.score.total - a.score.total || a.id.localeCompare(b.id));

  const roots: TaskMapRoot[] = acceptedRoots.map((root) => {
    ...
    const dependencyEdgeCount = acceptedEdges.filter((edge) => (
      ["depends_on", "blocks"].includes(edge.relation)
      && (
        rootTaskProposalIds.has(edge.fromProposalId)
        || rootTaskProposalIds.has(edge.toProposalId)
      )
    )).length;
    const scoreBreakdown = scoreRoot(
      rootTasks,
      evidence,
      dependencyEdgeCount,
      joinedBodyCount,
  ...
  const edges: TaskMapEdge[] = acceptedEdges.flatMap((edge) => {
  ...
  const referencedPointerIds = new Set<string>();
  for (const root of roots) {
    for (const citation of root.citations) referencedPointerIds.add(citation.pointerId);
  }
  for (const task of taskRows) {
    if (task.taskHomePointerId) referencedPointerIds.add(task.taskHomePointerId);
    for (const citation of task.citations) referencedPointerIds.add(citation.pointerId);
  }
  for (const edge of edges) {
    for (const citation of edge.citations) referencedPointerIds.add(citation.pointerId);
  }
  ...
  const invalidPointerIds = new Set(rejections
    ...
    .map((item) => item.proposalId));
  const sources: TaskMapProjectionSource[] = input.pointers
  ...
  if (projectedLeaks.length === 0) return projection;
  return {
    ...projection,
    runStatus: "rejected",
    brain: null,
...
export function taskMapMembershipSignature(projection: TaskMapProjectionV1): string {
...
export function runTaskMapAblation(
  input: TaskMapInput,
  brain: SemanticBrainOutput,
  options?: Omit<TaskMapHarnessOptions, "arm">,
): TaskMapAblationReport {
  const arms: TaskMapProjectionV1[] = (["E0", "E1", "E2", "E3", "E4"] as TaskMapExperimentArm[])
    .map((arm) => buildTaskMapProjection(input, brain, { ...options, arm }));
  const bodyMasked: TaskMapInput = {
    ...input,
  ...
  const bodyShuffled: TaskMapInput = {
    ...input,
  ...
  const rejectionSummary = (
    projection: TaskMapProjectionV1,
  ): TaskMapAblationReport["arms"][number]["rejectionSummary"] => (
    (["input", "root", "task", "edge"] as const).flatMap((kind) => {
