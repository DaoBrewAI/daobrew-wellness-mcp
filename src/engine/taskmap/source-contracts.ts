...
import { taskMapProjectionArtifactValidationReasons } from "./harness.js";
import {
  TASKMAP_ALGORITHM_POLICY_VERSION,
  TASKMAP_CONTRACT_VERSION,
  TASKMAP_CANONICAL_MEETING_VERSION,
  TASKMAP_CONNECTOR_CHECKPOINT_VERSION,
  TASKMAP_DISCOVERY_POINTER_VERSION,
  TASKMAP_GATE_DECISION_VERSION,
  TASKMAP_PROJECTION_DIFF_VERSION,
  TASKMAP_PROJECTION_PARITY_VERSION,
  TASKMAP_PROJECTION_TARGET_VERSION,
  TASKMAP_REPLAY_MANIFEST_VERSION,
  TASKMAP_SEMANTIC_PROPOSAL_REFERENCE_VERSION,
  TASKMAP_SOURCE_ENVELOPE_VERSION,
  TASKMAP_SOURCE_SNAPSHOT_VERSION,
  type TaskMapAuthorityContractV1,
  type TaskMapCanonicalMeetingV1,
  type TaskMapCapability,
  type TaskMapConnectorCheckpointV1,
  type TaskMapConnectorWatermarkV1,
  type TaskMapDeterministicGate,
  type TaskMapDiscoveryChannel,
  type TaskMapDiscoveryPointerV1,
  type TaskMapGateDecisionV1,
  type TaskMapMeetingIdentityHintV1,
  type TaskMapProjectionDiffEntryV1,
  type TaskMapProjectionDiffV1,
  type TaskMapProjectionParityFindingV1,
  type TaskMapProjectionParityV1,
  type TaskMapProjectionTargetRecordV1,
  type TaskMapProjectionTargetV1,
  type TaskMapProjectionV1,
  type TaskMapReplayManifestV1,
  type TaskMapReplayPolicyBindingV1,
  type SemanticBrainOutput,
  type TaskMapSemanticProposalReferenceV1,
  type TaskMapSourceAuthorityBindingV1,
  type TaskMapSourceEnvelopeV1,
  type TaskMapSourceKind,
  type TaskMapSourceObjectType,
...
const ENVELOPE_REQUIRED_KEYS = new Set(
  [...ENVELOPE_KEYS].filter((key) => key !== "meetingIdentity"),
);
const MEETING_HINT_REQUIRED_KEYS = new Set(
  [...MEETING_HINT_KEYS].filter((key) => (
    key !== "endAt" && key !== "calendarEventIdDigest"
  )),
);
const BUILT_ENVELOPE_REQUIRED_KEYS = new Set(
  [...BUILT_ENVELOPE_KEYS].filter((key) => key !== "meetingIdentity"),
);
...
const VALID_PARITY_CODES = new Set<TaskMapProjectionParityFindingV1["code"]>([
...
const VALID_CAPABILITIES = new Set<TaskMapCapability>([
  "read_task",
  "read_context",
  "create_task",
  "update_status",
...
const VALID_OBJECT_TYPES = new Set<TaskMapSourceObjectType>([
  "authoritative_task",
  "meeting_note",
  "calendar_event",
  "agent_session",
...
const VALID_EVIDENCE = new Set<TaskMapAuthorityContractV1["evidence"]>([
  "authoritative_task",
  "explicit_commitment",
...
const VALID_QUALITY = new Set<TaskMapAuthorityContractV1["quality"]>([
  "source_native",
...
const VALID_LIFECYCLE = new Set<TaskMapAuthorityContractV1["lifecycle"]>([
  "source_status",
  "explicit_user_policy",
  "none",
]);
const VALID_COMPLETION = new Set<TaskMapAuthorityContractV1["completion"]>([
  "source_status",
  "explicit_user_policy",
  "none",
]);
const VALID_RANK = new Set<TaskMapAuthorityContractV1["rank"]>([
  "accepted_work",
  "candidate_only",
  "context_only",
  "body_bonus_only",
]);
const SOURCE_OBJECT_TYPES: Record<TaskMapSourceKind, ReadonlySet<TaskMapSourceObjectType>> = {
...
const LOCAL_OR_BODY_SOURCE_KINDS = new Set<TaskMapSourceKind>([
  "codex_session",
  "claude_session",
  "cursor_session",
  "oura",
]);
const VALID_GATES = new Set<TaskMapDeterministicGate>([
  "identity",
  "privacy",
  "dedupe",
  "authority",
...
const VALID_GATE_OUTCOMES = new Set<TaskMapGateDecisionV1["outcome"]>([
  "accepted",
  "rejected",
  "candidate_only",
  "context_only",
...
const REQUIRED_POLICY_NAMES = new Set<TaskMapReplayPolicyBindingV1["name"]>([
  "source",
  "normalization",
  "identity",
  "privacy",
...
export type TaskMapSourceEnvelopeDraftV1 = Omit<
...
export function taskMapContractCanonicalJson(value: unknown): string {
...
export function taskMapContractDigest(value: unknown): string {
  return createHash("sha256").update(taskMapContractCanonicalJson(value)).digest("hex");
}
...
function assertSemanticAttestationKey(key: string): void {
  if (
    typeof key !== "string"
...
function semanticBrainAttestationMac(
  sourceSemanticInputDigest: string,
  brain: SemanticBrainOutput,
  key: string,
): string {
...
export function attestTaskMapSemanticBrain(
  sourceSemanticInputDigest: string,
  brain: SemanticBrainOutput,
  semanticBrainAttestationKey: string,
  ...
  return deepFreeze({
    sourceSemanticInputDigest,
    attestationKeyId: createHmac("sha256", semanticBrainAttestationKey)
      .update(`${SEMANTIC_ATTESTATION_DOMAIN}:key-id`)
      .digest("hex"),
    attestationMac: semanticBrainAttestationMac(
      sourceSemanticInputDigest,
      brain,
      semanticBrainAttestationKey,
    ),
...
function assertSemanticBrainAttestation(
  ...
  const expected = attestTaskMapSemanticBrain(
    artifact.sourceSemanticInputDigest,
    artifact.brain,
    key,
  );
  const keyIdMatches = timingSafeEqual(
    Buffer.from(artifact.attestationKeyId, "hex"),
    Buffer.from(expected.attestationKeyId, "hex"),
  );
  const macMatches = timingSafeEqual(
    Buffer.from(artifact.attestationMac, "hex"),
    Buffer.from(expected.attestationMac, "hex"),
  );
...
function assertBoundedRows(values: readonly unknown[], label: string): void {
...
function assertDigestOnlyIdentifier(value: unknown, label: string): asserts value is string {
...
function assertBinding(
  binding: TaskMapSourceAuthorityBindingV1,
  expectedSourceKind?: TaskMapSourceKind,
): void {
...
function assertMeetingHint(hint: TaskMapMeetingIdentityHintV1): void {
  assertContractKeys(
    hint,
    MEETING_HINT_KEYS,
    MEETING_HINT_REQUIRED_KEYS,
    "meeting identity hint",
  );
...
function assertAuthority(
  ...
  if (
    !VALID_EVIDENCE.has(authority.evidence)
  ...
  if (objectType === "body_context") {
    if (
      authority.evidence !== "body_context_only"
  ...
  if (objectType === "calendar_event") {
    if (
      authority.evidence !== "context_only"
  ...
  if (objectType === "meeting_note") {
    if (
      !["provider_generated_summary", "explicit_commitment", "context_only"].includes(
        authority.evidence,
      )
      || ![
        "structured_generated",
  ...
  if (objectType === "agent_session") {
    if (
      !["explicit_commitment", "context_only"].includes(authority.evidence)
  ...
  if (objectType === "authoritative_task") {
    if (
      authority.evidence !== "authoritative_task"
      || authority.quality !== "source_native"
    ...
    const expectedAuthority = sourceKind === "manual"
      ...
      : "source_status";
    if (
      authority.lifecycle !== expectedAuthority
  ...
  if (objectType === "strategy_context") {
    if (
      !["explicit_commitment", "provider_generated_summary", "context_only"].includes(
        authority.evidence,
      )
      || authority.lifecycle !== "none"
      || authority.completion !== "none"
...
export function buildTaskMapSourceEnvelope(
  draft: TaskMapSourceEnvelopeDraftV1,
): TaskMapSourceEnvelopeV1 {
  assertContractKeys(
    draft,
    ENVELOPE_KEYS,
    ENVELOPE_REQUIRED_KEYS,
    "source envelope draft",
  );
  assertDigest(draft.ownerScopeDigest, "ownerScopeDigest");
  assertBinding(draft.binding, draft.sourceKind);
  if (!VALID_SOURCE_KINDS.has(draft.sourceKind)) throw new Error("unsupported source kind");
  if (!VALID_OBJECT_TYPES.has(draft.objectType)) throw new Error("unsupported source object type");
  ...
  if (
    draft.sourceKind === "strategy"
    && draft.objectType === "authoritative_task"
    && !IMMUTABLE_GIT_COMMIT.test(draft.sourceRevision)
  ) {
    throw new Error(
      "Strategy authoritative tasks require an immutable 40-hex Git sourceRevision",
    );
  }
  ...
  if (["meeting_note", "calendar_event"].includes(draft.objectType)) {
    if (!draft.meetingIdentity) throw new Error("meeting sources require a bounded identity hint");
  ...
  return deepFreeze({
    contractVersion: TASKMAP_SOURCE_ENVELOPE_VERSION,
    envelopeId,
    ownerScopeDigest: draft.ownerScopeDigest,
    sourceObjectKeyDigest,
    sourceIdentityDigest,
...
export function canonicalGoogleDocumentId(url: string): string {
  let parsed: URL;
  try {
    parsed = new URL(url);
  } catch {
  ...
  if (!match) throw new Error("resolved meeting-notes URL has no canonical document ID");
  return match[1];
}

export function buildGeminiMeetSource(
  draft: TaskMapGeminiMeetDraftV1,
): { envelope: TaskMapSourceEnvelopeV1; discoveryPointers: TaskMapDiscoveryPointerV1[] } {
  ...
  const discoveryPointers = draft.gmailDiscoveries.map((discovery) => {
    ...
    const pointer: TaskMapDiscoveryPointerV1 = {
      contractVersion: TASKMAP_DISCOVERY_POINTER_VERSION,
...
function normalizeSourceEnvelope(envelope: TaskMapSourceEnvelopeV1): TaskMapSourceEnvelopeV1 {
  assertContractKeys(
    envelope,
    BUILT_ENVELOPE_KEYS,
    BUILT_ENVELOPE_REQUIRED_KEYS,
    "built source envelope",
  );
  if (envelope.contractVersion !== TASKMAP_SOURCE_ENVELOPE_VERSION) {
...
function normalizeDiscoveryPointer(
  pointer: TaskMapDiscoveryPointerV1,
): TaskMapDiscoveryPointerV1 {
  assertContractKeys(
    pointer,
    BUILT_DISCOVERY_KEYS,
    BUILT_DISCOVERY_KEYS,
    "built discovery pointer",
  );
  if (pointer.contractVersion !== TASKMAP_DISCOVERY_POINTER_VERSION) {
...
function sameBinding(
  left: TaskMapSourceAuthorityBindingV1,
  right: TaskMapSourceAuthorityBindingV1,
): boolean {
  return taskMapContractCanonicalJson(left) === taskMapContractCanonicalJson(right);
}

function assertWatermark(watermark: TaskMapConnectorWatermarkV1): void {
  assertContractKeys(
    watermark,
    WATERMARK_KEYS,
    WATERMARK_REQUIRED_KEYS,
    "connector watermark",
  );
...
export function advanceTaskMapConnectorCheckpoint(
  previous: TaskMapConnectorCheckpointV1 | null,
  poll: TaskMapConnectorPollDraftV1,
): TaskMapConnectorCheckpointV1 {
  if (previous) previous = normalizeConnectorCheckpoint(previous);
  assertBinding(poll.binding, poll.sourceKind);
  if (!VALID_SOURCE_KINDS.has(poll.sourceKind)) throw new Error("unsupported checkpoint source kind");
  ...
  for (const capability of capabilities) {
    if (!VALID_CAPABILITIES.has(capability)) throw new Error("unsupported connector capability");
  }
  if (previous) {
  ...
  let watermark: TaskMapConnectorWatermarkV1 | undefined;
  let watermarkHistoryDigests: string[];
  let lastSuccessfulPollAt: string | undefined;
  let acceptedSourceIdentityDigests: string[];
  let error: TaskMapConnectorCheckpointV1["error"];
  if (poll.state === "success") {
    if (!poll.proposedWatermark) throw new Error("successful polls require a watermark");
    ...
    if (
      previous?.watermark?.observedThrough
  ...
  return deepFreeze({
    contractVersion: TASKMAP_CONNECTOR_CHECKPOINT_VERSION,
...
function normalizeConnectorCheckpoint(
  ...
  if (checkpoint.contractVersion !== TASKMAP_CONNECTOR_CHECKPOINT_VERSION) {
...
export function assertTaskMapConnectorCheckpoint(
  checkpoint: TaskMapConnectorCheckpointV1,
): TaskMapConnectorCheckpointV1 {
...
function fallbackIdentityKeyDigest(fingerprintDigest: string): string {
  return taskMapContractDigest(`fingerprint:${fingerprintDigest}`);
}

function canonicalMeetingId(identityKeyDigest: string): string {
  return stableId("tmmeeting", { identityKeyDigest });
}

function normalizePriorCanonicalMeeting(
  meeting: TaskMapCanonicalMeetingV1,
): TaskMapCanonicalMeetingV1 {
  assertContractKeys(
    meeting,
    CANONICAL_MEETING_KEYS,
    CANONICAL_MEETING_KEYS,
    "prior canonical meeting",
  );
  if (meeting.contractVersion !== TASKMAP_CANONICAL_MEETING_VERSION) {
  ...
  for (const alias of identityAliases) assertDigest(alias, "prior meeting identity alias");
  if (
    !identityAliases.includes(meeting.identityKeyDigest)
  ...
  for (const code of conflictCodes) assertOpaque(code, "prior meeting conflict code");
  for (const id of calendarEnvelopeIds) assertOpaque(id, "prior Calendar envelope ID");
  for (const id of evidenceVariantEnvelopeIds) assertOpaque(id, "prior evidence envelope ID");
  if (
    meeting.identityMethod === "bounded_fingerprint"
  ...
  const normalized = {
    contractVersion: TASKMAP_CANONICAL_MEETING_VERSION,
...
export function canonicalizeTaskMapMeetings(
  envelopes: readonly TaskMapSourceEnvelopeV1[],
  previousMeetings: readonly TaskMapCanonicalMeetingV1[] = [],
): TaskMapCanonicalMeetingV1[] {
  ...
  const meetingEnvelopes = envelopes.filter((envelope) => (
    envelope.objectType === "meeting_note" || envelope.objectType === "calendar_event"
  ));
  for (const envelope of meetingEnvelopes) {
    if (!envelope.meetingIdentity) throw new Error("meeting envelope lacks an identity hint");
  }
  const calendarKeysByFingerprint = new Map<string, Set<string>>();
  for (const envelope of meetingEnvelopes) {
    ...
    if (hint.calendarEventIdDigest) {
      const keys = calendarKeysByFingerprint.get(fingerprint) ?? new Set<string>();
      keys.add(`${envelope.ownerScopeDigest}:${hint.calendarEventIdDigest}`);
      calendarKeysByFingerprint.set(fingerprint, keys);
    }
  }
  const groups = new Map<string, TaskMapSourceEnvelopeV1[]>();
  for (const envelope of meetingEnvelopes) {
    const hint = envelope.meetingIdentity!;
    const fingerprint = meetingFingerprint(envelope.ownerScopeDigest, hint);
  ...
  const canonical = [...groups.entries()].map(([key, variants]) => {
    ...
    const fingerprintDigests = [...new Set(variants.map((variant) => (
      meetingFingerprint(variant.ownerScopeDigest, variant.meetingIdentity!)
    )))].sort();
    ...
    const conflictingCalendarKeys = Math.max(...fingerprintDigests.map((fingerprint) => (
      calendarKeysByFingerprint.get(fingerprint)?.size ?? 0
    )));
...
function uniqueById<T>(values: readonly T[], id: (value: T) => string, label: string): T[] {
...
export function buildTaskMapSourceSnapshot(
  envelopesInput: readonly TaskMapSourceEnvelopeV1[],
  discoveryInput: readonly TaskMapDiscoveryPointerV1[],
  previousMeetings: readonly TaskMapCanonicalMeetingV1[] = [],
  ...
  if (envelopesInput.length === 0) throw new Error("source snapshot requires at least one envelope");
  const envelopes = uniqueById(
    envelopesInput.map(normalizeSourceEnvelope),
    (row) => row.envelopeId,
    "source envelope",
  );
  ...
  const objectRevisionOwners = new Map<string, string>();
  ...
  const discoveryPointers = uniqueById(
    discoveryInput.map(normalizeDiscoveryPointer),
    (row) => row.discoveryId,
    "discovery pointer",
  );
  for (const pointer of discoveryPointers) {
    const target = envelopes.find((envelope) => (
      envelope.sourceIdentityDigest === pointer.targetSourceIdentityDigest
    ));
    if (!target) {
      throw new Error("discovery pointer target is absent from the source snapshot");
    }
    if (
      pointer.ownerScopeDigest !== ownerScopeDigest
  ...
  const unbarrieredMeeting = envelopes.find((envelope) => (
    ["meeting_note", "calendar_event"].includes(envelope.objectType)
    && !meetingEnvelopeIds.has(envelope.envelopeId)
  ));
  if (unbarrieredMeeting) throw new Error("meeting evidence cannot bypass canonical identity");
  ...
  const snapshot = {
    contractVersion: TASKMAP_SOURCE_SNAPSHOT_VERSION,
...
function canonicalMeetingWithoutId(meeting: TaskMapCanonicalMeetingV1): object {
  const { canonicalMeetingId: _canonicalMeetingId, ...rest } = meeting;
  return rest;
}

function currentEvidenceContinuityProofs(
  storedMeetings: readonly TaskMapCanonicalMeetingV1[],
  baselineMeetings: readonly TaskMapCanonicalMeetingV1[],
  envelopes: readonly TaskMapSourceEnvelopeV1[],
): TaskMapCanonicalMeetingV1[] {
  ...
  const storedByIdentityKey = new Map<string, TaskMapCanonicalMeetingV1>();
  for (const stored of storedMeetings) {
    assertContractKeys(
      stored,
      CANONICAL_MEETING_KEYS,
      CANONICAL_MEETING_KEYS,
      "stored canonical meeting",
    );
  ...
  const proofs: TaskMapCanonicalMeetingV1[] = [];
  for (const baseline of baselineMeetings) {
    ...
    if (stored.canonicalMeetingId === baseline.canonicalMeetingId) continue;
    ...
    const currentFingerprints = [...new Set(envelopes
      ...
      .map((envelope) => meetingFingerprint(
        envelope.ownerScopeDigest,
        envelope.meetingIdentity!,
      )))].sort();
    const matchingFallbacks = currentFingerprints
      ...
      .filter(({ identityKeyDigest }) => (
        canonicalMeetingId(identityKeyDigest) === stored.canonicalMeetingId
      ));
...
function normalizeSourceSnapshotArtifact(
  ...
  if (snapshot.contractVersion !== TASKMAP_SOURCE_SNAPSHOT_VERSION) {
  ...
  assertDigest(
    snapshot.discoveryProvenanceDigest,
    "source discoveryProvenanceDigest",
  );
  assertPrivacy(snapshot.privacy);
  const baseline = buildTaskMapSourceSnapshot(
    snapshot.envelopes,
    snapshot.discoveryPointers,
  );
  const continuityProofs = currentEvidenceContinuityProofs(
    snapshot.canonicalMeetings,
    baseline.canonicalMeetings,
    baseline.envelopes,
  );
...
export function assertTaskMapSourceSnapshot(
  snapshot: TaskMapSourceSnapshotV1,
): TaskMapSourceSnapshotV1 {
  return normalizeSourceSnapshotArtifact(snapshot);
}

function sortedUniqueOpaque(values: readonly string[], label: string): string[] {
  const sorted = [...new Set(values)].sort();
  for (const value of sorted) assertOpaque(value, label);
  return sorted;
}

export function buildTaskMapSemanticProposalReference(
  draft: TaskMapSemanticProposalReferenceDraftV1,
): TaskMapSemanticProposalReferenceV1 {
  ...
  assertOnlyKeys(
    draft.semanticBrainArtifact,
    SEMANTIC_BRAIN_ARTIFACT_KEYS,
    "semantic brain artifact",
  );
  assertDigest(
    draft.semanticBrainArtifact.sourceSemanticInputDigest,
    "sourceSemanticInputDigest",
  );
  assertSemanticBrainAttestation(
    draft.semanticBrainArtifact,
    draft.semanticBrainAttestationKey,
  );
  ...
  return deepFreeze({
    contractVersion: TASKMAP_SEMANTIC_PROPOSAL_REFERENCE_VERSION,
    ...
    rootProposalIds: sortedUniqueOpaque(
      brain.roots.map((root) => root.proposalId),
      "root proposal ID",
    ),
    candidateProposalIds: sortedUniqueOpaque(
      brain.tasks.map((task) => task.proposalId),
      "candidate proposal ID",
    ),
    relationProposalIds: sortedUniqueOpaque(
      brain.edges.map((edge) => edge.proposalId),
      "relation proposal ID",
    ),
  });
}

function normalizeSemanticProposalReference(
  proposal: TaskMapSemanticProposalReferenceV1,
): TaskMapSemanticProposalReferenceV1 {
  assertOnlyKeys(proposal, BUILT_SEMANTIC_REFERENCE_KEYS, "built semantic proposal reference");
  if (proposal.contractVersion !== TASKMAP_SEMANTIC_PROPOSAL_REFERENCE_VERSION) {
  ...
  const normalized = {
    ...
    candidateProposalIds: sortedUniqueOpaque(
      proposal.candidateProposalIds,
      "candidate proposal ID",
    ),
    relationProposalIds: sortedUniqueOpaque(
      proposal.relationProposalIds,
      "relation proposal ID",
    ),
  };
...
export function assertTaskMapSemanticProposalCurrent(
  snapshot: TaskMapSourceSnapshotV1,
  proposal: TaskMapSemanticProposalReferenceV1,
): void {
...
export function buildTaskMapGateDecision(
  draft: TaskMapGateDecisionDraftV1,
): TaskMapGateDecisionV1 {
  assertOnlyKeys(draft, GATE_DECISION_KEYS, "gate decision");
  assertOpaque(draft.subjectId, "gate decision subjectId");
  if (!VALID_GATES.has(draft.gate)) throw new Error("unsupported deterministic gate");
  if (!VALID_GATE_OUTCOMES.has(draft.outcome)) throw new Error("unsupported gate outcome");
  ...
  return deepFreeze({
    contractVersion: TASKMAP_GATE_DECISION_VERSION,
    decisionId: stableId("tmdecision", core),
    ...core,
  });
}

function normalizeGateDecision(decision: TaskMapGateDecisionV1): TaskMapGateDecisionV1 {
  assertOnlyKeys(decision, BUILT_GATE_DECISION_KEYS, "built gate decision");
  if (decision.contractVersion !== TASKMAP_GATE_DECISION_VERSION) {
...
type ProjectionRow = {
  kind: TaskMapProjectionDiffEntryV1["kind"];
  id: string;
  value: object;
};

function assertProjectionArtifact(
  projection: TaskMapProjectionV1,
  label: string,
): void {
  assertOnlyKeys(projection, PROJECTION_KEYS, label);
  if (
    projection.contractVersion !== TASKMAP_CONTRACT_VERSION
    || projection.algorithmPolicyVersion !== TASKMAP_ALGORITHM_POLICY_VERSION
  ) {
  ...
  if (
    projection.privacy.sourceBodiesStored !== false
  ...
  for (const [rows, rowLabel] of [
    ...
    for (const row of rows) assertOpaque(row.id, `${label} ${rowLabel} id`);
  }
  ...
  const harnessValidationReasons = taskMapProjectionArtifactValidationReasons(projection);
  if (harnessValidationReasons.length > 0) {
    throw new Error(`${label} fails Task Map projection schema/privacy validation: ${
      harnessValidationReasons.join("; ")
    }`);
  }
}

function projectionRows(projection: TaskMapProjectionV1): ProjectionRow[] {
  const rows: ProjectionRow[] = [
    ...projection.sources.map((value) => ({ kind: "source" as const, id: value.id, value })),
    ...projection.roots.map((value) => ({ kind: "root" as const, id: value.id, value })),
    ...projection.tasks.map((value) => ({ kind: "task" as const, id: value.id, value })),
    ...projection.edges.map((value) => ({ kind: "edge" as const, id: value.id, value })),
...
function projectionRowMap(projection: TaskMapProjectionV1): Map<string, ProjectionRow> {
  const rows = new Map<string, ProjectionRow>();
  for (const row of projectionRows(projection)) {
    const key = `${row.kind}:${row.id}`;
    if (rows.has(key)) throw new Error("projection contains duplicate diff identities");
    rows.set(key, row);
  }
  return rows;
}

function projectionDigest(projection: TaskMapProjectionV1 | null): string {
  if (projection === null) return taskMapContractDigest(null);
  const {
    sources: _sources,
    roots: _roots,
    tasks: _tasks,
...
function changedFields(
  ...
  return [...new Set([...Object.keys(before), ...Object.keys(after)])]
    .filter((key) => (
      taskMapContractCanonicalJson(before[key]) !== taskMapContractCanonicalJson(after[key])
    ))
    .sort();
}

export function diffTaskMapProjections(
  previous: TaskMapProjectionV1 | null,
  current: TaskMapProjectionV1,
): TaskMapProjectionDiffV1 {
  if (previous) assertProjectionArtifact(previous, "previous projection");
  ...
  const before = previous ? projectionRowMap(previous) : new Map<string, ProjectionRow>();
  const after = projectionRowMap(current);
  const added: TaskMapProjectionDiffEntryV1[] = [];
  const removed: TaskMapProjectionDiffEntryV1[] = [];
  const changed: TaskMapProjectionDiffEntryV1[] = [];
  ...
  const sortEntries = (values: TaskMapProjectionDiffEntryV1[]) => values.sort((left, right) => (
    left.kind.localeCompare(right.kind) || left.id.localeCompare(right.id)
  ));
  const core = {
  ...
  return deepFreeze({
    contractVersion: TASKMAP_PROJECTION_DIFF_VERSION,
    diffId: stableId("tmdiff", core),
    ...core,
  });
}

export function buildTaskMapProjectionTarget(
  draft: TaskMapProjectionTargetDraftV1,
  ...
  return deepFreeze({
    contractVersion: TASKMAP_PROJECTION_TARGET_VERSION,
    target: draft.target,
...
function normalizeProjectionTarget(
  target: TaskMapProjectionTargetV1,
): TaskMapProjectionTargetV1 {
  assertOnlyKeys(target, PROJECTION_TARGET_KEYS, "built projection target");
  if (target.contractVersion !== TASKMAP_PROJECTION_TARGET_VERSION) {
  ...
  if (
    target.privacy.sourceBodiesStored !== false
...
function parityFinding(
  ...
  code: TaskMapProjectionParityFindingV1["code"],
): TaskMapProjectionParityFindingV1 {
  return { canonicalRecordId, code };
}

export function compareTaskMapProjectionTargets(
  strategy: TaskMapProjectionTargetV1,
  taskMap: TaskMapProjectionTargetV1,
  ...
  const intentionalDifferences: TaskMapProjectionParityFindingV1[] = [];
  const failures: TaskMapProjectionParityFindingV1[] = [];
  if (!sameDelta) failures.push(parityFinding("accepted-delta", "accepted_delta_mismatch"));
  for (const record of strategy.records) {
    ...
    if (!operational) {
      if (record.role === "strategy_durable_context") {
        intentionalDifferences.push(parityFinding(
  ...
  for (const record of taskMap.records) {
    if (strategyRows.has(record.canonicalRecordId)) continue;
  ...
  const sortFindings = (rows: TaskMapProjectionParityFindingV1[]) => rows.sort((left, right) => (
  ...
  return deepFreeze({
    contractVersion: TASKMAP_PROJECTION_PARITY_VERSION,
...
function sortedPolicies(
  bindings: readonly TaskMapReplayPolicyBindingV1[],
): TaskMapReplayPolicyBindingV1[] {
  assertBoundedRows(bindings, "replay policy bindings");
  const names = new Set<TaskMapReplayPolicyBindingV1["name"]>();
  const rows = bindings.map((binding) => {
    ...
    if (names.has(binding.name)) throw new Error("replay policy name must be unique");
  ...
  if (
    names.size !== REQUIRED_POLICY_NAMES.size
...
function normalizeProjectionParity(
  parity: TaskMapProjectionParityV1,
): TaskMapProjectionParityV1 {
  assertOnlyKeys(parity, PROJECTION_PARITY_KEYS, "projection parity");
  if (parity.contractVersion !== TASKMAP_PROJECTION_PARITY_VERSION) {
  ...
  const normalizeFindings = (
    findings: readonly TaskMapProjectionParityFindingV1[],
    label: string,
  ) => {
    ...
    return findings.map((finding) => {
      ...
      if (!VALID_PARITY_CODES.has(finding.code)) throw new Error("unsupported parity finding");
      return { ...finding };
    }).sort((left, right) => (
      left.canonicalRecordId.localeCompare(right.canonicalRecordId)
      || left.code.localeCompare(right.code)
  ...
  const core = {
    ...
    intentionalDifferences: normalizeFindings(
      parity.intentionalDifferences,
      "intentional parity findings",
    ),
...
export function buildTaskMapReplayManifest(
  draft: TaskMapReplayManifestDraftV1,
): TaskMapReplayManifestV1 {
  ...
  const projectionDiff = diffTaskMapProjections(
    draft.previousProjection,
  ...
  const brain = draft.semanticBrainArtifact.brain;
  if (
    draft.currentProjection.inputDigest !== brain.inputDigest
  ...
  const connectorCheckpoints = uniqueById(
    draft.connectorCheckpoints.map(normalizeConnectorCheckpoint),
    (checkpoint) => checkpoint.checkpointId,
    "connector checkpoint",
  );
  const snapshotSourceIdentities = new Set(
    sourceSnapshot.envelopes.map((envelope) => envelope.sourceIdentityDigest),
  );
  for (const checkpoint of connectorCheckpoints) {
    if (checkpoint.acceptedSourceIdentityDigests.some(
      (digest) => !snapshotSourceIdentities.has(digest),
    )) {
  ...
  const gateDecisions = uniqueById(
    draft.gateDecisions.map(normalizeGateDecision),
    (decision) => decision.decisionId,
    "gate decision",
  );
  const subjectGateOwners = new Set<string>();
  ...
  if (projectionParity && (
    projectionParity.failures.length > 0
  ...
  const manifest = {
    contractVersion: TASKMAP_REPLAY_MANIFEST_VERSION,
