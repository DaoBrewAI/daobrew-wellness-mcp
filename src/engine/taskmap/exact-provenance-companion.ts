...
export const TASKMAP_EXACT_PROVENANCE_VERSION =
  "taskmap-exact-provenance.v1" as const;
export const TASKMAP_EXACT_PROVENANCE_PRODUCER_VERSION =
  "taskmap-exact-provenance-producer.1" as const;

export const TASKMAP_EXACT_PROVENANCE_LIMITS_V1 = Object.freeze({
...
const PRODUCER_POLICY = Object.freeze({
  version: TASKMAP_EXACT_PROVENANCE_PRODUCER_VERSION,
  taskBinding:
    "projection_task_to_source_envelope_and_immutable_git_row_locator",
  legacyPointerMapping: "adapter_attested_not_source_native",
  currentness: "exhaustive_projection_bound_current_rows_only",
  rootMembership:
    "derived_from_projection_and_exact_current_task_proof_digests",
  edgeMembership:
    "derived_from_projection_and_exact_current_node_membership",
  executionEdgeEvidence:
    "operational_edge_requires_an_exact_task_source_citation",
  unversionedContext: "context_only_never_execution_provenance",
  privacy:
    "no_source_row_text_no_source_body_no_local_path_no_provider_fact_invention",
});

export const TASKMAP_EXACT_PROVENANCE_PRODUCER_POLICY_DIGEST =
  taskMapContractDigest(PRODUCER_POLICY);
...
const ADAPTER_ATTESTATION_DOMAIN =
  "taskmap-exact-provenance-adapter-attestation.1";
const REPOSITORY_BINDING_DOMAIN =
  "taskmap-exact-provenance-repository-binding.1";
const CANONICAL_ROW_DOMAIN = "taskmap-canonical-repository-row.1";
export const TASKMAP_MANUAL_RECEIPT_ADAPTER_VERSION =
  "taskmap-owner-receipt-adapter.1" as const;
export const TASKMAP_MANUAL_RECEIPT_LOCATOR =
  "owner-receipts/native-candidate-acceptance.v1.json" as const;
export const TASKMAP_MANUAL_RECEIPT_ADAPTER_POLICY_DIGEST =
  taskMapContractDigest({
    adapterVersion: TASKMAP_MANUAL_RECEIPT_ADAPTER_VERSION,
    policy: "durable-owner-receipt-store-v1",
  });

export const TASKMAP_MIXED_SOURCE_ADAPTER_VERSION =
  "taskmap-mixed-current-source-adapter.1" as const;
const MIXED_SOURCE_ADAPTER_POLICY_DOMAIN =
  "taskmap-mixed-current-source-adapter-policy.1";

const DIGEST = /^[a-f0-9]{64}$/;
const GIT_COMMIT = /^[a-f0-9]{40}$/;
const SAFE_ID = /^[A-Za-z0-9][A-Za-z0-9._:#-]{0,511}$/;
const SAFE_VERSION = /^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$/;
const OPERATIONAL_RELATIONS = new Set<TaskMapEdge["relation"]>([
  "advances",
  "depends_on",
  "blocks",
  "supersedes",
...
export interface TaskMapExactProvenanceRootProofV1 {
  ...
  producerDigest:
    typeof TASKMAP_EXACT_PROVENANCE_PRODUCER_POLICY_DIGEST;
...
export function taskMapExactProvenanceAdapterExpectation(
  bindings: readonly TaskMapExactProvenanceTaskBindingDraftV1[],
): { adapterVersion: string; adapterPolicyDigest: string } {
  ...
  const rows = bindings.map((binding) => {
    ...
    assertDigest(
      ...
      "task binding adapterPolicyDigest",
    );
    policies.set(
      binding.adapterVersion + ":" + binding.adapterPolicyDigest,
      {
  ...
  return {
    adapterVersion: TASKMAP_MIXED_SOURCE_ADAPTER_VERSION,
    adapterPolicyDigest: taskMapContractDigest({
...
function assertRepositoryRelativePath(value: string): void {
  if (
    ...
    || Buffer.byteLength(value, "utf8")
      > TASKMAP_EXACT_PROVENANCE_LIMITS_V1
...
function currentTaskIds(
  ...
  if (
    ...
    || currentness.taskDispositions.length
      > TASKMAP_EXACT_PROVENANCE_LIMITS_V1.maximumTasks
  ) {
  ...
  for (const row of currentness.taskDispositions) {
    ...
    if (seen.has(row.taskId)) fail("currentness taskId is duplicated");
    seen.add(row.taskId);
  }
  const projectionIds = new Set(projection.tasks.map((task) => task.id));
  if (
    seen.size !== projectionIds.size
  ...
  const current = currentness.taskDispositions
    ...
    .map((row) => row.taskId)
    .sort();
  if (current.length === 0) fail("exact provenance requires current tasks");
  return current;
}

function validateProjection(projection: TaskMapProjectionV1): string {
  if (
    ...
    || projection.roots.length
      > TASKMAP_EXACT_PROVENANCE_LIMITS_V1.maximumRoots
    || projection.edges.length
      > TASKMAP_EXACT_PROVENANCE_LIMITS_V1.maximumEdges
  ) {
...
function repositoryBindingDigest(
...
function canonicalProjectionCitations(
  projection: TaskMapProjectionV1,
  taskId: string,
  pointerId: string,
): {
  ...
  if (task === undefined) fail("task binding targets an absent task");
  ...
  return {
    sourceRefHash,
    citationDigest: taskMapContractDigest(citations),
  };
}

function assertExactTaskEnvelope(
  envelope: TaskMapSourceEnvelopeV1,
): void {
  const strategyRow = (
...
function canonicalUrlBindsLocator(
  canonicalUrl: string | undefined,
  commit: string,
  repositoryRelativePath: string,
): boolean {
  if (canonicalUrl === undefined) return false;
  try {
...
function buildTaskProof(
  projection: TaskMapProjectionV1,
  envelope: TaskMapSourceEnvelopeV1,
  binding: TaskMapExactProvenanceTaskBindingDraftV1,
): TaskMapExactProvenanceTaskProofV1 {
  assertId(binding.taskId, "task binding taskId");
  ...
  if (
    manualReceipt
    && (
      binding.repositoryRelativePath !== TASKMAP_MANUAL_RECEIPT_LOCATOR
      || binding.adapterVersion !== TASKMAP_MANUAL_RECEIPT_ADAPTER_VERSION
      || binding.adapterPolicyDigest
      ...
      || envelope.binding.tenantOrWorkspaceDigest !== taskMapContractDigest({
  ...
  const source = projection.sources.find(
    (row) => row.id === task.taskHomePointerId,
  );
  if (
    source === undefined
  ...
  const citation = canonicalProjectionCitations(
    projection,
    task.id,
  ...
  const attestationCore = {
    ...
    bindingAuthority: "adapter_attested" as const,
    adapterVersion: binding.adapterVersion,
    adapterPolicyDigest: binding.adapterPolicyDigest,
    taskId: task.id,
  ...
  const adapterAttestationDigest = taskMapContractDigest(attestationCore);
  const core = {
    taskId: task.id,
    pointerId: task.taskHomePointerId,
    projectionSourceRefHash: citation.sourceRefHash,
...
function buildRootProofs(
  projection: TaskMapProjectionV1,
  currentIds: readonly string[],
  tasks: readonly TaskMapExactProvenanceTaskProofV1[],
): TaskMapExactProvenanceRootProofV1[] {
  ...
  return projection.roots.flatMap((root) => {
    ...
    const currentTaskProofDigests = currentTaskIds.map(
      (taskId) => proofByTask.get(taskId)!.proofDigest,
    );
    const projectionRootDigest = taskMapContractDigest(root);
    const core = {
      rootId: root.id,
      factKind: "derived_projection" as const,
      currentTaskIds,
    ...
    return [{
      ...core,
      derivationDigest: taskMapContractDigest({
...
function buildEdgeProofs(
  projection: TaskMapProjectionV1,
  roots: readonly TaskMapExactProvenanceRootProofV1[],
  tasks: readonly TaskMapExactProvenanceTaskProofV1[],
): TaskMapExactProvenanceEdgeProofV1[] {
  const visible = new Set([
    ...roots.map((root) => root.rootId),
    ...tasks.map((task) => task.taskId),
  ]);
  const proofByCitationIdentity = new Map<string, Set<string>>();
  for (const task of tasks) {
    const key = `${task.pointerId}:${task.projectionSourceRefHash}`;
    const values = proofByCitationIdentity.get(key) ?? new Set<string>();
  ...
  return projection.edges
    ...
    .map((edge) => {
      const exactTaskProofDigests = [...new Set(edge.citations.flatMap(
        (citation) => [
          ...(proofByCitationIdentity.get(
            `${citation.pointerId}:${citation.sourceRefHash}`,
          ) ?? []),
        ],
      ))].sort();
      if (
      ...
      const executionEvidence =
        OPERATIONAL_RELATIONS.has(edge.relation)
          ? "exact_task_source" as const
          : "context_only" as const;
      const projectionEdgeDigest = taskMapContractDigest(edge);
      const core = {
        edgeId: edge.id,
        factKind: "derived_projection" as const,
        from: edge.from,
...
function artifactCore(
  artifact: TaskMapExactProvenanceV1,
): Omit<TaskMapExactProvenanceV1, "artifactDigest"> {
  const { artifactDigest: _artifactDigest, ...core } = artifact;
  return core;
}

export function taskMapCanonicalRepositoryRowDigest(
  input: TaskMapCanonicalRepositoryRowDigestInputV1,
): string {
  assertRepositoryRelativePath(input.repositoryRelativePath);
  assertId(input.sourceObjectId, "sourceObjectId");
  if (
    typeof input.rowText !== "string"
    || Buffer.byteLength(input.rowText, "utf8")
      > TASKMAP_EXACT_PROVENANCE_LIMITS_V1.maximumCanonicalRowBytes
  ) {
...
export function buildTaskMapExactProvenance(
  input: BuildTaskMapExactProvenanceInputV1,
): TaskMapExactProvenanceV1 {
  ...
  const snapshot = assertTaskMapSourceSnapshot(input.sourceSnapshot);
  ...
  if (
    snapshot.discoveryPointers.length !== 0
  ...
  if (
    !Array.isArray(input.taskBindings)
  ...
  if (
    adapterExpectation.adapterVersion !== input.expectedAdapterVersion
  ...
  const bindingEnvelopeIds = input.taskBindings.map(
    (binding) => binding.sourceEnvelopeId,
  );
  if (
    ...
    || snapshot.envelopes.some(
      (envelope) => !bindingEnvelopeIds.includes(envelope.envelopeId),
    )
  ) {
  ...
  const envelopeById = new Map(
    snapshot.envelopes.map((envelope) => [envelope.envelopeId, envelope]),
  );
  const tasks = input.taskBindings
    .map((binding) => {
      const envelope = envelopeById.get(binding.sourceEnvelopeId);
      if (envelope === undefined) fail("task binding envelope is absent");
  ...
  if (
    rootedTaskIds.size !== currentIds.length
  ...
  const core: Omit<TaskMapExactProvenanceV1, "artifactDigest"> = {
    contractVersion: TASKMAP_EXACT_PROVENANCE_VERSION,
    projection: {
      ...
      currentTaskSetDigest: taskMapContractDigest(currentIds),
    },
    producer: {
      version: TASKMAP_EXACT_PROVENANCE_PRODUCER_VERSION,
      policyDigest:
  ...
  const artifact: TaskMapExactProvenanceV1 = {
    ...core,
  ...
  if (
    Buffer.byteLength(taskMapContractCanonicalJson(artifact), "utf8")
      > TASKMAP_EXACT_PROVENANCE_LIMITS_V1.maximumArtifactBytes
  ) {
...
function assertArtifactShape(
  ...
  if (artifact.contractVersion !== TASKMAP_EXACT_PROVENANCE_VERSION) {
  ...
  assertDigest(
    artifact.projection.currentnessFileDigest,
    "currentnessFileDigest",
  );
  assertDigest(
    artifact.projection.currentTaskSetDigest,
    "currentTaskSetDigest",
  );
  ...
  if (
    artifact.producer.version
      !== TASKMAP_EXACT_PROVENANCE_PRODUCER_VERSION
  ...
  for (const task of artifact.tasks) {
    ...
    assertDigest(
      task.projectionSourceRefHash,
      "task proof projectionSourceRefHash",
    );
    assertDigest(
      task.projectionCitationDigest,
      "task proof projectionCitationDigest",
    );
    ...
    if (
      !GIT_COMMIT.test(task.sourceRevision)
    ...
    assertDigest(
      task.repositoryBindingDigest,
      "task proof repositoryBindingDigest",
    );
    ...
    assertDigest(
      task.adapterAttestationDigest,
      "task proof adapterAttestationDigest",
    );
  ...
  for (const root of artifact.roots) {
    ...
    if (
      root.factKind !== "derived_projection"
      || root.producerDigest
        !== TASKMAP_EXACT_PROVENANCE_PRODUCER_POLICY_DIGEST
    ) {
    ...
    if (
      !Array.isArray(root.currentTaskIds)
    ...
    root.currentTaskProofDigests.forEach(
      (digest) => assertDigest(digest, "root proof task digest"),
    );
  ...
  for (const edge of artifact.edges) {
    ...
    if (
      edge.factKind !== "derived_projection"
      || edge.producerDigest
        !== TASKMAP_EXACT_PROVENANCE_PRODUCER_POLICY_DIGEST
      || ![
        "exact_task_source",
    ...
    edge.exactTaskProofDigests.forEach(
      (digest) => assertDigest(digest, "edge proof task digest"),
    );
  ...
  if (
    artifact.privacy.sourceRowsStored !== false
...
export function taskMapExactProvenanceDigest(
  artifact: TaskMapExactProvenanceV1,
): string {
  return taskMapContractDigest({
    domain: ARTIFACT_DOMAIN,
...
export function assertTaskMapExactProvenance(
  artifact: TaskMapExactProvenanceV1,
  context: AssertTaskMapExactProvenanceContextV1,
): TaskMapExactProvenanceV1 {
  ...
  assertTaskMapSourceSnapshot(artifact.sourceSnapshot);
