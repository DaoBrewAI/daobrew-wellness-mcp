import { GranolaMeetingSignal, TranscriptSpan } from "../signals/granola.js";
...
const SEED_THREAD_VOCABULARY: SemanticFrame[] = [
  {
...
function spanTextIsCited(meeting: GranolaMeetingSignal, span: TranscriptSpan): boolean {
  const spanText = normalizeText(span.text);
  if (spanText.length < 16) return false;
  const body = normalizeText(meeting.body);
  return Boolean(body && body.includes(spanText));
}

export function citedTranscriptSpans(meeting: GranolaMeetingSignal): CitedTranscriptSpan[] {
...
export function meetingHasCitedTranscriptSpan(meeting: GranolaMeetingSignal): boolean {
  return citedTranscriptSpans(meeting).length > 0;
}

function textMatchesTerms(text: string, terms: string[]): number {
...
function frameHasRequiredTerm(frame: SemanticFrame, span: CitedTranscriptSpan, memories: MemoryInsightSignal[]): boolean {
  if (!frame.requiredAnyTerms?.length) return true;
  const haystack = [
    span.text,
...
function spanMatchesFrame(span: CitedTranscriptSpan, frame: SemanticFrame, memories: MemoryInsightSignal[]): boolean {
  if (!OPEN_LOOP_PATTERN.test(span.text)) return false;
  if (!frameHasRequiredTerm(frame, span, memories)) return false;
...
function memorySupportForFrame(
  memories: MemoryInsightSignal[],
  frame: SemanticFrame,
): LatentMemorySupport[] {
  return memories
    .map((memory) => {
      ...
      return {
        ...
        reason: "project_memory_semantic_support" as const,
        text: displayText(memory.insight_text).slice(0, 240),
        topics: memory.topics,
      };
    })
    .filter((entry): entry is LatentMemorySupport => entry !== null)
    .slice(0, 3);
}

function weekBucketForSpan(span: CitedTranscriptSpan, meetings: GranolaMeetingSignal[]): string {
  const meeting = meetings.find((entry) => entry.graph_source_ref === span.source_ref);
  const ts = meeting?.occurred_at_ts ?? 0;
  if (!Number.isFinite(ts) || ts <= 0) return span.source_ref;
  const week = Math.floor(ts / (7 * 24 * 60 * 60));
  return String(week);
}

function surfaceTermsFor(spans: CitedTranscriptSpan[], frame: SemanticFrame): string[] {
  const out: string[] = [];
...
export function proposeLatentHypotheses(context: LatentHypothesisContext): LatentHypothesisProposal[] {
  const citedSpans = context.meetings.flatMap(citedTranscriptSpans);
  const proposals: LatentHypothesisProposal[] = [];
  ...
  for (const frame of SEED_THREAD_VOCABULARY) {
    const evidenceSpans = citedSpans
      ...
      .sort((a, b) => (
        textMatchesTerms(b.text, frame.spanTerms) - textMatchesTerms(a.text, frame.spanTerms) ||
        a.source_ref.localeCompare(b.source_ref)
      ))
      .slice(0, 6);
    if (evidenceSpans.length === 0) continue;
  ...
  return proposals.sort((a, b) => (
    b.memory_support.length - a.memory_support.length ||
...
function proposalHasCitedSpans(proposal: LatentHypothesisProposal, meetings: GranolaMeetingSignal[]): boolean {
  if (proposal.evidence_spans.length === 0) return false;
  const citedByRef = new Map<string, Set<string>>();
  for (const meeting of meetings) {
    citedByRef.set(
      meeting.graph_source_ref,
      new Set(citedTranscriptSpans(meeting).map((span) => normalizeText(span.text))),
    );
  }
  return proposal.evidence_spans.every((span) => (
    citedByRef.get(span.source_ref)?.has(normalizeText(span.text)) ?? false
  ));
}

function titleBanPass(proposal: LatentHypothesisProposal, context: LatentHypothesisContext): boolean {
  ...
  return !bannedTitles.some((title) => (
    cause === title
...
function partitionSpansByAnchorDay(
  spans: CitedTranscriptSpan[],
  context: LatentHypothesisContext,
): { sameDay: CitedTranscriptSpan[]; otherDay: CitedTranscriptSpan[] } {
  if (!Number.isFinite(context.anchor_ts ?? NaN)) {
    return { sameDay: spans, otherDay: [] };
  }
  const anchorTs = context.anchor_ts as number;
  const byRef = new Map(context.meetings.map((meeting) => [meeting.graph_source_ref, meeting]));
  const sameDay: CitedTranscriptSpan[] = [];
  const otherDay: CitedTranscriptSpan[] = [];
...
function timePlausibilityPass(proposal: LatentHypothesisProposal, context: LatentHypothesisContext): boolean {
  if (!Number.isFinite(context.anchor_ts ?? NaN)) return true;
...
export function verifyLatentHypothesis(
  proposal: LatentHypothesisProposal,
  context: LatentHypothesisContext,
  ...
  } satisfies EvidenceVerification["gates"];

  const rejectedReasons: string[] = [];
  if (gates.citation === "fail") rejectedReasons.push("citation check failed: the meeting note needs a specific imported passage for this claim");
  if (gates.title_ban === "fail") rejectedReasons.push("title check failed: event or meeting titles can be context only, not final cause text");
  if (gates.time_plausibility === "fail") rejectedReasons.push("timing check failed: the source note is not close enough to the watch pattern");
  if (gates.biometrics_firewall === "fail") rejectedReasons.push("watch-pattern check failed: the pattern must come from watch data");
  if (gates.recurrence_cross_source === "fail") rejectedReasons.push("support check failed: project context is required before accepting this work thread");
  if (gates.non_clinical_language === "fail") rejectedReasons.push("language check failed: the proposal overstates causality or uses clinical language");
...
export function detectLatentHypothesis(context: LatentHypothesisContext): GapCandidate | null {
