...
import { CalendarEventSignal } from "../signals/calendar.js";
...
import {
  ...
  type MetricBaseline,
} from "./biometric-endorsement.js";
import { detectLatentHypothesis, GapCandidate, meetingHasCitedTranscriptSpan } from "./LatentHypothesis.js";
import {
...
let v2Active: ReasonerV2Context | null = null;
...
const SIGNATURE_TO_KEY: Record<string, PatternKey> = {
  TENSION: "tension",
  OVERDRIVE: "overdrive",
...
interface SourceCard {
  source_kind: "granola_note" | "calendar_event" | "project_memory_capsule";
...
const CANONICAL_THEME_IDS: Record<string, string> = {
...
type EvidenceGrade = "strong" | "candidate" | "weak" | "source_only";

interface TriggerBundle {
  events: CalendarEventSignal[];
  meetings: GranolaMeetingSignal[];
  usableMeetings: GranolaMeetingSignal[];
...
export const MVP_ROOT_CAUSE_ID =
  `ghost_root_${stableHash(["root", "local", DEFAULT_ROOT_THREAD_KEY])}`;

function scopedHashParts(userId: string, parts: readonly string[]): string[] {
  return [userId, ...parts];
}
...
export function persistentRootId(userId: string, threadKey = DEFAULT_ROOT_THREAD_KEY): string {
  const scopedUserId = requireUserId(userId, "Reasoner.persistentRootId");
  const normalized = normalizeThreadKey(threadKey);
  if (normalized === DEFAULT_ROOT_THREAD_KEY) {
    if (userId === "local") return MVP_ROOT_CAUSE_ID;
    return `ghost_root_${stableHash(["root", userId, normalized])}`;
  }
  return `ghost_root_${stableHash(["root", scopedUserId, normalized])}`;
}

export function rootCauseId(userId: string): string {
  return persistentRootId(userId, DEFAULT_ROOT_THREAD_KEY);
}

function persistentRootSourceRef(userId: string, state?: "watching"): string {
  ...
  if (userId === "local") {
    return `reasoner:${MVP_ROOT_CAUSE_ID}${state === "watching" ? ":watching" : ""}`;
  }
  return `reasoner:${rootCauseId(userId)}`;
}

function b2bGhostId(userId: string): string {
  return nodeId(userId, "ghost", "reasoner:ghost-b2b-track");
}

function nodeId(userId: string, kind: string, sourceRef: string): string {
  return `${kind}_${stableHash(scopedHashParts(userId, [kind, sourceRef]))}`;
}

function edgeId(userId: string, srcId: string, dstId: string, kind: GraphEdgeKind): string {
  return `edge_${stableHash(scopedHashParts(userId, [srcId, dstId, kind]))}`;
}

function addNode(nodes: Map<string, GraphNodeDelta>, node: GraphNodeDelta): void {
  if (!nodes.has(node.id)) nodes.set(node.id, node);
}

function addEdge(edges: Map<string, GraphEdgeDelta>, edge: GraphEdgeDelta): void {
  if (!edges.has(edge.id)) edges.set(edge.id, edge);
}

function patternDefinition(key: PatternKey): PatternDefinition {
  return PATTERNS.find((pattern) => pattern.key === key) ?? PATTERNS[0];
}

function patternNodeId(userId: string, key: PatternKey): string {
...
function themeRef(topic: string): string {
  return `theme:${normalizeTheme(topic)}`;
}

function themeNodeId(userId: string, topic: string): string {
  const normalized = normalizeTheme(topic);
  return nodeId(userId, "theme", themeRef(normalized));
}

function compactDateTime(ts: number | null | undefined): string | null {
...
function localDateParts(ts: number): { year: number; month: number; day: number } {
...
function sameLocalDay(aTs: number | null | undefined, bTs: number | null | undefined): boolean {
  if (!Number.isFinite(aTs ?? NaN) || !Number.isFinite(bTs ?? NaN)) return false;
  const a = localDateParts(aTs as number);
  const b = localDateParts(bTs as number);
...
function weekStartForTs(ts: number): string {
...
function compactDateForIso(iso: string): string {
  return new Intl.DateTimeFormat("en-US", {
    timeZone: "UTC",
    month: "short",
    day: "numeric",
...
function termSet(values: Array<string | null | undefined>): Set<string> {
...
function compactSentenceParts(text: string | null | undefined, max = 3): string[] {
...
function eventDescriptionText(event: CalendarEventSignal): string[] {
  ...
  const centralText = centralBrain && typeof centralBrain === "object" && !Array.isArray(centralBrain)
    ? [
      (centralBrain as Record<string, unknown>).summary,
      ...arrayFromUnknown((centralBrain as Record<string, unknown>).key_points),
      ...arrayFromUnknown((centralBrain as Record<string, unknown>).topics),
...
function tripletText(triplet: JoinedTriplet): string {
  return [
...
export function scorePatterns(triplet: JoinedTriplet): PatternScore[] {
...
function categoryFromScores(yin: number, yang: number): string {
  const yinOk = yin >= 50;
...
function normalizedStateCategory(triplet: JoinedTriplet): string {
  const state = triplet.episode.state;
  const raw = String(state.category ?? "").trim().toLowerCase();
  if (raw === "balanced" || raw === "peak") return "in_flow";
  if (raw === "yang_excess") return "pushing_it";
  if (raw === "yin_excess" || raw === "burnout") return "recharging";
  if (["in_flow", "pushing_it", "recharging", "running_on_empty"].includes(raw)) return raw;
  return categoryFromScores(state.yin_score, state.yang_score);
}

function biometricPatternScores(triplet: JoinedTriplet): PatternScore[] {
  ...
  if (v2Active) {
    const day = localDayKey(
      triplet.episode.occurred_at_ts,
      v2Active.timezoneOffsetHours,
    );
    ...
    if (signature && signature.quality === "real_inference") {
      for (const [label, key] of Object.entries(SIGNATURE_TO_KEY)) {
        const value = signature.scores[label as keyof typeof signature.scores];
        ...
        if (reasons[1]) scores.find((entry) => entry.key === key)?.reasons.push(reasons[1]);
      }
    }
  ...
  if (category === "pushing_it") {
    pushPatternScore(
      scores,
      "overdrive",
      3.2 + activation * 1.2 + recoveryDeficit * 0.8,
      "watch data shows high activation with low recovery",
    );
    pushPatternScore(
      scores,
      "tension",
      2.2 + activation * 0.8 + recoveryDeficit * 0.6,
      "watch data shows activation pressure with limited reserve",
    );
  } else if (category === "recharging") {
    pushPatternScore(
      scores,
      "depletion",
      3.0 + lowActivation * 1.1,
      "watch data shows low activation during recovery",
    );
    pushPatternScore(
      scores,
      "constriction",
      2.1 + lowActivation * 0.7,
      "watch data shows reduced outward activation",
    );
  } else if (category === "running_on_empty") {
    pushPatternScore(
      scores,
      "depletion",
      3.4 + recoveryDeficit * 0.9 + lowActivation * 0.7,
      "watch data shows low recovery and low activation",
    );
    pushPatternScore(
      scores,
      "stagnation",
      2.4 + lowActivation * 0.8,
      "watch data shows low movement and low activation",
    );
  }
 ...
 *  depletion claim. */
function primaryPattern(triplet: JoinedTriplet): PatternKey | null {
  return scorePatterns(triplet)[0]?.key ?? null;
}

function classifyPattern(triplet: JoinedTriplet): PatternKey | null {
...
function normalizedPolicy(value: unknown): string {
  return typeof value === "string" ? value.toLowerCase().trim() : "";
}

function meetingHasUsableSummary(meeting: GranolaMeetingSignal): boolean {
...
function meetingEvidenceWeight(meeting: GranolaMeetingSignal): number {
  if (meetingHasCitedTranscriptSpan(meeting)) return 1.15;
  return meetingHasUsableSummary(meeting) ? 0.4 : 0.15;
}

function sourceStartsBeforeOrNearTrigger(sourceTs: number | null | undefined, anchorTs: number): boolean {
  if (!Number.isFinite(sourceTs ?? NaN)) return false;
  return (sourceTs as number) <= anchorTs + TEMPORAL_PRECEDENCE_GRACE_SEC &&
...
function eventHasGranolaAlignment(event: CalendarEventSignal): boolean {
...
function plannedSourceStartsInsideContext(sourceTs: number | null | undefined, anchorTs: number): boolean {
  if (!Number.isFinite(sourceTs ?? NaN)) return false;
  const delta = (sourceTs as number) - anchorTs;
  return delta > TEMPORAL_PRECEDENCE_GRACE_SEC && delta <= PLANNED_CONTEXT_WINDOW_SEC;
}

function sourceInsideLongHorizonContext(sourceTs: number | null | undefined, anchorTs: number): boolean {
  if (!Number.isFinite(sourceTs ?? NaN)) return false;
  return Math.abs((sourceTs as number) - anchorTs) <= LONG_HORIZON_CONTEXT_WINDOW_SEC &&
    sameLocalDay(sourceTs, anchorTs);
}

function eventTimingAllowed(event: CalendarEventSignal, anchorTs: number): boolean {
  if (eventInsideLongHorizonContext(event, anchorTs)) return true;
  if (sourceStartsBeforeOrNearTrigger(event.start_ts, anchorTs)) return true;
  return eventHasGranolaAlignment(event) && plannedSourceStartsInsideContext(event.start_ts, anchorTs);
}

function eventEvidenceWeight(event: CalendarEventSignal): number {
  ...
  const importPolicy = normalizedPolicy(event.metadata?.import_policy);
  if (status === "cancelled" || response === "declined" || importPolicy.includes("declined")) return 0.15;
  if (transparency === "transparent" || importPolicy.includes("weak")) return 0.35;
  return 1;
}

function eventReferenceSet(event: CalendarEventSignal): Set<string> {
  return new Set(uniqueStrings([event.id, event.source_ref, event.graph_source_ref]));
}

function eventOverlapsTriggerWindow(event: CalendarEventSignal, anchorTs: number): boolean {
...
function eventInsideLongHorizonContext(event: CalendarEventSignal, anchorTs: number): boolean {
  const start = event.start_ts;
  const end = event.end_ts ?? event.start_ts;
  if (eventOverlapsTriggerWindow(event, anchorTs)) return true;
...
function meetingOverlapsTriggerWindow(meeting: GranolaMeetingSignal, anchorTs: number): boolean {
  const occurredAt = meeting.occurred_at_ts;
  if (!Number.isFinite(occurredAt ?? NaN)) return false;
  return Math.abs((occurredAt as number) - anchorTs) <= TRIGGER_SUPPORT_WINDOW_SEC;
}

function meetingInsideLongHorizonContext(meeting: GranolaMeetingSignal, anchorTs: number): boolean {
  const occurredAt = meeting.occurred_at_ts;
  return meetingOverlapsTriggerWindow(meeting, anchorTs) ||
    sourceInsideLongHorizonContext(occurredAt, anchorTs);
}

function meetingMatchesEvent(meeting: GranolaMeetingSignal, event: CalendarEventSignal): boolean {
  if (!meeting.event_id) return false;
  return eventReferenceSet(event).has(meeting.event_id);
}

function meetingTimingAllowed(
  meeting: GranolaMeetingSignal,
  anchorTs: number,
  events: CalendarEventSignal[],
): boolean {
  if (meetingInsideLongHorizonContext(meeting, anchorTs)) return true;
  if (sourceStartsBeforeOrNearTrigger(meeting.occurred_at_ts, anchorTs)) return true;
...
function loadSignalText(values: Array<string | null | undefined>): boolean {
  const text = values.filter(Boolean).join(" ").toLowerCase();
...
function meetingHasLoadSignal(meeting: GranolaMeetingSignal): boolean {
...
function isWelcomeDavidWithoutMeetingLoad(
  events: CalendarEventSignal[],
  meetings: GranolaMeetingSignal[],
): boolean {
  const titleText = [
  ...
  if (!/\bwelcome\s+david!?\b/i.test(titleText)) return false;
  return !meetings.some(meetingHasLoadSignal);
}

function eventMatchesMeetingRefs(event: CalendarEventSignal, refs: Set<string>): boolean {
...
function eventSupportsBundle(
  event: CalendarEventSignal,
  anchorTs: number,
  meetingEventRefs: Set<string>,
  meetings: GranolaMeetingSignal[],
): boolean {
  if (eventOverlapsTriggerWindow(event, anchorTs)) return true;
  if (eventMatchesMeetingRefs(event, meetingEventRefs)) return true;
  if (eventHasGranolaAlignment(event)) return true;
  if (!eventInsideLongHorizonContext(event, anchorTs)) return false;
...
function isLogisticsCalendarEvent(event: CalendarEventSignal): boolean {
...
function isProjectMemory(memory: MemoryInsightSignal): boolean {
  return /(codex|claude|project|session|capsule|memory)/i.test(memory.source);
}

function memoryContextTerms(
  events: CalendarEventSignal[],
  meetings: GranolaMeetingSignal[],
): Set<string> {
  return termSet([
 ...
 */
function memoryContextOverlap(
  memory: MemoryInsightSignal,
  contextTerms: Set<string>,
): number {
  if (!isProjectMemory(memory)) return 0;
...
function memorySupportsContext(
  memory: MemoryInsightSignal,
  events: CalendarEventSignal[],
  meetings: GranolaMeetingSignal[],
): boolean {
...
function sourceRefsForTriplet(
  triplet: JoinedTriplet,
  events: CalendarEventSignal[],
  meetings: GranolaMeetingSignal[],
  memories: MemoryInsightSignal[],
): string[] {
...
function triggerBundleFor(triplet: JoinedTriplet): TriggerBundle {
  ...
  const supportMeetings = triplet.meetings.filter((meeting) => (
    meetingTimingAllowed(meeting, anchorTs, timingAllowedEvents) &&
    (
  ...
  const supportEvents = timingAllowedEvents.filter((event) => (
    eventSupportsBundle(event, anchorTs, meetingEventRefs, supportMeetings)
  ));
  ...
  const usableMeetings = supportMeetings.filter((meeting) => (
    meetingHasUsableSummary(meeting) || meetingHasCitedTranscriptSpan(meeting)
  ));
  const gapCandidate = detectLatentHypothesis({
  ...
  const hasCalendar = supportEvents.some((event) => (
    eventEvidenceWeight(event) >= 0.35 &&
  ...
  const hasEarlyContext = (
    supportEvents.some((event) => !isLogisticsCalendarEvent(event)) ||
  ...
  const contextTitleValue = (
    ...
    (supportMemories[0] ? memoryDisplayTitle(supportMemories[0]) : null)
  ) ?? "local work context";
...
function tripletSourceStrength(triplet: JoinedTriplet): number {
...
function hasSourceEvidence(triplet: JoinedTriplet): boolean {
  return triggerBundleFor(triplet).eligible;
}

export function scoreTripletBreakdown(triplet: JoinedTriplet): TripletScoreBreakdown {
  const bundle = triggerBundleFor(triplet);
...
function scoreTriplet(triplet: JoinedTriplet): number {
  return scoreTripletBreakdown(triplet).total;
}

function compareTriplets(a: JoinedTriplet, b: JoinedTriplet): number {
  return (
    scoreTriplet(b) - scoreTriplet(a) ||
    a.episode.occurred_at_ts - b.episode.occurred_at_ts ||
    a.id.localeCompare(b.id)
  );
}

function patternScoreFor(triplet: JoinedTriplet, pattern: PatternKey): number {
  return scorePatterns(triplet).find((entry) => entry.key === pattern)?.score ?? 0;
}

function compareTripletsForPattern(pattern: PatternKey): (a: JoinedTriplet, b: JoinedTriplet) => number {
  return (a, b) => (
    patternScoreFor(b, pattern) - patternScoreFor(a, pattern) ||
    tripletSourceStrength(b) - tripletSourceStrength(a) ||
    compareTriplets(a, b)
  );
}

function chooseRootTriplet(triplets: JoinedTriplet[]): JoinedTriplet {
 ...
 */
function v2RootProps(rootTriplet: JoinedTriplet): Record<string, unknown> {
  if (!v2Active) return {};
  const day = localDayKey(
    rootTriplet.episode.occurred_at_ts,
    v2Active.timezoneOffsetHours,
  );
  ...
  if (axis) {
    const { gate, coverageCap } = runEnrichmentGateWithCoverage(
      axis,
      v2Active.coverage,
      v2Active.thresholds,
    );
    ...
    props.claim_level = claimLevelForVerdict(
      gate.verdict,
...
function contextKey(triplet: JoinedTriplet): string {
  const bundle = triggerBundleFor(triplet);
  const sourceKey = (
    bundle.meetings[0]?.graph_source_ref ??
...
function chooseGraphTriplets(triplets: JoinedTriplet[], rootTriplet: JoinedTriplet): JoinedTriplet[] {
  const selected = new Map<string, JoinedTriplet>([[rootTriplet.id, rootTriplet]]);
  const contextWinners = new Map<string, JoinedTriplet>();
  const rootWeek = triggerBundleFor(rootTriplet).weekStart;
  const assignedPatternCounts = new Map<string, number>();
  for (const score of scorePatterns(rootTriplet)) {
    assignedPatternCounts.set(`${rootWeek}:${score.key}`, 1);
  }
  const assignedWeekCounts = new Map<string, number>([[rootWeek, 1]]);

  for (const triplet of triplets) {
    if (!hasSourceEvidence(triplet) && triplet.id !== rootTriplet.id) continue;
  ...
  const byWeek = new Map<string, JoinedTriplet[]>();
  ...
  for (const [week, weekTriplets] of [...byWeek.entries()].sort((a, b) => b[0].localeCompare(a[0]))) {
    ...
    while ((assignedWeekCounts.get(week) ?? 0) < MAX_VISIBLE_TRIGGERS_PER_WEEK && madeProgress) {
      ...
      for (const pattern of PATTERNS) {
        if ((assignedWeekCounts.get(week) ?? 0) >= MAX_VISIBLE_TRIGGERS_PER_WEEK) break;
        const patternWeekKey = `${week}:${pattern.key}`;
        if ((assignedPatternCounts.get(patternWeekKey) ?? 0) >= MAX_EPISODES_PER_PATTERN) continue;
...
function watchingSourceTriplets(triplets: JoinedTriplet[]): JoinedTriplet[] {
  const seen = new Set<string>();
  const out: JoinedTriplet[] = [];
  ...
  for (const triplet of ranked) {
    const key = (
      triplet.meetings[0]?.graph_source_ref ??
    ...
    seen.add(key);
    out.push(triplet);
    if (out.length >= MAX_WATCHING_SOURCE_NODES) break;
  }

  return out;
}

function cappedPatternScores(
  triplets: JoinedTriplet[],
  rootTriplet: JoinedTriplet,
): Map<string, PatternScore[]> {
  const counts = new Map<string, number>();
  const byTriplet = new Map<string, PatternScore[]>();
  const ordered = [
    rootTriplet,
  ...
  for (const triplet of ordered) {
    const week = triggerBundleFor(triplet).weekStart;
    const visibleScores: PatternScore[] = [];
    for (const score of scorePatterns(triplet)) {
      const key = `${week}:${score.key}`;
      const count = counts.get(key) ?? 0;
      if (count >= MAX_EPISODES_PER_PATTERN) continue;
...
function confidenceFor(triplet: JoinedTriplet): number {
  const bundle = triggerBundleFor(triplet);
...
function contextTitle(triplet: JoinedTriplet): string {
  return triggerBundleFor(triplet).contextTitle;
}

function attributionTitle(triplet: JoinedTriplet): string {
  const gap = triggerBundleFor(triplet).gapCandidate;
  if (gap) return `${gap.cause} attribution candidate`;
...
function attributionCause(triplet: JoinedTriplet): string {
  const bundle = triggerBundleFor(triplet);
...
function calendarEvidence(event: CalendarEventSignal): string {
  const when = compactDateTime(event.start_ts);
  return `Calendar: ${event.title}${when ? ` at ${when}` : ""}`;
}

function meetingEvidence(meeting: GranolaMeetingSignal): string {
...
function memoryEvidence(memory: MemoryInsightSignal): string {
  const title = memoryDisplayTitle(memory);
  return `Local memory: ${title}`;
}

function sampleEvidence(triplet: JoinedTriplet): string[] {
...
function biometricPatternEvidence(triplet: JoinedTriplet): string {
...
function buildBrief(triplet: JoinedTriplet): DetonateBrief {
  const state = triplet.episode.state;
  const bundle = triggerBundleFor(triplet);
...
function triggerCountsByWeek(triplets: JoinedTriplet[]): Map<string, number> {
  const counts = new Map<string, number>();
  for (const triplet of triplets) {
    const week = triggerBundleFor(triplet).weekStart;
...
function bestTripletByWeek(triplets: JoinedTriplet[]): Map<string, JoinedTriplet> {
  const byWeek = new Map<string, JoinedTriplet>();
  for (const triplet of triplets) {
    const week = triggerBundleFor(triplet).weekStart;
...
function weeklyBriefSnapshot(
  triplet: JoinedTriplet,
  triggerCount: number,
  currentBrief?: DetonateBrief,
...
function rootCarryForwardProps(
  rootTriplet: JoinedTriplet,
  graphTriplets: JoinedTriplet[],
  brief: DetonateBrief,
): Record<string, unknown> {
  ...
  const weeklyBriefs = Object.fromEntries(activeWeeks.map((week) => {
    ...
    return [week, weeklyBriefSnapshot(
      triplet,
      counts.get(week) ?? 1,
      triplet.id === rootTriplet.id ? brief : undefined,
    )];
  }));
  const rootWeek = triggerBundleFor(rootTriplet).weekStart;
...
function timeAlignmentLine(triplet: JoinedTriplet): string {
...
function metadataCentralBrain(event: CalendarEventSignal): Partial<SourceCard> | null {
...
function sourceCardProps(card: SourceCard): Record<string, unknown> {
  return {
...
function sourceCardForMeeting(meeting: GranolaMeetingSignal, triplet: JoinedTriplet): SourceCard {
  const usableSummary = meetingHasUsableSummary(meeting);
  const hasTranscript = meetingHasCitedTranscriptSpan(meeting);
...
function sourceCardForEvent(event: CalendarEventSignal, triplet: JoinedTriplet): SourceCard {
  ...
  const summary = truncate(
    centralBrain?.summary ?? description,
    720,
  ) ?? `${event.title} was on the calendar${when ? ` on ${when}` : ""}${location}. It can add context, but without a matching meeting note it is not enough by itself to name a cause.`;
...
function memoryDisplayTitle(memory: MemoryInsightSignal): string {
  ...
  if (/(release|launch|delivery|readiness)/.test(text)) return "Delivery readiness";
  ...
  if (/(sentinel|swift|ui|screenshot|graph|pattern)/.test(text)) return "Sentinel graph UI repair";
  if (/(detonator|entitlement|license|paid|paywall)/.test(text)) return "Detonator entitlement gate";
  if (/(loop|tracker|handoff|verification|session)/.test(text)) return "Loop handoff and verification";
  return "Project memory capsule";
}

function memorySummary(memory: MemoryInsightSignal): string {
  if (memory.source === "codex_project_capsule") {
...
function sourceCardForMemory(memory: MemoryInsightSignal, triplet: JoinedTriplet): SourceCard {
...
function visibleMemories(triplet: JoinedTriplet): MemoryInsightSignal[] {
  return triplet.memories
    .filter((memory) => memory.source === "codex_project_capsule")
    .slice(0, 3);
}

function deriveThemes(triplet: JoinedTriplet): string[] {
  ...
  if (themes.size === 0) themes.add("#work-context");
  return [...themes].sort();
}
...
function memoryHitNodeId(userId: string, memoryId: string): string {
  return `memoryhit_${stableHash([userId, memoryId])}`;
}

/**
 * Watching-path overlay for memory_hit emission: watching context nodes must
 * carry the context-only invariant props, and their edge weight is capped at
 * the deliberate watching-context weight (0.28) so watching memories never
 * outweigh other watching evidence. Armed path passes no overlay and keeps
 * full clamped strength.
 */
interface MemoryHitOverlay {
  node_props: Record<string, unknown>;
  edge_weight_cap: number;
}
 ...
 */
function addMemoryHitNodes(
  nodes: Map<string, GraphNodeDelta>,
  edges: Map<string, GraphEdgeDelta>,
  userId: string,
  memories: MemoryInsightSignal[],
  weekStart: string,
  rootId: string,
  overlay?: MemoryHitOverlay,
): void {
  ...
  const nodeIdByDedupTuple = new Map<string, string>();
...
function heartRateValue(triplet: JoinedTriplet): number | undefined {
...
function supportCalendarProps(event: CalendarEventSignal): Record<string, unknown> {
  return {
    id: event.id,
    title: event.title,
...
function supportMeetingProps(meeting: GranolaMeetingSignal): Record<string, unknown> {
  ...
  return {
    ...
    has_cited_transcript_span: meetingHasCitedTranscriptSpan(meeting),
...
function citedSpanRefs(spans: GapCandidate["evidence_spans"] | undefined): Record<string, unknown>[] {
...
function supportMemoryProps(memory: MemoryInsightSignal): Record<string, unknown> {
  return {
    id: memory.id,
...
function triggerSummary(triplet: JoinedTriplet, bundle: TriggerBundle): string {
...
function triggerKeyPoints(bundle: TriggerBundle, triplet: JoinedTriplet): string[] {
  return uniqueStrings([
...
function triggerWhyLinked(triplet: JoinedTriplet): string {
  const when = compactDateTime(triplet.episode.occurred_at_ts);
...
function endorsementClaimLevel(_episodeClaimLevel: unknown): "context_only" | "attribution_candidate" {
  return "context_only";
}
 ...
 */
function biometricEndorsementProps(
  triplet: JoinedTriplet,
  visiblePatternScores: PatternScore[],
  claimLevel: unknown,
  baselines: MetricBaseline[] | undefined,
): { biometric_summary: unknown; pattern_endorsements: unknown } | undefined {
  const rawSamples = triplet.episode.samples;
  if (!rawSamples.length) return undefined;

  const samples: EndorsementSample[] = [];
  for (const sample of rawSamples) {
    const ts = Date.parse(sample.timestamp);
    if (!Number.isFinite(ts) || typeof sample.value !== "number") continue;
    samples.push({
  ...
  for (const sample of samples) {
    if (sample.start_time_ts < startTs) startTs = sample.start_time_ts;
    if (sample.start_time_ts > endTs) endTs = sample.start_time_ts;
  }

  const endorsement = buildBiometricEndorsement({
    samples,
    baselines: baselines ?? [],
  ...
  if (endorsement.biometric_summary.sample_count <= 0) return undefined;
  return {
...
function evidenceGrade(triplet: JoinedTriplet): EvidenceGrade {
  return triggerBundleFor(triplet).grade;
}

function rootScaleLabel(triplet: JoinedTriplet): string {
  const grade = evidenceGrade(triplet);
  if (grade === "strong") return "Primary";
  if (grade === "candidate") return "Candidate";
  if (grade === "weak") return "Weak";
  return "Source-only";
}

function patternLenses(triplet: JoinedTriplet): Record<string, Record<string, unknown>> {
  const context = contextTitle(triplet);
...
function daypart(ts: number): string {
...
function episodeTitle(triplet: JoinedTriplet): string {
...
function watchingBrief(reason: string): DetonateBrief {
  return {
 ...
 */
function memoryContextProps(memoryContext: ReasoningInput["memoryContext"]): Record<string, unknown> {
  if (!memoryContext || memoryContext.threads.length === 0) return {};
  return {
    memory_context: {
...
export class Reasoner implements AttributionReasoner {
  async buildWatchingDelta(input: ReasoningInput, reason = "No meeting-note detail is strong enough to name an open work thread yet"): Promise<GraphDelta> {
    const userId = requireUserId(input.user_id, "Reasoner.buildWatchingDelta");
    const nodes = new Map<string, GraphNodeDelta>();
    const edges = new Map<string, GraphEdgeDelta>();
    const activePatterns = new Set<PatternKey>(input.triplets.flatMap((triplet) => (
      scorePatterns(triplet).map((entry) => entry.key)
    )));
  ...
  async buildDelta(input: ReasoningInput): Promise<GraphDelta> {
  ...
  private async buildDeltaInner(input: ReasoningInput): Promise<GraphDelta> {
    const userId = requireUserId(input.user_id, "Reasoner.buildDelta");
    const nodes = new Map<string, GraphNodeDelta>();
    const edges = new Map<string, GraphEdgeDelta>();
    ...
    const activePatterns = new Set<PatternKey>(graphTriplets.flatMap((triplet) => (
      (patternScoresByTriplet.get(triplet.id) ?? []).map((entry) => entry.key)
    )));
    ...
    for (const triplet of graphTriplets) {
      ...
      for (const theme of themes) {
        if (theme === "#b2b-track") b2bThemes.add(theme);
      }
    ...
    if (b2bThemes.size > 0 || rootThemes.includes("#b2b-track")) {
      ...
      for (const triplet of graphTriplets.filter((candidate) => deriveThemes(candidate).some((theme) => theme === "#b2b-track" || theme === "#investor-facing"))) {
        const episodeId = nodeId(userId, "episode", triplet.episode.state.graph_source_ref);
        if (!nodes.has(episodeId)) continue;
        addEdge(edges, {
