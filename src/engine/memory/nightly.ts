...
import { armedEpisodeTs } from "../assignments.js";
import { postAlertWebhook } from "../ingest/jobs.js";
import { mapGraphClaimLevel, promotedClaimLevel } from "./claims.js";
import { deriveThreadKey, threadId, evidenceId, verificationId, THREAD_KEY_VERSION } from "./keys.js";
import { resolveHandledAtTs } from "./verify.js";
import { EmbeddingProvider } from "../embeddings/provider.js";
import { transferSalt } from "../transfer/consent.js";
import { emitTransferRecord, resolveTransferProvider } from "../transfer/records.js";
import { writeProfileVector } from "../transfer/profile.js";
import {
  computeStrength,
  decayStateFor,
  OBSERVATION_WEIGHT,
  INTERVENTION_OUTCOME_WEIGHT,
} from "./strength.js";
import { CLAIM_LEVELS, MemoryClaimLevel, ThreadStatus } from "./types.js";
...
function collectGhostRefs(props: Record<string, any>): string[] {
  const refs = new Set<string>();
  const push = (value: unknown): void => {
    if (typeof value === "string" && value.trim()) refs.add(value.trim());
    else if (value && typeof value === "object" && typeof (value as any).source_ref === "string") {
      const ref = String((value as any).source_ref).trim();
      if (ref) refs.add(ref);
    }
  };
 ...
 */
function patternKeyForNode(node: ContextNodeRow): string | null {
  const props = parseProps(node.props_json);
  if (typeof props.pattern === "string" && props.pattern) return props.pattern;
...
function patternKeysForGhost(
  ghostId: string,
  adjacency: Map<string, string[]>,
  nodesById: Map<string, ContextNodeRow>,
): string[] {
  const keys = new Set<string>();
  const visited = new Set<string>([ghostId]);
...
interface ResolvedSource {
  id: string;
  occurredAtTs: number | null;
}

async function recordFailure(
  deps: NightlyThreadMaintenanceDeps,
  err: unknown,
  startTs: number,
): Promise<void> {
  ...
  try {
    await exec(
      `INSERT INTO pipeline_metrics(id, job_name, duration_ms, rows_written, dedup_skips, gemini_calls_used, embedding_row_count, warm_db_bytes, warnings_json, run_at)\n` +
        `VALUES (${q(`metric_${randomUUID()}`)}, ${q(JOB_NAME)}, ${durationMs}, 0, 0, 0, 0, 0, ${q(JSON.stringify([`job_failed: ${message}`]))}::jsonb, ${nowTs()});`,
    );
  } catch {
...
export async function runNightlyThreadMaintenance(
  deps: NightlyThreadMaintenanceDeps,
): Promise<NightlyThreadMaintenanceResult> {
  if (!(deps.exec && deps.query) && graphStoreKind() !== "postgres") {
    throw new Error(
      "runNightlyThreadMaintenance requires DAOBREW_GRAPH_STORE=postgres; layer 2 memory is Postgres-only",
    );
  }
  const exec = deps.exec ?? execSql;
  ...
  try {
    const ghosts = await query(
      `SELECT id, title, subtitle, occurred_at_ts, created_at_ts, props_json FROM graph_nodes WHERE user_id = ${SCOPED_USER_ID_SQL} AND kind = 'ghost'`,
    );
    ghosts.sort((a, b) => String(a.id).localeCompare(String(b.id)));

    const contextNodes = ghosts.length
      ? ((await query(
          `SELECT id, kind, title, element, source_ref, props_json FROM graph_nodes WHERE user_id = ${SCOPED_USER_ID_SQL} AND kind IN ('pattern','episode')`,
        )) as ContextNodeRow[])
      : [];
    const edges = ghosts.length
      ? ((await query(
          `SELECT id, src_id, dst_id, kind FROM graph_edges WHERE user_id = ${SCOPED_USER_ID_SQL}`,
        )) as unknown as GraphEdgeRow[])
      : [];

    const nodesById = new Map<string, ContextNodeRow>(contextNodes.map((node) => [String(node.id), node]));
    const adjacency = new Map<string, string[]>();
    for (const edge of edges) {
      if (!TRAVERSAL_EDGE_KINDS.has(String(edge.kind))) continue;
    ...
    const ghostRefs = new Map<string, ClassifiedRef[]>();
    ...
    for (const ghost of ghosts) {
      ...
      for (const ref of collectGhostRefs(props)) {
        ...
        if (parsed.sourceTable) {
          const bucket = refsByTable.get(parsed.sourceTable) ?? new Set<string>();
    ...
    const resolvedByTable = new Map<string, Map<string, ResolvedSource>>();
    for (const table of ["meeting_notes", "events", "user_insights"]) {
      const refs = [...(refsByTable.get(table) ?? [])].sort();
      if (refs.length === 0) continue;
      const inList = refs.map((ref) => q(ref)).join(", ");
      const timestampColumn = table === "events" ? "start_ts AS occurred_at_ts" : "occurred_at_ts";
      const rows = await query(
        `SELECT id, source_ref, ${timestampColumn} FROM ${table} WHERE user_id = ${SCOPED_USER_ID_SQL} AND source_ref IN (${inList})`,
      );
      const resolved = new Map<string, ResolvedSource>();
      for (const row of rows) {
        if (typeof row.source_ref !== "string") continue;
    ...
    const baseClaimByRoot = new Map<string, MemoryClaimLevel>();
    const donePropsByRoot = new Map<string, Record<string, any>>();
    ...
    const heldByRoot = new Map<string, { props: Record<string, any>; createdAtTs: number | null }>();

    for (const ghost of ghosts) {
      const ghostId = String(ghost.id);
      const props = parseProps(ghost.props_json);
      const threadKey = deriveThreadKey({ graphRootId: ghostId });
      const id = threadId(userId, threadKey);
      const claim = mapGraphClaimLevel(typeof props.claim_level === "string" ? props.claim_level : undefined);
      // Accept numeric strings too — props round-trip through JSON serializers.
      const recurrenceRaw =
        typeof props.recurrence_count === "number" ||
      ...
      const recurrence = Number.isFinite(recurrenceRaw)
        ...
        : Array.isArray(props.active_weeks)
          ...
          : 1;
      const status: ThreadStatus = props.status === "done" ? "done" : "active";
      baseClaimByRoot.set(ghostId, claim);
      if (props.status === "done") donePropsByRoot.set(ghostId, props);
      ...
      threadStatements.push(
        `INSERT INTO causal_memory_threads(\n` +
          ...
          `) VALUES (${q(id)}, ${q(userId)}, ${q(threadKey)}, ${THREAD_KEY_VERSION}, ${q(ghostId)}, ${litText(title)}, ${litText(summary)}, ${q(claim)}, ${q(status)}, ${litJson(patternKeys)}, ${recurrence}, ${litNum(seenTs, "first_seen_ts")}, ${litNum(seenTs, "last_seen_ts")}, 1.0, 'active', ${now}, ${now})\n` +
      ...
      for (const ref of ghostRefs.get(ghostId) ?? []) {
        ...
        if (ref.sourceTable && !resolved) unresolvedRefs += 1;
        evidenceRows.push({
      ...
      if (props.status === "done") {
        ...
        evidenceRows.push({
          ...
          weight: INTERVENTION_OUTCOME_WEIGHT,
        });
      }
    }
    ...
    if (evidenceRows.length > 0) {
      const values = evidenceRows
        .map((row) => (
          `(${q(row.id)}, ${q(row.threadId)}, ${q(userId)}, ${q(row.evidenceKind)}, ${litText(row.sourceTable)}, ${litText(row.sourceId)}, ${litText(row.sourceRef)}, ${litText(row.graphNodeId)}, ${litText(row.graphEdgeId)}, ${q(row.claimLevel)}, ${litText(row.patternKey)}, ${q(row.evidenceRole)}, ${litNum(row.observedAtTs, "observed_at_ts")}, ${row.weight}, ${now})`
        ))
        .join(",\n");
      ...
      await exec(
        `INSERT INTO causal_thread_evidence(id, thread_id, user_id, evidence_kind, source_table, source_id, source_ref, graph_node_id, graph_edge_id, claim_level, pattern_key, evidence_role, observed_at_ts, weight, created_at_ts)\n` +
          `VALUES ${values}\n` +
    ...
    const transferWarnings = new Set<string>();
    ...
    const emitSalt = deps.transferSalt !== undefined ? deps.transferSalt : transferSalt();
    const emitProvider = deps.transferProvider !== undefined ? deps.transferProvider : resolveTransferProvider();
    for (const [ghostId, props] of [...donePropsByRoot.entries()].sort(([a], [b]) => a.localeCompare(b))) {
      const handled = await resolveHandledAtTs(userId, ghostId, props, query);
      ...
      if (now <= windowEnd) continue; // window still open — nothing to settle yet
      const threadKey = deriveThreadKey({ graphRootId: ghostId });
      const tid = threadId(userId, threadKey);
      const observations = await query(
        `SELECT observation, observed_at_ts FROM thread_verifications WHERE user_id = ${SCOPED_USER_ID_SQL} AND thread_id = ${q(tid)} AND kind = 'observation'`,
      );
      let triggersInWindow = 0;
      let quietRunsInWindow = 0;
      for (const row of observations) {
        const ts = finiteOrNull(row.observed_at_ts);
        if (ts === null || ts < handled || ts > windowEnd) continue;
        if (row.observation === "same_thread_trigger") triggersInWindow += 1;
        else if (row.observation === "engine_ran_no_trigger") quietRunsInWindow += 1;
      }
      const verdict =
        triggersInWindow > 0
          ? "pattern_recurred"
      ...
      const verdictRowId = verificationId({ threadId: tid, kind: "verdict", bucket: `vd:${handled}` });
      const details = {
      ...
      const existingVerdict = await query(
        `SELECT 1 FROM thread_verifications WHERE user_id = ${SCOPED_USER_ID_SQL} AND id = ${q(verdictRowId)} LIMIT 1`,
      );
      await exec(
        `INSERT INTO thread_verifications(id, user_id, thread_id, thread_key, graph_root_id, kind, verdict, handled_at_ts, horizon_weeks, predicted_outcome, observed_at_ts, details_json, created_at_ts)\n` +
          `VALUES (${q(verdictRowId)}, ${q(userId)}, ${q(tid)}, ${q(threadKey)}, ${q(ghostId)}, 'verdict', ${q(verdict)}, ${handled}, ${horizonWeeks}, 'no_same_thread_trigger_within_horizon', ${windowEnd}, ${litJson(details)}, ${now})\n` +
          `ON CONFLICT (id) DO NOTHING;`,
      );
      if (existingVerdict.length === 0) verdictsSettled += 1;
      ...
      try {
        const emitted = await emitTransferRecord({
          userId,
          ghostId,
          threadKey,
          threadId: tid,
        ...
        for (const warning of emitted.warnings) transferWarnings.add(warning);
      } catch (err: any) {
    ...
    for (const [ghostId, held] of [...heldByRoot.entries()].sort(([a], [b]) => a.localeCompare(b))) {
      const handled = armedEpisodeTs(held.props, held.createdAtTs);
      if (handled === null) continue;
    ...
    const verdictRows = await query(
      `SELECT thread_id, verdict, details_json ->> 'arm' AS arm FROM thread_verifications WHERE user_id = ${SCOPED_USER_ID_SQL} AND kind = 'verdict'`,
    );
    const verdictCounts = new Map<string, { confirmations: number; contradictions: number }>();
    for (const row of verdictRows) {
      ...
      if (String(row.arm ?? "") === "control") continue;
      const key = String(row.thread_id);
      const counts = verdictCounts.get(key) ?? { confirmations: 0, contradictions: 0 };
      if (row.verdict === "no_recurrence_observed") counts.confirmations += 1;
      else if (row.verdict === "pattern_recurred") counts.contradictions += 1;
      // insufficient_observation is neither
      verdictCounts.set(key, counts);
    }
    const claimRows = await query(
      `SELECT id, graph_root_id, claim_level, promotion_state_json FROM causal_memory_threads WHERE user_id = ${SCOPED_USER_ID_SQL}`,
    );
    ...
    for (const row of claimRows) {
      ...
      const current: MemoryClaimLevel = (CLAIM_LEVELS as readonly string[]).includes(String(row.claim_level))
        ? (row.claim_level as MemoryClaimLevel)
        : "correlation";
      ...
      const storedBase = (CLAIM_LEVELS as readonly string[]).includes(String(prevState.base_claim_level))
        ? (prevState.base_claim_level as MemoryClaimLevel)
        : null;
      ...
      const final = promotedClaimLevel(base, counts.confirmations, counts.contradictions);
      const history: unknown[] = Array.isArray(prevState.history) ? [...prevState.history] : [];
      if (final !== current) {
        history.push({
          from: current,
          to: final,
      ...
      await exec(
        `UPDATE causal_memory_threads SET claim_level = ${q(final)}, promotion_state_json = ${litJson(state)}, updated_at_ts = ${now} WHERE id = ${q(tid)};`,
      );
    }
    ...
    const threadRows = await query(
      `SELECT id, claim_level FROM causal_memory_threads WHERE user_id = ${SCOPED_USER_ID_SQL}`,
    );
    const allEvidence = await query(
      `SELECT thread_id, observed_at_ts, weight FROM causal_thread_evidence WHERE user_id = ${SCOPED_USER_ID_SQL}`,
    );
    const evidenceByThread = new Map<string, Array<{ observed_at_ts: number | null; weight: number }>>();
    for (const row of allEvidence) {
      ...
      bucket.push({
        ...
        weight: Number.isFinite(weight) ? weight : OBSERVATION_WEIGHT,
      });
    ...
    for (const thread of threadRows) {
      ...
      const claim: MemoryClaimLevel = (CLAIM_LEVELS as readonly string[]).includes(String(thread.claim_level))
        ? (thread.claim_level as MemoryClaimLevel)
        : "correlation";
      const strength = computeStrength(evidence, claim, now);
      ...
      await exec(
        `UPDATE causal_memory_threads SET strength = ${strength}, decay_state = ${q(decayStateFor(strength))}, last_reinforced_ts = ${litNum(lastReinforced, "last_reinforced_ts")}, updated_at_ts = ${now} WHERE id = ${q(tid)};`,
      );
    }
    ...
    const gcCandidates = await query(
      `SELECT id, graph_root_id FROM causal_memory_threads WHERE user_id = ${SCOPED_USER_ID_SQL} AND status = 'active' AND decay_state = 'dormant' AND strength = 0 AND created_at_ts < ${gcCutoffTs} AND NOT EXISTS (SELECT 1 FROM causal_thread_evidence e WHERE e.thread_id = causal_memory_threads.id AND e.observed_at_ts >= ${gcCutoffTs})`,
    );
    ...
    if (threadsArchived > 0) {
      await exec(
        `UPDATE causal_memory_threads SET status = 'archived', updated_at_ts = ${now} WHERE user_id = ${q(userId)} AND status = 'active' AND id IN (${gcVictims.map((id) => q(id)).join(", ")});`,
      );
    }
    ...
    try {
      const profile = await writeProfileVector({
        userId,
        provider: emitProvider,
        exec,
        query,
      ...
      for (const warning of profile.warnings) transferWarnings.add(warning);
    } catch (err: any) {
    ...
    const mrtComparison: MrtComparisonCounts = {
    ...
    for (const row of verdictRows) {
      ...
      if (row.verdict === "pattern_recurred") {
        if (control) mrtComparison.control_recurred += 1;
        else mrtComparison.treatment_recurred += 1;
      } else if (row.verdict === "no_recurrence_observed") {
        if (control) mrtComparison.control_quiet += 1;
        else mrtComparison.treatment_quiet += 1;
      }
    ...
    try {
      const counted =
        mrtComparison.treatment_recurred + mrtComparison.treatment_quiet +
        mrtComparison.control_recurred + mrtComparison.control_quiet;
      await exec(
        `INSERT INTO pipeline_metrics(id, job_name, duration_ms, rows_written, dedup_skips, gemini_calls_used, embedding_row_count, warm_db_bytes, warnings_json, run_at)\n` +
          `VALUES (${q(`metric_${randomUUID()}`)}, 'layer2:nightly:mrt_comparison', 0, ${counted}, 0, 0, 0, 0, ${q(JSON.stringify([{ mrt_comparison: mrtComparison }]))}::jsonb, ${nowTs()});`,
      );
    } catch (err: any) {
      warnings.push(`mrt comparison metric failed: ${String(err?.message ?? err).slice(0, 200)}`);
    }

    if (unresolvedRefs > 0) warnings.push(`unresolved_refs: ${unresolvedRefs}`);
    if (threadsArchived > 0) warnings.push(`threads_archived: ${threadsArchived}`);
    ...
    await exec(
      `INSERT INTO pipeline_metrics(id, job_name, duration_ms, rows_written, dedup_skips, gemini_calls_used, embedding_row_count, warm_db_bytes, warnings_json, run_at)\n` +
        `VALUES (${q(`metric_${randomUUID()}`)}, ${q(JOB_NAME)}, ${durationMs}, ${rowsWritten}, 0, 0, 0, 0, ${q(JSON.stringify(warnings))}::jsonb, ${nowTs()});`,
    );
