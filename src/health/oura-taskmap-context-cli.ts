...
import {
  ...
  summarizeOuraTaskMapContext,
  writeOuraTaskMapContextAtomic,
} from "./oura-taskmap-context.js";
...
export function parseOuraTaskMapContextCliArgs(
  argv: string[],
): OuraTaskMapContextCliArgs {
...
export function resolveOuraTaskMapContextRange(
  args: OuraTaskMapContextCliArgs,
...
export async function runOuraTaskMapContextCli(
  argv: string[],
): Promise<void> {
  ...
  const document = await fetchOuraTaskMapContext(range);
  writeOuraTaskMapContextAtomic(args.outputPath!, document);
  process.stdout.write(`${JSON.stringify(summarizeOuraTaskMapContext(document))}\n`);
}
