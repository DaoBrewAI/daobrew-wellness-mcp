// Yin/Yang quadrant states (legacy — backend may still emit; not exposed in MCP surface)
export type Quadrant = "peak" | "pushing_it" | "recharging" | "burnout";
export type Element = "wood" | "fire" | "earth" | "metal" | "water";
export type InterventionTier = "audio" | "full";

// User-facing stress pattern names (replace "element" terminology in MCP surface)
...
export const STRESS_PATTERN_TO_ELEMENT: Record<StressPattern, Element> = {
  tension: "wood",
  overdrive: "fire",
  stagnation: "earth",
  constriction: "metal",
...
export const ELEMENT_TO_STRESS_PATTERN: Record<Element, StressPattern> = {
  wood: "tension",
  fire: "overdrive",
  earth: "stagnation",
  metal: "constriction",
...
export const STRESS_PATTERN_LIST: StressPattern[] = [
  "tension",
  "overdrive",
  "stagnation",
  "constriction",
...
/**
 * Live breath-state response from `GET /api/v1/session/breath-state`.
 * Backend computes everything deterministically from
 * (cycle_start_unix, cycle_sec, current_params) plus latest HR sample.
 */
export type BreathState =
  ...
  | {
      ...
      pattern: Element;
      cycle_num: number;
      cycle_start_unix: number;
      cycle_sec: number;
      current_params: { bpm: number; ratio: number; depth: number };
      current_hr: number | null;
      current_hr_age_sec: number | null;
      prev_cycle_hr_mean: number | null;
      prev_cycle_hr_std: number | null;
...
export interface HealthkitHistorySample {
  value: number;
  timestamp: string;
}

export interface HealthkitHistory {
  metric: string;
  range: string;
  samples: HealthkitHistorySample[];
  aggregated: {
    avg: number | null;
    min: number | null;
    max: number | null;
